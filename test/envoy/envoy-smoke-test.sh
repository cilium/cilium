...
function cleanup {
  ...
  #  ${CILIUM_BIN} service delete --all 2> /dev/null || true
  #  ${CILIUM_BIN} policy delete --all 2> /dev/null || true
...
function service_init {
  ...
  ${CILIUM_BIN} service update --frontend "$SVC_IP4:80" --id 2233 \
			--backends "$SERVER1_IP4:80" \
			--backends "$SERVER2_IP4:80"

  ${CILIUM_BIN} service update --frontend "[$SVC_IP]:80" --id 2234 \
...
function proxy_init {
  ...
  while [ `${CILIUM_BIN} endpoint list -o jsonpath='{range [*]}{.status.identity.id}{" "}{.status.identity.labels}{"\n"}' | grep '^[1-9][0-9]* .*id.\(server\|client\)' | cut -d ' ' -f1 | sort | uniq | wc -l` -ne 2 ] ; do
    log "waiting..."
    sleep 1
  done
...
function policy_single_egress {
  ${CILIUM_BIN} policy delete policy=test
  cat <<EOF | policy_import_and_wait -
[{
    "labels": [{"key": "policy", "value": "test"}],
...
for state in "false" "true"; do
  ${CILIUM_BIN} config ConntrackLocal=$state
