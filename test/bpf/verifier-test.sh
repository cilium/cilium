...
function cg_prog_type_init {
	...
	$MAPTOOL eppolicymap "test_cilium_ep_to_policy" 2>/dev/null >/dev/null
	$MAPTOOL sockmap "test_sock_ops_map" 2>/dev/null >/dev/null
...
function load_sockops_prog {
	...
	ALL_MAPS="cilium_ipcache cilium_ep_to_policy cilium_lxc sock_ops_map \
		...
		cilium_lb6_reverse_nat cilium_lb6_services_v2 \
		cilium_lb6_backends cilium_lb4_reverse_nat \
		cilium_lb4_services_v2 cilium_lb4_backends cilium_events"
	...
	for p in ${CG_PROGS}; do
		...
		for section in $(get_section ${p}.c); do
			...
			if [ "$prog_type" = "sockaddr" ]; then
				local args="pin $pinpath obj ${prog}.o \
					type $prog_type \
					attach_type $attach_type sec $section"
