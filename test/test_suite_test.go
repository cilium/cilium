...
func TestTest(t *testing.T) {
	if config.CiliumTestConfig.TestScope != "" {
		helpers.PreDefinedScope = config.CiliumTestConfig.TestScope
		fmt.Printf("Force test scope value to %q", config.CiliumTestConfig.TestScope)
	}
...
var _ = BeforeAll(func() {
	...
	if err != nil {
		Fail(fmt.Sprintf("Cannot set scope for the test %s", err))
	}
...
var _ = AfterAll(func() {
	...
	// Logs are not checked here because it should fail on BeforeAll
