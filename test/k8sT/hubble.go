...
var _ = Describe("K8sHubbleTest", func() {
	...
	BeforeAll(func() {
		...
		hubbleCLIPath = helpers.ManifestGet(kubectl.BasePath(), "hubble-cli.yaml")
		...
		// Deploy hubble-cli DaemonSet which connects to the Hubble server running inside cilium-agent
		res := kubectl.Apply(helpers.ApplyOptions{FilePath: hubbleCLIPath, Namespace: hubbleNamespace})
