...
var _ = Describe("K8sFQDNTest", func() {
	...
	It("Validate that multiple specs are working correctly", func() {
		// To make sure that UUID in multiple specs are plumbin correctly to
		// Cilium Policy
		fqdnPolicy := helpers.ManifestGet("fqdn-proxy-multiple-specs.yaml")
		world1Target := "http://world1.cilium.test"
		world2Target := "http://world1.cilium.test"
		...
		res.ExpectSuccess("Can't connecto to a valid target when it should work")

		res = kubectl.ExecPodCmd(
			helpers.DefaultNamespace, appPods[helpers.App2],
			helpers.CurlFail(world2Target))
		res.ExpectSuccess("Can connecto to a valid target when it should NOT work")
