...
var _ = Describe("K8sPolicyTest", func() {
	...
	Context("Basic Test", func() {
		...
		importPolicy := func(file, name string) {
			...
			ExpectWithOffset(1, kubectl.WaitForEnforcingCNP(name)).To(BeNil(), "CNP is not in enforcing mode after timeout")
		}
		...
		It("checks all kind of Kubernetes policies", func() {
			...
			Expect(kubectl.WaitForEnforcingCNP("l3-l4-policy")).To(BeNil(), "CNP is not in enforcing mode after timeout")
			...
			Expect(kubectl.WaitForEnforcingCNP("l7-policy")).To(BeNil(), "CNP is not in enforcing mode after timeout")
		...
		It("ServiceAccount Based Enforcement", func() {
			...
			Expect(kubectl.WaitForEnforcingCNP("service-account")).To(BeNil(), "CNP is not in enforcing mode after timeout")
		...
		It("Denies traffic with cnp default-deny ingress policy", func() {
			...
			Expect(kubectl.WaitForEnforcingCNP("cnp-default-deny-ingress")).To(BeNil(), "CNP is not in enforcing mode after timeout")
		...
		It("Denies traffic with cnp default-deny egress policy", func() {
			...
			Expect(kubectl.WaitForEnforcingCNP("cnp-default-deny-egress")).To(BeNil(), "CNP is not in enforcing mode after timeout")
