...
EOF`, k, v)
	...
	Context("Namespaces policies", func() {
		...
		BeforeAll(func() {
			...
			res.ExpectSuccess("unable to apply manifest %s")
			...
			clusterIPNS, _, err = kubectl.GetServiceHostPort(namespace, app1Service)
		...
		It("Tests the same Policy in the different namespaces", func() {
			...
			res := kubectl.ExecPodCmd(
				...
				helpers.CurlFail(fmt.Sprintf("http://%s/public", clusterIPNS)))
		...
		It("Network Policy namespace selector", func() {
			...
			for _, pod := range []string{helpers.App2, helpers.App3} {
				res := kubectl.ExecPodCmd(
					helpers.DefaultNamespace, appPods[pod],
					helpers.CurlFail(fmt.Sprintf("http://%s/public", clusterIPNS)))
				res.ExpectFail("%q can curl to service and it should not. NS policy should be applied", appPods[pod])
			...
			By("Delete policy in %q namespace", namespace)
			_, err = kubectl.CiliumPolicyAction(
