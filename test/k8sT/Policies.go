...
EOF`, k, v)
	...
	Context("Namespaces policies", func() {
		...
		var (
			...
			netpolNsSelector   = fmt.Sprintf("%s -n %s", helpers.ManifestGet("netpol-namespace-selector.yaml"), namespace)
			l3l4PolicySecondNS = fmt.Sprintf("%s -n %s", l3L4Policy, namespace)
		...
		BeforeAll(func() {
			...
			res.ExpectSuccess("unable to create namespace %q", namespace)

			res = kubectl.Exec(fmt.Sprintf("kubectl label namespaces/%[1]s nslabel=%[1]s", namespace))
			...
			err := kubectl.WaitforPods(namespace, "-l zgroup=testapp", 300)
			Expect(err).To(BeNil(),
				"testapp pods are not ready after timeout in namspace %q", namespace)

			err = kubectl.WaitforPods(helpers.DefaultNamespace, "-l zgroup=testapp", 300)
			Expect(err).To(BeNil(),
				"testapp pods are not ready after timeout in namespace", helpers.DefaultNamespace)

			appPods = helpers.GetAppPods(apps, helpers.DefaultNamespace, kubectl, "id")
			appPodsNS = helpers.GetAppPods(apps, namespace, kubectl, "id")

			clusterIP, _, err = kubectl.GetServiceHostPort(helpers.DefaultNamespace, app1Service)
			Expect(err).To(BeNil(), "Cannot get service on %q namespace", helpers.DefaultNamespace)

			secondNSclusterIP, _, err = kubectl.GetServiceHostPort(namespace, app1Service)
		...
		It("Tests the same Policy in different namespaces", func() {
			...
			By("Applying Policy in %q namespace", namespace)
			_, err = kubectl.CiliumPolicyAction(
				helpers.KubeSystemNamespace, l3l4PolicySecondNS, helpers.KubectlApply, 300)
			Expect(err).Should(BeNil(),
				"%q Policy cannot be applied in %q namespace", l3l4PolicySecondNS, namespace)
		...
		It("Kubernetes Network Policy by namespace selector", func() {
			...
			for _, pod := range []string{helpers.App2, helpers.App3} {
				res := kubectl.ExecPodCmd(
					helpers.DefaultNamespace, appPods[pod],
					helpers.CurlFail(fmt.Sprintf("http://%s/public", secondNSclusterIP)))
				res.ExpectFail("%q can curl to service, policiy is not blocking"+
					"communication to %q namespace", appPods[pod], namespace)
			...
			By("Delete Kubernetes Network Policies in %q namespace", namespace)
			_, err = kubectl.CiliumPolicyAction(
