	...
	Context("Basic Test", func() {
		...
		validateNoPolicyEnabled := func() {
			...
			for _, app := range apps {
				...
				ExpectWithOffset(1, cep).NotTo(BeNil(), "Endpoint %q does not exit", pod)
		...
		validatePolicyStatus := func() {
			...
			ExpectWithOffset(1, cep).NotTo(BeNil(), "Endpoint %q does not exit", appPods[helpers.App1])
			ExpectWithOffset(1, cep.Status.Policy.Realized.PolicyEnabled).To(
		...
		It("tests PolicyEnforcement updates", func() {
			...
			for _, app := range apps {
				pod := appPods[app]
				cep := kubectl.CepGet(helpers.DefaultNamespace, pod)
				Expect(cep).NotTo(BeNil(), "Endpoint %q does not exit", pod)
