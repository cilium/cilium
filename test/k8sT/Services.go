...
Secondary Interface %s :: IPv4: (%s, %s), IPv6: (%s, %s)`, helpers.DualStackSupported(), privateIface, k8s1IP, k8s2IP, primaryK8s1IPv6, primaryK8s2IPv6,
		...
		SkipContextIf(helpers.RunsWithoutKubeProxy, "Tests NodePort (kube-proxy)", func() {
			...
			SkipItIf(func() bool { return helpers.SkipQuarantined() && helpers.SkipK8sVersions("==1.14") }, "with the host firewall and externalTrafficPolicy=Local", func() {
			...
			SkipItIf(func() bool { return helpers.SkipQuarantined() && helpers.SkipK8sVersions("1.14") }, "with externalTrafficPolicy=Local", func() {
				DeployCiliumAndDNS(kubectl, ciliumFilename)
				testExternalTrafficPolicyLocal()
			})
