...
var _ = Describe("K8sServicesTest", func() {
	...
	Context("Checks ClusterIP Connectivity", func() {
		...
		It("Checks service on same node", func() {
			...
			nodeNames, err := kubectl.GetNodeNames("cilium.io/ci-node=" + helpers.K8s1)
			Expect(err).To(BeNil(), "Cannot get node by label")
			status, err := kubectl.ExecInHostNetNS(context.TODO(), nodeNames[0],
	...
	Context("Checks service across nodes", func() {
		...
		testNodePort := func(bpfNodePort bool) {
			...
			doRequests := func(url string, count int) {
				...
				for i := 1; i <= count; i++ {
					res, err := kubectl.ExecInHostNetNS(context.TODO(), nodeNames[0], helpers.CurlFail(url))
			...
			ips, err := kubectl.GetNodeIPs("cilium.io/ci-node")
			Expect(err).Should(BeNil(), "Can not retrieve node IP")
			k8s1Ip := ips[helpers.K8s1]
			k8s2Ip := ips[helpers.K8s2]
