...
function test_images {
  ...
  # sed's -n does not print non-matching lines and the `#gp` prints only
  # matching groups, and the match is global. `#` is used as the sed delimiter
  # to avoid escaping `/` in the regex.
  DOCKER_IMAGES=$(grep -rI --no-filename "docker.io" test/ | sed -nEe 's#.*(docker.io/[-a-zA-Z0-9]+/[-a-zA-Z0-9]+:[-.a-zA-Z0-9]+)[^-.a-zA-Z0-9].*#\1#p' | sort | uniq)
  QUAY_IMAGES=$(grep -rI --no-filename "quay.io" test/     | sed -nEe 's#.*(quay.io/[-a-zA-Z0-9]+/[-a-zA-Z0-9]+:[-.a-zA-Z0-9]+)[^-.a-zA-Z0-9].*#\1#p'   | sort | uniq)
...
function cilium_images {
  ...
  CILIUM_IMGS=$(grep -rI --no-filename "quay.io" $CILIUM_DOCKERFILES   | sed -nEe 's#.*(quay.io/[-a-zA-Z0-9]+/[-a-zA-Z0-9]+:[-.a-zA-Z0-9]+)[^-.a-zA-Z0-9].*#\1#p')
