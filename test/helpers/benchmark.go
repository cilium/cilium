...
// ClientServerCheck verifies client can still ping server with
// both IPv4 and IPv6
func (s *SSHMeta) ClientServerCheck(client string, server string) {
	serverIpv4, serverIpv6, _ := s.ContainerInspectIP(server)

	ginkgoext.By("checking %s can ping to %s IPv6", client, server)
	res := s.ContainerExec(client, Ping6(serverIpv6))
	res.ExpectSuccess("ping %s -> %s failed", client, server)

	ginkgoext.By("checking %s can ping to %s IPv4", client, server)
	res = s.ContainerExec(Client, Ping(serverIpv4))
	res.ExpectSuccess("ping %s -> %s failed", client, server)
}
...
func (s *SSHMeta) superNetperfRRIPv4(client string, server string, num int) *CmdRes {
	...
	serverIpv4, _, _ := s.ContainerInspectIP(server)
	...
	s.ClientServerCheck(client, server)
	return res
}
