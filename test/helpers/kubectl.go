...
func (kub *Kubectl) WaitforDeployReady(namespace, name string, timeout time.Duration) error {
	...
	return WithTimeout(
		body,
		fmt.Sprintf("Deployment %s/%s not ready after %s (%d/%d ready)", namespace, name, timeout, specReplicas, currentReplicas),
		&TimeoutConfig{Timeout: timeout})
}
...
func (kub *Kubectl) WaitKubeDNS() error {
	return kub.WaitforDeployReady("kube-system", GetDNSEngine(), DNSHelperTimeout)
}
