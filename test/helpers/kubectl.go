...
// LogsWithPrevious returns a CmdRes with command output from the
// execution of `kubectl logs --previous <pod> -n <namespace>`.
func (kub *Kubectl) LogsWithPrevious(namespace string, pod string) *CmdRes {
	return kub.Exec(
		fmt.Sprintf("%s -n %s logs --previous %s", KubectlCmd, namespace, pod))
}

// LogsWithLabel returns a CmdRes with command output from the
// execution of `kubectl logs --previous -l <label string> -n <namespace>`.
func (kub *Kubectl) LogsWithLabel(namespace string, labelStr string) *CmdRes {
	return kub.Exec(
