...
func (kub *Kubectl) CiliumReport(namespace string, commands ...string) {
	...
	res := kub.ExecContext(ctx, fmt.Sprintf("%s get pods -o wide --all-namespaces", KubectlCmd), ExecOptions{
	...
	for _, pod := range pods {
		for _, cmd := range commands {
			res = kub.ExecPodCmdContext(ctx, namespace, pod, cmd, ExecOptions{
				SkipLog: true,
				Timeout: 10 * time.Second,
			})
			ginkgoext.GinkgoPrint(res.GetDebugMessage())
