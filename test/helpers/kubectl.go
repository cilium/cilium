...
func (kub *Kubectl) MicroscopeStart() (error, func() error) {
	...
	cb := func() error {
		...
		// FIXME: Fix this one when microscope process is stopped correctly.
		cmd := kub.Exec(fmt.Sprintf("%[1]s -n %[2]s delete pod %[3]s",
			KubectlCmd, KubeSystemNamespace, microscope))
		if !cmd.WasSuccessful() {
			return fmt.Errorf("Cannot delete microscrope: %s", cmd.CombineOutput())
		}
		return nil
	}
	return nil, cb
