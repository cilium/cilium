...
func (kub *Kubectl) CiliumPolicyRevision(pod string) (int, error) {
	...
	if err != nil {
		return -1, fmt.Errorf("cannot get revision from json output '%s': %w", res.CombineOutput(), err)
	}
...
func (kub *Kubectl) getPodRevisions() (map[string]int, error) {
	...
	for _, pod := range pods {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("Cannot retrieve cilium pod %s policy revision: %w", pod, err)
		}
...
func (kub *Kubectl) servicePreFlightCheck(serviceName, serviceNamespace string) error {
	...
	for _, pod := range kub.serviceCache.pods {
		...
		if err != nil {
			return fmt.Errorf("Error validating Cilium service on pod %v: %w", pod, err)
		}
	}
	return nil
}
