...
func (kub *Kubectl) CiliumServicePreFlightCheck() error {
	...
	k8sSvcAddrFilter := "kubectl get services kubernetes -o jsonpath='{.spec.clusterIP}:{.spec.ports[0].port}'"
	...
	endpointAddress, err := kub.GetJSONPath("default", "endpoints kubernetes", k8sEpAddrFilter)
	if err != nil {
		return err
	}
	...
	svcAddr, err := kub.GetJSONPath("default", "service kubernetes", k8sSvcAddrFilter)
	if err != nil {
		return err
	}
