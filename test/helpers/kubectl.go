...
func (kub *Kubectl) ciliumInstallHelm(options []string) error {
	defaultOptions := map[string]string{
		"global.hub":                    "k8s1:5000/cilium",
		"agent.image":                   "cilium-dev",
		"agent.tag":                     "latest",
		"operator.image":                "operator",
		"operator.tag":                  "latest",
		"managed-etcd.hub":              "docker.io/cilium",
		"global.debug":                  "true",
	...
	res := kub.ExecMiddle(fmt.Sprintf("helm template go/src/github.com/cilium/cilium/install/kubernetes/helm --namespace=kube-system %s > cilium.yaml", strings.Join(options, " ")))
	if !res.WasSuccessful() {
