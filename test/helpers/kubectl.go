...
func (kub *Kubectl) PrepareCluster() {
	...
	registry := config.RegistryDomain
	if len(config.CiliumTestConfig.Registry) > 0 {
		registry = config.CiliumTestConfig.Registry
	}
	err = kub.AddRegistryCredentials(config.CiliumTestConfig.RegistryCredentials, registry)
	if err != nil {
...
func (kub *Kubectl) overwriteHelmOptions(options map[string]string) error {
	...
	if len(config.CiliumTestConfig.RegistryCredentials) > 0 {
		options["global.imagePullSecrets[0].name"] = config.RegistrySecretName
	}
