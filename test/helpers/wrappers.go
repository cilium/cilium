...
func CurlFail(endpoint string, optionalValues ...interface{}) string {
	...
	return fmt.Sprintf(
		`curl -s -D /dev/stderr  --trace /dev/stderr --fail --connect-timeout %[1]d --max-time %[1]d %[2]s -w "%[3]s"`,
...
func CurlWithHTTPCode(endpoint string, optionalValues ...interface{}) string {
	...
	return fmt.Sprintf(
		`curl -s --fail --output /dev/stderr --trace /dev/stderr -w '%%{http_code}' --connect-timeout %d %s`,
