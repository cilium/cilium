...
# ── Phase 3: Per-Node CiliumVTEPNodeConfig ─────────────────
#
# The operator resolves each CiliumVTEPConfig's nodeSelector into one
# CiliumVTEPNodeConfig per matching node (name == node name): zone-a -> kind-worker,
# zone-b -> kind-worker2. The control-plane has no zone label, so it gets no node config.
# Each node's agent writes its own node config .status after BPF sync.

phase 3 "Per-Node CiliumVTEPNodeConfig"

# (a) Operator created one node config per matching node.
echo "  Checking CiliumVTEPNodeConfig objects exist for both workers..."
if kubectl get ciliumvtepnodeconfig kind-worker > /dev/null 2>&1; then
  pass "CiliumVTEPNodeConfig kind-worker exists (operator created from zone-a)"
else
  fail "CiliumVTEPNodeConfig kind-worker missing"
fi
if kubectl get ciliumvtepnodeconfig kind-worker2 > /dev/null 2>&1; then
  pass "CiliumVTEPNodeConfig kind-worker2 exists (operator created from zone-b)"
else
  fail "CiliumVTEPNodeConfig kind-worker2 missing"
fi

# control-plane has no zone label, so it matches no config: the operator must NOT
# create a CiliumVTEPNodeConfig for it (directly tests nodeSelector resolution).
if kubectl get ciliumvtepnodeconfig kind-control-plane > /dev/null 2>&1; then
  fail "CiliumVTEPNodeConfig kind-control-plane exists, but control-plane matches no config"
else
  pass "control-plane has no CiliumVTEPNodeConfig (operator correctly skips unmatched nodes)"
fi

# (b) nodeSelector resolved each node's spec.vtepEndpoints to the right zone endpoint.
echo "  Checking resolved spec.vtepEndpoints tunnelEndpoints..."
NC_A_TE=$(kubectl get ciliumvtepnodeconfig kind-worker \
  -o jsonpath="{.spec.vtepEndpoints[?(@.cidr=='10.1.5.0/24')].tunnelEndpoint}" 2>/dev/null || true)
if [ "$NC_A_TE" = "$VTEP_A_IP" ]; then
  pass "kind-worker node config resolved cidr 10.1.5.0/24 -> $VTEP_A_IP (zone-a)"
else
  fail "kind-worker node config tunnelEndpoint for 10.1.5.0/24 is '$NC_A_TE', expected $VTEP_A_IP"
fi
NC_B_TE=$(kubectl get ciliumvtepnodeconfig kind-worker2 \
  -o jsonpath="{.spec.vtepEndpoints[?(@.cidr=='10.1.5.0/24')].tunnelEndpoint}" 2>/dev/null || true)
if [ "$NC_B_TE" = "$VTEP_B_IP" ]; then
  pass "kind-worker2 node config resolved cidr 10.1.5.0/24 -> $VTEP_B_IP (zone-b)"
else
  fail "kind-worker2 node config tunnelEndpoint for 10.1.5.0/24 is '$NC_B_TE', expected $VTEP_B_IP"
fi

# (c) Each node config reports Ready == True (agent wrote .status after BPF sync).
echo "  Checking node config .status Ready conditions..."
NC_A_READY=$(kubectl get ciliumvtepnodeconfig kind-worker \
  -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || true)
if [ "$NC_A_READY" = "True" ]; then
  pass "kind-worker node config status Ready == True"
else
  fail "kind-worker node config status Ready == '$NC_A_READY', expected True"
fi
NC_B_READY=$(kubectl get ciliumvtepnodeconfig kind-worker2 \
  -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || true)
if [ "$NC_B_READY" = "True" ]; then
  pass "kind-worker2 node config status Ready == True"
else
  fail "kind-worker2 node config status Ready == '$NC_B_READY', expected True"
fi

# ── Phase 4: VXLAN Connectivity ─────────────────────────────
...
# ── Phase 5: Dynamic Update — Change MAC ────────────────────

phase 5 "Dynamic CRD Update"
...
kubectl patch ciliumvtepconfig zone-a --type=merge -p "{
  \"spec\": {\"vtepEndpoints\": [{\"name\": \"router-a\", \"cidr\": \"10.1.5.0/24\",
    \"tunnelEndpoint\": \"$VTEP_A_IP\", \"mac\": \"AA:BB:CC:DD:EE:99\"}]}}"

echo "  Waiting for reconciliation after sentinel MAC..."
STATUS=""
for i in $(seq 1 30); do
  STATUS=$(kubectl get ciliumvtepnodeconfig kind-worker \
    -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || true)
...
if [ "$STATUS" = "True" ]; then
  pass "kind-worker node config status is Ready after MAC patch"
else
  fail "kind-worker node config status not Ready after MAC patch: $STATUS"
fi
...
kubectl patch ciliumvtepconfig zone-b --type=merge -p "{
  \"spec\": {\"vtepEndpoints\": [{\"name\": \"router-b\", \"cidr\": \"10.1.5.0/24\",
    \"tunnelEndpoint\": \"$VTEP_B_IP\", \"mac\": \"AA:BB:CC:DD:EE:88\"}]}}"

echo "  Waiting for reconciliation after sentinel MAC..."
STATUS=""
for i in $(seq 1 30); do
  STATUS=$(kubectl get ciliumvtepnodeconfig kind-worker2 \
    -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || true)
  if [ "$STATUS" = "True" ]; then break; fi
  sleep 2
done
...
if [ "$STATUS" = "True" ]; then
  pass "kind-worker2 node config status is Ready after MAC patch"
else
  fail "kind-worker2 node config status not Ready after MAC patch: $STATUS"
fi
...
# The operator must garbage-collect the per-node config once no config matches the node.
echo "  Verifying operator removed kind-worker's CiliumVTEPNodeConfig..."
NC_GONE=1
for i in $(seq 1 15); do
  if ! kubectl get ciliumvtepnodeconfig kind-worker > /dev/null 2>&1; then NC_GONE=0; break; fi
  sleep 2
done
if [ "$NC_GONE" -eq 0 ]; then
  pass "operator removed CiliumVTEPNodeConfig kind-worker after zone-a deletion"
else
  fail "CiliumVTEPNodeConfig kind-worker still present after zone-a deletion"
fi
