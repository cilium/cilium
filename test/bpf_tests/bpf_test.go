...
func loadAndRunSpec(t *testing.T, entry fs.DirEntry, instrLog io.Writer) []*cover.Profile {
	...
	opts := ebpf.CollectionOptions{
		Programs: ebpf.ProgramOptions{
			LogSize: 32 << 20, // 32 MiB, not needed in most cases, but especially when running instrumented code.
		},
	}
	...
	if err != nil {
		...
		if errors.Is(err, ENOSPC) {
			t.Fatal("Verifier logs larger than 32MiB, increase the log size passed to ebpf.NewCollectionWithOptions " +
