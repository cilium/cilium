...
func validate(test *suite.ControlPlaneTest, start time.Time) error {
	...
	// Surrogate, ClusterIP and 2x NodePort
	expectedServices := 4 * numServicesAndBackends

	totalObjects := expectedServices + numServicesAndBackends /* backends */
	...
	if len(lbmap.ServiceByID) == expectedServices && len(lbmap.BackendByID) == numServicesAndBackends {
		...
		fmt.Printf("MemStats:\n  HeapAlloc: %dKB\n  HeapObjects: %d\n  HeapSys: %dKB\n", m.HeapAlloc/1024, m.HeapObjects, m.HeapSys/1024)
		return nil
	}
