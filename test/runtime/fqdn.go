...
[
	{
		...
		fqdnPolicyImport(fmt.Sprintf(policy, outsideIps[WorldHttpd1]))

		By("Testing connectivity to Cilium.test domain")
		res := vm.ContainerExec(helpers.App1, helpers.CurlWithHTTPCode("http://world1.cilium.test"))
		res.ExpectSuccess("Cannot access to world1.cilium.test")

		By("Testing connectivity to existing CIDR rule")
		res = vm.ContainerExec(helpers.App1, helpers.CurlWithHTTPCode(outsideIps[WorldHttpd1]))
		res.ExpectSuccess("Cannot access to CIDR rule when should work")

		By("Ensure connectivity to other domains is still block")
		res = vm.ContainerExec(helpers.App1, helpers.CurlWithHTTPCode("http://world2.outside.test"))
