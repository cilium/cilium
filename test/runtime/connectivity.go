...
var _ = Describe("RuntimeConnectivityTest", func() {
	...
	dumpOutput := func(res *helpers.CmdRes) {
		By(fmt.Sprintf("=======================================================\nstdout %q: \n%s", res.GetCmd(), res.GetStdOut()))
		By(fmt.Sprintf("=======================================================\nstderr %q: \n%s", res.GetCmd(), res.GetStdErr()))
	}
...
var _ = Describe("RunConntrackTest", func() {
	...
	BeforeEach(func() {
		...
		docker.ContainerCreate(helpers.Client, helpers.NetperfImage, helpers.CiliumDockerNetwork, "-l id.client")
		docker.ContainerCreate(helpers.Client, helpers.NetperfImage, helpers.CiliumDockerNetwork, "-l id.server")
		cilium.PolicyDelAll()
	})
