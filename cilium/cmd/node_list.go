...
func formatStatusResponse(w io.Writer, cluster *models.ClusterStatus) {
	nodesOutput := []string{"Name\tIPv4 Address\tEndpoint CIDR\tIPv6 Address\tEndpoint CIDR\tConfiguration\n"}
	...
	for _, node := range cluster.Nodes {
		...
		nodesOutput = append(nodesOutput, fmt.Sprintf("%s\t%s\t%s\t%s\t%s\t%s\n",
			node.Name, ipv4, ipv4Range, ipv6, ipv6Range, node.RoutingConfiguration.String()))
	}
