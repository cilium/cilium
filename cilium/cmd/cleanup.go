...
func init() {
	...
	bindEnv(allFlagName, cleanCiliumEnvVar)
	bindEnv(bpfFlagName, cleanBpfEnvVar)
...
func runCleanup() {
	...
	cleanAll = viper.GetBool(allFlagName)
	cleanBPF = viper.GetBool(bpfFlagName)

	// If no specific cleanup actions were requested, then remove all state.
	if !cleanBPF {
		cleanAll = true
	}
