...
func openMonitorSock() (conn net.Conn, version listener.ListenerVersion, err error) {
	errors := make([]string, 0)

	// try the 1.2 socket
	conn, err = net.Dial("unix", defaults.MonitorSockPath1_2)
	if err == nil {
		return conn, listener.ListenerVersion1_2, nil
	}
	...
	if err == nil {
		return conn, listener.ListenerVersion1_0, nil
	}
	errors = append(errors, defaults.MonitorSockPath1_0+": "+err.Error())

	return nil, listener.ListenerVersionUnsupported, fmt.Errorf("Cannot find or open a supported node-monitor socket. %s", strings.Join(errors, ","))
}
...
func consumeMonitorEvents(conn net.Conn, version listener.ListenerVersion) error {
	defer conn.Close()
...
func getMonitorParser(conn net.Conn, version listener.ListenerVersion) (parser eventParserFunc, err error) {
	switch version {
