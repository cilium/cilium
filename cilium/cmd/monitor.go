...
func endpointExists(endpoint format.Uint16Flags) bool {
	eps, err := client.EndpointList()
	if err != nil {
		Fatalf("cannot get endpoint list: %s\n", err)
	}
	for _, ep := range eps {
		if endpoint.Has(uint16(ep.ID)) {
			return true
		}
	}
	return false
...
func runMonitor(args []string) {
	...
	if len(printer.FromSource) > 0 && !endpointExists(printer.FromSource) {
		Fatalf("Endpoint %d doesn't exist\n", printer.FromSource[0])
	}
	if len(printer.ToDst) > 0 && !endpointExists(printer.ToDst) {
		Fatalf("Endpoint %d doesn't exist\n", printer.ToDst[0])
	}
	if len(printer.Related) > 0 && !endpointExists(printer.Related) {
		Fatalf("Endpoint %d doesn't exist\n", printer.Related[0])
	}
