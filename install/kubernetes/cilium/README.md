...
| envoy.accessLog.enabled | bool | `true` | Enable Envoy Access Logging. ref: https://www.envoyproxy.io/docs/envoy/latest/configuration/access_log |
| envoy.accessLog.format | string | `"text"` | Access log format Applicable values: text, json @schema type: [null, string] @schema |
| envoy.accessLog.path | string | `"/var/run/cilium/envoy-accesslog.log"` | Path to a separate Envoy log file, if any. |
| envoy.accessLog.pattern | string | `"[%START_TIME%] \"%REQ(:METHOD)% %REQ(X-ENVOY-ORIGINAL-PATH?:PATH)% %PROTOCOL%\" %RESPONSE_CODE% %RESPONSE_FLAGS% %BYTES_RECEIVED% %BYTES_SENT% %DURATION% %RESP(X-ENVOY-UPSTREAM-SERVICE-TIME)% \"%REQ(X-FORWARDED-FOR)%\" \"%REQ(USER-AGENT)%\" \"%REQ(X-REQUEST-ID)%\" \"%REQ(:AUTHORITY)%\" \"%UPSTREAM_HOST%\"\\n\n"` | Format string pattern for Envoy access logs. ref: https://www.envoyproxy.io/docs/envoy/latest/configuration/observability/access_log/usage" |
