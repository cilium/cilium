...
| identitySeedSalt | string | autogenerated | A seed used to make identity generation deterministic but difficult to predict. If absent, then a random value is generated the first time Cilium is installed. If you rely on offline templating (e.g. via Argo), then set this explicitly to a random value. Set to an empty string to disable. |
