...
| envoy.useOriginalSourceAddress | bool | `true` | For cases when CiliumEnvoyConfig is not used directly (Ingress, Gateway), configures Cilium BPF Metadata listener filter to use the original source address when extracting the metadata for a request. Controls `proxy-use-original-source-address` in the ConfigMap (`--proxy-use-original-source-address` agent option). |
