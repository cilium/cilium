...
| networkPolicy.aggregateIdentities | bool | `nil` | Enable policy-map identity aggregation. This can significantly reduce policy map size when using supported entities. The default is disabled. Changing from enabled to disabled may cause brief connectivity interruptions. |
