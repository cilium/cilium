...
| ingressController.enableSecretsSync | bool | `true` | EnableSecretsSync will make sure all TLS secrets used by Ingress are synced to secretsNamespace.name If disabled, TLS secrets must be maintained externally. |
...
| ingressController.secretsNamespace | object | `{"create":true,"name":"cilium-secrets"}` | SecretsNamespace is the namespace in which envoy SDS will retrieve TLS secrets from. |
