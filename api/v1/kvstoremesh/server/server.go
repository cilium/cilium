...
import (
	...
	"github.com/cilium/cilium/api/v1/kvstoremesh/models"
	swaggerserversrv "github.com/cilium/cilium/api/v1/kvstoremesh/server"
...
// APICell provides the restapi.KvstoreMeshAPI type, populated
...
func newAPI(p apiParams) *restapi.KvstoreMeshAPI {
	api := restapi.NewKvstoreMeshAPI(p.Spec.Document)
...
type ServerConfig struct {
	EnableKvstoreMeshServerAccess []string
}
...
var (
	...
	AdminEnableFlag = "enable-kvstore-mesh-server-access"
)

func (cfg ServerConfig) Flags(flags *pflag.FlagSet) {
	flags.StringSlice(AdminEnableFlag, cfg.EnableKvstoreMeshServerAccess,
		"List of kvstore mesh APIs which are administratively enabled. Supports '*'.")
}

var SpecCell = cell.Module(
	"kvstore-mesh-spec",
	"kvstore mesh Specification",
...
func newSpec(cfg ServerConfig) (*Spec, error) {
	...
	deniedAPIs, err := api.AllowedFlagsToDeniedPaths(swaggerSpec, cfg.EnableKvstoreMeshServerAccess)
	if err != nil {
...
// NewServer creates a new api kvstore mesh server but does not configure it
func NewServer(api *restapi.KvstoreMeshAPI) *Server {
	s := new(Server)
	s.api = api
	return s
}
...
// Server for the kvstore mesh API
type Server struct {
	EnabledListeners []string
...
func (s *Server) SetAPI(api *restapi.KvstoreMeshAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) GetAPI() *restapi.KvstoreMeshAPI {
	return s.api
}
...
func (s *Server) Start(cell.HookContext) (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		s.wg.Add(1)
		s.Logf("Serving kvstore mesh at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer s.wg.Done()
			if err := domainSocket.Serve(l); err != nil && err != http.ErrServerClosed {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving kvstore mesh at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		s.wg.Add(1)
		s.Logf("Serving kvstore mesh at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer s.wg.Done()
			if err := httpServer.Serve(l); err != nil && err != http.ErrServerClosed {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving kvstore mesh at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		s.wg.Add(1)
		s.Logf("Serving kvstore mesh at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer s.wg.Done()
			if err := httpsServer.Serve(l); err != nil && err != http.ErrServerClosed {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving kvstore mesh at https://%s", l.Addr())
