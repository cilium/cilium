...
func readMessageSet(r io.Reader, size int32, version int16) ([]*Message, error) {
	...
	for {
		...
		if version != -1 && version >= KafkaV1 {
			// timestamp
			fmt.Print("MK in readMessageSet skipping timestamp, since version :", version)
		...
		case CompressionNone:
			...
			if err := msgdec.Err(); err != nil {
				fmt.Print("MK in readMessageSet cannot decode message err:", err)
				return nil, err
			}
			set = append(set, msg)
		case CompressionGzip, CompressionSnappy:
			...
			case CompressionGzip:
				...
				if err != nil {
					fmt.Print("MK in readMessageSet error decoding gzip message err:", err)
					return nil, err
				}
			...
			case CompressionSnappy:
				...
				if err != nil {
					fmt.Print("MK in readMessageSet error decoding snappy message err:", err)
					return nil, err
				}
			}
			...
			set = append(set, msgs...)
		default:
			fmt.Print("MK in readMessageSet cannot handle compression method %d", compression, " err:", err)
			return nil, err
		}
	}
}
...
func ReadFetchResp(r io.Reader) (*FetchResp, error) {
	...
	for ti := range resp.Topics {
		...
		for pi := range topic.Partitions {
			...
			fmt.Print("MK in ReadFetchResp calling readMessageSet with msgSetSize:", msgSetSize)
			if part.Messages, err = readMessageSet(r, msgSetSize, -1); err != nil {
				return nil, err
			}
