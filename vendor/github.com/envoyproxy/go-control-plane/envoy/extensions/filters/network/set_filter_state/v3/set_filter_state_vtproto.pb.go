...
func (m *Config) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if len(m.OnDownstreamData) > 0 {
		for iNdEx := len(m.OnDownstreamData) - 1; iNdEx >= 0; iNdEx-- {
			if vtmsg, ok := interface{}(m.OnDownstreamData[iNdEx]).(interface {
				...
				encoded, err := proto.Marshal(m.OnDownstreamData[iNdEx])
				if err != nil {
					return 0, err
				}
				i -= len(encoded)
	...
	if len(m.OnNewConnection) > 0 {
		for iNdEx := len(m.OnNewConnection) - 1; iNdEx >= 0; iNdEx-- {
			if vtmsg, ok := interface{}(m.OnNewConnection[iNdEx]).(interface {
				...
				encoded, err := proto.Marshal(m.OnNewConnection[iNdEx])
				if err != nil {
					return 0, err
				}
				i -= len(encoded)
...
func (m *Config) SizeVT() (n int) {
	...
	if len(m.OnDownstreamData) > 0 {
		for _, e := range m.OnDownstreamData {
			if size, ok := interface{}(e).(interface {
				SizeVT() int
			}); ok {
				l = size.SizeVT()
