...
// Validate checks the field values on IPTagging_IPTags with the rules defined
// in the proto definition for this message. If any rules are violated, the
// first error encountered is returned, or nil if there are no violations.
func (m *IPTagging_IPTags) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on IPTagging_IPTags with the rules
// defined in the proto definition for this message. If any rules are
// violated, the result is a list of violation errors wrapped in
// IPTagging_IPTagsMultiError, or nil if none found.
func (m *IPTagging_IPTags) ValidateAll() error {
	return m.validate(true)
}

func (m *IPTagging_IPTags) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	for idx, item := range m.GetIpTags() {
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, IPTagging_IPTagsValidationError{
						field:  fmt.Sprintf("IpTags[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
			...
			if err := v.Validate(); err != nil {
				return IPTagging_IPTagsValidationError{
					field:  fmt.Sprintf("IpTags[%v]", idx),
					reason: "embedded message failed validation",
					cause:  err,
				}
	...
	if len(errors) > 0 {
		return IPTagging_IPTagsMultiError(errors)
	}

	return nil
}

// IPTagging_IPTagsMultiError is an error wrapping multiple validation errors
// returned by IPTagging_IPTags.ValidateAll() if the designated constraints
// aren't met.
type IPTagging_IPTagsMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m IPTagging_IPTagsMultiError) Error() string {
	msgs := make([]string, 0, len(m))
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m IPTagging_IPTagsMultiError) AllErrors() []error { return m }

// IPTagging_IPTagsValidationError is the validation error returned by
// IPTagging_IPTags.Validate if the designated constraints aren't met.
type IPTagging_IPTagsValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e IPTagging_IPTagsValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e IPTagging_IPTagsValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e IPTagging_IPTagsValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e IPTagging_IPTagsValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e IPTagging_IPTagsValidationError) ErrorName() string { return "IPTagging_IPTagsValidationError" }

// Error satisfies the builtin error interface
func (e IPTagging_IPTagsValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
	...
	return fmt.Sprintf(
		"invalid %sIPTagging_IPTags.%s: %s%s",
		key,
		e.field,
		e.reason,
		cause)
...
var _ error = IPTagging_IPTagsValidationError{}
...
var _ interface {
	...
	Cause() error
	ErrorName() string
} = IPTagging_IPTagsValidationError{}
