...
const (
	...
	tagCheckNilReceiverTagName = tagEnabledName + ":check-nil-receiver"
...
func extractCheckNilReceiverTypeTag(t *types.Type) *enabledTagValue {
	comments := append(append([]string{}, t.SecondClosestCommentLines...), t.CommentLines...)
	return extractTag(tagCheckNilReceiverTagName, comments)
}
...
func deepEqualMethod(t *types.Type) (*types.Signature, error) {
	...
	if privateMethod != nil && privateMethod.value == "true" {
...
func (g *genDeepEqual) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	methodName := "DeepEqual"
	privateMethod := extractPrivateMethodTypeTag(t)
	if privateMethod != nil && privateMethod.value == "true" {
		methodName = "deepEqual"
	}
	typeArgs["method"] = methodName
	checkNilReceiver := extractCheckNilReceiverTypeTag(t)
	if checkNilReceiver == nil || checkNilReceiver.value != "true" {
		typeArgs["description"] = " in must be non-nil."
	}

	if deepEqualMethodOrDie(t) == nil {
		sw.Do("// $.method$ is an autogenerated deepequal function, deeply comparing the \n", typeArgs)
		sw.Do("// receiver with other.$.description$\n", typeArgs)
...
func (g *genDeepEqual) doBuiltin(t *types.Type, sw *generator.SnippetWriter) {
	...
	if checkNilReceiver != nil && checkNilReceiver.value == "true" {
		sw.Do("switch {\n", nil)
		sw.Do("case (in == nil) != (other == nil):\n", nil)
		sw.Do("return false\n", nil)
		sw.Do("case (in == nil) && (other == nil):\n", nil)
		sw.Do("return true\n", nil)
		sw.Do("}\n", nil)
		sw.Do("return in != *other\n", nil)
	} else {
...
func (g *genDeepEqual) doMap(t *types.Type, sw *generator.SnippetWriter) {
	...
	if uet.IsPrimitive() {
		...
		if uet.Elem.IsPrimitive() {
			...
			sw.Do("if !inValue.$.method$(otherValue) {\n", typeArgs)
		}
...
func (g *genDeepEqual) doStruct(t *types.Type, sw *generator.SnippetWriter) {
	...
	if deepEqualMethodOrDie(t) != nil {
		...
		if checkNilReceiver != nil && checkNilReceiver.value == "true" {
			...
			sw.Do("if other == nil || !in.$.method$(other) {\n", typeArgs)
			sw.Do("return false\n", nil)
			sw.Do("}\n", nil)
		}
		return
	...
	for _, m := range ut.Members {
		...
		methodName := "DeepEqual"
		privateMethod := extractTag(tagPrivateMethodTagName, m.CommentLines)
		if privateMethod != nil && privateMethod.value == "true" {
			methodName = "deepEqual"
		}
		...
		case uft.Kind == types.Pointer:
			...
			if ufet.IsPrimitive() {
				...
				sw.Do("if !in.$.name$.$.method$(other.$.name$) {\n", typeArgs)
			}
			sw.Do("return false\n", nil)
			sw.Do("}\n", nil)
			sw.Do("}\n", nil)
