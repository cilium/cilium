...
const CASS_HDR_LEN = 9
const CASS_MAX_LEN = 268435456 // 256 MB, per spec
...
func (rule *CassandraRule) Matches(data interface{}) bool {
	...
	if rule.tableRegexCompiled != nil {
		regex_str = rule.tableRegexCompiled.String()
	}

	log.Debugf("Rule: action '%s', table '%s'", rule.queryActionExact, regex_str)
...
func CassandraRuleParser(rule *cilium.PortNetworkPolicyRule) []L7NetworkPolicyRule {
	...
	for _, l7Rule := range l7Rules.GetL7Rules() {
		...
		if len(cr.queryActionExact) > 0 {
			...
			} else if res == ACTION_NO_TABLE && cr.tableRegexCompiled != nil {
...
func (p *CassandraParser) OnData(reply, endStream bool, data_arr [][]byte, offset int) (OpType, int) {
	...
	if len(data) < CASS_HDR_LEN {
		// Partial header received, ask for more
		needs := CASS_HDR_LEN - len(data)
	...
	if requestLen > CASS_MAX_LEN {
		log.Errorf("Request length of %d is greater than 256 MB", requestLen)
		return ERROR, 0
	}

	data_missing := (9 + int(requestLen)) - len(data)
	if data_missing > 0 {
		// full header received, but only partial request

		log.Debugf("Hdr received, but need %d more bytes of request", data_missing)
	...
	if reply {
		...
		cassandraParseReply(p, data[0:(CASS_HDR_LEN+requestLen)])

		log.Debugf("reply, passing %d bytes", uint32(len(data)))
		return PASS, len(data)
	}

	err, paths := cassandraParseRequest(p, data[0:(CASS_HDR_LEN+requestLen)])
	if err != 0 {
	...
	if !matches {
		...
		return DROP, int(CASS_HDR_LEN + requestLen)
	}

	return PASS, int(CASS_HDR_LEN + requestLen)
}
...
const INVALID_ACTION = 0
const ACTION_WITH_TABLE = 1
const ACTION_NO_TABLE = 2
...
var queryActionMap = map[string]int{
	...
	"create-index":             ACTION_NO_TABLE, // TODO: we could tie this to table if we want
