...
const (
	CAS_WITH_NOREPLY_FIELDS     = 7
	STORAGE_WITH_NOREPLY_FIELDS = 6
	DELETE_WITH_NOREPLY_FIELDS  = 3
	INCR_WITH_NOREPLY_FIELDS    = 4
	TOUCH_WITH_NOREPLY_FIELDS   = 4
)
...
func (p *Parser) OnData(reply, endStream bool, dataBuffers [][]byte) (proxylib.OpType, int) {
	...
	if !reply {
		...
		frameLength := 0
		noreply := false
		if p.isCommandRetrieval(command) {
			// get, gets, gat, gats
		...
		} else if p.isCommandStorage(command) {
			// storage commands
			...
			if command[0] == 'c' { //storage command is "cas"
				noreply = len(tokens) == CAS_WITH_NOREPLY_FIELDS
			} else {
				noreply = len(tokens) == STORAGE_WITH_NOREPLY_FIELDS
			}
		} else if p.isCommandDelete(command) {
			meta.Keys = tokens[1:2]
			noreply = len(tokens) == DELETE_WITH_NOREPLY_FIELDS
			frameLength = linefeed + 2
		} else if p.isCommandIncrDecr(command) {
			meta.Keys = tokens[1:2]
			noreply = len(tokens) == INCR_WITH_NOREPLY_FIELDS
			frameLength = linefeed + 2
		} else if bytes.Equal(command, []byte("touch")) {
			meta.Keys = tokens[1:2]
			noreply = len(tokens) == TOUCH_WITH_NOREPLY_FIELDS
			frameLength = linefeed + 2
		} else if bytes.Equal(command, []byte("slabs")) ||
			...
			meta.Keys = [][]byte{}
			frameLength = linefeed + 2
		} else if bytes.Equal(command, []byte("flush_all")) ||
			...
			noreply = true
			frameLength = linefeed + 2
		} else if bytes.Equal(command, []byte("watch")) {
			meta.Keys = [][]byte{}
			frameLength = linefeed + 2
			p.watching = true
		} else {
