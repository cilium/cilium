	...
	if !reply {
		...
		case p.isCommandStorage(command):
			...
			if command[0] == 'c' { //storage command is "cas"
				noreply = len(tokens) == casWithNoreplyFields
			} else {
				noreply = len(tokens) == storageWithNoreplyFields
			}
		case p.isCommandDelete(command):
			meta.Keys = tokens[1:2]
			noreply = len(tokens) == deleteWithNoreplyFields
		case p.isCommandIncrDecr(command):
			meta.Keys = tokens[1:2]
			noreply = len(tokens) == incrWithNoreplyFields
		case bytes.Equal(command, []byte("touch")):
			meta.Keys = tokens[1:2]
			noreply = len(tokens) == touchWithNoreplyFields
