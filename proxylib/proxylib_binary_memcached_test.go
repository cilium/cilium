...
func TestBinaryMemcacheOnDataReqMore(t *testing.T) {
	...
	data := string(getHello[:10])
	CheckOnData(t, 1, false, false, &[]string{data}, []ExpFilterOp{{proxylib.MORE, 14}}, proxylib.OK, "")
...
func TestBinaryMemcacheOnDataReqSplit(t *testing.T) {
	...
	data := string(getHello)
	CheckOnData(t, 1, false, false, &[]string{data[:10], data[10:]}, []ExpFilterOp{
		{proxylib.PASS, len(data)}, {proxylib.MORE, 24},
	}, proxylib.OK, "")
