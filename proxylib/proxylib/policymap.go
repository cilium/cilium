...
func newPortNetworkPolicies(config []*cilium.PortNetworkPolicy) PortNetworkPolicies {
	...
	for _, rule := range config {
		...
		if _, found := policy.Rules[port]; found {
			panic(fmt.Errorf("NPDS: Duplicate port number %d", port))
		}
		...
		if ok {
			log.Debugf("NPDS::PortNetworkPolicies(): installing TCP policy for port %d", port)
...
var policyMap unsafe.Pointer // PolicyMap
...
func getPolicyMap() *PolicyMap {
	return (*PolicyMap)(atomic.LoadPointer(&policyMap))
}
...
func PolicyMatches(endpointPolicyName string, ingress bool, port, remoteId uint32, l7 interface{}) bool {
	...
	pMap := getPolicyMap()
	if pMap == nil {
		log.Warning("NPDS: No Policy Received!")
		return false
	}
	policy, found := (*pMap)[endpointPolicyName]

	if !found {
		log.Warningf("NPDS: Policy for %s not found", endpointPolicyName)
	}
...
// Assuming there is only one updater!
