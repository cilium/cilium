...
func (p *CassandraParserFactory) Create(connection *Connection) Parser {
	log.Infof("CassandraParserFactory: Create: %v", connection)
	return &CassandraParser{connection: connection}
}

func (p *CassandraParser) OnData(reply, endStream bool, str_data []string, offset uint32) (OpType, uint32) {
	...
	if hdr_missing > 0 {
		...
		return MORE, uint32(hdr_missing)
	}
	...
	if data_missing > 0 {
		...
		return MORE, uint32(data_missing)
	}
	...
	if reply {
		if len(data) == 0 {
			...
			return PASS, uint32(len(data))
		}
	}

	err, paths := cassandra_parse(p, data[0:(9+request_len)])
	if err != 0 {
	...
	if matches {
		...
		return DROP, (request_len + 9)
	}

}

// generate a full response (header + body) to be used as an
// "unauthorized" error to be sent to cassandra client as part of policy
// deny.   Original message is used to ensure that reply has matching
// protocol version, and matching stream-id.
...
func cassandra_parse(p *CassandraParser, data []byte) (OpError, []string) {

	direction := data[0] & 0x80 // top bit
	if direction != 0 {
		log.Infof("Direction bit is 'reply', but cassandra parser only parses requests")
		return ERROR_INVALID_FRAME_TYPE, nil
	}

	compression_flag := data[1] & 0x01
	if compression_flag == 1 {
		log.Infof("Compression flag set, unable to parse beyond the header")
		return ERROR_INVALID_FRAME_TYPE, nil
	}
