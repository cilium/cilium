...
type EnvoyCache interface {
	...
	UpsertEnvoyResources(context.Context, pkgEnvoy.Resources) error
	pkgEnvoy.PortAllocator
}
...
func (h *cecHandler) upsert(key resource.Key, spec *cilium_v2.CiliumEnvoyConfigSpec) {
	resources, err := pkgEnvoy.ParseResources(
		key.Namespace,
		key.Name,
		spec.Resources,
		true,
	...
	for _, svc := range spec.Services {
		name := loadbalancer.ServiceName{
			Authority: loadbalancer.AuthoritySVC,
	...
	// TODO: update subsystem status based on faultyConfigs.
}
