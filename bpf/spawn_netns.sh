...
# This script creates a network namespace (if no "--skip-net-init" is passed),
# configures the network connectivity for it, then runs a command within
# the namespace and waits for the command to exit. Cleans up the namespace and
# device configuration on termination.
# Must be run as root.
#
...
validate_args()
{
    ...
    [ -z "$SKIP_NETNS_INIT" ] && local NS="${NETNS}"
