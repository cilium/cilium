...
# ifdef ENABLE_IPSEC
	if (!decrypted) {
		...
		send_trace_notify(
			ctx, TRACE_TO_STACK, *identity, UNKNOWN_ID,
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV6_FROM_OVERLAY) int tail_handle_ipv6(
...
#ifdef ENABLE_IPV4
static __always_inline int
ipv4_host_delivery(struct __ctx_buff *ctx, struct iphdr *ip4)
{
# ifdef HOST_IFINDEX
	if (1) {
		union macaddr host_mac = HOST_IFINDEX_MAC;
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV4_INTER_CLUSTER_REVSNAT) int tail_handle_inter_cluster_revsnat(
	struct __ctx_buff *ctx)
{
	int ret;
	__u32 src_sec_identity = ctx_load_and_clear_meta(ctx, CB_SRC_LABEL);
	 ...
	 defined(ENABLE_INTER_CLUSTER_SNAT)
{
	...
	if (cluster_id_from_identity != 0 &&
		...
		return tail_call_internal(
			ctx, CILIUM_CALL_IPV4_INTER_CLUSTER_REVSNAT, ext_err);
	}
}
# endif
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV4_FROM_OVERLAY) int tail_handle_ipv4(
	struct __ctx_buff *ctx)
{
	__u32 src_sec_identity = ctx_load_and_clear_meta(ctx, CB_SRC_LABEL);
	__s8 ext_err = 0;
 ...
 */
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_ARP) int tail_handle_arp(
	struct __ctx_buff *ctx)
{
	...
	if (unlikely(ctx_get_tunnel_key(ctx, &key, key_size, 0) < 0))
		return send_drop_notify_error(
			ctx, UNKNOWN_ID, DROP_NO_TUNNEL_KEY, CTX_ACT_DROP,
			METRIC_INGRESS);
	...
	if (unlikely(ret != 0))
		return send_drop_notify_error(
			ctx, UNKNOWN_ID, ret, CTX_ACT_DROP, METRIC_EGRESS);
	if (info->tunnel_endpoint) {
		ret = __encap_and_redirect_with_nodeid(
			ctx, UNKNOWN_ID, info->tunnel_endpoint, LOCAL_NODE_ID,
			WORLD_IPV4_ID, WORLD_IPV4_ID, &trace);
		if (IS_ERR(ret))
			goto drop_err;
...
pass_to_stack:
	send_trace_notify(
		ctx, TRACE_TO_STACK, UNKNOWN_ID, UNKNOWN_ID, TRACE_EP_ID_UNKNOWN,
...
# else
			...
			if (unlikely(ctx_get_tunnel_key(ctx, &key, key_size, 0) <
				     0)) {
...
# endif /* ENABLE_HIGH_SCALE_IPCACHE */
			...
			src_sec_identity =
				get_id_from_tunnel_id(key.tunnel_id, proto);
...
#  if defined(ENABLE_DSR) && DSR_ENCAP_MODE == DSR_ENCAP_GENEVE
		if (ctx_load_meta(ctx, CB_HSIPC_ADDR_V4)) {
			...
			set_geneve_dsr_opt4(
				(__be16)ctx_load_meta(ctx, CB_HSIPC_PORT),
				ctx_load_meta(ctx, CB_HSIPC_ADDR_V4), &dsr_opt);
			...
			if (ctx_set_tunnel_opt(ctx, &dsr_opt, sizeof(dsr_opt)) <
			    0) {
				ret = DROP_WRITE_ERROR;
				goto out;
			}
