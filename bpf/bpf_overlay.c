...
#ifdef ENABLE_NODEPORT
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_ENCAP_NODEPORT_NAT) int tail_handle_nat_fwd(struct __sk_buff *skb)
{
	int ret;
...
int to_overlay(struct __sk_buff *skb)
{
	...
	if (unlikely(ret < 0))
		goto out;
#ifdef ENABLE_NODEPORT
	ep_tail_call(skb, CILIUM_CALL_ENCAP_NODEPORT_NAT);
	ret = DROP_MISSED_TAIL_CALL;
#endif
out:
	if (IS_ERR(ret))
