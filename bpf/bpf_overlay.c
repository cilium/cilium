				       ...
				       __s8 *ext_err __maybe_unused)
{
	...
	if (!decrypted) {
		...
		send_trace_notify_with_flags(ctx, TRACE_TO_STACK, *identity, UNKNOWN_ID,
 ...
 *
 * 1. will be traced with TRACE_FLAG_ENCRYPTED
 * 2. will be traced without TRACE_FLAG_ENCRYPTED
 * 3. will be traced without TRACE_FLAG_ENCRYPTED
 *
 * Note that 1. contains the ESP packets someone else generated.
 * In that case, we trace it as "encrypted", but it doesn't mean
 * "encrypted by Cilium".
 *
 * When IPSec is disabled, we won't use TRACE_FLAG_ENCRYPTED even
...
#ifdef ENABLE_IPSEC
	if (is_esp(ctx, proto))
		send_trace_notify_with_flags(ctx, TRACE_FROM_OVERLAY, src_sec_identity, UNKNOWN_ID,
					     TRACE_EP_ID_UNKNOWN, ctx->ingress_ifindex,
					     TRACE_REASON_UNKNOWN, 0, TRACE_FLAG_ENCRYPTED);
	else
#endif
