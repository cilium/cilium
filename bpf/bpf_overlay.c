...
#ifdef ENABLE_IPV6
static __maybe_unused int handle_ipv6(struct __ctx_buff *ctx,
				       __u32 *identity,
...
#ifdef ENABLE_IPV4
static __maybe_unused int ipv4_host_delivery(struct __ctx_buff *ctx, struct iphdr *ip4)
{
#ifdef HOST_IFINDEX
	if (1) {
		union macaddr host_mac = HOST_IFINDEX_MAC;
...
static __maybe_unused int handle_inter_cluster_revsnat(struct __ctx_buff *ctx,
...
static __maybe_unused int handle_ipv4(struct __ctx_buff *ctx,
				       __u32 *identity,
				       __s8 *ext_err __maybe_unused)
{
	struct remote_endpoint_info *info;
...
#ifdef ENABLE_IPSEC
static __maybe_unused bool is_esp(struct __ctx_buff *ctx, __u16 proto)
{
	void *data, *data_end;
	__u8 protocol = 0;
