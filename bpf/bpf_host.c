...
static __maybe_unused bool allow_vlan(__u32 __maybe_unused ifindex, __u32 __maybe_unused vlan_id) {
	VLAN_FILTER(ifindex, vlan_id);
}

#if defined(ENABLE_IPV4) || defined(ENABLE_IPV6)
static __maybe_unused int rewrite_dmac_to_host(struct __ctx_buff *ctx)
{
...
static __maybe_unused bool identity_from_ipcache_ok(void)
{
...
#if defined(ENABLE_IPSEC) && defined(TUNNEL_MODE)
static __maybe_unused int do_netdev_encrypt_encap(struct __ctx_buff *ctx, __u32 src_id)
{
...
static __maybe_unused int handle_l2_announcement(struct __ctx_buff *ctx)
{
 ...
 */
static __maybe_unused int do_encrypt_overlay(struct __ctx_buff *ctx)
{
	int ret = CTX_ACT_OK;
