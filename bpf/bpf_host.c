	...
	if (ret == CTX_ACT_OK) {
		...
		if (from_host) {
			if (is_defined(ENABLE_HOST_FIREWALL))
				ret = tail_call_internal(ctx, CILIUM_CALL_IPV6_CONT_FROM_HOST, &ext_err);
			else
				ret = tail_handle_ipv6_cont_from_host(ctx);
		} else {
			if (is_defined(ENABLE_HOST_FIREWALL))
				ret = tail_call_internal(ctx, CILIUM_CALL_IPV6_CONT_FROM_NETDEV, &ext_err);
			else
...
tail_handle_ipv4(struct __ctx_buff *ctx, __u32 ipcache_srcid, const bool from_host)
{
	...
	if (ret == CTX_ACT_OK) {
		...
		if (from_host) {
			if (is_defined(ENABLE_HOST_FIREWALL))
				ret = tail_call_internal(ctx, CILIUM_CALL_IPV4_CONT_FROM_HOST, &ext_err);
			else
				ret = tail_handle_ipv4_cont_from_host(ctx);
		} else {
			if (is_defined(ENABLE_HOST_FIREWALL))
				ret = tail_call_internal(ctx, CILIUM_CALL_IPV4_CONT_FROM_NETDEV, &ext_err);
			else
