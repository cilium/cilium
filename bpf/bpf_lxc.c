...
#  endif /* ENABLE_LOCAL_REDIRECT_POLICY && ENABLE_SOCKET_LB_FULL */
		ret = lb4_local(
			get_ct_map4(&tuple), ctx, ipv4_is_fragment(ip4),
...
#  if defined(ENABLE_LOCAL_REDIRECT_POLICY) && defined(ENABLE_SOCKET_LB_FULL)
		if (unlikely(lb6_svc_is_localredirect(svc)))
			goto skip_service_lookup;
#  endif /* ENABLE_LOCAL_REDIRECT_POLICY && ENABLE_SOCKET_LB_FULL */
		ret = lb6_local(get_ct_map6(&tuple), ctx, ETH_HLEN, l4_off, &key,
				&tuple, svc, &ct_state_new, false, ext_err);
...
static __always_inline int drop_for_direction(
	struct __ctx_buff *ctx, enum ct_dir dir, __u32 reason, __s8 ext_err)
{
	__u32 dst = 0;
	__u32 dst_id = 0;
...
# endif
	...
	return send_drop_notify_ext(
		ctx, src_label, dst, dst_id, reason, ext_err, CTX_ACT_DROP, m_dir);
}
...
# endif /* ENABLE_L7_LB */
	...
	verdict = policy_can_egress6(
		ctx, &POLICY_MAP, tuple, l4_off, SECLABEL_IPV6, *dst_sec_identity,
		&policy_match_type, &audited, ext_err, &proxy_port);

	if (verdict == DROP_POLICY_AUTH_REQUIRED) {
		auth_type = (__u8)*ext_err;
		verdict = auth_lookup(
			ctx, SECLABEL_IPV6, *dst_sec_identity, tunnel_endpoint,
			auth_type);
	}
...
# ifdef ENABLE_NODEPORT
		...
		if (ct_state->node_port && lb_is_svc_proto(tuple->nexthdr)) {
			...
			return tail_call_internal(
				ctx, CILIUM_CALL_IPV6_NODEPORT_REVNAT, ext_err);
		}
...
#  endif /* !ENABLE_NODEPORT && ENABLE_HOST_FIREWALL */
		...
		ret = encap_and_redirect_lxc(
			ctx, tunnel_endpoint, 0, 0, encrypt_key, &key,
			SECLABEL_IPV6, *dst_sec_identity, &trace);
		if (ret == CTX_ACT_OK)
			goto encrypt_to_stack;
...
# endif
	if (is_defined(ENABLE_HOST_ROUTING)) {
		...
		ret = fib_redirect_v6(
			ctx, ETH_HLEN, ip6, false, false, ext_err, &oif);
		if (fib_ok(ret))
			send_trace_notify(
				ctx, TRACE_TO_NETWORK, SECLABEL_IPV6,
...
# ifdef ENABLE_ROUTING
	if (is_defined(ENABLE_HOST_FIREWALL) && *dst_sec_identity == HOST_ID) {
		send_trace_notify(
			ctx, TRACE_TO_HOST, SECLABEL_IPV6, HOST_ID,
			TRACE_EP_ID_UNKNOWN, HOST_IFINDEX, trace.reason,
			trace.monitor);
...
#  ifdef ENABLE_IPSEC
	if (encrypt_key && tunnel_endpoint) {
		ret = set_ipsec_encrypt(
			ctx, encrypt_key, tunnel_endpoint, SECLABEL_IPV6, false,
			false);
		if (unlikely(ret != CTX_ACT_OK))
			return ret;
	} else
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV6_FROM_LXC_CONT) static __always_inline
	int tail_handle_ipv6_cont(struct __ctx_buff *ctx)
{
	...
	if (IS_ERR(ret))
		return send_drop_notify_ext(
			ctx, SECLABEL_IPV6, dst_sec_identity, TRACE_EP_ID_UNKNOWN,
			ret, ext_err, CTX_ACT_DROP, METRIC_EGRESS);

# ifdef ENABLE_CUSTOM_CALLS
	if (!encode_custom_prog_meta(ctx, ret, dst_sec_identity)) {
		tail_call_static(
			ctx, CUSTOM_CALLS_MAP, CUSTOM_CALLS_IDX_IPV6_EGRESS);
...
static __always_inline int
__tail_handle_ipv6(struct __ctx_buff *ctx, __s8 *ext_err __maybe_unused)
{
	void *data, *data_end;
	struct ipv6hdr *ip6;
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV6_FROM_LXC) int tail_handle_ipv6(
	struct __ctx_buff *ctx)
{
	...
	if (IS_ERR(ret))
		return send_drop_notify_error_ext(
			ctx, SECLABEL_IPV6, ret, ext_err, CTX_ACT_DROP,
			METRIC_EGRESS);
	return ret;
}
#endif /* ENABLE_IPV6 */
...
# endif /* ENABLE_L7_LB */
	...
	verdict = policy_can_egress4(
		ctx, &POLICY_MAP, tuple, l4_off, SECLABEL_IPV4, *dst_sec_identity,
		&policy_match_type, &audited, ext_err, &proxy_port);

	if (verdict == DROP_POLICY_AUTH_REQUIRED) {
		auth_type = (__u8)*ext_err;
		verdict = auth_lookup(
			ctx, SECLABEL_IPV4, *dst_sec_identity, tunnel_endpoint,
			auth_type);
	}
...
# ifdef ENABLE_EGRESS_GATEWAY_COMMON
	 ...
	 */
	ret = egress_gw_handle_packet(
		ctx, tuple, ct_status, SECLABEL_IPV4, *dst_sec_identity, &trace);
	if (ret != CTX_ACT_OK)
		return ret;
# endif
...
# if defined(ENABLE_VTEP)
	{
		...
		if (vtep->vtep_mac && vtep->tunnel_endpoint) {
			...
			return __encap_and_redirect_with_nodeid(
				ctx, 0, vtep->tunnel_endpoint, SECLABEL_IPV4,
...
#  endif
		...
		ret = encap_and_redirect_lxc(
			ctx, tunnel_endpoint, ip4->saddr, ip4->daddr, encrypt_key,
			&key, SECLABEL_IPV4, *dst_sec_identity, &trace);
...
# endif /* TUNNEL_MODE || ENABLE_HIGH_SCALE_IPCACHE */
	if (is_defined(ENABLE_HOST_ROUTING)) {
		...
		ret = fib_redirect_v4(
			ctx, ETH_HLEN, ip4, false, false, ext_err, &oif);
		if (fib_ok(ret))
			send_trace_notify(
				ctx, TRACE_TO_NETWORK, SECLABEL_IPV4,
				*dst_sec_identity, TRACE_EP_ID_UNKNOWN, oif,
...
# ifdef ENABLE_ROUTING
	if (is_defined(ENABLE_HOST_FIREWALL) && *dst_sec_identity == HOST_ID) {
		send_trace_notify(
			ctx, TRACE_TO_HOST, SECLABEL_IPV4, HOST_ID,
			TRACE_EP_ID_UNKNOWN, HOST_IFINDEX, trace.reason,
			trace.monitor);
		return ctx_redirect(ctx, HOST_IFINDEX, BPF_F_INGRESS);
	}
...
#  ifdef ENABLE_IPSEC
	if (encrypt_key && tunnel_endpoint) {
		ret = set_ipsec_encrypt(
			ctx, encrypt_key, tunnel_endpoint, SECLABEL_IPV4, false,
			false);
		if (unlikely(ret != CTX_ACT_OK))
			return ret;
	} else
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV4_FROM_LXC_CONT) static __always_inline
	int tail_handle_ipv4_cont(struct __ctx_buff *ctx)
{
	...
	if (IS_ERR(ret))
		return send_drop_notify_ext(
			ctx, SECLABEL_IPV4, dst_sec_identity, TRACE_EP_ID_UNKNOWN,
			ret, ext_err, CTX_ACT_DROP, METRIC_EGRESS);

# ifdef ENABLE_CUSTOM_CALLS
	if (!encode_custom_prog_meta(ctx, ret, dst_sec_identity)) {
		tail_call_static(
			ctx, CUSTOM_CALLS_MAP, CUSTOM_CALLS_IDX_IPV4_EGRESS);
		update_metrics(ctx_full_len(ctx), METRIC_EGRESS,
			       REASON_MISSED_CUSTOM_CALL);
	}
# endif
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV4_FROM_LXC) int tail_handle_ipv4(
	struct __ctx_buff *ctx)
{
	__s8 ext_err = 0;
	...
	if (IS_ERR(ret))
		return send_drop_notify_error_ext(
			ctx, SECLABEL_IPV4, ret, ext_err, CTX_ACT_DROP,
			METRIC_EGRESS);
	return ret;
}
...
#ifdef ENABLE_IPV6
static __always_inline int
ipv6_policy(struct __ctx_buff *ctx, struct ipv6hdr *ip6, int ifindex,
	    __u32 src_label, struct ipv6_ct_tuple *tuple_out, __s8 *ext_err,
	    __u16 *proxy_port, bool from_tunnel)
{
	struct ct_state *ct_state, ct_state_new = {};
	struct ipv6_ct_tuple *tuple;
	int ret, verdict, l4_off, zero = 0;
	...
	if (ret == CT_REPLY || ret == CT_RELATED) {
		...
		if (ct_state_is_from_l7lb(ct_state) ||
			 ...
			 */
			send_trace_notify6(
				ctx, TRACE_TO_PROXY, src_label, SECLABEL_IPV6,
				&orig_sip, TRACE_EP_ID_UNKNOWN, ifindex,
				trace.reason, trace.monitor);
			if (tuple_out)
		...
		if (unlikely(ct_state->rev_nat_index)) {
			...
			ret2 = lb6_rev_nat(
				ctx, l4_off, ct_state->rev_nat_index, tuple);
			if (IS_ERR(ret2))
				return ret2;
		}
	...
	verdict = policy_can_ingress6(
		ctx, &POLICY_MAP, tuple, l4_off, src_label, SECLABEL_IPV6,
		&policy_match_type, &audited, ext_err, proxy_port);
	if (verdict == DROP_POLICY_AUTH_REQUIRED) {
		struct remote_endpoint_info *sep =
			lookup_ip6_remote_endpoint(&orig_sip, 0);
...
skip_policy_enforcement:
	if (ret == CT_NEW) {
# ifdef ENABLE_NODEPORT
		ct_state_new.node_port = ct_has_nodeport_egress_entry6(
			get_ct_map6(tuple), tuple, NULL, false);
# endif /* ENABLE_NODEPORT */
	...
	if (*proxy_port > 0) {
		send_trace_notify6(
			ctx, TRACE_TO_PROXY, src_label, SECLABEL_IPV6,
			&orig_sip, bpf_ntohs(*proxy_port), ifindex,
			trace.reason, trace.monitor);
		if (tuple_out)
			memcpy(tuple_out, tuple, sizeof(*tuple));
		return POLICY_ACT_PROXY_REDIRECT;
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV6_TO_LXC_POLICY_ONLY) static __always_inline
...
# endif
	...
	ret = ipv6_policy(
		ctx, ip6, THIS_INTERFACE_IFINDEX, src_label, &tuple, &ext_err,
...
# ifdef ENABLE_CUSTOM_CALLS
	...
	if (!proxy_redirect && !encode_custom_prog_meta(ctx, ret, src_label)) {
		tail_call_static(
			ctx, CUSTOM_CALLS_MAP, CUSTOM_CALLS_IDX_IPV6_INGRESS);
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV6_TO_ENDPOINT) int tail_ipv6_to_endpoint(
	struct __ctx_buff *ctx)
{
...
# endif
	...
	ret = ipv6_policy(
		ctx, ip6, THIS_INTERFACE_IFINDEX, src_sec_identity, NULL,
		&ext_err, &proxy_port, false);
	switch (ret) {
	case POLICY_ACT_PROXY_REDIRECT:
...
TAIL_CT_LOOKUP6(
	...
	CT_INGRESS, __and(is_defined(ENABLE_IPV4), is_defined(ENABLE_IPV6)),
...
#ifdef ENABLE_IPV4
static __always_inline int
ipv4_policy(struct __ctx_buff *ctx, struct iphdr *ip4, int ifindex,
	    __u32 src_label, struct ipv4_ct_tuple *tuple_out, __s8 *ext_err,
	    __u16 *proxy_port, bool from_tunnel)
{
	struct ct_state *ct_state, ct_state_new = {};
	struct ipv4_ct_tuple *tuple;
...
# endif
	...
	if (ret == CT_REPLY || ret == CT_RELATED) {
		if (ct_state_is_from_l7lb(ct_state) ||
			 ...
			 */
			send_trace_notify4(
				ctx, TRACE_TO_PROXY, src_label, SECLABEL_IPV4,
				orig_sip, TRACE_EP_ID_UNKNOWN, ifindex,
				trace.reason, trace.monitor);
			if (tuple_out)
				*tuple_out = *tuple;
		...
		if (unlikely(ct_state->rev_nat_index)) {
			...
			ret2 = lb4_rev_nat(
				...
				ct_state->loopback, tuple, has_l4_header);
			if (IS_ERR(ret2))
				return ret2;
		}
...
# if defined(ENABLE_PER_PACKET_LB) && !defined(DISABLE_LOOPBACK_LB)
	...
	if (ret == CT_NEW && ip4->saddr == IPV4_LOOPBACK &&
	    ct_has_loopback_egress_entry4(
		    get_ct_map4(tuple), tuple, &ct_state_new.rev_nat_index)) {
...
# endif /* ENABLE_PER_PACKET_LB && !DISABLE_LOOPBACK_LB */
	...
	verdict = policy_can_ingress4(
		ctx, &POLICY_MAP, tuple, l4_off, is_untracked_fragment, src_label,
		SECLABEL_IPV4, &policy_match_type, &audited, ext_err, proxy_port);
	if (verdict == DROP_POLICY_AUTH_REQUIRED) {
		struct remote_endpoint_info *sep =
			lookup_ip4_remote_endpoint(orig_sip, 0);
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV4_TO_LXC_POLICY_ONLY) static __always_inline
...
# endif
	...
	ret = ipv4_policy(
		ctx, ip4, THIS_INTERFACE_IFINDEX, src_label, &tuple, &ext_err,
		&proxy_port, from_tunnel);
	switch (ret) {
	case POLICY_ACT_PROXY_REDIRECT:
...
# ifdef ENABLE_CUSTOM_CALLS
	...
	if (!proxy_redirect && !encode_custom_prog_meta(ctx, ret, src_label)) {
		tail_call_static(
			ctx, CUSTOM_CALLS_MAP, CUSTOM_CALLS_IDX_IPV4_INGRESS);
		update_metrics(ctx_full_len(ctx), METRIC_INGRESS,
			       REASON_MISSED_CUSTOM_CALL);
	}
# endif
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV4_TO_ENDPOINT) int tail_ipv4_to_endpoint(
	struct __ctx_buff *ctx)
{
	__u32 src_sec_identity = ctx_load_and_clear_meta(ctx, CB_SRC_LABEL);
	bool proxy_redirect __maybe_unused = false;
...
# endif
	...
	ret = ipv4_policy(
		ctx, ip4, THIS_INTERFACE_IFINDEX, src_sec_identity, NULL,
		&ext_err, &proxy_port, false);
	switch (ret) {
	case POLICY_ACT_PROXY_REDIRECT:
		if (!revalidate_data(ctx, &data, &data_end, &ip4)) {
 ...
 */
__section_tail(CILIUM_MAP_POLICY, TEMPLATE_LXC_ID) int handle_policy(
	struct __ctx_buff *ctx)
{
 ...
 */
#if defined(ENABLE_L7_LB)
__section_tail(CILIUM_MAP_EGRESSPOLICY, TEMPLATE_LXC_ID) int handle_policy_egress(
	struct __ctx_buff *ctx)
{
	__u16 proto;
	int ret;
	...
	send_trace_notify(
		ctx, TRACE_FROM_PROXY, SECLABEL, UNKNOWN_ID, TRACE_EP_ID_UNKNOWN,
		TRACE_IFINDEX_UNKNOWN, TRACE_REASON_UNKNOWN, TRACE_PAYLOAD_LEN);
...
__section_entry int cil_to_container(struct __ctx_buff *ctx)
{
	...
	if (magic == MARK_MAGIC_PROXY_INGRESS || magic == MARK_MAGIC_PROXY_EGRESS)
		...
		return send_drop_notify(
			ctx, identity, sec_label, LXC_ID, ret, CTX_ACT_DROP,
			METRIC_INGRESS);
	}
#endif
