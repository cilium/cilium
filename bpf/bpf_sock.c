...
static __maybe_unused __maybe_unused bool is_v4_loopback(__be32 daddr)
{
	/* Check for 127.0.0.0/8 range, RFC3330. */
	return (daddr & bpf_htonl(0xff000000)) == bpf_htonl(0x7f000000);
}

static __maybe_unused __maybe_unused bool is_v6_loopback(const union v6addr *daddr)
{
...
static __maybe_unused __maybe_unused __be16
...
static __maybe_unused __maybe_unused bool task_in_extended_hostns(void)
{
#ifdef ENABLE_MKE
...
static __maybe_unused int sock4_update_revnat(struct bpf_sock_addr *ctx,
...
static __maybe_unused struct lb4_service *
...
static __maybe_unused int __sock4_xlate_fwd(struct bpf_sock_addr *ctx,
...
#ifdef ENABLE_NODEPORT
static __maybe_unused int __sock4_post_bind(struct bpf_sock *ctx,
...
#ifdef ENABLE_HEALTH_CHECK
static __maybe_unused void sock4_auto_bind(struct bpf_sock_addr *ctx)
{
	ctx->user_ip4 = 0;
	ctx_set_port(ctx, 0);
}

static __maybe_unused int __sock4_pre_bind(struct bpf_sock_addr *ctx,
					    struct bpf_sock_addr *ctx_full)
{
...
static __maybe_unused int __sock4_xlate_rev(struct bpf_sock_addr *ctx,
					     struct bpf_sock_addr *ctx_full)
{
...
static __maybe_unused int sock6_update_revnat(struct bpf_sock_addr *ctx,
...
static __maybe_unused void ctx_get_v6_address(const struct bpf_sock_addr *ctx,
					       union v6addr *addr);
...
#ifdef ENABLE_NODEPORT
static __maybe_unused void ctx_get_v6_src_address(const struct bpf_sock *ctx,
						   union v6addr *addr)
{
...
static __maybe_unused void ctx_set_v6_address(struct bpf_sock_addr *ctx,
...
static __maybe_unused __maybe_unused struct lb6_service *
...
static __maybe_unused int __sock6_post_bind(struct bpf_sock *ctx)
{
	struct lb6_service *svc;
	struct lb6_key key = {
		.dport		= ctx_src_port(ctx),
...
#ifdef ENABLE_IPV6
static __maybe_unused void sock6_auto_bind(struct bpf_sock_addr *ctx)
{
	union v6addr zero = {};
...
static __maybe_unused int __sock6_pre_bind(struct bpf_sock_addr *ctx)
{
...
static __maybe_unused int __sock6_xlate_fwd(struct bpf_sock_addr *ctx,
					     const bool udp_only)
{
#ifdef ENABLE_IPV6
...
static __maybe_unused int __sock6_xlate_rev(struct bpf_sock_addr *ctx)
{
#ifdef ENABLE_IPV6
	struct ipv6_revnat_tuple key = {};
