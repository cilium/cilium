					     ...
					     const bool udp_only)
{
	...
	const bool in_hostns = ctx_in_hostns(ctx_full, &id.client_netns_cookie);
	 ...
	 * for the same service:port.
	 * lb4_update_affinity_by_{netns,sock}() below would then
	 * override the first created one if it didn't make it into
	 * the lookup yet for the other CPU.
	 */
	if (lb4_svc_is_affinity(svc)) {
		 ...
		 */
		id.client_sock_cookie = sock_local_cookie(ctx_full);
					     ...
					     const bool udp_only)
{
	 ...
	 * small race window. Two processes on two different
	 * CPUs but the same netns may select different backends
	 * for the same service:port. lb6_update_affinity_by_*()
