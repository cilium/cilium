...
static __always_inline int sock4_update_revnat(struct bpf_sock_addr *ctx,
					       ...
					       __u16 rev_nat_id, bool is_connect)
{
	...
	if (ctx->sk && is_connect) {
		struct ipv4_revnat_entry *sk_val =
			sk_storage_get(&cilium_lb4_reverse_sk_st, ctx->sk, &val,
				       BPF_SK_STORAGE_GET_F_CREATE);

		if (sk_val && memcmp(sk_val, &val, sizeof(val))) {
			sk_val->address = val.address;
			sk_val->port = val.port;
			sk_val->rev_nat_index = val.rev_nat_index;
		}
	}
					     ...
					     struct bpf_sock_addr *ctx_full)
{
	...
	if (val) {
		...
		if (!svc || svc->rev_nat_index != val->rev_nat_index ||
			...
			if (ctx_full->sk)
				sk_storage_delete(&cilium_lb4_reverse_sk_st, ctx_full->sk);
...
static __always_inline int sock6_update_revnat(struct bpf_sock_addr *ctx,
					       ...
					       __be16 dst_port,
					       __u16 rev_nat_index, bool is_connect)
{
	...
	if (ctx->sk && is_connect) {
		struct ipv6_revnat_entry *sk_val =
			sk_storage_get(&cilium_lb6_reverse_sk_st, ctx->sk, &val,
				       BPF_SK_STORAGE_GET_F_CREATE);
...
static __always_inline int __sock6_xlate_rev(struct bpf_sock_addr *ctx)
{
	...
	val = NULL;
	if (ctx->sk)
		val = sk_storage_get(&cilium_lb6_reverse_sk_st, ctx->sk, 0, 0);
	if (!val)
	...
	if (val) {
		...
		if (!svc || svc->rev_nat_index != val->rev_nat_index ||
			...
			if (ctx->sk)
				sk_storage_delete(&cilium_lb6_reverse_sk_st, ctx->sk);
			update_metrics(0, METRIC_INGRESS, REASON_LB_REVNAT_STALE);
			return -ENOENT;
		}
