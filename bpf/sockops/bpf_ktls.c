...
int bpf_redir_ktls(struct sk_msg_md *msg)
{
	...
	__u64 flags = 0;//BPF_F_INGRESS;
	__u32 dip4, dport;
	int err;

	if (msg->data + sizeof(struct sock_key) > msg->data_end) {
		bpf_printk("ktls redir abort no key\n");
		return SK_DROP;
	}
	...
	bpf_printk("ktls key: %d %d %d\n", key.sport, key.dport, key.family);
	bpf_printk("ktls pad: %d %d\n", key.pad7, key.pad8);
	bpf_printk("ktls key: %d %d\n", key.sip4, key.dip4);
	bpf_printk("ktls redirect %d\n", err);
	return err;
}
