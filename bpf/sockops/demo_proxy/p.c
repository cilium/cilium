...
int proxy_glue(int cfd, int pfd, int recv, struct msghdr *msg, char *type, struct byte_stats *stats, bool *more)
{
...
int main(int argc, char **argv)
{
	...
	while (running) {
		...
		if (ready) {
			...
			do {
				cfd_recv = proxy_glue(cfd, pfd, cfd_recv, &msg, "downstream", &down, &more);
				if (cfd_recv < 0)
					goto out;
			} while (more);
		}
		...
		if (ready) {
			...
			do {
				pfd_recv = proxy_glue(pfd, cfd, pfd_recv, &msg, "upstream", &up, &more); 
				if (pfd_recv < 0)
					goto out;
			} while (more);
		}
