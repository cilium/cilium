...
int bpf_redir_proxy(struct sk_msg_md *msg)
{
	...
	bpf_printk("skmsg ipv4: redir . (IP)  sip4 %u dip4 %u family %u\n", key.sip4, key.dip4, key.family);
	bpf_printk("skmsg ipv4: redir . (TCP) dport %u sport %u\n", key.dport, key.sport);
	...
	if (redirect_to_proxy(verdict)) {
		...
		if (map_update_elem(&cilium_proxy4, &key, &value, 0) < 0) {
			bpf_printk("skmsg error ipv4 update proxy map: (TCP) dport %u dip %u\n", key.dport, key.dip4);
			return SK_PASS;
		}

		bpf_printk("skmsg ipv4 redirect: (TCP) dport %u dip %u\n", key.dport, key.dip4);
		err = msg_redirect_hash(msg, &SOCK_OPS_MAP, &key, flags);
	} else if (!verdict) {
		bpf_printk("skmsg ipv4 redirect: not proxy: (TCP) dport %u dip %u\n", key.dport, key.dip4);
