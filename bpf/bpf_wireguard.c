...
ipv6_host_delivery(struct __ctx_buff *ctx, struct ipv6hdr *ip6 __maybe_unused)
{
...
handle_ipv6(struct __ctx_buff *ctx, __u32 identity, __s8 *ext_err __maybe_unused)
{
	...
	return ipv6_host_delivery(ctx, ip6);
}
...
out:
	if (IS_ERR(ret))
		return send_drop_notify_error_ext(ctx, identity, ret, ext_err, METRIC_INGRESS);
	return ret;
}
