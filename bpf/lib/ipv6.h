...
static __maybe_unused int ipv6_optlen(const struct ipv6_opt_hdr *opthdr)
{
	return (opthdr->hdrlen + 1) << 3;
}

static __maybe_unused int ipv6_authlen(const struct ipv6_opt_hdr *opthdr)
{
	return (opthdr->hdrlen + 2) << 2;
}

static __maybe_unused int ipv6_hdrlen_offset(struct __ctx_buff *ctx, __u8 *nexthdr, int l3_off)
{
...
static __maybe_unused int ipv6_hdrlen(struct __ctx_buff *ctx, __u8 *nexthdr)
{
	return ipv6_hdrlen_offset(ctx, nexthdr, ETH_HLEN);
}

static __maybe_unused void ipv6_addr_copy(union v6addr *dst,
					   const union v6addr *src)
{
	memcpy(dst, src, sizeof(*dst));
}

static __maybe_unused void ipv6_addr_copy_unaligned(union v6addr *dst,
						     const union v6addr *src)
{
	dst->d1 = src->d1;
	dst->d2 = src->d2;
...
static __maybe_unused bool ipv6_addr_equals(const union v6addr *a,
...
static __maybe_unused int ipv6_addr_in_net(const union v6addr *addr,
...
static __maybe_unused void ipv6_addr_clear_suffix(union v6addr *addr,
						   int prefix)
{
...
static __maybe_unused int ipv6_dec_hoplimit(struct __ctx_buff *ctx, int off)
{
	__u8 hl;
...
static __maybe_unused int ipv6_load_saddr(struct __ctx_buff *ctx, int off,
					   union v6addr *dst)
{
...
static __maybe_unused int ipv6_store_saddr(struct __ctx_buff *ctx, __u8 *addr,
					    int off)
{
	return ctx_store_bytes(ctx, off + offsetof(struct ipv6hdr, saddr), addr, 16, 0);
}

static __maybe_unused int ipv6_load_daddr(struct __ctx_buff *ctx, int off,
					   union v6addr *dst)
{
...
static __maybe_unused int ipv6_load_nexthdr(struct __ctx_buff *ctx, int off,
					     __u8 *nexthdr)
{
...
/* Assumes that caller fixes checksum csum_diff() and l4_csum_replace() */
static __maybe_unused int ipv6_store_nexthdr(struct __ctx_buff *ctx, __u8 *nexthdr,
					      int off)
{
...
static __maybe_unused int ipv6_load_paylen(struct __ctx_buff *ctx, int off,
					    __be16 *len)
{
...
/* Assumes that caller fixes checksum csum_diff() and l4_csum_replace() */
static __maybe_unused int ipv6_store_paylen(struct __ctx_buff *ctx, int off,
					     __be16 *len)
{
...
static __maybe_unused __be32 ipv6_pseudohdr_checksum(struct ipv6hdr *hdr,
						      __u8 next_hdr,
 ...
 */
static __maybe_unused int ipv6_addr_is_mapped(const union v6addr *addr)
{
