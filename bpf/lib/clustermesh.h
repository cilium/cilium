...
static __always_inline __u32 get_identity_len(void)
{
...
static __always_inline __u32 extract_cluster_id_from_identity(__u32 identity)
{
	return (__u32)(identity >> IDENTITY_LEN);
}

static __always_inline __u32 get_max_identity(void)
{
	return (__u32)((1 << IDENTITY_LEN) - 1);
}

static __always_inline __maybe_unused __u32 get_cluster_id_upper_mask(void)
{
	return (CLUSTER_ID_MAX & ~CLUSTER_ID_LOWER_MASK) << (8 + IDENTITY_LEN);
}

static __always_inline __maybe_unused __u32 get_mark_magic_cluster_id_mask(void)
{
