...
/* Deny precedence bit in 'precedence' */
#define PRECEDENCE_DENY (1 << 7)
/* Mask for deny/allow bit and proxy port precedence */
#define PRECEDENCE_ALLOW_MASK 0xFF
/* ALLOW_PRECEDENCE masks away the impact of redirect (priority) on the precedence */
#define ALLOW_PRECEDENCE(POLICY) ((POLICY)->precedence & ~PRECEDENCE_ALLOW_MASK)
	       ...
	       __u16 *proxy_port, __u32 *cookie)
{
	...
	*proxy_port = policy->proxy_port;
	...
	if (unlikely(policy2 && policy2->auth_type > auth_type &&
		     ...
		     ALLOW_PRECEDENCE(policy2) >= ALLOW_PRECEDENCE(policy))) {
		/* Both L4-only and L3/4 policy matched, the chosen more specific one does not have
		 * an explicit auth type and the precedence (apart from the deny/allow bits) of the
		 * chosen policy is the same or lower of the other policy: Propagate the auth type
		 * from the more general policy if its (explicit or propagated) auth_type is greater
		 * than the propagated auth_type of the chosen policy (policy entry may have an
		 * auth_type propagated from another entry with en explicit auth type. Numerically
		 * greater value has precedence in that case).
		 */
		    ...
		    __u16 *proxy_port, __u32 *cookie)
{
	 ...
	 * 2. If either entry is deny it is selected.
	 ...
	 */
	if (likely(policy && ~policy->precedence == 0)) {
		l4policy = NULL;
		goto check_policy;
	}
