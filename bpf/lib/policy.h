...
# endif
	 ...
	 * Entries with longer prefix lengths get precedence. If there is
	 * both an L4-only and L3/4 match and one of them is a deny entry, then precedence is given
	 * to the deny entry. Otherwise when both entries have the same prefix length, then the one
	 * with non-wildcard L3 is chosen.
	 */
