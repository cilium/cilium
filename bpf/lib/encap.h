...
static __always_inline int __encap_with_nodeid(
	struct __ctx_buff *ctx, __u32 src_ip, __be16 src_port,
	__be32 tunnel_endpoint, __u32 seclabel, __u32 dstid,
	__u32 vni __maybe_unused, enum trace_reason ct_reason, __u32 monitor,
	int *ifindex)
{
	__u32 node_id;
...
# endif
	...
	send_trace_notify(
		ctx, TRACE_TO_OVERLAY, seclabel, dstid, TRACE_EP_ID_UNKNOWN,
		*ifindex, ct_reason, monitor);

	return ctx_set_encap_info(
		ctx, src_ip, src_port, node_id, seclabel, vni, NULL, 0);
}

static __always_inline int __encap_and_redirect_with_nodeid(
	struct __ctx_buff *ctx, __u32 src_ip __maybe_unused, __be32 tunnel_endpoint,
	__u32 seclabel, __u32 dstid, __u32 vni, const struct trace_ctx *trace)
{
	int ifindex;
	int ret = 0;

	ret = __encap_with_nodeid(
		ctx, src_ip, 0, tunnel_endpoint, seclabel, dstid, vni,
 ...
 */
static __always_inline int encap_and_redirect_with_nodeid(
	struct __ctx_buff *ctx, __be32 tunnel_endpoint,
	__u8 encrypt_key __maybe_unused, __u32 seclabel, __u32 dstid,
	const struct trace_ctx *trace)
{
# ifdef ENABLE_IPSEC
	if (encrypt_key)
		return set_ipsec_encrypt(
			ctx, encrypt_key, tunnel_endpoint, seclabel, true, false);
# endif

	return __encap_and_redirect_with_nodeid(
		ctx, 0, tunnel_endpoint, seclabel, dstid, NOT_VTEP_DST, trace);
}

/* __encap_and_redirect_lxc() is a variant of encap_and_redirect_lxc()
 * that requires a valid tunnel_endpoint.
 */
static __always_inline int __encap_and_redirect_lxc(
	struct __ctx_buff *ctx, __be32 tunnel_endpoint,
	__u8 encrypt_key __maybe_unused, __u32 seclabel, __u32 dstid,
	const struct trace_ctx *trace)
{
...
# ifdef ENABLE_IPSEC
	if (encrypt_key)
		return set_ipsec_encrypt(
			ctx, encrypt_key, tunnel_endpoint, seclabel, false, false);
# endif

	return encap_and_redirect_with_nodeid(
		ctx, tunnel_endpoint, 0, seclabel, dstid, trace);
}
 ...
 * CTX_ACT_REDIRECT.
 */
static __always_inline int encap_and_redirect_lxc(
	struct __ctx_buff *ctx, __be32 tunnel_endpoint __maybe_unused,
	__u32 src_ip __maybe_unused, __u32 dst_ip __maybe_unused,
	__u8 encrypt_key __maybe_unused, struct tunnel_key *key __maybe_unused,
...
#  ifdef ENABLE_HIGH_SCALE_IPCACHE
	if (needs_encapsulation(dst_ip))
		return __encap_and_redirect_with_nodeid(
			ctx, src_ip, dst_ip, seclabel, dstid, NOT_VTEP_DST, trace);
	return DROP_NO_TUNNEL_ENDPOINT;
#  else /* ENABLE_HIGH_SCALE_IPCACHE */
	if (tunnel_endpoint)
		return __encap_and_redirect_lxc(
			ctx, tunnel_endpoint, encrypt_key, seclabel, dstid, trace);
...
#   ifdef ENABLE_IPSEC
	if (tunnel->key) {
		...
		return set_ipsec_encrypt(
			ctx, min_encrypt_key, tunnel->ip4, seclabel, false, false);
	}
#   endif
	return encap_and_redirect_with_nodeid(
		ctx, tunnel->ip4, 0, seclabel, dstid, trace);
#  endif /* ENABLE_HIGH_SCALE_IPCACHE */
}

static __always_inline int encap_and_redirect_netdev(
	struct __ctx_buff *ctx, struct tunnel_key *k,
...
#  ifdef ENABLE_IPSEC
	if (encrypt_key)
		return set_ipsec_encrypt(
			ctx, encrypt_key, tunnel->ip4, seclabel, true, false);
#  endif

	return encap_and_redirect_with_nodeid(
		ctx, tunnel->ip4, 0, seclabel, 0, trace);
}
...
static __always_inline int __encap_with_nodeid_opt(
	struct __ctx_buff *ctx, __u32 src_ip, __be16 src_port,
	__u32 tunnel_endpoint, __u32 seclabel, __u32 dstid, __u32 vni, void *opt,
...
#  endif
	...
	return ctx_set_encap_info(
		ctx, src_ip, src_port, node_id, seclabel, vni, opt, opt_len);
}
...
static __always_inline void set_geneve_dsr_opt6(
	__be16 port, const union v6addr *addr, struct geneve_dsr_opt6 *gopt)
{
	memset(gopt, 0, sizeof(*gopt));
	gopt->hdr.opt_class = bpf_htons(DSR_GENEVE_OPT_CLASS);
	gopt->hdr.type = DSR_GENEVE_OPT_TYPE;
