 ...
 */
static __maybe_unused void *mcast_lookup_subscriber_map(__be32 *group)
{
...
static __maybe_unused bool mcast_ipv4_is_igmp(const struct iphdr *ip4)
{
 ...
 */
static __maybe_unused __s32 mcast_ipv4_igmp_type(const struct iphdr *ip4,
						  const void *data,
						  const void *data_end)
{
...
static __maybe_unused __s32 mcast_ipv4_add_subscriber(void *map,
...
/* always returns 1 */
static __maybe_unused void mcast_ipv4_remove_subscriber(void *map,
							 struct mcast_subscriber_v4 *sub)
{
	map_delete_elem(map, &sub->saddr);
}

static __maybe_unused __s32 mcast_ipv4_handle_v3_membership_report(void *ctx,
								    void *group_map,
...
static __maybe_unused __s32 mcast_ipv4_handle_v2_membership_report(void *ctx,
								    void *group_map,
								    const struct iphdr *ip4,
								    const void *data,
								    const void *data_end)
...
static __maybe_unused __s32 mcast_ipv4_handle_igmp_leave(void *group_map,
							  const struct iphdr *ip4,
							  const void *data,
							  const void *data_end)
{
...
static __maybe_unused __s32 mcast_ipv4_handle_igmp(void *ctx,
						    struct iphdr *ip4,
						    void *data,
						    void *data_end)
{
 ...
 */
static __maybe_unused void mcast_encode_ipv4_mac(union macaddr *mac,
						  const __u8 group[4])
{
	mac->addr[0] = 0x01;
	mac->addr[1] = 0x00;
