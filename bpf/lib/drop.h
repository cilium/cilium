...
struct drop_notify {
	...
	__s8		ext_error;
	__u32		ifindex;
	__u8		flags; /* __u8 instead of cls_t so that it will error when
				* cls_t grows (we'll need to either borrow bits from pad2
				* or store it into flags_lower/flags_upper).
				*/
	__u8		pad2[3];
};
...
int __send_drop_notify(struct __ctx_buff *ctx)
{
	...
	__u64 cap_len = min_t(__u64, ctx_default_monitor_from_classifiers(flags), ctx_len);
...
#define __DROP_REASON_EXT_FLAGS(err, ext_err, flags) ({ \
	__u8 __flags = (cls_t)(flags); \
	__DROP_REASON_EXT(err, ext_err) | (__flags << 16); \
})
