			   ...
			   __s8 *ext_err)
{
	...
	*state = __snat_lookup(map, tuple);
	...
	if (*state) {
		...
		/* Recreate the SNAT and RevSNAT entries if any of:
		 * - Source IP is stale
		 * - RevSNAT entry is missing
		 * Otherwise, the packet will be erroneously SNATed with the stale source IP or dropped.
		 */
