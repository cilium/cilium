...
static __maybe_unused int icmp6_load_type(struct __ctx_buff *ctx, int l4_off, __u8 *type)
{
	return ctx_load_bytes(ctx, l4_off + ICMP6_TYPE_OFFSET, type, sizeof(*type));
}

static __maybe_unused int icmp6_send_reply(struct __ctx_buff *ctx, int nh_off)
{
...
static __maybe_unused __be32 compute_icmp6_csum(char data[80], __u16 payload_len,
...
static __maybe_unused int __icmp6_send_time_exceeded(struct __ctx_buff *ctx,
						      int nh_off)
{
 ...
 */
static __maybe_unused int icmp6_send_time_exceeded(struct __ctx_buff *ctx,
...
static __maybe_unused int __icmp6_handle_ns(struct __ctx_buff *ctx, int nh_off)
{
 ...
 * NOTE: This is terminal function and will cause the BPF program to exit
 */
static __maybe_unused int icmp6_handle_ns(struct __ctx_buff *ctx, int nh_off,
...
static __maybe_unused int icmp6_ndp_handle(struct __ctx_buff *ctx, int nh_off,
					    enum metric_dir direction,
					    __s8 *ext_err)
{
	__u8 type;
