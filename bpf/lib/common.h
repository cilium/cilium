...
validate_ethertype_l2_off(struct __ctx_buff *ctx, int l2_off, __u16 *proto)
{
...
static __always_inline bool
validate_ethertype(struct __ctx_buff *ctx, __u16 *proto)
{
	return validate_ethertype_l2_off(ctx, 0, proto);
}

static __always_inline __maybe_unused bool ____revalidate_data_pull(
	struct __ctx_buff *ctx, void **data_, void **data_end_, void **l3,
	const __u32 l3_len, const bool pull, __u32 l3_off)
{
...
static __always_inline __maybe_unused bool __revalidate_data_pull(
	struct __ctx_buff *ctx, void **data, void **data_end, void **l3,
	const __u32 l3_off, const __u32 l3_len, const bool pull)
{
	return ____revalidate_data_pull(
		ctx, data, data_end, l3, l3_len, pull, l3_off);
}
...
get_id_from_tunnel_id(__u32 tunnel_id, __u16 proto __maybe_unused)
{
#if defined ENABLE_IPV4 && defined ENABLE_IPV6
...
#define revalidate_data_pull(ctx, data, data_end, ip) \
	__revalidate_data_pull(                       \
		ctx, data, data_end, (void **)ip, ETH_HLEN, sizeof(**ip), true)

#define revalidate_data_l3_off(ctx, data, data_end, ip, l3_off) \
	__revalidate_data_pull(                                 \
		ctx, data, data_end, (void **)ip, l3_off, sizeof(**ip), false)
...
struct remote_endpoint_info {
	...
	__u16 pad;
	__u8 key;
	__u8 flag_skip_tunnel:1, pad2:7;
};
...
struct policy_entry {
	__be16 proxy_port;
	__u8 deny:1, wildcard_protocol:1, /* protocol is fully wildcarded */
...
#define __notify_common_hdr(t, s)                            \
	.type = (t), .subtype = (s), .source = EVENT_SOURCE, \
	.hash = get_hash_recalc(ctx)

#define __notify_pktcap_hdr(o, c) \
	.len_orig = (o), .len_cap = (c), .version = NOTIFY_CAPTURE_VER
...
#define DROP_NAT_NOT_NEEDED \
	-173 /* Mapped as drop code, though drop not necessary. */
...
#define MARK_MAGIC_PROXY_EGRESS_EPID \
	0x0900 /* mark carries source endpoint ID */
...
#define CB_ENCRYPT_IDENTITY \
	CB_CT_STATE /* Alias, non-overlapping,
						 * Not used by xfrm.
						 */
#define CB_ENCRYPT_DST \
	CB_CT_STATE		    /* Alias, non-overlapping,
						 * Not used by xfrm.
...
enum {
	...
	SVC_FLAG_L7LOADBALANCER =
		(1 << 2), /* tproxy redirect to local l7 loadbalancer */
	SVC_FLAG_LOOPBACK = (1 << 3), /* hostport with a loopback hostIP */
	SVC_FLAG_INT_LOCAL_SCOPE = (1 << 4), /* internalTrafficPolicy=Local */
	SVC_FLAG_TWO_SCOPES =
		(1 << 5), /* two sets of backends are used for external/internal connections */
};
...
struct ct_entry {
	...
	__u64 bytes;
	__u32 lifetime;
	__u16 rx_closing:1, tx_closing:1, reserved1:1, /* unused since v1.12 */
		lb_loopback:1, seen_non_syn:1, node_port:1,
...
static __always_inline bool
ct_state_is_from_l7lb(const struct ct_state *ct_state __maybe_unused)
{
#ifdef ENABLE_L7_LB
...
redirect_ep(struct __ctx_buff *ctx __maybe_unused, int ifindex __maybe_unused,
	    bool needs_backlog __maybe_unused, bool from_tunnel)
{
