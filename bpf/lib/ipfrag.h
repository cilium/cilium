 ...
 * and pass the __s64 bitmask around.
 *
...
static __always_inline __s64 ipfrag_encode_ipv4(const struct iphdr *ip4)
{
	__s64 fraginfo = (__u16)ip4->id; /* Store in network byte order. */
...
/* This function doesn't return errors. */
static __always_inline __s64 ipfrag_encode_ipv6(const struct ipv6_frag_hdr *exthdr)
{
	__s64 fraginfo = (__u32)exthdr->id; /* Store in network byte order. */
...
static __always_inline bool ipfrag_is_fragment(__s64 fraginfo)
{
	return fraginfo & IPFRAG_BIT_FRAGMENTED;
}

static __always_inline bool ipfrag_has_l4_header(__s64 fraginfo)
{
	return !(fraginfo & IPFRAG_BIT_NO_L4_HEADER);
}

static __always_inline __u8 ipfrag_get_protocol(__s64 fraginfo)
{
	return (fraginfo >> 32) & 0xf;
}

/* Downcast to __be16 for IPv4. */
static __always_inline __be32 ipfrag_get_id(__s64 fraginfo)
{
