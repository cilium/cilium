...
static __always_inline void *get_cluster_ct_map6(
	const struct ipv6_ct_tuple *tuple, __u32 cluster_id __maybe_unused)
{
#  ifdef ENABLE_CLUSTER_AWARE_ADDRESSING
...
static __always_inline void *get_cluster_ct_map4(
	const struct ipv4_ct_tuple *tuple, __u32 cluster_id __maybe_unused)
{
#  ifdef ENABLE_CLUSTER_AWARE_ADDRESSING
	if (cluster_id != 0 && cluster_id != CLUSTER_ID) {
		if (tuple->nexthdr == IPPROTO_TCP)
