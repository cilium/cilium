...
get_cluster_ct_map6(const struct ipv6_ct_tuple *tuple, __u32 cluster_id __maybe_unused)
{
	...
	if (cluster_id != 0 && cluster_id != CLUSTER_ID) {
		if (tuple->nexthdr == IPPROTO_TCP)
			return bpf_map_lookup_elem(&PER_CLUSTER_CT_TCP6, &cluster_id);

		return bpf_map_lookup_elem(&PER_CLUSTER_CT_ANY6, &cluster_id);
	}
#endif
	if (tuple->nexthdr == IPPROTO_TCP)
		return &CT_MAP_TCP6;
...
get_cluster_ct_map4(const struct ipv4_ct_tuple *tuple, __u32 cluster_id __maybe_unused)
{
	...
	if (cluster_id != 0 && cluster_id != CLUSTER_ID) {
		if (tuple->nexthdr == IPPROTO_TCP)
			return bpf_map_lookup_elem(&PER_CLUSTER_CT_TCP4, &cluster_id);

		return bpf_map_lookup_elem(&PER_CLUSTER_CT_ANY4, &cluster_id);
	}
#endif
	if (tuple->nexthdr == IPPROTO_TCP)
		return &CT_MAP_TCP4;
