...
/*
 * cilium_config is a global pinned array containing runtime configuration
 * information for the datapath. It is shared across all Cilium BPF programs on
 * the node.
 *
...
} cilium_config __section_maps_btf;
...
{
	__u64 *val = map_lookup_elem(&cilium_config, &index);
...
{
	map_update_elem(&cilium_config, &index, &value, BPF_ANY);
}
