...
wg_maybe_redirect_to_encrypt(struct __ctx_buff *ctx)
{
	...
	case bpf_htons(ETH_P_IPV6):
		if (!revalidate_data(ctx, &data, &data_end, &ip6))
			return DROP_INVALID;
		dst = lookup_ip6_remote_endpoint((union v6addr *)&ip6->daddr);
#ifndef ENABLE_NODE_ENCRYPTION
		src = lookup_ip6_remote_endpoint((union v6addr *)&ip6->saddr);
	...
	case bpf_htons(ETH_P_IP):
		if (!revalidate_data(ctx, &data, &data_end, &ip4))
			return DROP_INVALID;
		dst = lookup_ip4_remote_endpoint(ip4->daddr);
#ifndef ENABLE_NODE_ENCRYPTION
		src = lookup_ip4_remote_endpoint(ip4->saddr);
#endif /* ENABLE_NODE_ENCRYPTION */
		break;
#endif
	default:
	...
	if (((ctx->mark & MARK_MAGIC_WG_ENCRYPTED) !=
	     MARK_MAGIC_WG_ENCRYPTED) && (dst && dst->key)
#ifndef ENABLE_NODE_ENCRYPTION
	     /* host2pod */ && (src && src->sec_label != HOST_ID)
#endif /* ENABLE_NODE_ENCRYPTION */
	)
