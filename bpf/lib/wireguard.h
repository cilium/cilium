...
#  ifdef ENABLE_NODE_ENCRYPTION
		...
		if (ip6->nexthdr == IPPROTO_ICMPV6) {
			...
			if (icmp6_load_type(
				    ctx, ETH_HLEN + sizeof(struct ipv6hdr),
				    &icmp_type) < 0)
				return DROP_INVALID;
...
maybe_encrypt:
	...
	{
encrypt:
		__maybe_unused return ctx_redirect(ctx, WG_IFINDEX, 0);
	}
...
static __always_inline bool strict_allow(struct __ctx_buff *ctx, __be16 proto)
{
	struct remote_endpoint_info __maybe_unused *dest_info,
		__maybe_unused *src_info;
...
#  ifdef ENABLE_IPV4
	case bpf_htons(ETH_P_IP):
		...
		in_strict_cidr = ipv4_is_in_subnet(
			ip4->daddr, STRICT_IPV4_NET, STRICT_IPV4_NET_SIZE);
		in_strict_cidr &= ipv4_is_in_subnet(
			ip4->saddr, STRICT_IPV4_NET, STRICT_IPV4_NET_SIZE);
