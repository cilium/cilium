...
#define bpf_ktime_get_sec()                                \
	({                                                 \
		__u64 __x = ktime_get_ns() / NSEC_PER_SEC; \
		__x;                                       \
	})
#define bpf_ktime_get_msec()                                \
	({                                                  \
		__u64 __x = ktime_get_ns() / NSEC_PER_MSEC; \
		__x;                                        \
	})
#define bpf_ktime_get_usec()                                \
	({                                                  \
		__u64 __x = ktime_get_ns() / NSEC_PER_USEC; \
		__x;                                        \
	})
#define bpf_ktime_get_nsec()                \
	({                                  \
		__u64 __x = ktime_get_ns(); \
		__x;                        \
	})
...
#define bpf_jiffies_to_sec(j)                \
	({                                   \
		__u64 __x = (j) / KERNEL_HZ; \
		__x;                         \
	})
#define bpf_sec_to_jiffies(s)                \
	({                                   \
		__u64 __x = (s) * KERNEL_HZ; \
		__x;                         \
	})
