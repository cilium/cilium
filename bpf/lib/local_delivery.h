...
static __always_inline bool should_fast_redirect_egress(struct __ctx_buff *ctx, bool from_host)
{
...
static __always_inline int redirect_ep(struct __ctx_buff *ctx,
				       int ifindex,
				       bool use_fast_redirect_egress,
				       bool from_tunnel)
{
	if (!use_fast_redirect_egress)
 ...
 */
	use_fast_redirect_egress = should_fast_redirect_egress(ctx, from_host);
	if (is_defined(USE_BPF_PROG_FOR_INGRESS_POLICY) && !use_fast_redirect_egress) {
...
# endif /* !ENABLE_NODEPORT */
		...
		return redirect_ep(ctx, ep->ifindex, use_fast_redirect_egress, from_tunnel);
	}
