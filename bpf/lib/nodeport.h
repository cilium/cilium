...
#  endif /* ENABLE_IPV4 */
	...
	return __encap_with_nodeid(
		ctx, src_ip, src_port, info, src_sec_identity, info->sec_identity,
		NOT_VTEP_DST, opt, opt_len, ct_reason, monitor, ifindex, proto);
}
	...
	enum trace_reason ct_reason, __u32 monitor, int *ifindex, __be16 proto)
{
	return nodeport_add_tunnel_encap_opt(
		ctx, src_ip, src_port, info, src_sec_identity, NULL, 0,
		ct_reason, monitor, ifindex, proto);
}
# endif /* HAVE_ENCAP */

static __always_inline int nodeport_l7_lb_redirect(
	struct __ctx_buff *ctx __maybe_unused, struct iphdr *ip4 __maybe_unused,
	__be16 proxy_port __maybe_unused, __u32 src_sec_identity __maybe_unused,
...
static __always_inline bool
nodeport_skip_xlate6(const struct lb6_service *svc, bool backend_local)
{
...
static __always_inline int dsr_set_ext6(
	struct __ctx_buff *ctx, struct ipv6hdr *ip6, const union v6addr *svc_addr,
	__be16 svc_port, bool need_dsr_info, int *ohead)
{
...
static __always_inline int encap_geneve_dsr_opt6(
	struct __ctx_buff *ctx, struct ipv6hdr *ip6, const union v6addr *svc_addr,
	__be16 svc_port, bool need_opt, int *ifindex, int *ohead)
{
...
#   elif DSR_ENCAP_MODE == DSR_ENCAP_GENEVE
	ret = encap_geneve_dsr_opt6(
		ctx, ip6, &addr, port, need_dsr_info, &oif, &ohead);
	if (!IS_ERR(ret))
...
#  if defined(ENABLE_L7_LB)
	if (lb6_svc_is_l7_loadbalancer(svc))
		return nodeport_l7_lb_redirect(
			ctx, NULL, (__be16)svc->l7_lb_proxy_port,
			src_sec_identity, punt_to_stack);
#  endif
...
static __always_inline bool
nodeport_skip_xlate4(const struct lb4_service *svc, bool backend_local)
{
	/* Under DSR with IPIP dispatch the inner packet is shipped to the
	 * remote backend unchanged, so skip the DNAT. When the backend is
	 * local though (LB node with local backend, or backend node after
...
dsr_set_opt4(struct __ctx_buff *ctx, struct iphdr *ip4, __be32 svc_addr,
	     __be16 svc_port, bool need_dsr_info, __be16 *ohead)
{
...
static __always_inline int encap_geneve_dsr_opt4(
	struct __ctx_buff *ctx, struct iphdr *ip4, __be32 svc_addr,
	__be16 svc_port, bool need_opt, int *ifindex, __be16 *ohead)
{
	const struct remote_endpoint_info *info;
...
#   elif DSR_ENCAP_MODE == DSR_ENCAP_GENEVE
	ret = encap_geneve_dsr_opt4(
		ctx, ip4, addr, port, need_dsr_info, &oif, &ohead);
#   else
#    error "Invalid load balancer DSR encapsulation mode!"
#   endif
	if (!IS_ERR(ret)) {
...
#  endif
	if (lb4_to_lb6_service(svc)) {
		...
		if (CONFIG(enable_ipip_termination)) {
			__be32 forced_addr = ctx_load_and_clear_meta(
				ctx, CB_FORCED_BACKEND_V4);
			if (forced_addr) {
