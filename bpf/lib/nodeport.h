...
select_nat_port_range_ipv6(struct ipv6_nat_target *target)
{
	if (CONFIG(nodeport_port_max_nat_ext) && (get_prandom_u32() & 0x1)) {
...
#    if DSR_ENCAP_MODE == DSR_ENCAP_NONE || DSR_ENCAP_MODE == DSR_ENCAP_GENEVE
	...
	ret = snat_v6_rewrite_headers(
		ctx, tuple.nexthdr, l3_off, ipfrag_has_l4_header(fraginfo),
		l4_off, &tuple.daddr, svc_addr, IPV6_DADDR_OFF, tuple.sport,
		dport, TCP_DPORT_OFF);
	if (IS_ERR(ret))
		goto drop_err;
#    endif
...
#  endif
	...
	ret = lb6_local(
		...
		&ct_state_svc, &backend, &need_dsr_info, ext_err, forced_be_p);
	if (IS_ERR(ret)) {
...
# ifdef ENABLE_IPV4
static __always_inline __maybe_unused void
select_nat_port_range_ipv4(struct ipv4_nat_target *target)
{
	if (CONFIG(nodeport_port_max_nat_ext) && (get_prandom_u32() & 0x1)) {
		target->min_port = CONFIG(nodeport_port_min_nat_ext);
		target->max_port = CONFIG(nodeport_port_max_nat_ext);
...
#    if DSR_ENCAP_MODE == DSR_ENCAP_NONE || DSR_ENCAP_MODE == DSR_ENCAP_GENEVE
	...
	/* Packet was DNATed from Service IP/Port to backend. Revert this again. */
	ret = snat_v4_rewrite_headers(
		ctx, tuple.nexthdr, l3_off, ipfrag_has_l4_header(fraginfo),
		l4_off, ip4->daddr, svc_addr, IPV4_DADDR_OFF, tuple.sport,
		dport, TCP_DPORT_OFF, 0);
	if (IS_ERR(ret))
		goto drop_err;
#    endif
...
#  endif
	if (lb4_to_lb6_service(svc)) {
		...
		ret = lb4_local(
			get_ct_map4(tuple), ctx, fraginfo, l4_off, key, tuple, svc,
			&ct_state_svc, &backend, &need_dsr_info, ext_err, tmp);
		if (IS_ERR(ret)) {
			if (ret == DROP_NO_SERVICE) {
				if (!CONFIG(enable_no_service_endpoints_routable))
