...
nodeport_rev_dnat_fwd_ipv6(struct __ctx_buff *ctx, bool *snat_done,
			   __u32 revdnat_only __maybe_unused,
...
static __always_inline int
__handle_nat_fwd_ipv6(struct __ctx_buff *ctx, __u32 revdnat_only, struct trace_ctx *trace,
		      __s8 *ext_err)
{
...
handle_nat_fwd_ipv6(struct __ctx_buff *ctx, struct trace_ctx *trace,
		    __s8 *ext_err)
{
	__u32 revdnat_only = ctx_load_and_clear_meta(ctx, CB_REVDNAT_ONLY);
...
__handle_nat_fwd_ipv4(struct __ctx_buff *ctx, __u32 cluster_id __maybe_unused,
		      __u32 revdnat_only, struct trace_ctx *trace, __s8 *ext_err)
{
	bool snat_done = false;
	int ret;
...
handle_nat_fwd_ipv4(struct __ctx_buff *ctx, struct trace_ctx *trace,
		    __s8 *ext_err)
{
	__u32 revdnat_only = ctx_load_meta(ctx, CB_REVDNAT_ONLY);
	__u32 cluster_id = ctx_load_and_clear_meta(ctx, CB_CLUSTER_ID_EGRESS);
 ...
 * nodeport. If revdnat_only is set to 1, fib_lookup and bpf_snat are skipped.
 * The typical use case of handle_nat_fwd(revdnat_only=1) is for handling reply
 * traffic that requires revDNAT prior to wireguard/IPsec encryption.
 */
static __always_inline int
handle_nat_fwd(struct __ctx_buff *ctx, __u32 cluster_id, __be16 proto,
	       __u32 revdnat_only, struct trace_ctx *trace __maybe_unused,
	       __s8 *ext_err __maybe_unused)
{
	int ret = CTX_ACT_OK;

	ctx_store_meta(ctx, CB_CLUSTER_ID_EGRESS, cluster_id);
	ctx_store_meta(ctx, CB_REVDNAT_ONLY, revdnat_only);
