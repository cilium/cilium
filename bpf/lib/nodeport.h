...
redo:
		...
		ret = snat_v6_create_dsr(&tuple, &addr,
					 (__be16)ctx_load_meta(ctx, CB_PORT));
		if (IS_ERR(ret))
			goto drop_err;
	...
	case CT_ESTABLISHED:
	case CT_REPLY:
		/* Recreate CT entry, as the existing one is stale and
		 * belongs to a different flow.
		 */
		if (unlikely(!ct_state.dsr))
			goto redo;
	...
	/* Be paranoid about leaking info to the main path: */
...
nodeport_rev_dnat_fwd_ipv6(struct __ctx_buff *ctx, struct trace_ctx *trace)
{
	...
	if (!ct_has_nodeport_or_dsr_egress_entry6(get_ct_map6(&tuple), &tuple))
		return CTX_ACT_OK;
...
redo:
		...
		ret = snat_v4_create_dsr(&tuple, ctx_load_meta(ctx, CB_ADDR_V4),
					 (__be16)ctx_load_meta(ctx, CB_PORT));
		if (IS_ERR(ret))
			goto drop_err;
...
nodeport_rev_dnat_fwd_ipv4(struct __ctx_buff *ctx, struct trace_ctx *trace)
{
	...
	if (!ct_has_nodeport_or_dsr_egress_entry4(get_ct_map4(&tuple), &tuple))
		return CTX_ACT_OK;
