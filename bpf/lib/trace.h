 ...
 * @obs_point:	observation point (TRACE_*)
 * @reason:	reason for forwarding the packet (TRACE_REASON_*)
 * @line: life of code where the function is being called
 * @file: file from which the function is being called
 *
		   ...
		   enum trace_reason reason, __u32 monitor, __u16 line, __u8 file)
{
	...
	update_trace_metrics(ctx, obs_point, reason, line, file);
