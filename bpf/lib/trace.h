...
_update_trace_metrics(struct __ctx_buff *ctx, enum trace_point obs_point,
		      cls_t flags, __u16 line, __u8 file)
{
	switch (obs_point) {
	case TRACE_TO_LXC:
...
struct trace_notify {
	...
	__u8		flags; /* __u8 instead of cls_t so that it will error when
				* cls_t grows (we'll need to store flags_lower/flags_upper).
				*/
	__u32		ifindex;
	union {
		struct {
...
_send_trace_notify(struct __ctx_buff *ctx, enum trace_point obs_point,
		   ...
		   enum trace_reason reason, __u32 monitor, cls_t flags,
		   __u16 line, __u8 file)
{
	...
	__u64 cap_len = min_t(__u64,
				  monitor && monitor != TRACE_PAYLOAD_LEN
				     ? monitor
					 : ctx_default_monitor_from_classifiers(flags),
			      ctx_len);
	...
	else if ((flags & CLS_FLAG_IPV6) == CLS_FLAG_IPV6)
...
_send_trace_notify(struct __ctx_buff *ctx, enum trace_point obs_point,
		   ...
		   __u32 monitor __maybe_unused, cls_t flags,
...
/* Same as _send_trace_notify() but with additional `__be32 orig_addr` parameter */
static __always_inline void
_send_trace_notify4(struct __ctx_buff *ctx, enum trace_point obs_point,
		    __u32 src, __u32 dst, __be32 orig_addr, __u16 dst_id,
			__u32 ifindex, enum trace_reason reason, __u32 monitor, cls_t flags,
			__u16 line, __u8 file)
{
...
/* Same as _send_trace_notify() but with additional `const union v6addr *orig_addr` parameter */
static __always_inline void
...
#define send_trace_notify(ctx, obs_point, src, dst, dst_id, ifindex, reason, monitor) \
		_send_trace_notify(ctx, obs_point, src, dst, dst_id, NULL, ifindex, reason, \
		monitor, ctx_base_classifiers(ctx), __MAGIC_LINE__, __MAGIC_FILE__)

#define send_trace_notify_flags(ctx, obs_point, src, dst, dst_id, ifindex, reason, monitor, flags) \
		_send_trace_notify(ctx, obs_point, src, dst, dst_id, NULL, ifindex, reason, \
		monitor, ctx_base_classifiers(ctx) | (cls_t)(flags), __MAGIC_LINE__, __MAGIC_FILE__)

#define send_trace_notify4(ctx, obs_point, src, dst, orig_addr, dst_id, ifindex, reason, monitor) \
		_send_trace_notify4(ctx, obs_point, src, dst, orig_addr, dst_id, ifindex, reason, \
		monitor, ctx_base_classifiers4(ctx), __MAGIC_LINE__, __MAGIC_FILE__)

#define send_trace_notify6(ctx, obs_point, src, dst, orig_addr, dst_id, ifindex, reason, monitor) \
		_send_trace_notify6(ctx, obs_point, src, dst, orig_addr, dst_id, ifindex, reason, \
		monitor, ctx_base_classifiers6(ctx), __MAGIC_LINE__, __MAGIC_FILE__)
