...
lb6_xlate(struct __ctx_buff *ctx, const union v6addr *new_dst,
	  const union v6addr *new_saddr __maybe_unused,
	  const union v6addr *old_saddr __maybe_unused, __u8 nexthdr,
...
update_state:
	...
	return lb_skip_l4_dnat() ? CTX_ACT_OK :
	       lb6_xlate(ctx, &new_daddr, &new_saddr, &tuple->saddr,
