...
int __tail_no_service_ipv4(struct __ctx_buff *ctx)
{
	...
	#define body(i) if ((i) > sample_len) \
			goto csumend; \
		if (data + sizeof(struct ethhdr) + (i) + sizeof(__u16) > data_end) \
			goto csumend; \
		csum += *(__u16 *)(data + sizeof(struct ethhdr) + (i));

	#define body5(i) body(i)\
		body(i + 2) \
		body(i + 4) \
		body(i + 6) \
		body(i + 8)
...
csumend :
	...
	xdp_adjust_head(ctx, 0 - (int)(sizeof(struct iphdr) + sizeof(struct icmphdr)));
#else
	skb_adjust_room(ctx, sizeof(struct iphdr) + sizeof(struct icmphdr), BPF_ADJ_ROOM_MAC, 0);
#endif
	...
	icmphdr->code = ICMP_HOST_UNREACH;
	icmphdr->checksum = 0;
...
#endif /* SERVICE_NO_BACKEND_RESPONSE == 1 */
...
int __tail_no_service_ipv6(struct __ctx_buff *ctx)
{
	...
	/* Unrolled loop to calculate the checksum of the ICMPv6 sample
	 * Done manually because the compiler refuses with #pragma unroll
	 */
	csum = 0;
...
csumend :
...
#if __ctx_is == __ctx_xdp
	xdp_adjust_head(ctx, 0 - (int)(sizeof(struct ipv6hdr) + sizeof(struct icmp6hdr)));
#else
	skb_adjust_room(ctx, sizeof(struct ipv6hdr) + sizeof(struct icmp6hdr), BPF_ADJ_ROOM_MAC, 0);
#endif
	...
	icmphdr->icmp6_code = ICMPV6_ADDR_UNREACH;
