...
static __always_inline
struct lb6_service *lb6_lookup_wildcard(struct lb6_key *key)
{
				       ...
				       const bool east_west)
{
	...
	if (!svc) {
		...
		return lb6_lookup_wildcard(key);
	}
...
static __always_inline
struct lb4_service *lb4_lookup_wildcard(struct lb4_key *key)
{
				       ...
				       const bool east_west)
{
	...
	if (!svc) {
		...
		return lb4_lookup_wildcard(key);
	}
