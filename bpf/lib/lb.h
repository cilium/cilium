...
static inline int __inline__ lb6_extract_key_v2(struct __sk_buff *skb,
							...
							int l4_off, struct lb6_key_v2 *key,
							struct csum_offset *csum_off, int dir)
{
	union v6addr *addr;

    key->proto = 0; // FIXME: set after adding support for different protocols in LB
...
static inline struct lb6_service_v2 *__lb6_lookup_service_v2(struct lb6_key_v2 *key)
{
#ifdef LB_L4
	if (key->dport) {
...
static inline struct lb6_service_v2 *lb6_lookup_service_v2(struct __sk_buff *skb,
							...
							__u16 backend_id)
{
	...
	if (!backend) {
		cilium_dbg_lb(skb, DBG_LB6_LOOKUP_SLAVE, backend_id, backend_id);
		cilium_dbg_lb(skb, DBG_LB6_LOOKUP_MASTER_FAIL, backend_id, 0);
	}
...
static inline struct lb6_service_v2 *lb6_lookup_slave_v2(struct __sk_buff *skb,
...
static inline int __inline__ lb6_xlate_v2(struct __sk_buff *skb, union v6addr *new_dst, __u8 nexthdr,
				       int l3_off, int l4_off, struct csum_offset *csum_off,
				       struct lb6_key_v2 *key,
					   struct lb6_service_v2 *svc, struct lb6_backend *backend)
{
...
static inline int __inline__ lb6_local(void *map, struct __sk_buff *skb, int l3_off, int l4_off,
				       struct csum_offset *csum_off, struct lb6_key_v2 *key,
				       struct ipv6_ct_tuple *tuple, struct lb6_service_v2 *svc_v2,
				       struct ct_state *state)
{
    ...
    if (state->backend_id == 0) {
        if ((svc = lb6_lookup_service(skb, (struct lb6_key *)key)) == NULL) {
            goto drop_no_service;
        }
		if ((svc = lb6_lookup_slave(skb, (struct lb6_key *)key, state->slave)) == NULL) {
            goto drop_no_service;
		}
	 ...
	 */
	if ((backend = lb6_lookup_backend(skb, state->backend_id)) == NULL) {
        key->slave = 0;
		if ((svc_v2 = lb6_lookup_service_v2(skb, key)) == NULL) {
            goto drop_no_service;
		}
		slave = lb6_select_slave(skb, svc_v2->count, svc_v2->weight);
		if ((slave_svc = lb6_lookup_slave_v2(skb, key, slave)) == NULL) {
...
static inline struct lb4_service_v2 *__lb4_lookup_service_v2(struct lb4_key_v2 *key)
{
#ifdef LB_L4
	if (key->dport) {
...
static inline struct lb4_service_v2 *lb4_lookup_service_v2(struct __sk_buff *skb,
...
static inline struct lb4_service_v2 *lb4_lookup_slave_v2(struct __sk_buff *skb,
				       ...
				       struct ct_state *state, __be32 saddr)
{
	...
	switch(ret) {
	case CT_NEW:
        // TODO(brb) document why it's fine to rely on v2-only here
	...
	// CT hasn't been migrated to use v2 yet, so do it now
    if (state->backend_id == 0) {
        if ((svc = lb4_lookup_service(skb, (struct lb4_key *)key)) == NULL) {
            goto drop_no_service;
        }
