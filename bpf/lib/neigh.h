...
static __maybe_unused int neigh_record_ip6(struct __ctx_buff *ctx)
{
...
static __maybe_unused union macaddr *neigh_lookup_ip6(const union v6addr *addr)
{
	return map_lookup_elem(&NODEPORT_NEIGH6, addr);
}
#else
static __maybe_unused union macaddr *
...
static __maybe_unused int neigh_record_ip4(struct __ctx_buff *ctx)
{
	union macaddr smac = {}, *mac;
	void *data, *data_end;
	struct iphdr *ip4;
...
static __maybe_unused union macaddr *neigh_lookup_ip4(const __be32 *addr)
{
