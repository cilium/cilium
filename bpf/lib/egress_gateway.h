...
static __always_inline
int egress_gw_fib_lookup_and_redirect_v6(struct __ctx_buff *ctx, union v6addr *egress_ip, 
                                        union v6addr *daddr, __u32 egress_ifindex, __s8 *ext_err)
{
    struct bpf_fib_lookup_padded fib_params = {};
    int oif = 0;
    int ret;
...
struct {
    ...
    __uint(max_entries, EGRESS_POLICY_MAP_SIZE);
    __uint(map_flags, BPF_F_NO_PREALLOC);
} EGRESS_POLICY_MAP6 __section_maps_btf;
...
static __always_inline
struct egress_gw_policy_entry6 *lookup_ip6_egress_gw_policy(union v6addr *saddr, 
                                                             union v6addr *daddr)
{
    ...
    return map_lookup_elem(&EGRESS_POLICY_MAP6, &key);
}
