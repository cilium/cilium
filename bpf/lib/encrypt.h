 ...
 */
static __always_inline int
ipsec_redirect_sec_id_ok(__u32 sec_id, int ip_proto) {
...
# endif /* ENABLE_IPv6 */
	...
	if (!ipsec_redirect_sec_id_ok(dst->sec_identity, ip_proto) ||
	    !ipsec_redirect_sec_id_ok(src->sec_identity, ip_proto))
		return CTX_ACT_OK;
