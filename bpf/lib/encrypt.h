...
#  if defined(TUNNEL_MODE)
		...
		if (ctx_is_overlay(ctx)) {
			...
			/* in v1.17 backport code we may see in-flight packets
			 * processed by v1.16 to_overlay program which does
			 * not mark the packets for the subsequent
			 * "ctx_is_overlay_encrypted" check.
			 *
			 * in this scenario we must parse headers to determine
			 * if the tunnel traffic is encapsulated ESP and should
			 * be encrypted per v1.17 IPsec rules.
			 *
			 * we can reuse the vxlan related function for geneve
			 * as well since IPsec does not currently support DSR,
			 * meaning both VXLan and Geneve headers are 8 bytes
			 * large with no additional options for Geneve.
			 */
			l4_off = ETH_HLEN + ipv4_hdrlen(ip4);
			encrypt_handle_vxlan_inner_packet(ctx, data, data_end,
							  &l4_off, &l4_proto,
							  &dst);
