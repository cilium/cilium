...
static __maybe_unused int ipv4_load_daddr(struct __ctx_buff *ctx, int off,
					   __u32 *dst)
{
	return ctx_load_bytes(ctx, off + offsetof(struct iphdr, daddr), dst, 4);
}

static __maybe_unused int ipv4_dec_ttl(struct __ctx_buff *ctx, int off,
					struct iphdr *ip4)
{
...
static __maybe_unused int ipv4_hdrlen(const struct iphdr *ip4)
{
	return ip4->ihl * 4;
}

static __maybe_unused bool ipv4_is_fragment(const struct iphdr *ip4)
{
...
static __maybe_unused bool ipv4_is_not_first_fragment(const struct iphdr *ip4)
{
...
static __maybe_unused bool ipv4_has_l4_header(const struct iphdr *ip4)
{
	return !ipv4_is_not_first_fragment(ip4);
}

static __maybe_unused bool ipv4_is_in_subnet(__be32 addr,
