...
static __always_inline int ipv4_csum_update_by_value(
	struct __ctx_buff *ctx, int l3_off, __u64 old_val, __u64 new_val, __u32 len)
{
	return l3_csum_replace(
		ctx, l3_off + offsetof(struct iphdr, check), old_val, new_val,
		len);
}
...
{
	return l3_csum_replace(
		ctx, l3_off + offsetof(struct iphdr, check), 0, diff, 0);
}
...
static __always_inline int
ipv4_dec_ttl(struct __ctx_buff *ctx, int off, struct iphdr *ip4)
{
...
ipv4_is_in_subnet(__be32 addr, __be32 subnet, int prefixlen)
{
	return (addr & bpf_htonl(~((1 << (32 - prefixlen)) - 1))) == subnet;
}

#ifdef ENABLE_IPV4_FRAGMENTS
static __always_inline int ipv4_frag_get_l4ports(
	const struct ipv4_frag_id *frag_id, struct ipv4_frag_l4ports *ports)
{
...
static __always_inline int ipv4_handle_fragmentation(
	struct __ctx_buff *ctx, const struct iphdr *ip4, int l4_off,
	enum ct_dir ct_dir, struct ipv4_frag_l4ports *ports, bool *has_l4_header)
{
	...
	if (unlikely(is_fragment)) {
		 ...
		 */
		if (map_update_elem(
			    &IPV4_FRAG_DATAGRAMS_MAP, &frag_id, ports, BPF_ANY))
			update_metrics(
				ctx_full_len(ctx), ct_to_metrics_dir(ct_dir),
...
static __always_inline int ipv4_load_l4_ports(
	struct __ctx_buff *ctx, struct iphdr *ip4 __maybe_unused, int l4_off,
	enum ct_dir dir __maybe_unused, __be16 *ports,
	bool *has_l4_header __maybe_unused)
{
#ifdef ENABLE_IPV4_FRAGMENTS
	return ipv4_handle_fragmentation(
		ctx, ip4, l4_off, dir, (struct ipv4_frag_l4ports *)ports,
		has_l4_header);
#else
