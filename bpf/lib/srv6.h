...
#  define SRV6_VRF_STATIC_PREFIX4                                            \
	  (8 *                                                               \
	   (sizeof(struct srv6_vrf_key4) - sizeof(struct bpf_lpm_trie_key) - \
	    4))
...
#  define SRV6_POLICY_STATIC_PREFIX4          \
	  (8 *                                \
	   (sizeof(struct srv6_policy_key4) - \
	    sizeof(struct bpf_lpm_trie_key) - 4))
#  define SRV6_POLICY_PREFIX4_LEN(PREFIX) \
	  (SRV6_POLICY_STATIC_PREFIX4 + (PREFIX))
...
# define SRV6_VRF_STATIC_PREFIX6 \
	 (8 *                    \
	  (sizeof(struct srv6_vrf_key6) - sizeof(struct bpf_lpm_trie_key) - 4))
...
# define SRV6_POLICY_STATIC_PREFIX6                                            \
	 (8 *                                                                  \
	  (sizeof(struct srv6_policy_key6) - sizeof(struct bpf_lpm_trie_key) - \
	   4))
...
static __always_inline int srv6_encapsulation(
	struct __ctx_buff *ctx, int growth, __u16 new_payload_len, __u8 nexthdr,
	union v6addr *saddr, struct in6_addr *sid)
{
...
#  endif /* ENABLE_SRV6_SRH_ENCAP */
	...
	if (ctx_adjust_hroom(
		    ctx, growth, BPF_ADJ_ROOM_MAC, BPF_F_ADJ_ROOM_ENCAP_L3_IPV6))
		return DROP_INVALID;
...
#  ifdef ENABLE_SRV6_SRH_ENCAP
	case NEXTHDR_ROUTING:
	{
		...
		if ((void *)srh + sizeof(struct srv6_srh) + sizeof(struct in6_addr) >
		    data_end)
			return DROP_INVALID;
...
static __always_inline int srv6_handling4(
	struct __ctx_buff *ctx, union v6addr *src_sid, struct in6_addr *dst_sid)
{
	 ...
	 */
	new_payload_len =
		bpf_ntohs(ip4->tot_len) - (__u16)(ip4->ihl << 2) +
		sizeof(struct iphdr);

	if (ctx_store_bytes(ctx, offsetof(struct ethhdr, h_proto), &outer_proto,
			    sizeof(outer_proto), 0) < 0)
		return DROP_WRITE_ERROR;

#  ifdef ENABLE_SRV6_SRH_ENCAP
	growth += sizeof(struct ipv6hdr) + sizeof(struct srv6_srh) +
...
#  endif /* ENABLE_SRV6_SRH_ENCAP */
	...
	return srv6_encapsulation(
		ctx, growth, new_payload_len, nexthdr, src_sid, dst_sid);
}

static __always_inline int srv6_handling6(
	struct __ctx_buff *ctx, union v6addr *src_sid, struct in6_addr *dst_sid)
{
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_SRV6_ENCAP) int tail_srv6_encap(
	struct __ctx_buff *ctx)
{
	...
	if (ret < 0)
		return send_drop_notify_error(
			ctx, SECLABEL_IPV6, ret, CTX_ACT_DROP, METRIC_EGRESS);

	send_trace_notify(
		ctx, TRACE_TO_STACK, SECLABEL_IPV6, UNKNOWN_ID, TRACE_EP_ID_UNKNOWN,
		TRACE_IFINDEX_UNKNOWN, TRACE_REASON_SRV6_ENCAP, 0);

	return ret;
}

__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_SRV6_DECAP) int tail_srv6_decap(
	struct __ctx_buff *ctx)
{
	int ret = 0;
