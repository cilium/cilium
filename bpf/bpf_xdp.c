...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV4_FROM_NETDEV) int tail_lb_ipv4(
	  ...
	  DSR_ENCAP_MODE == DSR_ENCAP_GENEVE
		{
			...
			if (l4_load_port(ctx, l4_off + UDP_DPORT_OFF, &dport) <
			    0) {
				ret = DROP_INVALID;
				goto out;
			}
			 ...
			 */
			if (ctx_load_bytes(
				    ctx, l4_off + offsetof(struct udphdr, check),
			...
			if (ctx_load_bytes(ctx, l4_off + sizeof(struct udphdr),
					   &geneve, sizeof(geneve)) < 0) {
				ret = DROP_INVALID;
				goto out;
			}
			...
			inner_l2_off = l4_off + sizeof(struct udphdr) +
			...
			if (!revalidate_data_l3_off(
				    ctx, &data, &data_end, &ip4, l3_off)) {
				ret = DROP_INVALID;
				goto out;
			}
		}
...
out:
	if (IS_ERR(ret))
		return send_drop_notify_error_ext(
			ctx, UNKNOWN_ID, ret, ext_err, CTX_ACT_DROP,
			METRIC_INGRESS);
...
static __always_inline int check_v4_lb(struct __ctx_buff *ctx)
{
	...
	return send_drop_notify_error_ext(
		ctx, UNKNOWN_ID, ret, ext_err, CTX_ACT_DROP, METRIC_INGRESS);
}
# else
...
#ifdef ENABLE_IPV6
# ifdef ENABLE_NODEPORT_ACCELERATION
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV6_FROM_NETDEV) int tail_lb_ipv6(
	struct __ctx_buff *ctx)
{
	int ret = CTX_ACT_OK;
	__s8 ext_err = 0;
...
__section_entry int cil_xdp_entry(struct __ctx_buff *ctx)
{
