...
static __maybe_unused int check_v4_lb(struct __ctx_buff *ctx)
{
	__s8 ext_err = 0;
	int ret;
...
#else
static __maybe_unused int check_v4_lb(struct __ctx_buff *ctx __maybe_unused)
{
	return CTX_ACT_OK;
}
...
#ifdef ENABLE_PREFILTER
static __maybe_unused int check_v4(struct __ctx_buff *ctx)
{
...
static __maybe_unused int check_v6_lb(struct __ctx_buff *ctx)
{
	__s8 ext_err = 0;
	int ret;
...
#else
static __maybe_unused int check_v6_lb(struct __ctx_buff *ctx __maybe_unused)
{
	return CTX_ACT_OK;
}
#endif /* ENABLE_NODEPORT_ACCELERATION */
...
#ifdef ENABLE_PREFILTER
static __maybe_unused int check_v6(struct __ctx_buff *ctx)
{
	void *data_end = ctx_data_end(ctx);
	void *data = ctx_data(ctx);
...
static __maybe_unused int check_filters(struct __ctx_buff *ctx)
{
	int ret = CTX_ACT_OK;
	__u16 proto;
