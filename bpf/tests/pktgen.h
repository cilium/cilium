...
void pktgen__init(struct pktgen *builder, struct __ctx_buff *ctx)
{
	...
	for (int i = 0; i < PKT_BUILDER_LAYERS; i++) {
		builder->layers[i] = PKT_LYR_NONE;
		builder->layer_offsets[i] = 0;
	}
};

static __always_inline
int pktgen__free_layer(struct pktgen *builder)
{
	#pragma unroll
	for (int i = 0; i < PKT_BUILDER_LAYERS; i++) {
		if (builder->layers[i] == PKT_LYR_NONE)
			return i;
	}
...
struct ethhdr *pktgen__push_ethhdr(struct pktgen *builder)
{
	...
	struct ethhdr *layer = ctx_data(ctx) + builder->cur_off;
	int layer_idx = pktgen__free_layer(builder);

	if (layer_idx < 0)
		return 0;

	builder->layers[layer_idx] = PKT_LYR_ETH;
...
struct iphdr *pktgen__push_iphdr(struct pktgen *builder)
{
	...
	struct iphdr *layer = ctx_data(ctx) + builder->cur_off;
	int layer_idx = pktgen__free_layer(builder);

	if (layer_idx < 0)
		return 0;

	builder->layers[layer_idx] = PKT_LYR_IPV4;
	builder->layer_offsets[layer_idx] = builder->cur_off;
...
struct tcphdr *pktgen__push_tcphdr(struct pktgen *builder)
{
	...
	struct tcphdr *layer = ctx_data(ctx) + builder->cur_off;
	int layer_idx = pktgen__free_layer(builder);

	if (layer_idx < 0)
		return 0;

	builder->layers[layer_idx] = PKT_LYR_TCP;
	builder->layer_offsets[layer_idx] = builder->cur_off;
...
void *pktgen__push_data_room(struct pktgen *builder, int len)
{
	...
	void *layer = ctx_data(ctx) + builder->cur_off;
	int layer_idx = pktgen__free_layer(builder);

	if (layer_idx < 0)
		return 0;

	builder->layers[layer_idx] = PKT_LYR_DATA;
	builder->layer_offsets[layer_idx] = builder->cur_off;
	builder->cur_off += len;
 ...
 */
static __always_inline
void pktgen__finish(struct pktgen *builder)
{
	...
	for (int i = 0; i < PKT_BUILDER_LAYERS; i++) {
		...
		case PKT_LYR_IPV4:
			...
			__u16 v4len = (__be16)(builder->cur_off - builder->layer_offsets[i]);
		...
		case PKT_LYR_IPV6:
			...
			__be16 v6len = (__be16)(builder->cur_off + sizeof(struct ipv6hdr) -
		...
		case PKT_LYR_TCP:
			...
			if (builder->layers[i + 1] == PKT_LYR_NONE) {
