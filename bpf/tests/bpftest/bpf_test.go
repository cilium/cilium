...
func loadAndRunSpec(t *testing.T, entry fs.DirEntry, instrLog io.Writer, testNetNS *netns.NetNS) []*cover.Profile {
	...
	for _, name := range testNames {
		t.Run(name, subTest(testNameToPrograms[name], coll.Maps[suiteResultMap], skbMdMap, testNetNS))
	}
...
func subTest(progSet programSet, resultMap *ebpf.Map, skbMdMap *ebpf.Map, testNetNS *netns.NetNS) func(t *testing.T) {
