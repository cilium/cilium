...
#ifdef ENABLE_IPV6
static __always_inline int egressgw_pktgen_v6(struct __ctx_buff *ctx, struct egressgw_test_ctx test_ctx)
{
	union v6addr ext_svc_ip = EXTERNAL_SVC_IP_V6;
	union v6addr client_ip = CLIENT_IP_V6;
	union v6addr egress_ip = EGRESS_IP_V6;
	union v6addr egress_ip2 = EGRESS_IP2_V6;
	struct pktgen builder;
	struct tcphdr *l4;
	struct ethhdr *l2;
	struct ipv6hdr *l3;
	...
	if (test_ctx.dir == CT_INGRESS) {
		...
		if (test_ctx.tuple_collision) {
			ipv6_addr_copy((union v6addr *)&l3->daddr, &egress_ip2);
		} else {
	...
	if (test_ctx.dir == CT_INGRESS) {
		...
		struct ipv6_nat_entry *nat_entry = __snat_lookup(&SNAT_MAPPING_IPV6, &tuple);
						  ...
						  struct egressgw_test_ctx test_ctx)
{
	...
	if (test_ctx.dir == CT_INGRESS) {
		...
		if (test_ctx.redirect) {
			...
			if (test_ctx.tuple_collision)
				ipv6_addr_copy((union v6addr *)&expected_saddr, &egress_ip2);
...
static __always_inline int create_ct_entry_v6(struct __ctx_buff *ctx, __be16 client_port)
{
	...
	tuple.nexthdr = IPPROTO_TCP;
	ipv6_addr_copy_unaligned((union v6addr *)&tuple.daddr, &ext_svc_ip);
	tuple.sport = EXTERNAL_SVC_PORT;
	ipv6_addr_copy_unaligned((union v6addr *)&tuple.saddr, &client_ip);
	tuple.dport = client_port;
	__ipv6_ct_tuple_reverse(&tuple);

	return ct_create6(get_ct_map6(&tuple), &CT_MAP_ANY6, &tuple, ctx,
					 CT_EGRESS, &ct_state, NULL);
}
#endif
