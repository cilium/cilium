...
struct remote_ep_flags {
	/* This must be kept in sync with remote_endpoint_info in bpf/lib/eps.h. Perhaps these
	 * could be split out of the remote_endpoint_info in future, for reuse here.
	 */
...
__ipcache_add_entry(struct ipcache_key *key, __u32 sec_identity,
		    const union v6addr *tunnel_ep, __u8 spi,
		    struct remote_ep_flags flags)
{
	struct remote_endpoint_info value = {};

	value.sec_identity = sec_identity;
	if (flags.flag_ipv6_tunnel_ep) {
	...
	value.key = spi;
	value.flag_skip_tunnel = flags.flag_skip_tunnel;
	if (tunnel_ep->p1)
		value.flag_has_tunnel_ep = true;

	value.flag_null_route = flags.flag_null_route;
...
__ipcache_v4_add_entry(__be32 addr, __u8 cluster_id, __u32 sec_identity,
		       ...
		       bool ipv6_underlay, bool flag_null_route, __u32 mask_size)
{
	...
	flags.flag_skip_tunnel = flag_skip_tunnel,
	flags.flag_ipv6_tunnel_ep = ipv6_underlay,
	flags.flag_null_route = flag_null_route,

	__ipcache_add_entry(&key, sec_identity, tunnel_ep, spi, flags);
}
...
ipcache_v4_add_world_entry()
{
	...
	__ipcache_v4_add_entry(v4_all, 0, WORLD_IPV4_ID, &tunnel_ep, 0, false, false, false, 0);
}
		     ...
		     __u32 tunnel_ep, __u8 spi)
{
	__ipcache_v4_add_entry(addr, cluster_id, sec_identity, (union v6addr *)&tunnel_ep, spi,
			       false, false, false, V4_CACHE_KEY_LEN);
}

static __always_inline void
ipcache_v4_add_entry_null_route(__be32 addr, __u8 cluster_id, __u32 sec_identity,
				__u32 tunnel_ep, __u8 spi)
{
	__ipcache_v4_add_entry(addr, cluster_id, sec_identity, (union v6addr *)&tunnel_ep, spi,
...
{
	__ipcache_v4_add_entry(addr, cluster_id, sec_identity, (union v6addr *)&tunnel_ep, spi,
			       flag_skip_tunnel, false, false, V4_CACHE_KEY_LEN);
}
...
static __always_inline void
ipcache_v6_add_entry_null_route(const union v6addr *addr, __u8 cluster_id, __u32 sec_identity,
				__u32 tunnel_ep, __u8 spi)
{
	__ipcache_v6_add_entry(addr, cluster_id, sec_identity, (union v6addr *)&tunnel_ep, spi,
...
{
	__ipcache_v6_add_entry(addr, cluster_id, sec_identity, (union v6addr *)&tunnel_ep, spi,
			       flag_skip_tunnel, false, false, V6_CACHE_KEY_LEN);
}

static __always_inline void
ipcache_v6_add_entry_with_mask_size_ipv6_underlay(const union v6addr *addr, __u8 cluster_id,
						  __u32 sec_identity, const union v6addr *tunnel_ep,
						  __u8 spi, __u32 mask_size)
{
	__ipcache_v6_add_entry(addr, cluster_id, sec_identity, tunnel_ep, spi,
			       false, true, false, mask_size);
}
