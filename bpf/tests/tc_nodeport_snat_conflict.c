...
/*
 * Create a non-transparent request from the egress proxy sourced from a node
 * IP, subjecting it to the nat fwd path. No endpoint entry for the source IP.
 */
PKTGEN("tc", "tc_nodeport_snat_conflict_egressproxy_no_ep_ipv4")
int tc_nodeport_snat_conflict_egressproxy_no_ep_ipv4_pktgen(struct __ctx_buff *ctx)
{
	struct pktgen builder;
	struct udphdr *udp;
...
SETUP("tc", "tc_nodeport_snat_conflict_egressproxy_no_ep_ipv4")
int tc_nodeport_snat_conflict_egressproxy_no_ep_ipv4_setup(struct __ctx_buff *ctx)
{
	/* No endpoint entry for NODE_IP */
	ipcache_v4_add_entry(NODE_IP, 0, HOST_ID, 0, 0);
	ipcache_v4_add_world_entry();
...
CHECK("tc", "tc_nodeport_snat_conflict_egressproxy_no_ep_ipv4")
int tc_nodeport_snat_conflict_egressproxy_no_ep_ipv4_check(struct __ctx_buff *ctx)
{
	test_init();
	assert(tc_nodeport_snat_conflict_assert_status(ctx));
	assert(tc_nodeport_snat_conflict_assert_ipv4());
...
/*
 * IPv6: Create a non-transparent request from the egress proxy sourced from a
 * node IP, subjecting it to the nat fwd path. No endpoint entry for the source IP.
 */
PKTGEN("tc", "tc_nodeport_snat_conflict_egressproxy_no_ep_ipv6")
int tc_nodeport_snat_conflict_egressproxy_no_ep_ipv6_pktgen(struct __ctx_buff *ctx)
{
	struct pktgen builder;
	struct udphdr *udp;
...
SETUP("tc", "tc_nodeport_snat_conflict_egressproxy_no_ep_ipv6")
int tc_nodeport_snat_conflict_egressproxy_no_ep_ipv6_setup(struct __ctx_buff *ctx)
{
	union v6addr node_ip6 = NODE_IP6_ADDR;

	/* No endpoint entry for NODE_IP6 */
	ipcache_v6_add_entry(&node_ip6, 0, HOST_ID, 0, 0);
	ipcache_v6_add_world_entry();
...
CHECK("tc", "tc_nodeport_snat_conflict_egressproxy_no_ep_ipv6")
int tc_nodeport_snat_conflict_egressproxy_no_ep_ipv6_check(struct __ctx_buff *ctx)
{
	test_init();
	assert(tc_nodeport_snat_conflict_assert_status(ctx));
	assert(tc_nodeport_snat_conflict_assert_ipv6());
