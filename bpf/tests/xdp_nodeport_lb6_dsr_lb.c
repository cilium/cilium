...
int nodeport_dsr_fwd_pktgen(struct __ctx_buff *ctx)
{
	...
	l4 = pktgen__push_ipv6_tcp_packet(
		&builder, (__u8 *)client_mac, (__u8 *)lb_mac, (__u8 *)&client_ip,
...
int nodeport_dsr_fwd_check(__maybe_unused const struct __ctx_buff *ctx)
{
	...
	if (memcmp(l2->h_source, (__u8 *)lb_mac, ETH_ALEN) != 0)
		test_fatal("src MAC is not the LB MAC") if (
			...
			if (l3->nexthdr != NEXTHDR_DEST) test_fatal(
				"l3 header doesn't indicate DSR extension");
	...
	if (opt->hdr.nexthdr != IPPROTO_TCP)
		test_fatal("nexthdr in DSR extension is bad") if (opt->hdr.hdrlen != DSR_IPV6_EXT_LEN) test_fatal(
			"length in DSR extension is bad") if (opt->opt_type != DSR_IPV6_OPT_TYPE)
			test_fatal("opt_type in DSR extension is bad") if (
				opt->opt_len !=
				DSR_IPV6_OPT_LEN) test_fatal("opt_len in DSR extension is bad")

				if (opt->port != FRONTEND_PORT) test_fatal("port in DSR extension is bad") if (
					!ipv6_addr_equals((union v6addr *)&opt->addr,
							  &frontend_ip))
