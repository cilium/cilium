...
check_egress_policy(struct __ctx_buff *ctx, __u32 dst_id, __u8 proto, __be16 dport)
{
	...
	return policy_can_egress(ctx, 0 /* src_id ignored */, dst_id,
				 0 /* ethertype */,
...
static __always_inline int
check_egress_policy_port(struct __ctx_buff *ctx, __u32 dst_id,
			 __u8 proto, __be16 dport,
			 __u16 *proxy_port_out)
{
	__u8 match_type;
	__u8 audited;
	__s8 ext_err;
	...
	ret = policy_can_egress(ctx, 0 /* src_id ignored */, dst_id,
				0 /* ethertype */,
				 dport, proto, 0 /* l4_off */,
				 &match_type, &audited, &ext_err, &proxy_port,
				 &cookie);
	if (proxy_port_out)
		*proxy_port_out = proxy_port;
	return ret;
}
...
int l3_l4_precedence_test(struct __ctx_buff *ctx)
{
	...
	TEST("Precedence: L3+L4 match should win over L4-only match", {
		...
		policy_add_shared_entry(HANDLE_PRECEDENCE, REMOTE_IDENTITY,
					1 /* egress */, IPPROTO_TCP,
					__bpf_htons(9090), true /* deny */);
...
int proxy_redirect_test(struct __ctx_buff *ctx)
{
	...
	TEST("Proxy redirect policy", {
		...
		ret = check_egress_policy_port(ctx, REMOTE_IDENTITY,
					       IPPROTO_TCP,
					       __bpf_htons(80), &proxy_port);
		assert(ret == CTX_ACT_OK);
		assert(proxy_port == __bpf_htons(15001));
	});
...
int cidr_policy_test(struct __ctx_buff *ctx)
{
	...
	TEST("CIDR-based policy", {
		...
		policy_add_shared_entry(HANDLE_CIDR, CIDR_IDENTITY_1, 1,
					IPPROTO_TCP, __bpf_htons(443), false);
...
int cidr_l4_only_test(struct __ctx_buff *ctx)
{
	...
	TEST("CIDR combined with L4-only", {
		...
		policy_add_shared_entry(HANDLE_CIDR_L4, 0, 1,
					IPPROTO_TCP, __bpf_htons(8443), false);
...
int fqdn_policy_test(struct __ctx_buff *ctx)
{
	...
	TEST("FQDN-based policy", {
		...
		policy_add_shared_entry(HANDLE_FQDN, FQDN_IDENTITY_1, 1,
					IPPROTO_TCP, __bpf_htons(443), false);
...
int world_identity_test(struct __ctx_buff *ctx)
{
	...
	TEST("World identity", {
		...
		policy_add_shared_entry(HANDLE_WORLD, WORLD_IDENTITY, 1,
					IPPROTO_TCP, __bpf_htons(443), false);
...
int health_identity_test(struct __ctx_buff *ctx)
{
	...
	TEST("Health identity", {
		...
		policy_add_shared_entry(HANDLE_HEALTH, HEALTH_IDENTITY, 1,
					IPPROTO_TCP, __bpf_htons(4240), false);
...
int remote_node_identity_test(struct __ctx_buff *ctx)
{
	...
	TEST("Remote-node identity", {
		...
		policy_add_shared_entry(HANDLE_REMOTE_NODE, REMOTE_NODE_ID, 1,
					IPPROTO_TCP, __bpf_htons(6443), false);
...
int mixed_policy_test(struct __ctx_buff *ctx)
{
	...
	TEST("Mixed policies", {
		...
		policy_add_shared_entry(HANDLE_MIXED, REMOTE_IDENTITY, 1,
					IPPROTO_TCP, __bpf_htons(80), false);
		policy_add_shared_entry(HANDLE_MIXED, FQDN_IDENTITY_1, 1,
					IPPROTO_TCP, __bpf_htons(443), false);
