...
static __always_inline int
validate_icmpv6_reply(const struct __ctx_buff *ctx, __u32 retval)
{
	void *data, *data_end;
	__u32 *status_code;
	struct ethhdr *l2;
...
{
	return validate_icmpv6_reply(ctx, CTX_ACT_REDIRECT);
}
...
{
	return validate_icmpv6_reply(ctx, CTX_ACT_OK);
}
