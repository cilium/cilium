...
#define test_fail() __test_set_result(TEST_FAIL)

/* Mark the current test as failed and exit the current TEST/CHECK */
#define test_fail_now() __test_set_result_now(TEST_FAIL)

/* Mark the current test as skipped */
#define test_skip() __test_set_result(TEST_SKIP)

/* Mark the current test as skipped and exit the current TEST/CHECK */
#define test_skip_now() __test_set_result_now(TEST_SKIP)
...
do {										\
	...
	__curr_test = map_lookup_elem(&suite_result_map2, &__suite_test_cnt);	\
	if (!__curr_test) return TEST_FAIL;					\
...
#define __test_finish() 							\
		if (!__curr_test->valid && __curr_test->n_asserts > 0) {	\
