...
int nodeport_lb6_fragment1_setup(struct __ctx_buff *ctx)
{
	...
	endpoint_v6_add_entry(&backend_ip, BACKEND_IFINDEX, 0, 0, 0,
			      (__u8 *)mac_one, (__u8 *)mac_two);
	lb_v6_add_service_with_flags(&frontend_ip, FRONTEND_PORT, IPPROTO_TCP, BACKEND_COUNT,
				     NAT_REV_INDEX, SVC_FLAG_ROUTABLE | SVC_FLAG_NODEPORT, 0);
	lb_v6_add_backend(&frontend_ip, FRONTEND_PORT, BACKEND_COUNT, BACKEND_ID,
...
int nodeport_lb6_fragment1_check(struct __ctx_buff *ctx)
{
	...
	struct ipv6_frag_l4ports *l4ports = map_lookup_elem(&cilium_ipv6_frag_datagrams, &frag_id);
	...
	/* Ensure expected xlation happened. */
