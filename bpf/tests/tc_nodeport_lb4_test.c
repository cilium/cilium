...
static inline int ctx_recalc(struct __ctx_buff *ctx, int offset)
{
	return ctx_full_len(ctx) + offset;
}

SETUP("tc", "forward_to_other_node")
int test1_setup(struct __ctx_buff *ctx)
{
	...
	bpf_skb_change_tail(ctx, ctx_recalc(ctx, offset), 0);
...
CHECK("tc", "forward_to_other_node")
int test1_check(__maybe_unused const struct __ctx_buff *ctx)
{
	...
	if (*status_code != TC_ACT_REDIRECT)
		test_fatal("status code != TC_ACT_REDIRECT");
