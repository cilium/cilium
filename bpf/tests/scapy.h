...
bool __assert_map_add_failure(const char *name, const __u8 name_len,
			      const int cfi, const char *first_layer,
			      const __u8 first_layer_len,
			      const unsigned char *buf,
			      const __u16 len, void *data, void *data_end)
{
	__scapy_assert.cfi = cfi;
...
#define __ASSERT_CTX_BUF_OFF(NAME, CFI, FIRST_LAYER, CTX, OFF, BUF_NAME, _BUF,		\
			    _BUF_LEN, LEN)						\
	do {										\
		...
		if ((_BUF_LEN) < (LEN)) {						\
			_ok = false;							\
			__ASSERT_TRACE_FAIL_LEN(CFI, BUF_NAME, _BUF_LEN, LEN);		\
		}									\
		if (_ok && scapy_memcmp(__DATA, _BUF, LEN) != 0) {			\
			_ok = false;							\
			__ASSERT_TRACE_FAIL_BUF(CFI, BUF_NAME, _BUF_LEN, LEN);		\
		}									\
		if (!_ok) {								\
			if (!__assert_map_add_failure(NAME, sizeof(NAME), CFI,		\
						      FIRST_LAYER,			\
						      sizeof(FIRST_LAYER),		\
						      _BUF, LEN, __DATA,		\
						      __DATA_END)) {			\
...
#define ASSERT_CTX_BUF_OFF2(NAME, FIRST_LAYER, CTX, OFF, BUF_NAME, _BUF,	\
			    _BUF_LEN, LEN)					\
	__ASSERT_CTX_BUF_OFF(NAME, -1, FIRST_LAYER, CTX, OFF, BUF_NAME, _BUF,	\
			     _BUF_LEN, LEN)

/**
 * Compare a packet (ctx) to a scapy buffer (pointer BUF) starting from ctx's
 * OFF byte.
 *
 * As opposed to ASSERT_CTX_BUF_OFF2, which receives the name of the buffer as a
 * string literal, this version receives a Callee Function Id (CFI), an int,
 * a user-generated ID to correlate with the test code. This allows reusing
 * test functions with asserts and not loose track of which was the function
 * callee, hence the assert that actually failed.
 *
 * NOTE: CFI is necessary as - today - test_log doesn't support '%s'.
 *
 * NAME: assertion quoted name (unique in the test): "test_a".
 * CFI: Callee Function Id (integer)
 * FIRST_LAYER: Scapy layer name (e.g. Ether, IP), quoted: "Ether".
 * CTX: ctx ptr.
 * OFF: start to compare against ctx->data + OFF.
 * _BUF: pointer to the first byte of the scapy buffer BUF_DECL() / BUF().
 ...
 * LEN: how many bytes to compare.
 */
#define ASSERT_CTX_BUF_OFF2_CFI(NAME, CFI, FIRST_LAYER, CTX, OFF, _BUF,		\
				 _BUF_LEN, LEN)					\
	__ASSERT_CTX_BUF_OFF(NAME, CFI, FIRST_LAYER, CTX, OFF, "", _BUF,	\
			     _BUF_LEN, LEN)
