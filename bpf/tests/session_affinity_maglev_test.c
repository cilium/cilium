...
} test_cilium_lb4_maglev __section_maps_btf = {
	.values = {[TEST_REVNAT] = &test_lb4_maglev_map_inner, },
};

#define cilium_lb4_maglev_map test_cilium_lb4_maglev
	...
	__maybe_unused int plen, __maybe_unused __u32 flags)
{
	...
	if (params->ipv4_dst == CLIENT_IP1) {
		...
		__bpf_memcpy_builtin(
			params->dmac, (__u8 *)new_backend_mac, ETH_ALEN);
	}
...
generate_packet(struct __ctx_buff *ctx, int client_id, __u16 src_port)
{
	...
	l4 = pktgen__push_ipv4_tcp_packet(
		&builder, src_mac, (__u8 *)lb_mac, src_ip, FRONTEND_IP,
...
static __always_inline void setup_test()
{
	...
	for (__u16 backend_id = 1; backend_id <= LB_MAGLEV_LUT_SIZE;
	     backend_id++) {
		__lb_v4_add_backend(
			FRONTEND_IP, FRONTEND_PORT, /*backend_slot*/ backend_id,
