...
int hairpin_flow_forward_check_v6(__maybe_unused const struct __ctx_buff *ctx)
{
    ...
    if (memcmp(&l3->saddr, (const void *)&CONFIG(service_loopback_ipv6).addr, sizeof(l3->saddr)) != 0)
    ...
    tuple.flags = TUPLE_F_OUT;
    tuple.nexthdr = IPPROTO_TCP;
    memcpy(&tuple.saddr, (const void *)&CONFIG(service_loopback_ipv6).addr, sizeof(tuple.saddr));
    tuple.sport = tcp_src_one;
...
int hairpin_flow_rev_check_v6(__maybe_unused const struct __ctx_buff *ctx)
{
	...
	if (memcmp(&l3->daddr, (const void *)&CONFIG(service_loopback_ipv6).addr, sizeof(l3->daddr)) != 0)
		test_fatal("dest IPv6 changed");
