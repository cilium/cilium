	...
	__maybe_unused int plen, __maybe_unused __u32 flags)
{
	...
	if (params->ipv4_dst == BACKEND_NODE_IP) {
		...
		__bpf_memcpy_builtin(
			params->dmac, (__u8 *)backend_node_mac, ETH_ALEN);
	} else {
		return CTX_ACT_DROP;
	}
...
int nodeport_geneve_dsr_lb_xdp1_local_backend_setup(struct __ctx_buff *ctx)
{
	...
	endpoint_v4_add_entry(
		BACKEND_IP_LOCAL, 0, 0, 0, 0, (__u8 *)local_backend_mac,
		(__u8 *)node_mac);
...
int nodeport_geneve_dsr_lb_xdp2_fwd_setup(struct __ctx_buff *ctx)
{
	...
	lb_v4_add_backend(
		FRONTEND_IP_REMOTE, FRONTEND_PORT, 1, backend_id,
...
int nodeport_geneve_dsr_lb_xdp_fwd_check(
