...
#define DST_IP IPv4(10,0, 0, 1)
#define DST_PORT 6000
#define DST_PORT_HOSTNS 6001
#define BACKEND_IP IPv4(10, 0, 0, 2)
...
int my_get_netns_cookie(__maybe_unused struct bpf_sock_addr *addr)
{
...
#define SVC_KEY_VALUE(_port, _beslot, _beid, _scope) { \
    .key = { .address = DST_IP, .dport = (_port), .scope = (_scope), .backend_slot = (_beslot) }, \
    .value = { .flags = SVC_FLAG_ROUTABLE, .count = 1, .rev_nat_index = 1, .backend_id = (_beid) } \
}
...
int test1_check(__maybe_unused struct xdp_md *ctx)
{
	...
	for (unsigned long i=0; i < NELEMS(services); i++) {
    		map_update_elem(&LB4_SERVICES_MAP_V2, &services[i].key, &services[i].value, BPF_ANY);
	}

        struct { __u32 key; struct lb4_backend value; } backends[] = {
        	BE_KEY_VALUE(1, BACKEND_IP),
        };

	for (unsigned long i=0; i < NELEMS(backends); i++) {
    		map_update_elem(&LB4_BACKEND_MAP_V2, &backends[i].key, &backends[i].value, BPF_ANY);
	}
	...
	int ret = __sock4_xlate_fwd(&addr, &addr, false);
	assert(addr.user_ip4 == DST_IP);
