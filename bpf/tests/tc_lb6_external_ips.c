...
int lb6_external_ips_pktgen(struct __ctx_buff *ctx)
{
	...
	BUF_DECL(LB6_EXTERNAL_IP, lb6_external_ip);
	BUILDER_PUSH_BUF(builder, LB6_EXTERNAL_IP);
...
int lb6_external_ips_check(__maybe_unused const struct __ctx_buff *ctx)
{
	...
	BUF_DECL(LB6_EXTERNAL_IP_POST_DNAT, lb6_external_ip_post_dnat);
	ASSERT_CTX_BUF_OFF("lb6_external_ip_post_dnat", "Ether", ctx, sizeof(__u32),
			   LB6_EXTERNAL_IP_POST_DNAT, sizeof(BUF(LB6_EXTERNAL_IP_POST_DNAT)));
