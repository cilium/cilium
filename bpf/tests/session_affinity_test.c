...
static __always_inline int craft_packet(struct __ctx_buff *ctx) {
        ...
        struct ethhdr *eh = pktgen__push_ethhdr(&builder);
        if (!eh) return TEST_ERROR;
        *eh = (struct ethhdr){.h_source = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF},
                              .h_dest = {0x12, 0x23, 0x34, 0x45, 0x56, 0x67},
                              .h_proto = bpf_htons(ETH_P_IP)};

        struct iphdr *iph = pktgen__push_default_iphdr(&builder);
        if (!iph) return TEST_ERROR;

        iph->saddr = CLIENT_IP;
        iph->daddr = FRONTEND_IP;

        struct tcphdr *tcph = pktgen__push_default_tcphdr(&builder);
        if (!tcph) return TEST_ERROR;
...
int test1_setup(struct __ctx_buff *ctx) {
        ...
        for (unsigned long i = 0; i < NELEMS(services); i++) {
        ...
        for (unsigned long i = 0; i < NELEMS(backends); i++) {
