...
static inline void __setup_v4(void)
{
	struct {
		...
		SVC_KEY_VALUE(0, HOSTPORT_EXISTS_LOCALHOST, SVC_FLAG_HOSTPORT,
			      SVC_FLAG_LOOPBACK),
	};
	unsigned long i;
...
int test_v4_check(__maybe_unused struct xdp_md *ctx)
{
	...
	TEST("nodeport", {
		...
		ret = sock4_wildcard_lookup(
			&key, DONT_CARE(0), NODEPORT_LOOKUP, HOST_NS);
		assert(ret);
		...
		key.dport = bpf_htons(NODEPORT_EXISTS);
		ret = sock4_wildcard_lookup(
			&key, DONT_CARE(0), NODEPORT_LOOKUP, OTHER_NS);
		assert(!ret);
	...
	TEST("hostport", {
		...
		ret = sock4_wildcard_lookup(
			&key, DONT_CARE(0), HOSTPORT_LOOKUP, HOST_NS);
		assert(ret);
		...
		key.dport = bpf_htons(HOSTPORT_EXISTS);
		ret = sock4_wildcard_lookup(
			&key, DONT_CARE(0), HOSTPORT_LOOKUP, OTHER_NS);
		assert(!ret);
...
static inline void __setup_v6_nodeport(const union v6addr *HOST_IP6)
{
	...
	for (i = 0; i < ARRAY_SIZE(services); i++)
		map_update_elem(&LB6_SERVICES_MAP_V2, &services[i].key,
				&services[i].value, BPF_ANY);
}
