 ...
 * populated in actual CGROUP_SOCK_ADDR hooks, we use it only for testing to
...
int test_get_netns_cookie(__maybe_unused const struct bpf_sock_addr *addr)
{
	...
	if (!sk)
		...
		return sk->src_ip4 == v4_pod_one ? NETNS_COOKIE : NETNS_COOKIE2;
}
...
int test_sock4_xlate_fwd_skip_lb(__maybe_unused struct xdp_md *ctx)
{
	...
	/* Skip LB xlate when pod_one is connecting to v4_svc_one:tcp_dst_one. */
	...
	/* LB xlate happens when pod_one is connecting to v4_svc_two:tcp_dst_two. */
	...
	/* LB xlate happens when pod_two is connecting to v4_svc_one:tcp_dst_one. */
