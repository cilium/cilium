...
test_get_netns_cookie(__maybe_unused const struct bpf_sock_addr *addr)
{
	...
	if (sk->src_ip4 == v4_pod_one ||
	    (sk->src_ip6[0] == bpf_htonl(0xfd040000) && sk->src_ip6[1] == 0 &&
	     sk->src_ip6[2] == 0 && sk->src_ip6[3] == bpf_htonl(1)))
		return NETNS_COOKIE;
	else
...
int test_sock4_xlate_fwd_skip_lb(__maybe_unused struct xdp_md *ctx)
{
	...
	__u16 revnat_id = 1;
	__u16 revnat_id2 = 2;
	struct bpf_sock sk = { .src_ip4 = v4_pod_one };
	...
	lb_v4_add_service_with_flags(
		v4_svc_one, tcp_svc_one, 1, revnat_id, 0, SVC_FLAG_LOCALREDIRECT);
	lb_v4_add_service_with_flags(
		v4_svc_two, tcp_svc_two, 1, revnat_id2, 0, SVC_FLAG_LOCALREDIRECT);
	lb_v4_add_backend(
		v4_svc_one, tcp_svc_one, 1, 124, v4_pod_one, tcp_dst_one,
		IPPROTO_TCP, 0);
	lb_v4_add_backend(
		v4_svc_two, tcp_svc_two, 1, 124, v4_pod_one, tcp_dst_one,
		IPPROTO_TCP, 0);
	...
	addr.user_ip4 = v4_svc_one, addr.user_port = tcp_svc_one,
...
int test_sock6_xlate_fwd_skip_lb(__maybe_unused struct xdp_md *ctx)
{
	...
	lb_v6_add_service_with_flags(
		&frontend_ip1, tcp_svc_one, 1, revnat_id, 0,
		SVC_FLAG_LOCALREDIRECT);
	lb_v6_add_service_with_flags(
		&frontend_ip2, tcp_svc_two, 1, revnat_id2, 0,
		SVC_FLAG_LOCALREDIRECT);
	lb_v6_add_backend(
		&frontend_ip1, tcp_svc_one, 1, 124, (union v6addr *)V6_BACKEND1,
		tcp_dst_one, IPPROTO_TCP, 0);
	lb_v6_add_backend(
		&frontend_ip2, tcp_svc_two, 1, 124, (union v6addr *)V6_BACKEND1,
		tcp_dst_one, IPPROTO_TCP, 0);
	...
	test_log("ret: %d", ret);
	test_log("pod_one [%lx] -> svc_one [%lx]", addr.sk->src_ip6[0],
		 addr.user_ip6[0]);
	...
	ret = __sock6_xlate_fwd(&addr, false);
	test_log("ret: %d", ret);
	test_log("pod_one [%lx] -> svc_two [%lx]", addr.sk->src_ip6[0],
		 addr.user_ip6[0]);
	...
	ret = __sock6_xlate_fwd(&addr, false);
	test_log("ret: %d", ret);
	test_log("pod_two [%lx] -> svc_one [%lx]", addr.sk->src_ip6[0],
		 addr.user_ip6[0]);
	assert(addr.user_ip6[0] == bpf_htonl(0xfd040000));
	assert(addr.user_ip6[1] == 0);
	assert(addr.user_ip6[2] == 0);
