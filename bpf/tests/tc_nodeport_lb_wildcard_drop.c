...
static const union v6addr client_ip6 = { .addr = v6_ext_node_one_addr };
static const union v6addr frontend_ip6 = { .addr = v6_svc_one_addr };
static const union v6addr backend_ip6 = { .addr = v6_pod_one_addr };
...
static __always_inline int build_packet6(struct __ctx_buff *ctx,
					 const __be16 fe_dport, const __u8 proto)
{
	struct pktgen builder;
	void *data;
	...
	case IPPROTO_TCP: {
		struct tcphdr *l4 = pktgen__push_ipv6_tcp_packet(&builder,
					(__u8 *)client_mac,
					(__u8 *)lb_mac,
					(__u8 *)client_ip6.addr,
					(__u8 *)frontend_ip6.addr,
					CLIENT_PORT,
					fe_dport);
		if (!l4)
			return TEST_ERROR;
...
static __always_inline void setup_services6(struct __ctx_buff *ctx __maybe_unused)
{
	__u16 revnat_id = 2;

	/* Insert the real Load Balancer VIP */
	lb_v6_add_service(&frontend_ip6, FRONTEND_PORT, IPPROTO_TCP, 1, revnat_id);

	/* Insert the wildcard entry for the VIP */
	lb_v6_add_service(&frontend_ip6, LB_SVC_WILDCARD_DPORT,
			  LB_SVC_WILDCARD_PROTO, 0, revnat_id);

	/* Not sure if this is needed! */
	ipcache_v6_add_entry(&backend_ip6, 0, 112233, 0, 0);
}

static __always_inline int validate_packet6(const struct __ctx_buff *ctx,
					    const __u8 *smac, const __u8 *dmac,
...
int tc_nodeport_lb6_wildcard_drop_unknown_dport_check(__maybe_unused const struct __ctx_buff *ctx)
{
	return validate_packet6(ctx, (__u8 *)client_mac, (__u8 *)lb_mac, client_ip6.addr,
				frontend_ip6.addr, UNKNOWN_PORT, IPPROTO_TCP, CTX_ACT_DROP);
}
...
{
	return validate_packet6(ctx, (__u8 *)client_mac, (__u8 *)lb_mac, client_ip6.addr,
				frontend_ip6.addr, FRONTEND_PORT, IPPROTO_UDP, CTX_ACT_DROP);
}
...
{
	/* We should receive an ICMP Unreachable */
	return validate_packet6(ctx, (__u8 *)lb_mac, (__u8 *)client_mac, frontend_ip6.addr,
...
int tc_nodeport_lb6_wildcard_drop_not_unknown2_check(__maybe_unused const struct __ctx_buff *ctx)
{
	/* We should receive an ICMP Unreachable */
	return validate_packet6(ctx, (__u8 *)lb_mac, (__u8 *)client_mac, frontend_ip6.addr,
				client_ip6.addr, FRONTEND_PORT, IPPROTO_ICMPV6, CTX_ACT_OK);
}
