...
pipeline {
    ...
    environment {
        ...
        BASE_IMAGE="""${sh(
                returnStdout: true,
                script: 'if [ "${run_with_race_detection}" = "" ]; then echo -n "scratch"; else echo -n "quay.io/cilium/cilium-runtime:f962d31d5f1b9939171cf2eb61d79f4dc9eed500@sha256:f51f573458442c95cbe5df1e9d6207b678f15a129bd1ff6a53af67cd125d7b66"; fi'
            )}"""
    }
