...
pipeline {
    ...
    environment {
        ...
        BASE_IMAGE="""${sh(
                returnStdout: true,
                script: 'if [ "${run_with_race_detection}" = "" ]; then echo -n "scratch"; else echo -n "quay.io/cilium/cilium-runtime:72cdf227ea0eb1a09b9f422dfbc9a8fec8bf96c0@sha256:ec9dce80197f10e1f46ce92f79738c3e56b51b355516d48277eeea1675e13b41"; fi'
            )}"""
    }
