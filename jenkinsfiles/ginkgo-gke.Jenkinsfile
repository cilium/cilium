            ...
            parallel {
                stage('Wait for images') {
                    ...
                    steps {
                        retry(25) {
                            sleep(time: 60)
                            sh 'docker buildx imagetools inspect --raw ${IMAGE_REGISTRY}/cilium-ci:${DOCKER_TAG} &> /dev/null'
                            sh 'docker buildx imagetools inspect --raw ${IMAGE_REGISTRY}/operator-generic-ci:${DOCKER_TAG} &> /dev/null'
                            sh 'docker buildx imagetools inspect --raw ${IMAGE_REGISTRY}/hubble-relay-ci:${DOCKER_TAG} &> /dev/null'
                        }
                    }
                }
