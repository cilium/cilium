...
type namespaceWatcher struct {
	...
	// Track all known namespaces for backfill operations
	allNamespaces sets.Set[string]
...
func (nw *namespaceWatcher) updateNamespace(ns *slim_corev1.Namespace) {
	...
	// Track this namespace as known
	nw.allNamespaces.Insert(ns.Name)

	wasAnnotated := nw.annotated.Has(ns.Name)
	wasGlobal := nw.global.Has(ns.Name)
	wasFilteringActive := nw.filteringActive
	...
	transitionedToInactive := wasFilteringActive && !nw.filteringActive

	// Check if we transitioned from filtering inactive to active (cleanup case)
	transitionedToActive := !wasFilteringActive && nw.filteringActive

	// Copy processors while holding the lock to avoid the double locking issue
	var processors []NamespaceProcessor
	if needsProcessing || transitionedToInactive || transitionedToActive {
		processors = make([]NamespaceProcessor, len(nw.processors))
		copy(processors, nw.processors)
	}

	// Get all known namespaces for backfill if needed
	var namespacesToBackfill []string
	if transitionedToInactive {
		namespacesToBackfill = nw.allNamespaces.UnsortedList()
	}

	// Get all known namespaces for cleanup if needed
	var namespacesToCleanup []string
	if transitionedToActive {
		namespacesToCleanup = nw.allNamespaces.UnsortedList()
	}
	...
	if transitionedToActive {
		...
		for _, namespaceName := range namespacesToCleanup {
			...
			if namespaceName != ns.Name {
				...
				shouldBeGlobal := nw.isGlobalNamespaceUnlocked(namespaceName)
				if !shouldBeGlobal {
...
func (nw *namespaceWatcher) deleteNamespace(ns *slim_corev1.Namespace) {
	...
	nw.allNamespaces.Delete(ns.Name)
	nw.annotated.Delete(ns.Name)
	nw.global.Delete(ns.Name)
	...
	var processors []NamespaceProcessor
	if needsProcessing || transitionedToInactive {
		processors = make([]NamespaceProcessor, len(nw.processors))
		copy(processors, nw.processors)
	}
