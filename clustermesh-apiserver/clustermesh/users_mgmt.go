...
func (us *usersManager) Start(cell.HookContext) error {
	...
	if err != nil {
		us.logger.Error("Unable to setup config watcher", slog.Any(logfields.Error, err))
...
func (us *usersManager) sync(ctx context.Context) error {
	if us.client == nil {
		...
		if err != nil {
			us.logger.Error("Unable to retrieve the kvstore client", slog.Any(logfields.Error, err))
			return err
		}
		us.client = client
	}
	...
	for _, user := range users.Users {
		...
		if !found || role != user.Role {
			...
			us.logger.Info("User successfully configured", slog.Any(logfields.User, user.Name))
		}
	...
	for user := range stale {
		...
		us.logger.Info("User successfully removed", slog.Any(logfields.User, user))
