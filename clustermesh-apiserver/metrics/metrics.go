...
func (mm *metricsManager) Start(cell.HookContext) error {
	...
	go func() {
		mm.logger.Info(
			...
			slog.Any("address", mm.server.Addr),
		)
		if err := mm.server.ListenAndServe(); !errors.Is(err, http.ErrServerClosed) {
			logging.Fatal(mm.logger, "Unable to start metrics server", slog.Any(logfields.Error, err))
		}
	}()
...
func (mm *metricsManager) Stop(ctx cell.HookContext) error {
	...
	if err := mm.server.Shutdown(ctx); err != nil {
		mm.logger.Error("Shutdown metrics server failed", slog.Any(logfields.Error, err))
		return err
	}
