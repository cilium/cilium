		...
		t.Run(name, func(t *testing.T) {
			...
			hive := hive.New(
				...
				cell.Provide(func() Config {
					return Config{EnableStaleCiliumEndpointCleanup: true}
				}),
				cell.Provide(func() *option.DaemonConfig {
					return &option.DaemonConfig{
						DisableCiliumEndpointCRD:  false,
						EnableCiliumEndpointSlice: test.enableCES,
					}
				}),
			...
			if err := testutils.WaitUntil(func() bool {
				mu.Lock()
				defer mu.Unlock()
				slices.Sort(deletedSet)
				slices.Sort(test.expectedDeletedSet)
				return reflect.DeepEqual(deletedSet, test.expectedDeletedSet)
			}, time.Second); err != nil {
				t.Fatalf("expected deleted set to be %v, got %v", test.expectedDeletedSet, deletedSet)
			}
