...
var (
	...
	// ErrControllerNotFound is returned when a controller lookup fails.
	ErrControllerNotFound = errors.New("controller not found")

	// ErrControllerMapEmpty is returned when the controller map has not been initialized.
...
func (m *Manager) RemoveControllerAndWait(name string) error {
	...
	if err != nil {
		if errors.Is(err, ErrControllerNotFound) || errors.Is(err, ErrControllerMapEmpty) {
			return nil
		}
		return err
	}
