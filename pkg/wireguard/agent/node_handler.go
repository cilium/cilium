...
func (a *Agent) nodeUpsert(node nodeTypes.Node) error {
	...
	podAllocCIDRs := append(node.GetIPv4AllocCIDRs(), node.GetIPv6AllocCIDRs()...)

	if err := a.UpdatePeer(node.Fullname(), node.WireguardPubKey, newIP4, newIP6, podAllocCIDRs); err != nil {
		log.WithError(err).
