...
func TestCell(t *testing.T) {
	...
	testRunDir, err := os.MkdirTemp("", "wireguard_go_test")
	require.NoError(t, err)
	// Save it to option.Config.StateDir, used by non-cellified modules.
	oldConfig := *option.Config
	defer func() { *option.Config = oldConfig }()
	option.Config.RunDir = testRunDir
	option.Config.StateDir = testRunDir

	// Register cleanup actions to clean cilium_wg0 interface and the temporary StateDir.
	t.Cleanup(func() {
	...
	// First, we test the Hive with WireGuard enabled.
	t.Run("WireguardEnabled", func(t *testing.T) {
		...
		// Ensure wireguard Agent is enabled.
		assert.True(t, wgAgent.Enabled())

		// Ensure the link cilium_wg0 has been created.
		link, err := safenetlink.LinkByName(types.IfaceName)
		require.NoError(t, err)

		// Now we want to ensure also that the other kvstore-sync-obsolete job worked.
		// To do that, let's add an obsolete peer to the wireguard Device (but not to the agent),
		// then trigger the node discovery and see if the agent removed the obsolete peer.
		dev, err := wgAgent.wgClient.Device(types.IfaceName)
		require.NoError(t, err)
		assert.Empty(t, dev.Peers)

		// Inject the obsolete peer.
		cfg := wgtypes.Config{
			PrivateKey:   nil,
			ListenPort:   nil,
			FirewallMark: nil,
		...
		assert.EventuallyWithT(t, func(c *assert.CollectT) {
			// Ensure MTU is set accordingly (mtu-reconciler job).
			assert.Equal(c, mtu.EthernetMTU-mtu.WireguardOverhead, link.Attrs().MTU)

			// Ensure the obsolete peer has been removed (peer-reconciler job).
			dev, err = wgAgent.wgClient.Device(types.IfaceName)
			require.NoError(c, err)
			assert.Empty(c, dev.Peers)

			// Ensure local node has been updated (localnode-updater job).
		...
		// Upsert an identity with ipcache.
		ipCache.Upsert(pod2IPv4Str, k8s2NodeIPv4, 0, nil, ipcache.Identity{ID: 1, Source: source.Kubernetes})

		assert.EventuallyWithT(t, func(c *assert.CollectT) {
			// Ensure the agent subscribed to node events (nodemanger-subscribe job).
			assert.Contains(t, wgAgent.nodeNameByNodeIP, k8s2NodeIPv4.String())

			// Ensure the agent reacts to the new ipcache identity (ipcache-listener job).
			assert.Contains(t, wgAgent.peerByNodeName, k8s2NodeName)
		...
		// Ensure the link cilium_wg0 is not deleted.
		_, err = safenetlink.LinkByName(types.IfaceName)
		require.NoError(t, err)
	})

	// Now test with WireGuard disabled.
	t.Run("WireguardDisabled", func(t *testing.T) {
		...
		// Assert wireguard Agent is disabled.
