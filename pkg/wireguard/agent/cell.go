...
const (
	...
	NodeEncryptionOptOutLabelsFlag   = "node-encryption-opt-out-labels"
)
...
func (def UserConfig) Flags(flags *pflag.FlagSet) {
	flags.Bool(EnableWireguardFlag, def.EnableWireguard, "Enable WireGuard")
	flags.Duration(WireguardPersistentKeepaliveFlag, def.WireguardPersistentKeepalive, "The Wireguard keepalive interval as a Go duration string")
	flags.Bool(WireguardTrackAllIPsFallbackFlag, def.WireguardTrackAllIPsFallback, "Force WireGuard to track all IPs")
	flags.MarkHidden(WireguardTrackAllIPsFallbackFlag)
	flags.String(NodeEncryptionOptOutLabelsFlag, def.NodeEncryptionOptOutLabels, "Label selector for nodes which will opt-out of node-to-node encryption")
}
