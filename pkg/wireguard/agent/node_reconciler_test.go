...
type immediateReconciler struct {
	called chan struct{}
}

func (r *immediateReconciler) Prune() {}

func (r *immediateReconciler) WaitUntilReconciled(
	_ context.Context,
	revision statedb.Revision,
) (statedb.Revision, statedb.Revision, error) {
	close(r.called)
...
func TestNodeReconciler(t *testing.T) {
	...
	require.NoError(t, a.Delete(t.Context(), nil, 3, n))
}
...
func TestWaitForNodeReconciliationWaitsForRetries(t *testing.T) {
	...
	require.NoError(t, err)
	revision := nodes.Revision(txn.Commit())

	r := &immediateReconciler{called: make(chan struct{})}
	a := &Agent{
		db:             db,
		nodes:          nodes,
		nodeReconciler: r,
	...
	go func() {
		done <- a.waitForNodeReconciliation(t.Context(), revision)
	}()
	<-r.called
