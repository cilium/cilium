...
func (a *Agent) Init() error {
	...
	if option.Config.EnableIPv4 {
		if err := sysctl.Write(fmt.Sprintf("net.ipv4.conf.%s.rp_filter", types.IfaceName), "0"); err != nil {
			return nil
		}
	}
...
func (a *Agent) UpdatePeer(nodeName, pubKeyHex string, nodeIPv4, nodeIPv6 net.IP) error {
	...
	if allowedIPs == nil {
		...
		if option.Config.EnableIPv4 && nodeIPv4 != nil {
			lookupIPv4 = nodeIPv4
			allowedIPs = append(allowedIPs, *iputil.IPToPrefix(nodeIPv4))
		}
		if option.Config.EnableIPv6 && nodeIPv6 != nil {
			lookupIPv6 = nodeIPv6
			allowedIPs = append(allowedIPs, *iputil.IPToPrefix(nodeIPv6))
		}
...
func (a *Agent) OnIPIdentityCacheChange(modType ipcache.CacheModification, ipnet net.IPNet, oldHostIP, newHostIP net.IP,
	_ *identity.NumericIdentity, _ identity.NumericIdentity, _ uint8, _ *ipcache.K8sMetadata) {
