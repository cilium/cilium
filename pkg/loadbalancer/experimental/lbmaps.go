...
type maglevMaps interface {
	/*
		TODO: Perhaps it would be better to split UpdateMaglev into the following two methods:
			AddInnerMaglev([]loadbalancer.BackendID) (lbmap.MaglevOuterVal, error)
			UpdateMaglev(lbmap.MaglevOuterKey, lbmap.MaglevOuterVal, ipv6 bool) error
	*/
...
var (
	...
	maglevInnerMapSpec = &ebpf.MapSpec{
		...
		ValueSize:  lbmap.MaglevBackendLen * uint32(option.Config.MaglevTableSize),
		MaxEntries: 1,
	}
...
func (r *BPFLBMaps) UpdateMaglev(key lbmap.MaglevOuterKey, backendIDs []loadbalancer.BackendID, ipv6 bool) error {
	/*
		TODO: Should we delete the previous inner map (if it exists) from the kernel? Will it be garbage-collected automatically?
		I'm following the logic of updateMaglevTable() from pkg/maps/lbmap/maglev.go, but it's somehow strange that it creates
		a new inner map on every update.
	*/
...
func (r *BPFLBMaps) DeleteMaglev(key lbmap.MaglevOuterKey, ipv6 bool) error {
	...
	var val lbmap.MaglevOuterVal
	err := ebpfmap.LookupAndDelete(outerKey, &val)
	if errors.Is(err, ebpf.ErrKeyNotExist) {
		return nil
	}
	/*
		TODO: Should we use val to delete the inner table from the kernel or is it garbage-collected automatically?
		In deleteMaglevTable() from pkg/maps/lbmap/maglev.go, only the method Delete() is called
		(similar to LookupAndDelete() used above), so the inner map seems not to be explicitly deleted.
		However, the doc string for deleteMaglevTable() claims that the inner table is deleted,
		so perhaps it happens automatically.
	*/
	return err
}
...
func (r *BPFLBMaps) DumpMaglev(cb func(lbmap.MaglevOuterKey, lbmap.MaglevOuterVal, lbmap.MaglevInnerKey, *lbmap.MaglevInnerVal, bool)) error {
	...
	cbWrap := func(key, value any, ipv6 bool) {
		...
		if err != nil {
			errs = append(errs, fmt.Errorf("lookup up first inner map key (backends) for map with id %d: %w", maglevValue.FD, err))
		}
...
func (f *FakeLBMaps) UpdateMaglev(key lbmap.MaglevOuterKey, backendIDs []loadbalancer.BackendID, ipv6 bool) error {
	...
	var singletonKey lbmap.MaglevInnerKey
	// value, ok := outer.Map.Load(bpfKey(&key))
	// if ok {
	// 	currentID := value.b.(lbmap.MaglevOuterVal).FD
	// 	inner := f.inners[currentID]
