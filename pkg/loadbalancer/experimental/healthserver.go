...
func registerHealthServer(params healthServerParams) {
	if !params.ExtConfig.EnableHealthCheckNodePort {
		return
	}
...
func (s *healthServer) controlLoop(ctx context.Context, health cell.Health) error {
	...
	for {
		...
		for change := range changes {
			...
			if port == 0 || svc.ProxyRedirect != nil || found || change.Deleted {
				continue
			}
...
func isExtLocal(fe *Frontend) bool {
	...
	case lb.SVCTypeNodePort, lb.SVCTypeLoadBalancer, lb.SVCTypeExternalIPs:
		return fe.service.ExtTrafficPolicy == lb.SVCTrafficPolicyLocal
	default:
		return false
	}
}
...
func (h *httpHealthServer) getLocalEndpointCount() int {
	...
	for be := range h.backends.List(txn, BackendByServiceName(h.name)) {
		if len(be.NodeName) > 0 && be.NodeName != h.nodeName {
			continue
		}
