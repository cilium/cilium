...
func (c *lrpController) processLRP(wtxn experimental.WriteTxn, lrpID k8s.ServiceID) (*statedb.WatchSet, func(experimental.WriteTxn)) {
	...
	case lrpConfigTypeSvc:
		...
		// Grab a watch channel for all the associated frontends to re-set redirects
		// if the frontends change.
		_, watch := c.p.Writer.Frontends().ListWatch(wtxn, experimental.FrontendByServiceName(targetName))
		ws.Add(watch)
