	...
	for {
		...
		for change := range changes {
			...
			if change.Deleted || !backend.IsAlive() {
				opSupported := terminateUDPConnectionsToBackend(p, sd, backend.Address)
				if !opSupported {
...
func terminateUDPConnectionsToBackend(p socketTerminationParams, sd sockets.SocketDestroyer, l3n4Addr lb.L3n4Addr) (opSupported bool) {
	opSupported = true
	...
	destroy := func(nsName string, ns *netns.NetNS) error {
		err := p.NetNSOps.do(ns, func() error {
			return sd.Destroy(sockets.SocketFilter{
				Family:    family,
				Protocol:  protocol,
				DestIp:    ip,
