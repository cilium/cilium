...
func TestPrefixMask(t *testing.T) {
	...
	for i := minID; i <= maxID; i++ {
		...
		require.NotEqual(t, id, unmaskedID)
		require.Equal(t, id.String(), val)
	}
...
func testAllocator(t *testing.T, maxID idpool.ID, allocatorName string, suffix string) {
	backend := newDummyBackend()
	...
	for i := idpool.ID(1); i <= maxID; i++ {
		key := TestAllocatorKey(fmt.Sprintf("key%04d", i))
		require.Nil(t, allocator.localKeys.keys[allocator.encodeKey(key)])
	}
...
func TestAllocateCached(t *testing.T) {
	testAllocator(t, idpool.ID(256), randomTestName(), "a") // enable use of local cache
}
