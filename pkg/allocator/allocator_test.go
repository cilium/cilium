...
func (s *AllocatorSuite) TestObserveAllocatorChanges(c *C) {
	backend := newDummyBackend()
	...
	for i := 0; i < numAllocations; i++ {
		...
		c.Assert(change.Kind, Equals, AllocatorChangeUpsert)
	}

	// After replay we should see a sync event.
	change := <-changes
	c.Assert(change.Kind, Equals, AllocatorChangeSync)
	...
	change = <-changes
	c.Assert(change.Kind, Equals, AllocatorChangeDelete)
	c.Assert(change.Key, Equals, TestAllocatorKey("remote"))
