...
type Allocator struct {
	...
	//FIXME:used in lockedAllocate. Should it be here? probably since it is agent-level and not per-backend
...
// FIXME: is this kvstore.KVLocker? Comparator moved here
type Lock interface {
	Unlock() error
...
// FIXME: What is missing? *IfLocked?
type Backend interface {
	// DeleteAllKeys will delete all keys
	DeleteAllKeys()

	//FIXME these 2 are missing in kvstoreBackend? :/
	...
	//FIXME: stakeKeysPrevRound added in first rebase
	RunGC(staleKeysPrevRound map[string]uint64) (map[string]uint64, error)

	//FIXME: missing
	Status() (string, error)

	//FIXME: kvstore.KVLock -> Lock on thse 3 functions
	//FIXME: rename this?
...
//FIXME: Add the backend name to the error
...
type AllocatorKey interface {
	...
	//FIXME: added by tgraf. the map is labels?
...
//FIXME: master calls createValueNode from here, but it also has:
...
func (a *Allocator) lockedAllocate(ctx context.Context, key AllocatorKey) (idpool.ID, bool, error) {
	...
	k := key.GetKey()
	lock, err := a.backend.Lock(ctx, key) //FIXME: was a.lockPath
	if err != nil {
		return 0, false, err
	}
	...
	//FIXME: added on master. double check
	...
	if value == 0 { //FIXME:added on master
	...
	if value != 0 {
		...
		// mark the key as verified in the local cache
		if err := a.localKeys.verify(k); err != nil { //FIXME: this replaces an invocation from createValueNodeKey ?
			log.WithError(err).Error("BUG: Unable to verify local key")
		}

		//FIXME: should this happen before the verify?
	...
	//FIXME: may also be missing lease=false bool but all invocations use false? WTF
...
// lock.
//FIXME: should this be on backend or allocator?
...
func (a *Allocator) syncLocalKeys() error {
	...
	for id, value := range ids {
		//FIXME: this was a.recreateMasterKey(id, value, false) on master
	...
	go func(a *Allocator) {
		for {
			if err := a.syncLocalKeys(); err != nil {
				//FIXME: misssing .WithFields(logrus.Fields{fieldPrefix: a.idPrefix}).
