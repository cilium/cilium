...
func registerNodeReconciler(
	...
	lifecycle.Append(cell.Hook{
		OnStart: func(cell.HookContext) error {
			nodeTable := nodeWriter.Table().(statedb.RWTable[*node.Node])
			nodeWriter.RegisterReconciler(node.IPCacheNodeReconciler)
			_, err := reconciler.Register(
				params,
				nodeTable,
				(*node.Node).DeepCopy,
				func(n *node.Node, status reconciler.Status) *node.Node {
					n.Statuses = n.Statuses.Set(node.IPCacheNodeReconciler.String(), status)
					return n
				},
				func(n *node.Node) reconciler.Status {
					return n.Statuses.Get(node.IPCacheNodeReconciler.String())
				},
				&nodeReconcilerOps{
					db:          params.DB,
					nodes:       nodeTable,
				...
				nil,
				reconciler.WithName(node.IPCacheNodeReconciler.String()),
				reconciler.WithoutPruning(),
			)
			if err != nil {
				nodeWriter.UnregisterReconciler(node.IPCacheNodeReconciler)
...
func (ops *nodeReconcilerOps) invalidateRemoteNodes() error {
	...
	for n := range ops.nodes.All(txn) {
		...
		status := n.Statuses.Get(node.IPCacheNodeReconciler.String())
		if status.IsPendingOrRefreshing() {
			continue
		}
		n = n.DeepCopy()
		n.Statuses = n.Statuses.Set(
			node.IPCacheNodeReconciler.String(),
...
func nodePrefixCluster(
	n *node.Node,
	prefix netip.Prefix,
) cmtypes.PrefixCluster {
