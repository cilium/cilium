...
func (nh *nodeOps) Prune(context.Context, statedb.ReadTxn, statedb.Iterator[node.Node]) error {
	return nil
}
...
func registerNodeReconciler(g job.Group, p nodeOpsParams) error {
	...
	config := reconciler.Config[node.Node]{
		Table:                   p.Nodes,
	...
	return reconciler.Register(config, p.ReconcilerParams)
}
