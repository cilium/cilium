...
func (s *IPCacheTestSuite) TestIPCache(c *C) {
	...
	IPIdentityCache.Upsert(endpointIP, nil, nil, Identity{
		ID:     identity,
		Source: FromKVStore,
	})
	...
	updated := IPIdentityCache.Upsert(endpointIP, nil, nil, Identity{
		ID:     identity,
	...
	for index := range endpointIPs {
		IPIdentityCache.Upsert(endpointIPs[index], nil, nil, Identity{
			ID:     identities[index],
			Source: FromKVStore,
		})
