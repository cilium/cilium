...
func TestNodeReconcilerInvalidatesRemoteNodes(t *testing.T) {
	...
	local.Statuses = local.Statuses.Set(node.IPCacheNodeReconciler.String(), reconciler.StatusDone())
	remote := &node.Node{Node: nodeTypes.Node{Name: "remote"}}
	remote.Statuses = remote.Statuses.Set(node.IPCacheNodeReconciler.String(), reconciler.StatusDone())
	txn := db.WriteTxn(nodes)
	_, _, err := nodes.Insert(txn, local)
	require.NoError(t, err)
	...
	require.Equal(t, reconciler.StatusKindPending,
		remote.Statuses.Get(node.IPCacheNodeReconciler.String()).Kind,
	)
	local, _, found = nodes.Get(db.ReadTxn(), node.LocalNodeQuery)
	require.True(t, found)
	require.Equal(t, reconciler.StatusKindDone,
		local.Statuses.Get(node.IPCacheNodeReconciler.String()).Kind,
	)
}
