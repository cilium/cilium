	...
	if err != nil {
		if err2 := identity.ReleaseSlice(prefixIdentities); err2 != nil {
			log.WithError(err2).WithFields(logrus.Fields{
				"identities": prefixIdentities,
			}).Debug("Failed during CIDR allocation")
		}
	}
...
func ReleaseCIDRs(prefixes []*net.IPNet) (err error) {
	...
	prefixIdentities, err2 := identity.LookupCIDRIdentities(prefixes)
	if err2 != nil {
		if err == nil {
			err = err2
		}
		scopedLog.WithError(err2).Debug(
			"Could not locate identities for CIDRs during release")
	}
