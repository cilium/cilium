...
func newHealthV2Provider(params HealthProviderParams) (types.Provider, *HealthProvider, error) {
	p := &HealthProvider{
		db: params.DB,
	}
	p.PrimaryIndex = statedb.Index[types.Status, types.HealthID]{
		Name: "identifier",
	...
	p.LevelIndex = statedb.Index[types.Status, types.Level]{
		Name: "level",
		FromObject: func(s types.Status) index.KeySet {
	...
	var err error
	p.StatusTable, err = statedb.NewTable(HealthTable,
		p.PrimaryIndex,
		p.LevelIndex)
	p.Statuses = p.StatusTable
	if err != nil {
		return nil, nil, err
	}
...
func (p *HealthProvider) ForModule(mid types.FullModuleID) types.Health {
	return &moduleReporter{
		...
		upsert: func(s types.Status) error {
			...
			defer tx.Abort()
			old, _, found := p.StatusTable.First(tx, p.PrimaryIndex.QueryFromObject(s))
		...
		deletePrefix: func(i types.Identifier) error {
			...
			tx := p.db.WriteTxn(p.StatusTable)
			defer tx.Abort()
			q := p.PrimaryIndex.Query(types.HealthID(i.String()))
		...
		stop: func(i types.Identifier) error {
			...
			tx := p.db.WriteTxn(p.StatusTable)
			defer tx.Abort()
			old, _, found := p.StatusTable.First(tx, p.PrimaryIndex.QueryFromObject(types.Status{ID: i}))
			if found && !old.Stopped.IsZero() {
