...
func newServer(lc cell.Lifecycle, log logging.FieldLogger, cfg Config) Server {
	if !cfg.Pprof {
		return nil
	}
...
func (s *server) Start(ctx cell.HookContext) error {
	...
	s.logger = s.logger.With(
		slog.Any("ip", s.listener.Addr().(*net.TCPAddr).IP),
		slog.Int("port", s.listener.Addr().(*net.TCPAddr).Port),
	)
	...
	go func() {
		if err := s.httpSrv.Serve(s.listener); !errors.Is(err, http.ErrServerClosed) {
			s.logger.Error("server stopped unexpectedly", slog.Any(logfields.Error, err))
		}
	}()
