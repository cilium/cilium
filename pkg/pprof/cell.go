...
func (def Config) Flags(flags *pflag.FlagSet) {
	...
	flags.Int(PprofBlockProfileRate, def.PprofBlockProfileRate, "Enable goroutine blocking profiling and set the rate of sampled events in nanoseconds (set to 1 to sample all event [warning: performance overhead])")
}
...
func newServer(lc cell.Lifecycle, log *slog.Logger, cfg Config) Server {
	...
	if cfg.PprofMutexProfileFraction > 0 {
		...
		log.Info("Enabled mutex contention profiling", "fraction", cfg.PprofMutexProfileFraction)
	}
	if cfg.PprofBlockProfileRate > 0 {
		runtime.SetBlockProfileRate(cfg.PprofBlockProfileRate)
		log.Info("Enabled goroutine block profiling", "rate", cfg.PprofBlockProfileRate)
	}
