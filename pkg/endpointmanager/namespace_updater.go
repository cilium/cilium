...
func registerNamespaceUpdater(jg job.Group, log *slog.Logger, cfg *option.DaemonConfig, db *statedb.DB, namespaces statedb.Table[daemonk8s.Namespace], em EndpointManager) {
	if !option.NetworkPolicyEnabled(cfg) {
		// Namespace labels are only needed when network policies are enabled
		return
	}
