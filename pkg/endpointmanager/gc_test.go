...
func fakeCheckHealthy(_ *endpoint.Endpoint) error {
	return nil
}

func isMarkedEndpoint(mgr *endpointManager, id uint16) bool {
	mgr.mutex.RLock()
...
func TestMarkAndSweepNoDoubleSweep(t *testing.T) {
	...
	require.False(t, isMarkedEndpoint(mgr, unhealthyID), "markedEndpoints should be empty after endpoint is deleted")
	...
	require.True(t, isMarkedEndpoint(mgr, healthyID), "healthyID should be marked in markedEndpoints after round 3 GC")
...
func TestMarkAndSweepEndpointIDRecover(t *testing.T) {
	...
	require.True(t, isMarkedEndpoint(mgr, unhealthyID), "unhealthyID should be marked in markedEndpoints after round 1 GC")
	...
	require.False(t, isMarkedEndpoint(mgr, unhealthyID), "recovered endpoint should be removed from markedEndpoints after round 2 GC")
}
