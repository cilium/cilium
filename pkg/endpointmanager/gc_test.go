...
//
// The old GC logic (mark first, then sweep) would cause:
//   - Round 1: mark the endpoint, then sweep (empty)
//   - Round 2: mark the same endpoint again, then sweep deletes it
//   - Round 3: if the endpoint ID is reused to create a new healthy endpoint,
//     the old mark list still contains that ID, causing the newly created
//     healthy endpoint to be incorrectly swept
//
// The new GC logic (sweep first, then mark) avoids this issue:
//   - Round 1: sweep (empty), then mark the unhealthy endpoint
//   - Round 2: sweep deletes the previously marked endpoint, then mark re-checks
//     (endpoint no longer exists at this point)
//   - Round 3: sweep (empty, because the endpoint didn't exist during the previous
//     mark phase), newly created healthy endpoint is not affected
func TestMarkAndSweepNoDoubleSweep(t *testing.T) {
	...
	require.Len(t, mgr.GetEndpoints(), 0)
	...
	ep2, err := endpoint.NewEndpointFromChangeModel(t.Context(), logger, nil, &endpoint.MockEndpointBuildQueue{}, nil, nil, nil, nil, nil, identitymanager.NewIDManager(logger), nil, nil, s.repo, testipcache.NewMockIPCache(), &endpoint.FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), ctmap.NewFakeGCRunner(), nil, model2)
	require.NoError(t, err)
	ep2.Start(healthyID)
	t.Cleanup(ep2.Stop)
	err = mgr.expose(ep2)
