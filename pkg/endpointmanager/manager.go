...
func SetPolicyRevision(endpointIDSet map[uint16]struct{}, revision uint64) {
	mutex.RLock()
	defer mutex.RUnlock()

	for endpointID := range endpointIDSet {
		ep := endpoints[endpointID]
		ep.Logger("endpointmanager").Info("setting policy revision for endpoint instead of regenerating")
		ep.SetPolicyRevision(revision)
	}
}
