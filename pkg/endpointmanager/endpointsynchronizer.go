	...
	e.UpdateController(controllerName,
		controller.ControllerParams{
			...
			DoFunc: func(ctx context.Context) (err error) {
				...
				// was created.
				scopedLog = e.Logger(subsysEndpointSync).With(slog.String("controller", controllerName))
				...
				if needInit {
					...
					case err == nil:
						...
						if err := updateCEPUID(scopedLog, e, localCEP); err != nil {
							scopedLog.Warn("could not take ownership of existing ciliumendpoint", slog.Any(logfields.Error, err))
							return err
						}
					case k8serrors.IsNotFound(err):
						...
						if err != nil {
							...
							scopedLog.Error("Cannot create CEP", slog.Any(logfields.Error, err))
							return err
						}

						scopedLog.Debug("storing CEP UID after create", slog.Any(logfields.CEPUID, localCEP.UID))
					...
					default:
						scopedLog.Warn("Error getting CEP", slog.Any(logfields.Error, err))
						return err
					}
				...
				if localCEP == nil {
					...
					case k8serrors.IsInvalid(err):
						scopedLog.Warn("Invalid CEP during update", slog.Any(logfields.Error, err))
						needInit = true
						return nil

					// A real error
					default:
						scopedLog.Error("Cannot get CEP during update", slog.Any(logfields.Error, err))
						return err
					}
				}
				...
				case err != nil && k8serrors.IsConflict(err):
					scopedLog.Warn("Cannot update CEP due to a revision conflict. The next controller execution will try again", slog.Any(logfields.Error, err))
					needInit = true
					return nil
				...
				case err != nil:
					...
					scopedLog.Error("Cannot update CEP", slog.Any(logfields.Error, err))
...
func updateCEPUID(scopedLog *slog.Logger, e *endpoint.Endpoint, localCEP *cilium_v2.CiliumEndpoint) error {
	...
	if cepUID := e.GetCiliumEndpointUID(); cepUID == "" {
		scopedLog.Debug(
			...
			slog.String(logfields.Node, types.GetName()),
			slog.Any("old"+logfields.CEPUID, cepUID),
			slog.Any(logfields.CEPUID, localCEP.UID),
		)
		e.SetCiliumEndpointUID(localCEP.UID)
...
func deleteCEP(ctx context.Context, scopedLog *slog.Logger, ciliumClient v2.CiliumV2Interface, e *endpoint.Endpoint) error {
	...
	scopedLog.Debug("deleting CEP with UID", slog.Any(logfields.CEPUID, cepUID))
	if err := ciliumClient.CiliumEndpoints(cepOwner.GetNamespace()).Delete(ctx, cepName, meta_v1.DeleteOptions{
		...
		if !k8serrors.IsNotFound(err) && !k8serrors.IsConflict(err) {
			scopedLog.Warn("Unable to delete CEP", slog.Any(logfields.Error, err))
		}
	}
	return nil
}
