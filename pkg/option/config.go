...
func (c *DaemonConfig) Populate() {
	...
	if len(hostServicesProtos) != 1 && len(hostServicesProtos) != 2 {
	...
	for i := 0; i < len(hostServicesProtos); i++ {
		...
		case HostServicesTCP:
			if c.EnableHostServicesTCP {
				log.Fatalf("Duplicate protocol %s for host reachable services!",
					HostServicesTCP)
			}
			c.EnableHostServicesTCP = true
		case HostServicesUDP:
			if c.EnableHostServicesUDP {
				log.Fatalf("Duplicate protocol %s for host reachable services!",
					HostServicesUDP)
			}
