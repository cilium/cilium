...
const (
	...
	CTMapEntriesTimeoutSrvTCPName    = "bpf-ct-timeout-service-tcp"
	CTMapEntriesTimeoutSrvTCPDefault = "6h"
	CTMapEntriesTimeoutSrvAnyName    = "bpf-ct-timeout-service-any"
	CTMapEntriesTimeoutSrvAnyDefault = "1m"
...
type DaemonConfig struct {
	...
	CTMapEntriesTimeoutSrvTCP time.Duration
	CTMapEntriesTimeoutSrvAny time.Duration
...
func (c *DaemonConfig) Validate() error {
	...
	timeCheck := []time.Duration{
		...
		c.CTMapEntriesTimeoutSrvTCP,
		c.CTMapEntriesTimeoutSrvAny,
...
func (c *DaemonConfig) Populate() {
	...
	if err != nil {
		log.WithError(err).Fatal("Unable to parse %s duration!", CTMapEntriesTimeoutTCPName)
	}
	...
	if err != nil {
		log.WithError(err).Fatal("Unable to parse %s duration!", CTMapEntriesTimeoutAnyName)
	}
	c.CTMapEntriesTimeoutAny = c.CTMapEntriesTimeoutAny.Round(time.Second)

	c.CTMapEntriesTimeoutSrvTCP, err = time.ParseDuration(viper.GetString(CTMapEntriesTimeoutSrvTCPName))
	if err != nil {
		log.WithError(err).Fatal("Unable to parse %s duration!", CTMapEntriesTimeoutSrvTCPName)
	}
	c.CTMapEntriesTimeoutSrvTCP = c.CTMapEntriesTimeoutSrvTCP.Round(time.Second)

	c.CTMapEntriesTimeoutSrvAny, err = time.ParseDuration(viper.GetString(CTMapEntriesTimeoutSrvAnyName))
	if err != nil {
		log.WithError(err).Fatal("Unable to parse %s duration!", CTMapEntriesTimeoutSrvAnyName)
	}
	c.CTMapEntriesTimeoutSrvAny = c.CTMapEntriesTimeoutSrvAny.Round(time.Second)

	c.CTMapEntriesTimeoutSYN, err = time.ParseDuration(viper.GetString(CTMapEntriesTimeoutSYNName))
	if err != nil {
		log.WithError(err).Fatal("Unable to parse %s duration!", CTMapEntriesTimeoutSYNName)
	}
	...
	if err != nil {
		log.WithError(err).Fatal("Unable to parse %s duration!", CTMapEntriesTimeoutFINName)
	}
