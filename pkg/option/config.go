...
const (
	...
	// BPRRoot is the Path to BPF filesystem
	BPFRoot = "bpf-root"
	...
	// IPv4Range is the Kubernetes IPv4 services CIDR if not inside cluster prefix
	IPv4ServiceRange = "ipv4-service-range"

	// IPv6Range is the Kubernetes IPv6 services CIDR if not inside cluster prefix
	...
	// Prefilter mode { "+ModePreFilterNative+" | "+ModePreFilterGeneric+" } (default: "+option.ModePreFilterNative+")
	...
	// CMDRefDir is the path to cmdref output directory
	CMDRef = "cmdref"
...
// getEnv returns the environment variable to be used for the given option name.
func getEnv(option string) string {
...
func BindEnv(optName string) {
	...
	viper.BindEnv(optName, getEnv(optName))
}
