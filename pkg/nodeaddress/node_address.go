...
func UseNodeCIDR(node *node.Node) error {
	scopedLog := log.WithField(logfields.CiliumNode, node.Name)
	...
	if node.IPv6AllocCIDR != nil {
		...
		if err := SetIPv6NodeRange(node.IPv6AllocCIDR); err != nil {
			scopedLog.WithError(err).WithField(logfields.V4Prefix, node.IPv6AllocCIDR).Warning("k8s: Can't use IPv6 CIDR range from kubernetes")
		}
	}
