...
func PrintBGPPeersCaps(w io.Writer, peers []*models.BgpPeer) {
	...
	for _, peer := range peers {
		...
		if err != nil {
			fmt.Fprintf(w, "Could not decode local capabilities: %s\n", err)
			return
		}
		remoteCaps, err := decodeBgpCapabilities(peer.RemoteCapabilities)
		if err != nil {
			fmt.Fprintf(w, "Could not decode remote capabilities: %s\n", err)
			return
		}
...
func decodeBgpCapabilities(caps []*models.BgpCapabilities) ([]bgppacket.ParameterCapabilityInterface, error) {
	...
	for i, cap := range caps {
		...
		if err != nil {
			return nil, fmt.Errorf("could not decode from base64 to bytes %s: %w", cap.Capabilities, err)
		}
		decodedCap, err := bgppacket.DecodeCapability(bin)
		if err != nil {
			return nil, fmt.Errorf("could not decode from bytes to capability %s: %w", cap.Capabilities, err)
		}
...
func printLocalRemoteCaps(w io.Writer, localCaps, remoteCaps []bgppacket.ParameterCapabilityInterface) {
	...
	for _, cap := range caps {
		...
		if formatter, found := capabilityFormatters[cap.Code()]; found {
			...
			formatDefaultCap(w, cap, support)
		}
	}
}
...
func formatDefaultCap(w io.Writer, cap bgppacket.ParameterCapabilityInterface, support string) {
	fmt.Fprintf(w, "\t%s: %s\n", cap.Code(), support)
}
