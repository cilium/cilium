...
func exportAdvertisementsReconciler(params *advertisementsReconcilerParams) ([]*types.Path, error) {
	...
	log.Debug("Begin reconciling advertisements for virtual router with local ASN", slog.String("name", params.name), slog.Int64("asn", params.newc.LocalASN), logAttr)

	// if advertisement is turned off withdraw any previously advertised
	// cidrs and early return nil.
	if !params.enabled {
		log.Debug("advertisements disabled for virtual router with local ASN", slog.String("name", params.name), slog.Int64("asn", params.newc.LocalASN), logAttr)

		for _, advrt := range params.currentAdvertisements {
			log.Debug("Withdrawing advertisement for local ASN", slog.String("name", params.name), slog.Any("NLRI", advrt.NLRI), slog.Int64("asn", params.newc.LocalASN), logAttr)
	...
	if len(toAdvertise) == 0 && len(toWithdraw) == 0 {
		log.Debug("No reconciliation necessary", logAttr)
		return append([]*types.Path{}, params.currentAdvertisements...), nil
	}

	// create new adverts
	for _, advrt := range toAdvertise {
		log.Debug("Advertising for policy with local ASN", slog.String("name", params.name), slog.Any("NLRI", advrt.NLRI), slog.Int64("asn", params.newc.LocalASN), logAttr)
	...
	for _, advrt := range toWithdraw {
		log.Debug("Withdrawing for policy with local ASN", slog.String("name", params.name), slog.Any("NLRI", advrt.NLRI), slog.Int64("asn", params.newc.LocalASN), logAttr)
		if err := params.sc.Server.WithdrawPath(params.ctx, types.PathRequest{Path: advrt}); err != nil {
			return nil, err
		}
	}
