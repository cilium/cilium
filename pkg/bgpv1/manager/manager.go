		...
		job.OneShot("bgp-state-observer", func(ctx context.Context, health cell.Health) (err error) {
			for {
				...
				case <-m.state.reconcileSignal:
					...
					if err != nil {
						m.Logger.Error("failed to reconcile state", slog.Any(logfields.Error, err))
					}
...
func (m *BGPRouterManager) ConfigurePeers(ctx context.Context,
	...
	if rd.empty() {
		m.Logger.Debug("GoBGP peering topology up-to-date with CiliumBGPPeeringPolicy for this node.", logAttr)
		return nil
	}
	m.Logger.Debug("Reconciling new CiliumBGPPeeringPolicy", slog.Any("diff", rd), logAttr)
...
func (m *BGPRouterManager) register(ctx context.Context, rd *reconcileDiff) {
	...
	for _, asn := range rd.register {
		...
		if config, ok = rd.seen[asn]; !ok {
			m.Logger.Error("Work diff (add) contains unseen ASN, skipping", slog.Int64("asn", asn), logAttr)
			continue
		}
		if err := m.registerBGPServer(ctx, config, rd.ciliumNode); err != nil {
			...
			m.Logger.Error("Error while registering new BGP server for local ASN", slog.Int64("asn", config.LocalASN), slog.Any(logfields.Error, err), logAttr)
		}
	}
}
...
func (m *BGPRouterManager) registerBGPServer(ctx context.Context,
	...
	m.Logger.Info("Registering BGP servers for policy with local ASN", slog.Int64("asn", c.LocalASN), logAttr)
	...
	m.Logger.Info("Successfully registered GoBGP servers for policy with local ASN", slog.Int64("asn", c.LocalASN), logAttr)
...
func (m *BGPRouterManager) withdraw(ctx context.Context, rd *reconcileDiff) error {
	...
	for _, asn := range rd.withdraw {
		...
		if s, ok = m.Servers[asn]; !ok {
			m.Logger.Warn("Server with local ASN marked for deletion but does not exist", slog.Int64("asn", asn), logAttrs)
			continue
		}
		for _, r := range m.Reconcilers {
			r.Cleanup(s)
		...
		m.Logger.Info("Removed BGP server with local ASN", slog.Int64("asn", asn), logAttrs)
	}
	return nil
}
...
func (m *BGPRouterManager) reconcile(ctx context.Context, rd *reconcileDiff) {
	...
	for _, asn := range rd.reconcile {
		...
		if sc == nil {
			m.Logger.Error("Virtual router with local ASN marked for reconciliation but missing from Manager", slog.Int64("asn", newc.LocalASN), logAttr) // really shouldn't happen
			continue
		}
		if newc == nil {
			m.Logger.Error("Virtual router with local ASN marked for reconciliation but missing from incoming configurations", slog.Int64("asn", sc.Config.LocalASN), logAttr) // also really shouldn't happen
			continue
		}
		if err := m.reconcileBGPConfig(ctx, sc, newc, rd.ciliumNode); err != nil {
			m.Logger.Error("Encountered error reconciling virtual router with local ASN", slog.Any(logfields.Error, err), slog.Int64("asn", newc.LocalASN), logAttr)
		}
	}
}
...
func (m *BGPRouterManager) ReconcileInstances(ctx context.Context,
	...
	if rd.empty() {
		m.Logger.Debug("BGP instance up-to-date with CiliumBGPNodeConfig", slog.String(types.BGPNodeConfigLogField, nodeObj.Name))
		return nil
	}
	m.Logger.Debug("Reconciling BGP instances", slog.Any("diff", rd), slog.String(types.BGPNodeConfigLogField, nodeObj.Name))
...
func (m *BGPRouterManager) registerV2(ctx context.Context, rd *reconcileDiffV2) error {
	...
	for _, name := range rd.register {
		...
		if config, ok = rd.seen[name]; !ok {
			m.Logger.Debug("Work diff (add) contains unseen instance, skipping", slog.String(types.InstanceLogField, name))
		...
		if rErr := m.registerBGPInstance(ctx, config, rd.ciliumNode); rErr != nil {
			...
			m.Logger.Debug("Error registering new BGP instance", slog.Any(logfields.Error, rErr), slog.String(types.InstanceLogField, name))
			instancesWithError = append(instancesWithError, name)
			lastErr = rErr
		}
	}
...
func (m *BGPRouterManager) registerBGPInstance(ctx context.Context,
	...
	m.Logger.Info("Registering BGP instance", slog.String(types.InstanceLogField, c.Name))
	...
	i, err := instance.NewBGPInstance(ctx, m.Logger.With(types.InstanceLogField, c.Name), c.Name, globalConfig)
	if err != nil {
	...
	m.Logger.Info(
		...
		slog.Int64(types.LocalASNLogField, localASN),
		slog.Int64(types.ListenPortLogField, int64(localPort)),
		slog.String(types.RouterIDLogField, routerID),
		slog.String(types.InstanceLogField, c.Name),
	)
	...
	for _, name := range rd.withdraw {
		...
		if i, ok = m.BGPInstances[name]; !ok {
			m.Logger.Warn(
				...
				slog.String(types.InstanceLogField, name),
			)
			continue
		}
		for _, r := range m.ConfigReconcilers {
