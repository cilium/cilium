...
func getRouterID(config *v2.CiliumBGPNodeInstance, ciliumNode *v2.CiliumNode, asn int64) (string, error) {
	...
	case option.BGPRouterIDAllocationModeNode:
		if nodeIP := ciliumNode.GetIP(false); nodeIP != nil {
			routerID = nodeIP.String()
		} else {
	...
	default:
		return "", fmt.Errorf("invalid router-id allocation mode: %s (supported modes: %s, %s)", option.Config.BGPRouterIDAllocationMode, option.BGPRouterIDAllocationModeNode, option.BGPRouterIDAllocationModeIPPool)
	}
}
