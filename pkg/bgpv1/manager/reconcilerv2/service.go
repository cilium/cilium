...
func (r *ServiceReconciler) getServicePrefixes(p ReconcileParams, svc *loadbalancer.Service, advert v2.BGPAdvertisement, rx statedb.ReadTxn) (map[v2.BGPServiceAddressType][]netip.Prefix, error) {
	...
	for _, svcAdv := range advert.Service.Addresses {
		...
		case v2.BGPLoadBalancerIPAddr:
			res[svcAdv] = append(res[svcAdv], r.getLoadBalancerPaths(p, svc, frontends, advert)...)
		case v2.BGPClusterIPAddr:
			res[svcAdv] = append(res[svcAdv], r.getClusterIPPaths(p, frontends, advert)...)
		case v2.BGPExternalIPAddr:
			res[svcAdv] = append(res[svcAdv], r.getExternalIPPaths(p, frontends, advert)...)
		}
	}
	return res, nil
}
...
func (r *ServiceReconciler) getLoadBalancerPaths(p ReconcileParams, svc *loadbalancer.Service, frontends []*loadbalancer.Frontend, advert v2.BGPAdvertisement) []netip.Prefix {
	var desiredRoutes []netip.Prefix
