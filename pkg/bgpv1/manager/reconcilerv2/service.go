...
func (r *ServiceReconciler) reconcileServices(ctx context.Context, p ReconcileParams, desiredPeerAdverts PeerAdvertisements, fullReconcile bool) error {
	...
	/*
		//1) no epSlice, BGPNoEndpointRoutable=true, tp=Local => NO
		//2) no epSlice, BGPNoEndpointRoutable=false, tp=Local => NO
		//3) no epSlice, BGPNoEndpointRoutable=true, tp=Cluster => YES
		//4) no epSlice, BGPNoEndpointRoutable=false, tp=Cluster => NO
		//3) epSlice, BGPNoEndpointRoutable=true, tp=Local => YES
		//4) epSlice, BGPNoEndpointRoutable=false, tp=Local => YES
		//5) epSlice, BGPNoEndpointRoutable=true, tp=Cluster => YES
		//6) epSlice, BGPNoEndpointRoutable=false, tp=Cluster => YES
	*/
...
func (r *ServiceReconciler) getServicePrefixes(svc *slim_corev1.Service, advert v2.BGPAdvertisement, ls sets.Set[resource.Key]) ([]netip.Prefix, error) {
	...
	if !hasValidEndpoints(svc, ls) && !r.routesConfig.BGPNoEndpointsRoutable {
		return nil, nil
	}
