...
func (r *ServiceReconciler) processFrontendEvents(ctx context.Context, health cell.Health) error {
	txn := r.db.WriteTxn(r.frontends)
	iter, err := r.frontends.Changes(txn)
	txn.Commit()
	if err != nil {
		return err
	}
	...
	for {
		changes, watch := iter.Next(r.db.ReadTxn())

		signal := false
		for range changes {
			signal = true
			// do not break - consume all changes
		}
		if signal {
			// trigger BGP CP reconciliation
...
func (r *ServiceReconciler) Reconcile(ctx context.Context, p ReconcileParams) error {
	...
	// ensure that first reconciliation for the instance is always full
	serviceMetadata := r.getMetadata(p.BGPInstance)
	if !serviceMetadata.Reconciled {
		reqFullReconcile = true
	}
	serviceMetadata.Reconciled = true
		...
		for fam, adverts := range afAdverts {
			...
			for _, advert := range adverts {
				...
				for advertType, prefixes := range typedPrefixes {
					...
					policy, err := r.getServiceRoutePolicy(peer, agentFamily, svc, prefixes, advert, advertType)
					if err != nil {
...
func (r *ServiceReconciler) getServicePrefixes(p ReconcileParams, svc *loadbalancer.Service, advert v2.BGPAdvertisement, rx statedb.ReadTxn) (map[v2.BGPServiceAddressType][]netip.Prefix, error) {
	...
	var lbFrontends, cipFrontends, eipFrontends []*loadbalancer.Frontend
	for fe := range r.frontends.List(rx, loadbalancer.FrontendByServiceName(svc.Name)) {
		if fe.Type == loadbalancer.SVCTypeLoadBalancer {
			lbFrontends = append(lbFrontends, fe)
		}
		if fe.Type == loadbalancer.SVCTypeClusterIP {
			cipFrontends = append(cipFrontends, fe)
		}
		if fe.Type == loadbalancer.SVCTypeExternalIPs {
			eipFrontends = append(eipFrontends, fe)
		}
	}

	// Loop over the service adverts and determine the desired routes
	res := make(map[v2.BGPServiceAddressType][]netip.Prefix)
	for _, svcAdv := range advert.Service.Addresses {
		switch svcAdv {
		case v2.BGPLoadBalancerIPAddr:
			res[svcAdv] = append(res[svcAdv], r.getLoadBalancerPaths(p, lbFrontends, advert)...)
		case v2.BGPClusterIPAddr:
			res[svcAdv] = append(res[svcAdv], r.getClusterIPPaths(p, cipFrontends, advert)...)
		case v2.BGPExternalIPAddr:
			res[svcAdv] = append(res[svcAdv], r.getExternalIPPaths(p, eipFrontends, advert)...)
		}
	}
	return res, nil
}

func (r *ServiceReconciler) getExternalIPPaths(p ReconcileParams, frontends []*loadbalancer.Frontend, advert v2.BGPAdvertisement) []netip.Prefix {
...
func (r *ServiceReconciler) getClusterIPPaths(p ReconcileParams, frontends []*loadbalancer.Frontend, advert v2.BGPAdvertisement) []netip.Prefix {
	var desiredRoutes []netip.Prefix
...
func (r *ServiceReconciler) getLoadBalancerPaths(p ReconcileParams, frontends []*loadbalancer.Frontend, advert v2.BGPAdvertisement) []netip.Prefix {
	var desiredRoutes []netip.Prefix
