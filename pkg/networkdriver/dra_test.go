...
// TestDeviceClaimConfigs_InvalidJSON verifies deviceClaimConfigs returns an
// error when the opaque parameters cannot be unmarshalled.
func TestDeviceClaimConfigs_InvalidJSON(t *testing.T) {
...
// TestDeviceClaimConfigs_Empty verifies that a claim without any config is still valid.
func TestDeviceClaimConfigs_Empty(t *testing.T) {
	driver := &Driver{logger: hivetest.Logger(t)}
	claim := &resourceapi.ResourceClaim{
		Status: resourceapi.ResourceClaimStatus{
			Allocation: &resourceapi.AllocationResult{
...
// TestPrepareResourceClaim_WrongReservedForLength verifies that a claim with
// zero or more than one ReservedFor entry is rejected with errUnexpectedInput.
func TestPrepareResourceClaim_WrongReservedForLength(t *testing.T) {
	driver := &Driver{
...
// TestApplyAddressToConfig_EmptyAddress verifies that an empty prefix
// does not return an error
func TestApplyAddressToConfig_EmptyAddress(t *testing.T) {
	driver := &Driver{logger: hivetest.Logger(t)}
...
// TestApplyAddressToConfig_RequestNotFound verifies that referencing a request
// not present in devicesCfg returns an error.
func TestApplyAddressToConfig_RequestNotFound(t *testing.T) {
	driver := &Driver{logger: hivetest.Logger(t)}
	claim := &resourceapi.ResourceClaim{ObjectMeta: metav1.ObjectMeta{Name: "test"}}
