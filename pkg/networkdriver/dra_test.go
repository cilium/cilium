...
func TestNetworkDriverIPAMPool(t *testing.T) {
	...
	ipPool := v2alpha1.CiliumResourceIPPool{
		ObjectMeta: metav1.ObjectMeta{
			Name: ipPoolName,
		},
	...
	hive := hive.New(
		...
		cell.Invoke(func(c *k8sClient.FakeClientset) {
			...
			_, err := c.CiliumFakeClientset.CiliumV2alpha1().CiliumResourceIPPools().Create(t.Context(), &ipPool, metav1.CreateOptions{})
			assert.NoError(t, err)
		...
		cell.Invoke(func(
			...
			d *statedb.DB,
			poolsResource statedb.Table[resourceIPPool],
		) {
			mgr = m
			cs = c
			db = d
			resourceIPPools = poolsResource
		}),
	)
	...
	// wait for the reflector to propagate CiliumResourceIPPool upsert into stateDB
	var found bool
	for !found {
		_, _, watch, found := resourceIPPools.GetWatch(db.ReadTxn(), ResourceIPPoolByName(ipPoolName))
		if found {
			break
		}
		<-watch
	...
	driver := &Driver{
		...
		ipv6Enabled:     daemonCfg.IPv6Enabled(),
		db:              db,
		resourceIPPools: resourceIPPools,
		localNodeStore: node.NewTestLocalNodeStore(node.LocalNode{
			Node: nodetypes.Node{Name: localNodeName},
		}),
	}
...
func TestNetworkDriverIPAMPoolNodeSelector(t *testing.T) {
	t.Cleanup(func() {
		testutils.GoleakVerifyNone(t)
	})
	...
	// leave the "ip-pool" parameter empty to trigger automatic selection of ResourceIPPool
	rawParam, err := json.Marshal(map[string]string{"ip-pool": ""})
	assert.NoError(t, err)
	...
	hive := hive.New(
		...
		cell.Invoke(func(c *k8sClient.FakeClientset) {
			...
			localNode := v2.CiliumNode{
				ObjectMeta: metav1.ObjectMeta{
					Name:   localNodeName,
					Labels: map[string]string{"kubernetes.io/hostname": localNodeName},
				},
			}
			_, err = c.CiliumFakeClientset.CiliumV2().CiliumNodes().Create(t.Context(), &localNode, metav1.CreateOptions{})
			assert.NoError(t, err)
