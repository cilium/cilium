...
func (driver *Driver) addrsForDevice(ctx context.Context, device string, pool string, v4Needed, v6Needed bool) (netip.Addr, netip.Addr, error) {
	...
	if err := resiliency.Retry(ctx, AddrAddRetryInterval, AddrAddMaxRetries, func(ctx context.Context, retries int) (bool, error) {
		...
		errs := []error{err}
		if v4Addr.IsValid() {
			if err := driver.multiPoolMgr.ReleaseIP(v4Addr.AsSlice(), ipam.Pool(pool), ipam.IPv4, true); err != nil {
				errs = append(errs, fmt.Errorf("failed to release IPv4 address %s: %w", v4Addr, err))
			}
		}
		if v6Addr.IsValid() {
			if err := driver.multiPoolMgr.ReleaseIP(v6Addr.AsSlice(), ipam.Pool(pool), ipam.IPv6, true); err != nil {
				errs = append(errs, fmt.Errorf("failed to release IPv6 address %s: %w", v6Addr, err))
			}
			errs = append(errs, driver.multiPoolMgr.ReleaseIP(v6Addr.AsSlice(), ipam.Pool(pool), ipam.IPv6, true))
		}
		return netip.Addr{}, netip.Addr{}, fmt.Errorf("failed to get IP addresses for device %s from pool %s: %w", device, pool, errors.Join(errs...))
	}
