...
the capacity assigned to that share. `PreparedDevice` holds the
allocation-specific device returned by `Setup`, while `Config` records driver
...
Agent restart
  ...
  │              └─ restores PreparedDevice, Config, and ConsumedCapacity
  │                   from serialized device data
