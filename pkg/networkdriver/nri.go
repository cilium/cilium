	...
	err := driver.withLock(func() error {
		...
		for i := range podAllocations {
			...
			if err != nil {
				...
				// Setup implementations for on-demand devices must make this recovery
				// safe to retry.
				...
				prepared, setupErr := a.Device.Setup(a.deviceAllocation())
				if setupErr != nil {
					return fmt.Errorf("failed to re-create device %s on demand: %w", a.Device.KernelIfName(), setupErr)
				}
				if prepared == nil {
					return fmt.Errorf("device %s returned no prepared device", a.Device.KernelIfName())
				}
				a.Device = prepared
