...
func cidrsContainAddress[T ~string](cidrs []T, addr netip.Addr) bool {
...
func nodeResourcePoolCIDRs(allocations []ipamTypes.IPAMPoolAllocation, poolName string) ([]ipamTypes.IPAMCIDR, bool) {
	for _, allocation := range allocations {
		if allocation.Pool != poolName {
			continue
		}
		return allocation.CIDRs, len(allocation.CIDRs) > 0
	}
	return nil, false
}
