...
func (driver *Driver) buildPoolsFromTable() map[string]resourceslice.Pool {
	...
	for d := range driver.deviceTable.All(txn) {
		...
		if _, ok := pools[pool]; !ok {
			// No pool matches — do not advertise this device.
			continue
		}

		if d.ClaimUID != "" && d.PodUID != "" {
			// if an allocation exists, the device was allocated
			// from a pool. if the pool we just resolved differs
			// from the pool used to allocate a device,
			// we should advertise the device with the previous pool
			if pool != d.Pool {
				driver.logger.Warn(
					"pool resolved does not match pool from an already allocated device. keeping old pool",
					logfields.Device, d.Dev.IfName(),
					logfields.PoolName, d.Pool,
					logfields.New, pool,
				)
		...
		entry, ok := pools[pool]
		if !ok {
			// The sticky (previously allocated) pool no longer exists in the
			// current config at all — do not advertise this device.
			continue
		}
