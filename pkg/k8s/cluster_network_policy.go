...
func WithServiceResolver(resolver fqdnconfig.ServiceResolver) ParseClusterNetworkPolicyOption {
	return func(c *parseCNPConfig) {
		...
		if len(eRule.To) > 0 {
			for _, rule := range eRule.To {
				...
				case rule.DomainNames != nil:
					...
					dnsL3, dnsL4Rules := cfg.fqdn.DNSSelectors(cfg.serviceResolver, dnsL4)
					if len(dnsL3) > 0 {
