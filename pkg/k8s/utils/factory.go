...
// lister returns a function that, when called, returns a versioned.ComparableMap
// of all objects the lister function can retrieve.
type lister func(client interface{}) func() (*versioned.ComparableMap, error)
...
//    is received.
//  * missingFunc will return a versioned.ComparableMap for the objects that are
//    considered missing from the versioned.ComparableMap provided.
...
func ResourceEventHandlerFactory(
	...
	missingFunc func(comparableMap *versioned.ComparableMap) *versioned.ComparableMap,
	resourceObj runtime.Object,
	...
	if addFunc != nil && delFunc != nil {
		...
		k8sSyncCM.UpdateController(fmt.Sprintf("k8s-sync-%s", resourceNameOf(resourceObj)),
			controller.ControllerParams{
				DoFunc: func() error {
					err := scm.DoLocked(nil, replaceFunc)
					if err == nil {
						s.Do(func() {
							// Close the hasSynced chnnale to signalize the that
							// all other functions queued were serialized. This
							// will make sure the all events received after the
							// cached was synced were processed by the queue.
...
func listerFactory(client, i interface{}) func() (*versioned.ComparableMap, error) {
...
func replaceFuncFactory(
	...
	return func(oldMap *versioned.ComparableMap) (*versioned.ComparableMap, error) {
		...
		var (
			added       = versioned.ComparableMap{}
			deleted     = versioned.ComparableMap{}
			newEqualMap = &versioned.ComparableMap{
				Map:        newMap.Map,
				DeepEquals: oldMap.DeepEquals,
			}
		)
		for k, oldVS := range oldMap.Map {
			newVS, ok := newMap.Get(k)
			if ok {
				// If the old element is not new nor different than the one
				// fetch by the lister then it's not going to be added.
				if !newEqualMap.AddEqual(k, oldVS) {
					added.Add(k, newVS)
				}
			} else {
				// If the old object no longer exists in the source of truth
				// from the lister then it needs to be deleted.
				deleted.Add(k, oldVS)
			}
		}

		if missingFunc != nil {
			// Check which objects are still missing with the missingFunc.
			// The ones considered missing will be re-added again.
			missing := missingFunc(newMap)
			for k, newVS := range missing.Map {
				added.Add(k, newVS)
			}
		}

		for k, newVS := range newMap.Map {
			// If we have new elements that are not present in the oldMap then
			// then will definitely need to be added.
			_, ok := oldMap.Get(k)
			if !ok {
				added.Add(k, newVS)
			}
		...
		for _, v := range added.Map {
			fqueue.Enqueue(addFunc(v.Data), serializer.NoRetry)
		}
		for _, v := range deleted.Map {
