...
func replaceFuncFactory(
	...
	return func(oldEqualMap *versioned.ComparableMap) (*versioned.ComparableMap, error) {
		...
		var (
			...
			newEqualMap = &versioned.ComparableMap{
				Map:        newMap,
				DeepEquals: oldEqualMap.DeepEquals,
			}
		)
		...
		// - when an object exists in the oldMap and it's older and different
		//   than the one from kubernetes.
		for k, v := range newEqualMap.Map {
			if !oldEqualMap.AddEqual(k, v) {
				added.Add(k, v)
			}
		}
