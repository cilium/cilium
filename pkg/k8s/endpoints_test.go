	...
	tests := []struct {
			...
			fields: fields{
				svcEP: &Endpoints{
					Backends: map[cmtypes.IPCluster]*Backend{
						cmtypes.NewIPCluster("172.20.0.1", 0): {
		...
		},
		{
			...
			args: args{
				o: &Endpoints{
					Backends: map[cmtypes.IPCluster]*Backend{
						cmtypes.NewIPCluster("172.20.0.2", 0): {
							Ports: map[string]*loadbalancer.L4Addr{
								"foo": {
									Protocol: loadbalancer.NONE,
									Port:     1,
...
func (s *K8sSuite) Test_parseK8sEPv1(c *check.C) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupWanted: func() *Endpoints {
				svcEP := newEndpoints()
				svcEP.Backends[cmtypes.NewIPCluster("172.0.0.1", 0)] = &Backend{
		...
		},
		{
			...
			setupWanted: func() *Endpoints {
				...
				svcEP.Backends[cmtypes.NewIPCluster("172.0.0.2", 0)] = &Backend{
					Ports: serviceStore.PortConfiguration{
						"http-test-svc":   loadbalancer.NewL4Addr(loadbalancer.TCP, 8080),
						"http-test-svc-2": loadbalancer.NewL4Addr(loadbalancer.TCP, 8081),
					},
