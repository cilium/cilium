...
func TestGetUniqueServiceFrontends(t *testing.T) {
	...
	cache := NewServiceCache(hivetest.Logger(t), loadbalancer.DefaultConfig, false, db, nodeAddrs, NewSVCMetricsNoop())
...
func testServiceCache(t *testing.T,
	...
	db, nodeAddrs := newDB(t)
	svcCache := NewServiceCache(hivetest.Logger(t), loadbalancer.DefaultConfig, false, db, nodeAddrs, NewSVCMetricsNoop())
...
func TestExternalServiceMerging(t *testing.T) {
	...
	k8sSvcID, _ := ParseService(hivetest.Logger(t), loadbalancer.DefaultConfig, false, k8sSvc, nil)
...
func BenchmarkCorrelateEndpoints(b *testing.B) {
	...
	cache := NewServiceCache(hivetest.Logger(b), loadbalancer.DefaultConfig, false, db, nodeAddrs, NewSVCMetricsNoop())
