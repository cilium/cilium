...
// +kubebuilder:resource:singular="ciliumenvoyconfig",path="ciliumenvoyconfigs",scope="Cluster"
...
type CiliumEnvoyConfigSpec struct {
	// Service specifies Kubernetes service and port for which
	// traffic is forwarded to an Envoy listener.
	//
	// +kubebuilder:validation:Optional
	Service *ServiceListener `json:"service,omitempty"`
	...
	// in 'resources' must be either ingress or egress listeners.
	//
	// This setting directs policy principal selection
	// (destination for ingress, source for egress) and must be
	// set properly if Cilium policy enforcement filters are
	// configured.
	//
	// +kubebuilder:validation:Optional
