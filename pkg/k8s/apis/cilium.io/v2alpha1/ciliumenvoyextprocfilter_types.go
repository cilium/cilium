...
type CiliumEnvoyExtProcFilterSpec struct {
	...
	// +kubebuilder:validation:XValidation:rule="duration(self) <= duration('1h')",message="messageTimeout must not exceed 1h"
...
// ExtProcHeaderMode controls whether request or response headers and trailers
// are sent to the ext_proc service.
type ExtProcHeaderMode string

const (
	ExtProcHeaderModeSend ExtProcHeaderMode = "SEND"
	ExtProcHeaderModeSkip ExtProcHeaderMode = "SKIP"
)

// ExtProcBodyMode controls how request or response bodies are sent to the
// ext_proc service.
type ExtProcBodyMode string

const (
	ExtProcBodyModeNone            ExtProcBodyMode = "NONE"
	ExtProcBodyModeStreamed        ExtProcBodyMode = "STREAMED"
	ExtProcBodyModeBuffered        ExtProcBodyMode = "BUFFERED"
	ExtProcBodyModeBufferedPartial ExtProcBodyMode = "BUFFERED_PARTIAL"
)
...
type ExtProcProcessingMode struct {
	...
	RequestHeaderMode *ExtProcHeaderMode `json:"requestHeaderMode,omitempty"`
	...
	// +kubebuilder:validation:Optional
	// +kubebuilder:validation:Enum=SEND;SKIP
	ResponseHeaderMode *ExtProcHeaderMode `json:"responseHeaderMode,omitempty"`
	...
	RequestBodyMode *ExtProcBodyMode `json:"requestBodyMode,omitempty"`
	...
	// +kubebuilder:validation:Optional
	// +kubebuilder:validation:Enum=NONE;STREAMED;BUFFERED;BUFFERED_PARTIAL
	ResponseBodyMode *ExtProcBodyMode `json:"responseBodyMode,omitempty"`
	...
	// +kubebuilder:validation:Optional
	// +kubebuilder:validation:Enum=SEND;SKIP
	RequestTrailerMode *ExtProcHeaderMode `json:"requestTrailerMode,omitempty"`
	...
	// +kubebuilder:validation:Optional
	// +kubebuilder:validation:Enum=SEND;SKIP
	ResponseTrailerMode *ExtProcHeaderMode `json:"responseTrailerMode,omitempty"`
}
