...
func getEndpointSelector(namespace, clusterName string, labelSelector *slim_metav1.LabelSelector, addK8sPrefix, matchesInit bool) api.EndpointSelector {
	...
	if clusterName != "" && !es.HasKey(clusterPrefixLbl) && !es.HasKey(clusterAnyPrefixLbl) {
...
func parseToCiliumIngressCommonRule(namespace, clusterName string, es api.EndpointSelector, ing api.IngressCommonRule) api.IngressCommonRule {
	...
	if ing.FromEndpoints != nil {
		...
		for j, ep := range ing.FromEndpoints {
			retRule.FromEndpoints[j] = getEndpointSelector(namespace, clusterName, ep.LabelSelector, true, matchesInit)
		}
	}
	...
	if ing.FromRequires != nil {
		...
		for j, ep := range ing.FromRequires {
			retRule.FromRequires[j] = getEndpointSelector(namespace, clusterName, ep.LabelSelector, false, matchesInit)
		}
	}
...
func parseToCiliumIngressRule(namespace, clusterName string, es api.EndpointSelector, inRules []api.IngressRule) []api.IngressRule {
	...
	if inRules != nil {
		...
		for i, ing := range inRules {
			...
			retRules[i].IngressCommonRule = parseToCiliumIngressCommonRule(namespace, clusterName, es, ing.IngressCommonRule)
...
func parseToCiliumIngressDenyRule(namespace, clusterName string, es api.EndpointSelector, inRules []api.IngressDenyRule) []api.IngressDenyRule {
...
func parseToCiliumEgressCommonRule(namespace, clusterName string, es api.EndpointSelector, egr api.EgressCommonRule) api.EgressCommonRule {
	matchesInit := matchesPodInit(es)
	...
	if egr.ToEndpoints != nil {
		...
		for j, ep := range egr.ToEndpoints {
			endpointSelector := getEndpointSelector(namespace, clusterName, ep.LabelSelector, true, matchesInit)
	...
	if egr.ToRequires != nil {
		...
		for j, ep := range egr.ToRequires {
			retRule.ToRequires[j] = getEndpointSelector(namespace, clusterName, ep.LabelSelector, false, matchesInit)
		}
	}
...
func parseToCiliumEgressRule(namespace, clusterName string, es api.EndpointSelector, inRules []api.EgressRule) []api.EgressRule {
	...
	if inRules != nil {
		...
		for i, egr := range inRules {
			...
			retRules[i].EgressCommonRule = parseToCiliumEgressCommonRule(namespace, clusterName, es, egr.EgressCommonRule)
...
func parseToCiliumEgressDenyRule(namespace, clusterName string, es api.EndpointSelector, inRules []api.EgressDenyRule) []api.EgressDenyRule {
...
func ParseToCiliumRule(logger *slog.Logger, namespace, clusterName, name string, uid types.UID, r *api.Rule) *api.Rule {
	retRule := &api.Rule{}
	...
	retRule.Ingress = parseToCiliumIngressRule(namespace, clusterName, r.EndpointSelector, r.Ingress)
	retRule.IngressDeny = parseToCiliumIngressDenyRule(namespace, clusterName, r.EndpointSelector, r.IngressDeny)
	retRule.Egress = parseToCiliumEgressRule(namespace, clusterName, r.EndpointSelector, r.Egress)
	retRule.EgressDeny = parseToCiliumEgressDenyRule(namespace, clusterName, r.EndpointSelector, r.EgressDeny)
