...
func RulesToPolicyEntries(rules api.Rules) types.PolicyEntries {
	...
	for _, rule := range rules {
		...
		for _, iRule := range rule.Ingress {
			...
			l3 = append(l3, types.ToEndpointSelectorInterfaceSlice(iRule.FromCIDR)...)
			l3 = append(l3, types.ToEndpointSelectorInterfaceSlice(iRule.FromCIDRSet)...)
		...
		for _, eRule := range rule.Egress {
			...
			l3 = append(l3, types.ToEndpointSelectorInterfaceSlice(eRule.ToCIDR)...)
			l3 = append(l3, types.ToEndpointSelectorInterfaceSlice(eRule.ToCIDRSet)...)
