...
// CreateCustomResourceDefinitionFromBytes creates the given CRD in the
// Kubernetes cluster. The given CRD is expected to be serialized into bytes,
// from its YAML structure.
func CreateCustomResourceDefinitionFromBytes(
	clientset apiextensionsclient.Interface,
	crd []byte,
) error {
	var ciliumCRD apiextensionsv1.CustomResourceDefinition
	if err := yaml.Unmarshal(crd, &ciliumCRD); err != nil {
		return err
	}
