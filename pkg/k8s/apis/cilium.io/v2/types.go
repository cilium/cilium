...
// CiliumDerivativePolicyStatus is the status of a Cilium derived policies.
...
func (r *CiliumNetworkPolicy) SetDerivedPolicyStatus(derivativePolicyName string, status CiliumDerivativePolicyStatus) {
	if r.Status.DerivativesPolicies == nil {
		r.Status.DerivativesPolicies = map[string]CiliumDerivativePolicyStatus{}
	}
	r.Status.DerivativesPolicies[derivativePolicyName] = status
}
