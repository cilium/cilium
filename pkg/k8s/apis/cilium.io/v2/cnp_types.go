...
func (r *CiliumNetworkPolicy) Parse(logger *slog.Logger, clusterName string) (api.Rules, error) {
	...
	if r.Spec != nil {
		...
		cr := k8sCiliumUtils.ParseToCiliumRule(logger, namespace, clusterName, name, uid, r.Spec)
	...
	if r.Specs != nil {
		for _, rule := range r.Specs {
			...
			cr := k8sCiliumUtils.ParseToCiliumRule(logger, namespace, clusterName, name, uid, rule)
			retRules = append(retRules, cr)
		}
	}
