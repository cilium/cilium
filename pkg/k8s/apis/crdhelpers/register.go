...
func CreateUpdateCRD(
	...
	scopedLog := log.With(slog.String("name", crd.Name))
	...
	if crd.Spec.Versions[0].Schema != nil && needsUpdateV1(clusterCRD, crdSchemaVersionLabelKey, minCRDSchemaVersion) {
		...
		err := poller.Poll(500*time.Millisecond, 60*time.Second, func() (bool, error) {
			...
			if needsUpdateV1(clusterCRD, crdSchemaVersionLabelKey, minCRDSchemaVersion) {
				...
				scopedLog.Debug("Unable to update CRD validation", slog.Any(logfields.Error, err))
		...
		if err != nil {
			scopedLog.Error("Unable to update CRD", slog.Any(logfields.Error, err))
			return err
		}
	}
		...
		for _, cond := range crd.Status.Conditions {
			...
			case apiextensionsv1.NamesAccepted:
				if cond.Status == apiextensionsv1.ConditionFalse {
					...
					scopedLog.Error("Name conflict for CRD", slog.Any(logfields.Error, err))
					return false, err
				}
			}
		}
