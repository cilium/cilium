...
func TestParseClusterNetworkPolicy(t *testing.T) {
	...
	mockResolver := fqdnconfig.ServiceResolver(func(ns, name string) (map[string]string, []string) {
	...
	fqdnCfg := fqdnconfig.FQDNPolicyDNSServerConfig{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := ParseClusterNetworkPolicy(logger, clusterName, tt.cnp,
				WithFQDNPolicyDNSServerConfig(fqdnCfg),
				WithServiceResolver(mockResolver),
			)
			if tt.wantErr {
				require.Error(t, err)
				return
