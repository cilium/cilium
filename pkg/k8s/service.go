...
func (si Service) IsExternal() bool {
	return len(si.Selector) == 0
}

// Equals returns true if both services are equal
func (si *Service) DeepEquals(o *Service) bool {
	switch {
	case (si == nil) != (o == nil):
		return false
	case (si == nil) && (o == nil):
	...
	if si.IsHeadless == o.IsHeadless &&
		si.FrontendIP.Equal(o.FrontendIP) &&
...
// Ports returns a map of all unique ports configured in the service
