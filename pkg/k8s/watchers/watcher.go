...
func (k *K8sWatcher) delK8sSVCs(svc k8s.ServiceID, svcInfo *k8s.Service, se *k8s.Endpoints) error {
	...
	for portName, svcPort := range svcInfo.Ports {
		...
		for _, feIP := range svcInfo.FrontendIPs {
			fe := loadbalancer.NewL3n4Addr(svcPort.Protocol, cmtypes.NewIPCluster(feIP.String(), 0), svcPort.Port, loadbalancer.ScopeExternal)
		...
		for _, k8sExternalIP := range svcInfo.K8sExternalIPs {
			frontends = append(frontends, loadbalancer.NewL3n4Addr(svcPort.Protocol, cmtypes.NewIPCluster(k8sExternalIP.String(), 0), svcPort.Port, loadbalancer.ScopeExternal))
		}

		for _, ip := range svcInfo.LoadBalancerIPs {
			frontends = append(frontends, loadbalancer.NewL3n4Addr(svcPort.Protocol, cmtypes.NewIPCluster(ip.String(), 0), svcPort.Port, loadbalancer.ScopeExternal))
			if svcInfo.TrafficPolicy == loadbalancer.SVCTrafficPolicyLocal {
				frontends = append(frontends, loadbalancer.NewL3n4Addr(svcPort.Protocol, cmtypes.NewIPCluster(ip.String(), 0), svcPort.Port, loadbalancer.ScopeInternal))
			}
		}
	}
	...
	for _, fe := range frontends {
		if found, err := k.svcManager.DeleteService(*fe); err != nil {
			...
			scopedLog.Debugf("# cilium lb delete-service %s %d 0", fe.IPCluster.IPString(), fe.Port)
		}
	}
	return nil
}
...
func genCartesianProduct(
	...
	for fePortName, fePort := range ports {
		...
		for ipCluster, backend := range bes.Backends {
			if backendPort := backend.Ports[string(fePortName)]; backendPort != nil && feFamilyIPv6 == ipCluster.IsIPv6() {
		...
		svcs = append(svcs,
			loadbalancer.SVC{
				Frontend: loadbalancer.L3n4AddrID{
					L3n4Addr: loadbalancer.L3n4Addr{
						IPCluster: cmtypes.NewIPCluster(fe.String(), 0),
...
func datapathSVCs(svc *k8s.Service, endpoints *k8s.Endpoints) (svcs []loadbalancer.SVC) {
	...
	for fePortName := range clusterIPPorts {
		for _, nodePortFE := range svc.NodePorts[fePortName] {
			...
			dpSVC := genCartesianProduct(net.ParseIP(nodePortFE.IPCluster.IPString()), svc.TrafficPolicy, loadbalancer.SVCTypeNodePort, nodePortPorts, endpoints)
			svcs = append(svcs, dpSVC...)
		}
	}
		...
		for svcHash, oldSvc := range oldSVCMap {
			if _, ok := svcMap[svcHash]; !ok {
				if found, err := k.svcManager.DeleteService(oldSvc); err != nil {
					...
					scopedLog.Debugf("# cilium lb delete-service %s %d 0", oldSvc.IPCluster.IPString(), oldSvc.Port)
				}
			}
		}
	}
