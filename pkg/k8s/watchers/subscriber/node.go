...
func (l *NodeChain) OnAddNode(node *slim_corev1.Node) error {
	l.RLock()
	defer l.RUnlock()
...
func (l *NodeChain) OnUpdateNode(oldNode, newNode *slim_corev1.Node) error {
	l.RLock()
	defer l.RUnlock()
	errs := make([]error, 0, len(l.subs))
	for _, s := range l.subs {
...
func (l *NodeChain) OnDeleteNode(node *slim_corev1.Node) error {
	l.RLock()
	defer l.RUnlock()
	errs := make([]error, 0, len(l.subs))
	for _, s := range l.subs {
