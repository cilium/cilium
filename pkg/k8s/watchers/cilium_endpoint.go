	...
	if oldEndpoint != nil && oldEndpoint.Networking != nil {
		...
		defer func() {
			for _, oldPair := range oldEndpoint.Networking.Addressing {
				...
				if !v4Added {
					portsChanged := ipcache.IPIdentityCache.DeleteOnMetadataMatch(oldPair.IPV4, source.CustomResource, endpoint.Namespace, endpoint.Name)
					if portsChanged {
						namedPortsChanged = true
					}
				}
				if !v6Added {
					portsChanged := ipcache.IPIdentityCache.DeleteOnMetadataMatch(oldPair.IPV6, source.CustomResource, endpoint.Namespace, endpoint.Name)
					if portsChanged {
						namedPortsChanged = true
					}
				}
