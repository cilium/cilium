...
func (k *K8sWatcher) endpointUpdated(oldEndpoint, endpoint *types.CiliumEndpoint) {
	...
	for _, pair := range endpoint.Networking.Addressing {
		...
		if pair.IPV4 != "" {
			...
			namedPortsChanged, _ = k.ipcache.Upsert(pair.IPV4, nodeIP, encryptionKey, &k8sMeta, cid)
		}
		if pair.IPV6 != "" {
			ipsAdded = append(ipsAdded, pair.IPV6)
			namedPortsChanged, _ = k.ipcache.Upsert(pair.IPV6, nodeIP, encryptionKey, &k8sMeta, cid)
		}
	}
