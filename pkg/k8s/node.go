...
func ParseNode(k8sNode *v1.Node, source node.Source) *node.Node {
	...
	if len(k8sNode.Spec.PodCIDR) != 0 {
		if _, allocCIDR, err := net.ParseCIDR(k8sNode.Spec.PodCIDR); err != nil {
			...
			if allocCIDR.IP.To4() != nil {
				newNode.IPv4AllocCIDR = cidr.NewCIDR(allocCIDR)
			} else {
				newNode.IPv6AllocCIDR = cidr.NewCIDR(allocCIDR)
			}
		}
	}
	...
	if newNode.IPv4AllocCIDR == nil {
		if ipv4CIDR, ok := k8sNode.Annotations[annotation.V4CIDRName]; !ok {
			...
			_, allocCIDR, err := net.ParseCIDR(ipv4CIDR)
			if err != nil {
	...
	if newNode.IPv6AllocCIDR == nil {
		if ipv6CIDR, ok := k8sNode.Annotations[annotation.V6CIDRName]; !ok {
			...
			_, allocCIDR, err := net.ParseCIDR(ipv6CIDR)
			if err != nil {
