...
func AnnotateNode(cs kubernetes.Interface, nodeName string, nd nodeTypes.Node, encryptKey uint8) (nodeAnnotation, error) {
	logAttrs := []any{
		...
		slog.Any(logfields.V4HealthIP, nd.IPv4HealthIP),
		slog.Any(logfields.V6HealthIP, nd.IPv6HealthIP),
		slog.Any(logfields.V4IngressIP, nd.IPv4IngressIP),
		slog.Any(logfields.V6IngressIP, nd.IPv6IngressIP),
		slog.Any(logfields.V4CiliumHostIP, nd.GetCiliumInternalIP(false)),
