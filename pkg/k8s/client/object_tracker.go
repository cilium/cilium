...
// Some parts of this code is originally from k8s.io/client-go/testing
...
func (s *statedbObjectTracker) Update(gvr schema.GroupVersionResource, obj runtime.Object, ns string, opts ...metav1.UpdateOptions) error {
	...
	newMeta, err := meta.Accessor(obj)
	if err != nil {
		s.log.Debug("Update", logfields.Error, err)
		return err
	}
	if len(newMeta.GetNamespace()) == 0 {
		newMeta.SetNamespace(ns)
	...
	if !found {
		...
		log.Debug("Update", logfields.Error, err)
		return err
	}
	wtxn.Commit()
	log.Debug("Update")
