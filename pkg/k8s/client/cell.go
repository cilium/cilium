...
func newClientset(lc fx.Lifecycle, log logrus.FieldLogger, cfg Config) (Clientset, error) {
	if !cfg.isEnabled() {
		return &compositeClientset{enabled: false}, nil
	}
...
func (s *standaloneLifecycle) Append(hook fx.Hook) {
