...
func NewGlobalServiceCache(logger logging.FieldLogger, metricTotalGlobalServices metric.Gauge) *GlobalServiceCache {
...
func (c *GlobalServiceCache) delete(globalService *GlobalService, clusterName string, serviceNN types.NamespacedName) bool {
	...
	if _, ok := globalService.ClusterServices[clusterName]; !ok {
		c.logger.Debug("Ignoring delete request for unknown cluster", logAttrs...)
		return false
	}

	c.logger.Debug("Deleted service definition of remote cluster", logAttrs...)
	...
	if len(globalService.ClusterServices) == 0 {
		c.logger.Debug("Deleted global service", logAttrs)
...
func (c *GlobalServiceCache) OnDelete(svc *serviceStore.ClusterService) bool {
	...
	c.logger.Debug("Delete event for service", logAttr)
	...
	if globalService, ok := c.byName[svc.NamespaceServiceName()]; ok {
		...
		c.logger.Debug("Ignoring delete request for unknown global service", logAttr)
		return false
	}
}
