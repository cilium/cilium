...
func (r *remoteServiceObserver) processKey(handler remoteServiceHandler, svcMergerFunc ServiceMergerFunc, key interface{}) {
...
func (r *remoteServiceObserver) OnUpdate(key store.Key) {
	r.processKey(r.remoteCluster.mesh.globalServices.onUpdate, merger.MergeExternalServiceUpdate, key)
}

// OnDelete is called when a service in a remote cluster is deleted
func (r *remoteServiceObserver) OnDelete(key store.NamedKey) {
	r.processKey(r.remoteCluster.mesh.globalServices.onDelete, merger.MergeExternalServiceDelete, key)
}
