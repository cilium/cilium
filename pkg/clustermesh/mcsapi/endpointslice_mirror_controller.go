...
func (r *mcsAPIEndpointSliceMirrorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if derivedServiceName == "" {
		if derivedEpSlice != nil {
			...
			if localEpSlice != nil {
				r.Logger.Warn(
					"Restarting reconciliation with the derived EndpointSlice "+
						"to recover from tampered mirrored derived EndpointSlice",
					logfields.Request, req.NamespacedName,
					logfields.K8sEndpointName, localEpSlice.Name,
				)
				return r.Reconcile(ctx, ctrl.Request{
					NamespacedName: client.ObjectKeyFromObject(localEpSlice),
				})
			}
