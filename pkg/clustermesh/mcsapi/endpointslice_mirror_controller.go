...
func computePlan(
	...
	clusterName string, derivedService *corev1.Service,
...
func updateDerivedEndpointSlice(
	...
	derivedEndpointSlice.Annotations = maps.Clone(derivedService.Annotations)
	derivedEndpointSlice.AddressType = localEndpointSlice.AddressType

	// We need to deep copy the slices as those struct are full of pointers
	derivedEndpointSlice.Endpoints = make([]discoveryv1.Endpoint, 0, len(localEndpointSlice.Endpoints))
	for _, endpoint := range localEndpointSlice.Endpoints {
		derivedEndpointSlice.Endpoints = append(derivedEndpointSlice.Endpoints, *endpoint.DeepCopy())
	}
	derivedEndpointSlice.Ports = make([]discoveryv1.EndpointPort, 0, len(localEndpointSlice.Ports))
	for _, port := range localEndpointSlice.Ports {
		derivedEndpointSlice.Ports = append(derivedEndpointSlice.Ports, *port.DeepCopy())
	}
}

func newDerivedEndpointSlice(
	localEndpointSlice *discoveryv1.EndpointSlice,
	clusterName string, derivedService *corev1.Service,
...
func (r *mcsAPIEndpointSliceMirrorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	toUpdateSuffixes, toAddSuffixes, toDeleteSuffixes := computePlan(
		derivedEndpointSlicesMap, localEndpointSlicesMap, r.clusterName, &derivedService,
	)
	r.Logger.Debug(
		"MCSAPI EndpointSlice Mirror Reconciler",
		logAttrReq, req.String(),
		logAttrToUpdateSuffixes, fmt.Sprint(toUpdateSuffixes),
		logAttrToAddSuffixes, fmt.Sprint(toAddSuffixes),
		logAttrToDeleteSuffixes, fmt.Sprint(toDeleteSuffixes),
	)
...
func needUpdate(
	...
	for i := range derivedEndpointSlice.Ports {
		if !equalsEndpointPort(&derivedEndpointSlice.Ports[i], &localEndpointSlice.Ports[i]) {
			return true
		}
	}
	...
	for i := range derivedEndpointSlice.Endpoints {
		if !equalsEndpoint(&derivedEndpointSlice.Endpoints[i], &localEndpointSlice.Endpoints[i]) {
			return true
		}
	}
