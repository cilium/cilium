...
func (cm *ClusterMesh) NewRemoteCluster(name string, status common.StatusFunc) common.RemoteCluster {
	...
	rc.remoteServices = cm.conf.StoreFactory.NewWatchStore(
		...
		store.RWSWithEntriesMetric(cm.conf.Metrics.TotalGlobalServices.WithLabelValues(cm.conf.ClusterInfo.Name, cm.nodeName, rc.name)),
	)
	...
	rc.ipCacheWatcher = ipcache.NewIPIdentityWatcher(
		...
		store.RWSWithEntriesMetric(cm.conf.Metrics.TotalIdentities.WithLabelValues(cm.conf.ClusterInfo.Name, cm.nodeName, rc.name)),
	)
