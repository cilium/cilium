...
var Cell = cell.Module(
	...
	cell.Invoke(func(info types.ClusterInfo, dcfg *option.DaemonConfig, cnimgr cni.CNIConfigManager, log logging.FieldLogger, quirks types.QuirksConfig) error {
		err := info.ValidateBuggyClusterID(dcfg.IPAM, cnimgr.GetChainingMode())
		if err != nil && quirks.AllowUnsafePolicySKBUsage {
			log.Error("Detected clustermesh ID configuration that may cause connection impact", slog.Any(logfields.Error, err))
			return nil
		}
		return err
	}),
