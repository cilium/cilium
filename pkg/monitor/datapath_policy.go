...
const (
	...
	// PolicyActionRedirect is the value of Action for redirected traffic
	PolicyActionRedirect = 3
...
func getPolicyMatchTypeString(matchType uint8) string {
	switch matchType {
...
func (n *PolicyVerdictNotify) DumpInfo(data []byte) {
	fmt.Printf("Policy verdict log: flow %#x local EP ID %d, remote ID %d, ingress %v, action %s, match %s, %s\n",
		n.Hash, n.Source, n.RemoteLabel, n.IsTrafficIngress(), GetPolicyActionString(n.Action),
		getPolicyMatchTypeString(n.MatchType), GetConnectionSummary(data[PolicyVerdictNotifyLen:]))
}
