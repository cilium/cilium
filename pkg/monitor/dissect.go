...
var (
	...
	parserL3DevIPv4 *gopacket.DecodingLayerParser
	parserL3DevIPv6 *gopacket.DecodingLayerParser
...
func initParser() {
	if cache == nil {
		...
		parserL3DevIPv4 = gopacket.NewDecodingLayerParser(layers.LayerTypeIPv4, decoders...)
		parserL3DevIPv6 = gopacket.NewDecodingLayerParser(layers.LayerTypeIPv6, decoders...)

		parserL2Dev.IgnoreUnsupported = true
		parserL3DevIPv4.IgnoreUnsupported = true
		parserL3DevIPv6.IgnoreUnsupported = true
	}
}
...
func GetConnectionSummary(data []byte, opts *decodeOpts) string {
	...
	if len(data) > 0 {
		...
		case opts.IsIPv6:
			err = parserL3DevIPv6.DecodeLayers(data, &cache.decoded)
		default:
			err = parserL3DevIPv4.DecodeLayers(data, &cache.decoded)
		}
