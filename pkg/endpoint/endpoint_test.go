...
func (s *EndpointSuite) TestEndpointEventQueueDeadlockUponStop(c *C) {
	...
	ep := NewEndpointWithState(s, s, &FakeEndpointProxy{}, testidentity.NewFakeIdentityAllocator(nil), 12345, StateReady)
...
func BenchmarkEndpointGetModel(b *testing.B) {
	e := NewEndpointWithState(&suite, &FakeEndpointProxy{}, testidentity.NewFakeIdentityAllocator(nil), 123, StateWaitingForIdentity)
