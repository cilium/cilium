...
func (s *EndpointSuite) TestEndpointEventQueueDeadlockUponStop(c *C) {
	...
	ep := NewEndpointWithState(s, &FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), 12345, StateReady)
...
func BenchmarkEndpointGetModel(b *testing.B) {
	e := NewEndpointWithState(&suite, &FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), 123, StateWaitingForIdentity)
