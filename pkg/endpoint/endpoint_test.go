...
func setupEndpointSuite(tb testing.TB) *EndpointSuite {
	...
	s.fetcher = compute.NewIdentityPolicyRecomputer(compute.Params{Repo: repo, Logger: logger})
...
func TestEndpointUpdateLabels(t *testing.T) {
	...
	e, err := NewEndpointFromChangeModel(t.Context(), hivetest.Logger(t), nil, &MockEndpointBuildQueue{}, nil, s.orchestrator, nil, nil, nil, identitymanager.NewIDManager(logger), nil, nil, s.repo, s.fetcher, testipcache.NewMockIPCache(), &FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), ctmap.NewFakeGCRunner(), nil, model)
	require.NoError(t, err)
...
func TestEndpointEventQueueDeadlockUponStop(t *testing.T) {
	...
	ep, err := NewEndpointFromChangeModel(t.Context(), hivetest.Logger(t), nil, &MockEndpointBuildQueue{}, nil, s.orchestrator, nil, nil, nil, identitymanager.NewIDManager(logger), nil, nil, s.repo, s.fetcher, testipcache.NewMockIPCache(), &FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), ctmap.NewFakeGCRunner(), nil, model)
	require.NoError(t, err)
