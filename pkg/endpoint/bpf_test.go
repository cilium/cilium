...
func (s *EndpointSuite) TestWriteInformationalComments(c *C) {
	e := NewEndpointWithState(s, &FakeEndpointProxy{}, &testutils.FakeIdentityAllocator{}, 100, StateCreating)
...
func BenchmarkWriteHeaderfile(b *testing.B) {
	e := NewEndpointWithState(&suite, &FakeEndpointProxy{}, &testutils.FakeIdentityAllocator{}, 100, StateCreating)
