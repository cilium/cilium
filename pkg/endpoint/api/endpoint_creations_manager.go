...
func (m *endpointCreationManager) NewCreateRequest(ep *endpoint.Endpoint, cancel context.CancelFunc) {
	...
	if req, ok := m.requests[cepName]; ok {
		ep.Logger(endpointAPISubsys).Warning("Cancelling obsolete endpoint creating due to new create for same cep name")
		req.cancel()
	}
...
func (m *endpointCreationManager) CancelCreateRequest(ep *endpoint.Endpoint) {
	if m.EndCreateRequest(ep) {
		ep.Logger(endpointAPISubsys).Warning("Cancelled endpoint create request due to receiving endpoint delete request")
	}
}
