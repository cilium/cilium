...
func setupSharedMapsPrivileged(t *testing.T) {
	...
	policymap.SetSharedPolicyMap(policymap.NewSharedPolicyBPFMap())
	policymap.SetPolicyOverlayMap(policymap.NewPolicyOverlayBPFMap())

	err := policymap.GetSharedPolicyMap().OpenOrCreate()
	require.NoError(t, err)
	err = policymap.GetPolicyOverlayMap().OpenOrCreate()
	require.NoError(t, err)
}
...
func TestPrivilegedEndpointPolicySyncShared(t *testing.T) {
	...
	defer func() {
		policymap.GetSharedPolicyMap().Close()
		policymap.GetPolicyOverlayMap().Close()
	}()
	...
	logger := hivetest.Logger(t)
	repo := policy.NewPolicyRepository(logger, cmtypes.DefaultClusterInfo.ID, nil, nil, nil, nil, testpolicy.NewPolicyMetricsNoop())
	...
	val, err := policymap.GetPolicyOverlayMap().Lookup(&overlayKey)
	require.NoError(t, err)
	...
	sharedKey1 := policymap.SharedPolicyKey{
		...
		Nexthdr:          6,
		DestPortNetwork:  byteorder.HostToNetwork16(443),
	}
	entry1, err := policymap.GetSharedPolicyMap().Lookup(&sharedKey1)
	require.NoError(t, err)
	...
	entry2, err := policymap.GetSharedPolicyMap().Lookup(&sharedKey2)
	require.NoError(t, err)
	...
	entry3, err := policymap.GetSharedPolicyMap().Lookup(&sharedKey3)
	require.NoError(t, err)
	...
	entry4, err := policymap.GetSharedPolicyMap().Lookup(&sharedKey4)
	require.NoError(t, err)
