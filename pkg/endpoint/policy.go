...
func (e *Endpoint) computeDesiredL4PolicyMapEntries(keysToAdd map[policymap.PolicyKey]uint16) {
	if keysToAdd == nil {
		keysToAdd = map[policymap.PolicyKey]uint16{}
	}
	...
	for _, filter := range e.DesiredL4Policy.Ingress {
		...
		for _, keyFromFilter := range keysFromFilter {
			...
			keysToAdd[keyFromFilter] = e.lookupRedirectPort(&filter)
		}
	}
...
func (e *Endpoint) computeDesiredPolicyMapState(owner Owner, labelsMap *identityPkg.IdentityCache,
	...
	desiredPolicyKeys := make(map[policymap.PolicyKey]uint16)
...
func (e *Endpoint) determineAllowLocalhost(desiredPolicyKeys map[policymap.PolicyKey]uint16) {

	if desiredPolicyKeys == nil {
		desiredPolicyKeys = map[policymap.PolicyKey]uint16{}
	}

	if option.Config.AlwaysAllowLocalhost() || (e.DesiredL4Policy != nil && e.DesiredL4Policy.HasRedirect()) {
		desiredPolicyKeys[localHostKey] = 0
	}
}
...
func (e *Endpoint) determineAllowFromWorld(desiredPolicyKeys map[policymap.PolicyKey]uint16) {
	...
	if option.Config.HostAllowsWorld && localHostAllowed {
		desiredPolicyKeys[worldKey] = 0
	}
}

func (e *Endpoint) computeDesiredL3PolicyMapEntries(owner Owner, identityCache *identityPkg.IdentityCache, repo *policy.Repository, desiredPolicyKeys map[policymap.PolicyKey]uint16) {
