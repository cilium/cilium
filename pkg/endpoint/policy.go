...
// May ONLY be called from applyPolicyMapChanges() that makes sure the selector cache version handle
// is up-to-date (via ConsumeMapChanges).
...
func (e *Endpoint) useCurrentNetworkPolicy(proxyWaitGroup *completion.WaitGroup) {
	if e.SecurityIdentity != nil && e.desiredPolicy != nil && !e.IsProxyDisabled() {
...
func (e *Endpoint) regeneratePolicy(stats *regenerationStatistics, datapathRegenCtxt *datapathRegenerationContext) (*policyGenerateResult, error) {
	...
	// Add new redirects before Consume() so that all required redirects are available for it.
	// Ingress endpoint needs no redirects
	var desiredRedirects map[string]uint16
	if !e.isProperty(PropertySkipBPFPolicy) {
		stats.proxyConfiguration.Start()
		desiredRedirects, err, ff, rf = e.addNewRedirects(result.selectorPolicy, datapathRegenCtxt.proxyWaitGroup)
		...
		// Add a finalize function to clear out stale redirects. This will be called after new
		// redirects have been acknowledged, and policy maps and NetworkPolicy have been updated.
		// We are not waiting for an acknowledgement for the removal.
		...
		datapathRegenCtxt.finalizeList.Append(func() {
			// At the point of this call, traffic is no longer redirected to the proxy for
...
func (e *Endpoint) setDesiredPolicy(res *policyGenerateResult, datapathRegenCtxt *datapathRegenerationContext) error {
	defer func() {
		// Detach the endpoint policy if it was not assigned to 'e.desiredPolicy'
