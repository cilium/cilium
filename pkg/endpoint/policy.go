...
func (e *Endpoint) setDesiredPolicy(datapathRegenCtxt *datapathRegenerationContext) error {
	...
	if res.endpointPolicy != nil && res.endpointPolicy != e.desiredPolicy {
		...
		// Capture the enforcement mode of the policy we are about to detach so
		// that, if we end up reverting to an empty realizedPolicy stub below,
		// we know whether that stub needs wildcard entries to accurately
		// represent "policy enforcement disabled". This must happen before
		// Detach(), which is called on the revert path below.
		...
		datapathRegenCtxt.revertStack.Push(func() error {
			...
			if e.policyMap != nil && e.desiredPolicy != e.realizedPolicy {
				...
				// If this is the endpoint's very first regeneration, e.realizedPolicy
				// is still the empty stub created by NewEndpointPolicy (no PolicyOwner,
				// no wildcard entries). If policy enforcement is disabled and the BPF
				// policy map already holds wildcard allow entries (written by the
				// pre-compile sync in runPreCompilationSteps), syncing against the
				// empty stub below would delete them, leaving the datapath dropping
				// all traffic with no successful regeneration ever able to recover
				// (since desiredPolicy == realizedPolicy going forward). Repair the
				// stub first so the sync reconciles instead of wiping it. See GH-44855.
				//
				// Guard on !IsValid() (PolicyOwner == nil), not just Empty(): a real,
				// previously-realized policy can legitimately be Empty() too (e.g.
				// enforcement enabled on both directions with a default-deny outcome
				// and no selecting rules). Injecting wildcard allow entries into such
				// a policy would silently open traffic that must stay denied, turning
				// this availability fix into a security regression. Only the pristine
				// NewEndpointPolicy stub has PolicyOwner == nil, so IsValid() is the
				// precise "is this the untouched first-regen stub" check.
				if !e.realizedPolicy.IsValid() && e.realizedPolicy.Empty() && (!ingressPolicyEnabled || !egressPolicyEnabled) {
					e.realizedPolicy.AllowAllIdentities(!ingressPolicyEnabled, !egressPolicyEnabled)
				}
