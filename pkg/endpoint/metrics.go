...
var (
	epdpointPolicyStatus = new(endpointPolicyStatusMap)
)
...
func (s *regenerationStatistics) SendMetrics() {
	epdpointPolicyStatus.AddEndpoint(s.endpointID, s.policyStatus)
...
// enforcement status. Is used only for send metrics to prometheus.
type endpointPolicyStatusMap struct {
	sync.Map
}

// AddEndpoint add a new endpoint to the map and update the metrics related
func (epPolicyMaps *endpointPolicyStatusMap) AddEndpoint(endpointID uint16, policyStatus models.EndpointPolicyEnabled) {
	epPolicyMaps.Store(endpointID, policyStatus)
	epdpointPolicyStatus.SendMetrics()
}

// DeleteEndpoint delete a endpoint from the map and update the metrics
func (epPolicyMaps *endpointPolicyStatusMap) DeleteEndpoint(endpointID uint16) {
	epPolicyMaps.Delete(endpointID)
	epPolicyMaps.SendMetrics()
}

// SendMetrics update the policy enforcement metrics statistics for the endpoints.
func (epPolicyMaps *endpointPolicyStatusMap) SendMetrics() {
