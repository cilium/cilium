...
func newDatapathRegenerationContext() *datapathRegenerationContext {
	...
	bpfRegenCtx := &datapathRegenerationContext{
...
func (e *Endpoint) runPreCompilationSteps(owner Owner, datapathRegenCtx *datapathRegenerationContext, currentDir, nextDir string, regenContext *RegenerationContext) (func(err error), error) {
	...
	if err = datapathRegenCtx.completionCtx.Err(); err != nil {
		return nil, err
	}
	...
	if !e.ctCleaned {
		go func() {
			...
			close(datapathRegenCtx.ctCleaned)
		}()
	} else {
		close(datapathRegenCtx.ctCleaned)
	}
	...
	if option.Config.DryMode {
		...
		if err = e.regeneratePolicy(owner); err != nil {
			return nil, fmt.Errorf("Unable to regenerate policy: %s", err)
		}
		...
		if err = e.writeHeaderfile(nextDir, owner); err != nil {
			return nil, fmt.Errorf("Unable to write header file: %s", err)
		}
	...
	proxyCleanupFunc := func(err error) {
		if err == nil {
	...
	if e.SecurityIdentity != nil {
		...
		if err != nil {
			return proxyCleanupFunc, fmt.Errorf("unable to regenerate policy for '%s': %s", e.PolicyMap.String(), err)
		}
		...
		err = e.syncPolicyMap()
		if err != nil {
			return proxyCleanupFunc, fmt.Errorf("unable to regenerate policy because PolicyMap synchronization failed: %s", err)
		}
		...
		err, networkPolicyRevertFunc = e.updateNetworkPolicy(owner, datapathRegenCtx.proxyWaitGroup)
		stats.proxyPolicyCalculation.End(err == nil)
		if err != nil {
			e.Unlock()
			return proxyCleanupFunc, err
		}
	...
	if e.DesiredL4Policy != nil {
		desiredRedirects, err, finalizeFunc, revertFunc = e.addNewRedirects(owner, e.DesiredL4Policy, datapathRegenCtx.proxyWaitGroup)
		if err != nil {
	...
	finalizeFunc, revertFunc = e.removeOldRedirects(owner, desiredRedirects, datapathRegenCtx.proxyWaitGroup)
	finalizeList.Append(finalizeFunc)
	revertStack.Push(revertFunc)
	...
	if err = e.writeHeaderfile(nextDir, owner); err != nil {
		...
		e.Unlock()
		return proxyCleanupFunc, fmt.Errorf("unable to write header file: %s", err)
	}

	// Avoid BPF program compilation and installation if the headerfile for the endpoint
	// or the node have not changed.
	datapathRegenCtx.bpfHeaderfilesHash, err = hashEndpointHeaderfiles(nextDir)
	if err != nil {
		e.getLogger().WithError(err).Warn("Unable to hash header file")
		datapathRegenCtx.bpfHeaderfilesHash = ""
		datapathRegenCtx.bpfHeaderfilesChanged = true
	} else {
		datapathRegenCtx.bpfHeaderfilesChanged = (datapathRegenCtx.bpfHeaderfilesHash != e.bpfHeaderfileHash)
		e.getLogger().WithField(logfields.BPFHeaderfileHash, datapathRegenCtx.bpfHeaderfilesHash).
	...
	// but still access information about the endpoint later if needed.
	if datapathRegenCtx.bpfHeaderfilesChanged {
		datapathRegenCtx.epInfoCache = e.createEpInfoCache(nextDir)
	} else {
		datapathRegenCtx.epInfoCache = e.createEpInfoCache(currentDir)
	}
	if datapathRegenCtx.epInfoCache == nil {
		stats.prepareBuild.End(false)
		return proxyCleanupFunc, fmt.Errorf("Unable to cache endpoint information")
	}
...
func (e *Endpoint) regenerateBPF(owner Owner, currentDir, nextDir string, regenContext *RegenerationContext) (revnum uint64, compiled bool, reterr error) {
	...
	// Cache endpoint ID for use later.
	e.UnconditionalRLock()
	epID := e.StringID()
	e.RUnlock()

	bpfRegenContext := newDatapathRegenerationContext()
	defer bpfRegenContext.completionCancel()

	// Wrap logic which requires the endpoint to be locked within a function
	// & call within regenerateBPF so that endpoint lock release can be deferred
	// within the function. If this isn't done, and something else panics
	// while the endpoint regeneration lock is held, and a deferred function
	// within regenerateBPF tries to grab the endpoint lock, it will deadlock
	...
	// lock.
	revFunc, err := e.runPreCompilationSteps(owner, bpfRegenContext, currentDir, nextDir, regenContext)

	// Note that revFunc may hold the endpoint lock.
	if revFunc != nil {
		defer revFunc(reterr)
	}
	...
	<-bpfRegenContext.ctCleaned
	stats.waitingForCTClean.End(true)

	// The endpoint lock should not be held while compiling BPF, as this is a
	// long-lived operation.
	e.getLogger().WithField("bpfHeaderfilesChanged", bpfRegenContext.bpfHeaderfilesChanged).Debug("Preparing to compile BPF")

	stats.prepareBuild.End(true)
	if bpfRegenContext.bpfHeaderfilesChanged || regenContext.ReloadDatapath {
		...
		if bpfRegenContext.bpfHeaderfilesChanged {
			stats.bpfCompilation.Start()
			err = loader.CompileAndLoad(bpfRegenContext.completionCtx, bpfRegenContext.epInfoCache)
			...
			err = loader.ReloadDatapath(bpfRegenContext.completionCtx, bpfRegenContext.epInfoCache)
		...
		if err != nil {
			return bpfRegenContext.epInfoCache.revision, compilationExecuted, err
		}
		e.bpfHeaderfileHash = bpfRegenContext.bpfHeaderfilesHash
	} else {
		e.getLogger().WithField(logfields.BPFHeaderfileHash, bpfRegenContext.bpfHeaderfilesHash).
			Debug("BPF header file unchanged, skipping BPF compilation and installation")
	}

	// Hook the endpoint into the endpoint and endpoint to policy tables then expose it
	stats.mapSync.Start()
	epErr := eppolicymap.WriteEndpoint(bpfRegenContext.epInfoCache.keys, e.PolicyMap.Fd)
	err = lxcmap.WriteEndpoint(bpfRegenContext.epInfoCache)
	stats.mapSync.End(err == nil)
	if epErr != nil {
	...
	err = e.WaitForProxyCompletions(bpfRegenContext.proxyWaitGroup)
