...
func (e *Endpoint) removeOldRedirects(owner Owner, desiredRedirects map[string]bool, proxyWaitGroup *completion.WaitGroup) (FinalizeFunc, RevertFunc) {
	...
	return e.FinalizeFunc(&finalizeList),
		func() error {
			e.Logger().Debug("Reverting removal of old redirects")
...
func (e *Endpoint) regenerateBPF(owner Owner, currentDir, nextDir string, regenContext *RegenerationContext) (revnum uint64, compiled bool, reterr error) {
	...
	defer func() {
		if reterr == nil {
			...
			e.UnconditionalLock()
			e.Logger().Error("Finalizing successful endpoint regeneration")
			e.Finalize(&finalizeList)
			e.Unlock()
		} else {
			if err := e.LockAlive(); err != nil {
				e.Logger().WithError(err).Debug("Skipping unnecessary restoring endpoint state")
				return
			}
