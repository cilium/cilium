...
func (e *Endpoint) syncAckedRedirects(ackedRedirects *sync.Map) {
	if e.LockAlive() == nil {
		...
		ackedRedirects.Range(func(key, value interface{}) bool {
			...
			if !keyOk || !redirectOk {
				e.Logger().Warningf("Error syncing acknowledged redirects!")
				return false
			}
			...
			if !l4Ok {
				e.Logger().Warningf("Error syncing acknowledged redirects, missing l4 policy for %s!", redirect.key)
				return false
			}
