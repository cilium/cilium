	...
	if datapathRegenCtxt.regenerationLevel > regeneration.RegenerateWithoutDatapath {
		if debugEnabled {
			debugFunc := func(format string, args ...interface{}) {
				e.getLogger().Debug(fmt.Sprintf(format, args), slog.String(logfields.EndpointID, e.StringID()))
			}
...
func (e *Endpoint) runPreCompilationSteps(regenContext *regenerationContext) (preCompilationError error) {
	...
	if datapathRegenCtxt.bpfHeaderfilesHash != e.bpfHeaderfileHash {
		if logger := e.getLogger(); logger.Enabled(context.Background(), slog.LevelDebug) {
			e.getLogger().Debug(
				fmt.Sprintf("BPF header file hashed (was: %q)", e.bpfHeaderfileHash),
				slog.Any(logfields.BPFHeaderfileHash, datapathRegenCtxt.bpfHeaderfilesHash),
			)
		}
...
func (e *Endpoint) applyPolicyMapChangesLocked(regenContext *regenerationContext, hasNewPolicy bool) error {
	...
	if e.policyMap == nil {
		if logging.CanLogAt(log, slog.LevelDebug) {
			log.Debug("Skipping bpf updates due to endpoint not having policy map yet", slog.Any(logfields.EndpointID, e.ID))
		}
		return nil
	}
