...
// addNewRedirectsFromSelectorPolicy must be called while holding the endpoint build mutex.
// On success, returns nil; otherwise, returns an error indicating the
// problem that occurred while adding an l7 redirect for the specified policy.
// Only called after a new selector policy has been computed.
func (e *Endpoint) addNewRedirectsFromSelectorPolicy(selectorPolicy policy.SelectorPolicy, desiredRedirects map[string]uint16, proxyWaitGroup *completion.WaitGroup) (revert.FinalizeFunc, revert.RevertFunc) {
...
func (e *Endpoint) addNewRedirects(selectorPolicy policy.SelectorPolicy, proxyWaitGroup *completion.WaitGroup) (desiredRedirects map[string]uint16, err error, finalizeFunc revert.FinalizeFunc, revertFunc revert.RevertFunc) {
	...
	ff, rf = e.addNewRedirectsFromSelectorPolicy(selectorPolicy, desiredRedirects, proxyWaitGroup)
...
func (e *Endpoint) applyPolicyMapChanges(regenContext *regenerationContext, hasNewPolicy bool) error {
	...
	if updateEnvoy {
		...
		err, rf = e.updateNetworkPolicy(proxyWaitGroup)
