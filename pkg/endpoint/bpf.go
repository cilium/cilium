...
func (e *Endpoint) realizeBPFState(regenContext *regenerationContext) (compilationExecuted bool, err error) {
	...
	if datapathRegenCtxt.regenerationLevel > RegenerateWithoutDatapath {
		...
		if datapathRegenCtxt.regenerationLevel == RegenerateWithDatapathRebuild {
			...
			if err == nil {
				e.getLogger().Debug("Rewrote endpoint BPF program")
			} else {
				e.getLogger().WithError(err).Warning("Error while rewriting endpoint BPF program")
			}
			...
			if err == nil {
				e.getLogger().Debug("Reloaded endpoint BPF program")
			} else {
				e.getLogger().WithError(err).Warning("Error while reloading endpoint BPF program")
			}
		}
		close(closeChan)
