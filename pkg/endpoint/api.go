...
func toSlice(idMap map[uint32]struct{}) (res []int64) {
	for id := range idMap {
...
func (e *Endpoint) getRealizedPolicyIdentities() (ing, eg, ingDeny, egDeny []int64) {
	...
	ingressMap := make(map[uint32]struct{})
	ingressDenyMap := make(map[uint32]struct{})
	egressMap := make(map[uint32]struct{})
	egressDenyMap := make(map[uint32]struct{})
