	...
	e.controllers.UpdateController(controllerName,
		controller.ControllerParams{
			...
			DoFunc: func() (err error) {
				...
				// This is inside the controller to avoid deadlocking; calls to
				// RunK8sCiliumEndpointSync hold ep.Mutex and that conflicts with
				// HasLabels.
				if isHealthEP := e.HasLabels(pkgLabels.LabelHealth); isHealthEP {
					// use an error object so we can stop the controller. This is NOT an
					// error
					msg := "Suspending unnecessary CEP controller for cilium-health endpoint"
					scopedLog.Debug(msg)
					return controller.MakeExitReason(msg)
				}
