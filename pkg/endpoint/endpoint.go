...
type Endpoint struct {
	...
	// Opts determine the datapath configuration of the endpoint.
...
func NewEndpointWithState(ID uint16, state string) *Endpoint {
	return &Endpoint{
		ID:     ID,
		Opts:   option.NewIntOptions(&EndpointMutableOptionLibrary),
...
func (e *Endpoint) applyOptsLocked(opts map[string]string) bool {
	return e.Opts.ApplyValidated(opts, optionChanged, e) > 0
}
...
func (e *Endpoint) SetDefaultOpts(opts *option.IntOptions) {
	if e.Opts == nil {
		e.Opts = option.NewIntOptions(&EndpointMutableOptionLibrary)
	}
	...
	if opts != nil {
		...
		for k := range epOptLib {
			e.Opts.SetValidated(k, opts.GetValue(k))
		}
	}
}
