...
type Endpoint struct {
	...
	loggerAttrs lock.Map[string, slog.Value]

	// loggerAttrs are attributes.
	policyLoggerAttrs lock.Map[string, slog.Value]
...
func (e *Endpoint) waitForProxyCompletions(proxyWaitGroup *completion.WaitGroup) error {
	...
	e.getLogger().Debug("Wait time for proxy updates", slog.Duration("duration", time.Since(start)))
...
func (e *Endpoint) logStatusLocked(typ StatusType, code StatusCode, msg string) {
	...
	if logger := e.getLogger(); logging.CanLogAt(logger, slog.LevelDebug) {
		logger.Debug(
			msg,
			slog.Any("code", sts.Status.Code),
			slog.Any("type", sts.Status.Type),
			slog.String(logfields.EndpointState, sts.Status.State),
			slog.Uint64(logfields.PolicyRevision, e.policyRevision),
		)
	}
}
...
func (e *Endpoint) Update(cfg *models.EndpointConfigurationSpec) error {
	...
	e.getLogger().Debug("updating endpoint configuration options", slog.Any("configuration-options", cfg))
...
func (e *Endpoint) SetK8sMetadata(containerPorts []slim_corev1.ContainerPort) {
	...
	for _, cp := range containerPorts {
		...
		if err != nil {
			e.getLogger().Warn("Adding named port failed", slog.Any(logfields.Error, err))
			continue
		}
	}
...
func (e *Endpoint) setState(toState State, reason string) bool {
	...
	if toState != fromState {
		...
		e.getLogger().Info(
			...
			slog.Any(logfields.EndpointState+".from", fromState),
			slog.Any(logfields.EndpointState+".to", toState),
			slog.String("file", fileName),
			slog.Int("line", fileLine),
		)
	}
...
func (e *Endpoint) UpdateProxyStatistics(proxyType, l4Protocol string, port, proxyPort uint16, ingress, request bool, verdict accesslog.FlowVerdict) {
	...
	if !ok {
		e.getLogger().Info(
			...
			slog.String(logfields.L4PolicyID, key),
		)
		return
	}
...
func (e *Endpoint) InitWithIngressLabels(ctx context.Context, launchTime time.Duration) {
	...
	if errors.Is(newCtx.Err(), context.DeadlineExceeded) {
		e.getLogger().Warn("Timed out while updating security identify for host endpoint", slog.Any(logfields.Error, newCtx.Err()))
	}
}
...
func (e *Endpoint) UpdateLabels(ctx context.Context, sourceFilter string, identityLabels, infoLabels labels.Labels, blocking bool) (regenTriggered bool) {
	e.getLogger().Debug(
		...
		slog.String(logfields.SourceFilter, sourceFilter),
		slog.Any(logfields.IdentityLabels, identityLabels),
		slog.Any(logfields.InfoLabels, infoLabels),
	)
...
func (e *Endpoint) UpdateLabelsFrom(oldLbls, newLbls map[string]string, source string) error {
	...
	e.getLogger().Debug(
		...
		slog.Any(logfields.Labels, newIdtyLabels),
	)
	return nil
}
...
func (e *Endpoint) runIdentityResolver(ctx context.Context, blocking bool) (regenTriggered bool) {
	...
	e.runlock()
	scopedLog := e.getLogger().With(slog.Any(logfields.IdentityLabels, newLabels))
	...
	if blocking || identity.IdentityAllocationIsLocal(newLabels) {
		...
		if err != nil {
			...
			scopedLog.Warn("Error changing endpoint identity", slog.Any(logfields.Error, err))
		}
	} else {
...
func (e *Endpoint) identityLabelsChanged(ctx context.Context) (regenTriggered bool, err error) {
	...
	releaseNewlyAllocatedIdentity := func() {
		...
		if err != nil {
			...
			scopedLog.Warn(
				...
				slog.Any(logfields.Error, err),
				slog.Any(logfields.Identity, allocatedIdentity.ID),
			)
		}
	}
	...
	scopedLog.Debug(
		...
		slog.String(logfields.Identity, allocatedIdentity.StringID()),
	)
	...
	if oldIdentity != nil {
		...
		if err != nil {
			scopedLog.Warn(
				...
				slog.Any(logfields.Error, err),
				slog.Any(logfields.Identity, oldIdentity.ID),
			)
		}
	}
...
func (e *Endpoint) Delete(conf DeleteConfig) []error {
	...
	if option.Config.IPAM == ipamOption.IPAMENI || option.Config.IPAM == ipamOption.IPAMAzure || option.Config.IPAM == ipamOption.IPAMAlibabaCloud ||
		...
		e.getLogger().Debug(
			...
			slog.Any("ep", e.GetID()),
			slog.Any("ipAddr", e.GetIPv4Address()),
		)
