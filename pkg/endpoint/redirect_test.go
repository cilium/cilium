...
func setupRedirectSuite(tb testing.TB) *RedirectSuite {
	...
	s.do.repo = policy.NewPolicyRepository(identityCache, nil, envoypolicy.NewEnvoyL7RulesTranslator(hivetest.Logger(tb), certificatemanager.NewMockSecretManagerInline()), s.do.idmgr, api.NewPolicyMetricsNoop())
