...
func ReadEPsFromDirNames(ctx context.Context, owner regeneration.Owner, prg policyRepoGetter, basePath string, eptsDirNames []string) map[uint16]*Endpoint {
	...
	for _, epDirName := range completeEPDirNames {
		...
		ep, err := parseEndpoint(ctx, owner, prg, bEp)
		if err != nil {
