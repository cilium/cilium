...
func (s *EndpointSuite) endpointCreator(id uint16, secID identity.NumericIdentity) *Endpoint {
	...
	ep := NewTestEndpointWithState(s, nil, s.orchestrator, nil, nil, nil, identitymanager.NewIDManager(), nil, s, testipcache.NewMockIPCache(), &FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), ctmap.NewFakeGCRunner(), id, StateReady)
...
func TestReadEPsFromDirNames(t *testing.T) {
	...
	eps := ReadEPsFromDirNames(context.TODO(), s, nil, s.orchestrator, nil, nil, nil, nil, nil, s, s, tmpDir, epsNames)
...
func TestReadEPsFromDirNamesWithRestoreFailure(t *testing.T) {
	...
	epResult := ReadEPsFromDirNames(context.TODO(), s, nil, s.orchestrator, nil, nil, nil, nil, nil, s, s, tmpDir, epNames)
