...
func TestReadEPsFromDirNames(t *testing.T) {
	...
	p.PolicyRepo = s.repo
	eps := ReadEPsFromDirNames(context.TODO(), logger, &fakeParser{p: p}, tmpDir, epsNames)
...
func TestReadEPsFromDirNamesWithRestoreFailure(t *testing.T) {
	...
	p.Orchestrator = s.orchestrator
	p.PolicyRepo = s.repo
	epResult := ReadEPsFromDirNames(context.TODO(), logger, &fakeParser{p: p}, tmpDir, epNames)
...
func (f *fakeParser) ParseEndpoint(epJSON []byte) (*Endpoint, error) {
	return ParseEndpoint(f.p, nil, nil, nil, epJSON)
}
