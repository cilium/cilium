...
func (c *Client) IPAMAllocate(family string) (*models.IPAMResponse, error) {
	params := ipam.NewPostIPAMParams().WithTimeout(defaults.ClientTimeout)
...
func (c *Client) IPAMAllocateIP(ip string) error {
	params := ipam.NewPostIPAMIPParams().WithIP(ip).WithTimeout(defaults.ClientTimeout)
...
func (c *Client) IPAMReleaseIP(ip string) error {
	params := ipam.NewDeleteIPAMIPParams().WithIP(ip).WithTimeout(defaults.ClientTimeout)
