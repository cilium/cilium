...
func (cw *configWatcher) watch(ctx context.Context, health cell.Health) error {
	for {
		...
		entry, found, w := dynamicconfig.WatchKey(cw.DB.ReadTxn(), cw.DynamicConfigTable, DynamicFeaturesConfigKey)
		fs := cw.ManagerConfig.DynamicFeatureConfig
		if found {
			fs = entry.Value
		}
...
func registerWatcher(p watcherParams) error {
	...
	p.JobGroup.Add(job.OneShot("dynamic-config-watcher", func(ctx context.Context, health cell.Health) error {
		if err := waitForDynamicFeatureTableInit(ctx, p); err != nil {
			return err
		}
...
func waitForDynamicFeatureTableInit(ctx context.Context, p watcherParams) error {
	//Wait for DynamicFeatureTable initialization
	for {
		_, channel := p.DynamicFeatureTable.AllWatch(p.DB.ReadTxn())
		if p.DynamicFeatureTable.NumObjects(p.DB.ReadTxn()) == 0 {
			select {
			case <-ctx.Done():
				return ctx.Err()
			case <-channel:
