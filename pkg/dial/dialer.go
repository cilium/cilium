...
// A lastConnectedIP pointer could be provided which the dialer will use to store the last
// successfully connected IP address which will be always attempted first if this
// IP is still valid.
func NewStaticContextDialerWithFallback(
	log *slog.Logger, hostname string, ips []netip.Addr, fallback dialContextFn,
	...
	return newStaticContextDialerWithFallback(log, func(ctx context.Context, address string) (net.Conn, error) {
		return (&net.Dialer{}).DialContext(ctx, "tcp", address)
	}, hostname, ips, fallback, lastConnectedIP)
}

// newStaticContextDialerWithFallback is mostly adapted from Go's net/dial.go
// dialSerial adapted for our static use case
func newStaticContextDialerWithFallback(
	log *slog.Logger, dialContext dialContextFn, hostname string, ips []netip.Addr,
	fallback dialContextFn, lastConnectedIP *netip.Addr,
) dialContextFn {
...
func shuffleIPsWithLastConnectedIPAffinity(
	...
	lastIPIndex := -1
	toShuffle := retIPs
	if lastConnectedIP != nil && lastConnectedIP.IsValid() {
