...
var (
	log = logging.DefaultSlogLogger.With(logfields.LogSubsys, "pod-cidr")
...
func syncToK8s(nodeGetterUpdater ipam.CiliumNodeGetterUpdater, ciliumNodesToK8s map[string]*ciliumNodeK8sOp) (retErr error) {
	for nodeName, nodeToK8s := range ciliumNodesToK8s {
		var (
			...
			log           = log.With("node-name", nodeName)
		)
		switch nodeToK8s.op {
		case k8sOpCreate:
		...
		case k8sOpUpdate:
			...
			log.Debug("Updated Node", logfields.Error, err)
			if err != nil {
		...
		case k8sOpUpdateStatus:
			...
			log.Debug("UpdatedStatus Node", logfields.Error, err)
			switch {
		...
		case k8sOpDelete:
			...
			if err != nil && k8sErrors.IsNotFound(err) {
				...
				log.Warn("Received a CiliumNode delete event, but the resource may not have been deleted (see error)",
					logfields.Error, err)
			}
		}
		switch {
...
func (n *NodesPodCIDRManager) allocateNode(node *v2.CiliumNode) (cn *v2.CiliumNode, allocated, updateStatus bool, err error) {
	...
	log = log.With("node-name", node.Name)
	...
	if len(node.Spec.IPAM.PodCIDRs) == 0 {
		...
		log.Debug("Allocated new CIDRs", "cidrs", cidrs.String(), "allocated", allocated)
	} else {
		...
		log.Debug("Allocated existing CIDRs",
			"cidrs", cidrs.String(),
			"allocated", allocated,
...
func releaseCIDRs(cidrAllocators []cidralloc.CIDRAllocator, cidrsToRelease []*net.IPNet) {
	...
	for _, ipNet := range cidrsToRelease {
		for _, clusterCIDR := range cidrAllocators {
			...
			if err != nil {
				log.Error("failed to release cidr", "cidr", ipNet.String(), logfields.Error, err)
				continue
			}
			log.Info("node released cidrs", "cidr", ipNet.String())
			break
		}
	}
}
...
func (n *NodesPodCIDRManager) reuseIPNets(
	...
	if canAllocateIPv4PodCIDRs && len(n.v4CIDRAllocators) != 0 {
		...
		log.Debug(fmt.Sprintf("Allocated v4CIDR %s", v4CIDR))
		allocated = true
	}
	...
	if canAllocateIPv6PodCIDRs && len(n.v6CIDRAllocators) != 0 {
		...
		log.Debug(fmt.Sprintf("Allocated v6CIDR %s", v6CIDR))
		allocated = true
	}
...
func (n *NodesPodCIDRManager) allocateNext(nodeName string) (*nodeCIDRs, bool, error) {
	...
	if len(n.v4CIDRAllocators) != 0 {
		...
		log.Debug("v4 allocated CIDR", "CIDR", v4CIDR)
	...
	if len(n.v6CIDRAllocators) != 0 {
		...
		log.Debug("v6 allocated CIDR", "CIDR", v6CIDR)
