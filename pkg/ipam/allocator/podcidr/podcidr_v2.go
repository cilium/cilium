...
func (e aggregatedErrors) Error() string {
	...
	default:
		...
		return fmt.Sprintf("encountered %d errors: %s", len(e), strings.Join(errStrs, "; "))
	}
}
...
) (result []*net.IPNet, changed bool, errs aggregatedErrors) {
...
func (n *NodesPodCIDRManager) allocateNodeV2(node *v2.CiliumNode) (cn *v2.CiliumNode, updateSpec, updateStatus bool, err error) {
	...
	errs = append(errs, v4Errors...)
	errs = append(errs, v6Errors...)

	updateStatus = len(errs) != 0
	...
	// If errs is empty, this will reset Error to an empty string
	cn.Status.IPAM.OperatorStatus.Error = errs.Error()
