...
func TestPoolUpdateWithCIDRInUse(t *testing.T) {
	...
	assert.NoError(t, err)
	defaultPool, exists := p.pools["test-pool"]
	assert.True(t, exists)
	assert.Equal(t, 24, defaultPool.v4MaskSize)
	assert.Equal(t, 96, defaultPool.v6MaskSize)
...
func TestOrphanCIDRs(t *testing.T) {
	...
	// all previously allocated CIDRs are now orphaned
	assert.Empty(t, p.AllocatedPools(node1.Name))
	assert.Empty(t, p.AllocatedPools(node2.Name))

	assert.Equal(t, map[string]cidrSets{
		node1.Name: {
			v4: cidrSet{netip.MustParsePrefix("10.100.0.0/24"): {}},
			v6: cidrSet{netip.MustParsePrefix("fd00:100::/96"): {}},
		},
		node2.Name: {
			v4: cidrSet{netip.MustParsePrefix("10.100.1.0/24"): {}},
			v6: cidrSet{netip.MustParsePrefix("fd00:100::1:0:0/96"): {}},
		},
	}, p.orphans)
