...
func TestPoolAllocator_getRemainingIPs(t *testing.T) {
	...
	p = NewPoolAllocator(hivetest.Logger(t), mockMetrics)
	...
	assert.True(t, len(mockMetrics.calls) >= 2, "Should have called metrics for both IPv4 and IPv6")
	...
	if ipv4Call != nil {
		...
		assert.True(t, ipv4Call.remaining.Cmp(big.NewInt(0)) >= 0, "IPv4 remaining should be non-negative")
	}
	if ipv6Call != nil {
		assert.Equal(t, "test-pool", ipv6Call.poolName)
		assert.Equal(t, "ipv6", ipv6Call.family)
		assert.True(t, ipv6Call.remaining.Cmp(big.NewInt(0)) >= 0, "IPv6 remaining should be non-negative")
	}
...
func TestPoolAllocator_PrometheusIntegration(t *testing.T) {
	...
	p := NewPoolAllocator(hivetest.Logger(t), metricsAPI)
...
func (p *prometheusMetricsAPI) SetRemainingIPs(poolName, family string, remaining *big.Int) {
	p.gauge.WithLabelValues(poolName, family).Set(float64(remaining.Int64()))
}
