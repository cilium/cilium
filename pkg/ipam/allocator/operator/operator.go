...
func (a *AllocatorOperator) Init() error {
	if len(option.Config.IPAMClusterPoolV4CIDR) != 0 {
		if !option.Config.EnableIPv4 {
			return errors.New("IPv4CIDR can not be set if IPv4 is not enabled")
		}
		v4Allocators, err := newCIDRSets(false, option.Config.IPAMClusterPoolV4CIDR, operatorOption.Config.NodeCIDRMaskSizeIPv4)
		if err != nil {
			return fmt.Errorf("unable to initialize IPv4 allocator %w", err)
		}
		a.v4CIDRSet = v4Allocators
	}
	if len(option.Config.IPAMClusterPoolV6CIDR) != 0 {
		if !option.Config.EnableIPv6 {
			return errors.New("IPv6CIDR can not be set if IPv6 is not enabled")
		}
		v6Allocators, err := newCIDRSets(true, option.Config.IPAMClusterPoolV6CIDR, operatorOption.Config.NodeCIDRMaskSizeIPv6)
		if err != nil {
	...
	if len(a.v4CIDRSet)+len(a.v6CIDRSet) == 0 {
		return fmt.Errorf("either '%s' or '%s' need to be set", option.IPAMClusterPoolV4CIDR, option.IPAMClusterPoolV6CIDR)
	}
	return nil
}
