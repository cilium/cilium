...
func reserveLocalRoutes(ipam *Config) {
	...
	if err != nil || link == nil {
		log.WithError(err).Warning("Unable to find net_device cilium_host")
		return
	}

	routes, err := netlink.RouteList(nil, netlink.FAMILY_V4)
	if err != nil {
		log.WithError(err).Warningf("Unable to retrieve local routes")
		return
	}
	...
	for _, r := range routes {
		...
		if r.LinkIndex == link.Attrs().Index {
			log.WithFields(log.Fields{
				"route":  r,
				"reason": "points to cilium_host",
			}).Debug("Ignoring route")
			continue
		}
		...
		log.WithField("route", r).Debug("Considering route")
