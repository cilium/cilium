...
func TestManager_handlePoolEvent_UpsertAndDelete(t *testing.T) {
	...
	require.ErrorIs(t, err, &ManagerPoolsNotSyncedError{})
...
func TestNamespaceSelector(t *testing.T) {
	...
	t.Run("match by namespace only", func(t *testing.T) {
		manager.compiledPools = make(map[string]compiledPool)
		manager.setCompiledPool(compiledPool{
			name:              "dev-pool",
			namespaceSelector: selectorFromLabels(map[string]string{"env": "dev"}),
			hasV4:             true,
		})

		pool, err := manager.GetIPPoolForPod("dev/pod", ipam.IPv4)
		require.NoError(t, err)
		assert.Equal(t, "dev-pool", pool)

		pool, err = manager.GetIPPoolForPod("prod/pod", ipam.IPv4)
		require.NoError(t, err)
	...
	t.Run("require-pool-match on namespace blocks fallback", func(t *testing.T) {
		...
		_, err := manager.GetIPPoolForPod("restricted/pod", ipam.IPv4)
		require.Error(t, err)
