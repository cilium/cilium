...
func newNodeStore(nodeName string) *nodeStore {
	...
	for {
		if store.hasMinimumIPsInPool(minimumIPs) {
			break
		}

		log.WithField(fieldName, nodeName).Info("Waiting for initial IP to become available")
...
func (n *nodeStore) hasMinimumIPsInPool(required int) bool {
	n.mutex.RLock()
...
func (n *nodeStore) refreshNode() error {
	...
	case k8sCapabilities.UpdateStatus:
		_, err = ciliumClient.CiliumV2().CiliumNodes(k8s.DefaultNamespace).UpdateStatus(node)
	default:
		_, err = ciliumClient.CiliumV2().CiliumNodes(k8s.DefaultNamespace).Update(node)
	}
...
func (a *crdAllocator) buildAllocationResult(ip net.IP) (result *AllocationResult, err error) {
...
func (a *crdAllocator) Allocate(ip net.IP, owner string) (*AllocationResult, error) {
	...
	return a.buildAllocationResult(ip)
}
