...
func (r *IPAMInitializer) setDefaultPrefix(device string, localNode *node.LocalNode) {
	if r.daemonConfig.EnableIPv4 {
		ip, err := node.FirstGlobalV4Addr(device, localNode.GetCiliumInternalIPv4())
		if err != nil {
			return
		}
	...
	if r.daemonConfig.EnableIPv6 {
		...
		if localNode.GetNodeIP(true) == nil {
			// Find a IPv6 node address first
			addr, _ := node.FirstGlobalV6Addr(device, localNode.GetCiliumInternalIPv6())
			if addr == nil {
...
func (r *IPAMInitializer) ValidatePostInit(ctx context.Context) error {
	...
	if r.daemonConfig.EnableIPv4 && ln.GetCiliumInternalIPv4() == nil {
