...
func (r *IPAMInitializer) setDefaultPrefix(device string, localNode *node.LocalNode) {
	if r.daemonConfig.EnableIPv4 {
		ip, err := node.FirstGlobalV4Addr(device, localNode.GetCiliumInternalIPv4().AsSlice())
		if err != nil {
			return
		}

		if !localNode.GetNodeIP(false).IsValid() {
			localNode.SetNodeInternalIP(iputil.AddrFromIP(ip))
		}
	...
	if r.daemonConfig.EnableIPv6 {
		...
		if !localNode.GetNodeIP(true).IsValid() {
			// Find a IPv6 node address first
			firstAddr, _ := node.FirstGlobalV6Addr(device, localNode.GetCiliumInternalIPv6().AsSlice())
			addr := iputil.AddrFromIP(firstAddr)
			if !addr.IsValid() {
		...
		if !ipv6range.IsValid() {
			...
			if ipv4range.IsValid() {
				...
				nodeIP := localNode.GetNodeIP(true)
				if !nodeIP.IsValid() {
