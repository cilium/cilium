...
func (n *NodeManager) Upsert(resource *v2.CiliumNode) {
	...
	if !ok {
		...
		poolMaintainer, err := trigger.NewTrigger(trigger.Parameters{
			...
			TriggerFunc: func(reasons []string) {
				if err := node.MaintainIPPool(ctx); err != nil {
					node.logger().Warn("Unable to maintain ip pool of node", logfields.Error, err)
					backoff.Wait(ctx)
				}
			},
			ShutdownFunc: cancel,
		...
		if err != nil {
			node.logger().Error("Unable to create pool-maintainer trigger", logfields.Error, err)
			return
		}
		...
		if err != nil {
			node.logger().Error("Unable to create pool-maintainer-retry trigger", logfields.Error, err)
			return
		}
		node.retry = retry
		...
		if err != nil {
			...
			node.logger().Error("Unable to create k8s-sync trigger", logfields.Error, err)
			return
		}
		...
		if err != nil {
			poolMaintainer.Shutdown()
			k8sSync.Shutdown()
			node.logger().Error("Unable to create instance-sync trigger", logfields.Error, err)
			return
		}
		...
		log.Info("Discovered new CiliumNode custom resource", fieldName, resource.Name)
	}
