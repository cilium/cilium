...
const (
	...
	MIN_FILE_NAME_LEN int    = 9 // format: <ns>-<pod>:<ip>
)
...
func cleanStaleIpPinnings(ipam *IPAM) error {
	...
	if err != nil {
		log.Errorf("Read %s failed: %s, skip cleaning", IPAM_STATE_DIR, err)
		return err
	}
...
func getPinnedIP(owner string) (*AllocationResult, error) {
	...
	for _, f := range files {
		...
		if len(filename) < MIN_FILE_NAME_LEN {
