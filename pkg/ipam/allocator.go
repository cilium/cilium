...
func (ipam *IPAM) allocateNextFamily(family Family, allocator Allocator, owner string) (result *AllocationResult, err error) {
	if EnableFixedIP && owner != "loopback" && owner != "health" {
		...
		// But the determine step involves an further call to K8S apiserver.
		...
		if !ipam.blacklist.Contains(result.IP) {
			...
			if EnableFixedIP && owner != "loopback" && owner != "health" {
				...
				if isSts, err = isStsPod(owner); err != nil {
					log.Errorf("Determine whether %s is a sts pod failed: %s"+
						", skip pinning this IP", owner, err)
					return
				}
				...
				if isSts {
					...
					log.Infof("IP %v pinned to %s", owner)
				} else {
