...
func (n *Node) loggerLocked() (logger *slog.Logger) {
	...
	if n != nil {
		logger = logger.With(fieldName, n.name)
		if n.resource != nil {
			logger = logger.With("instanceID", n.resource.InstanceID())
		}
	}
	return
}
...
func (n *Node) recalculate() {
	...
	scopedLog.Debug("Recalculated needed addresses",
...
func (n *Node) createInterface(ctx context.Context, a *AllocationAction) (created bool, err error) {
	...
	if err != nil {
		...
		scopedLog.Warn("Unable to create interface on instance", logfields.Error, err)
		return false, err
	}
...
func (n *Node) determineMaintenanceAction() (*maintenanceAction, error) {
	...
	if err != nil {
		if n.logLimiter.Allow() {
			scopedLog.Warn("Unable to compute pending pods, will not surge-allocate", logfields.Error, err)
		}
...
func (n *Node) handleIPRelease(ctx context.Context, a *maintenanceAction) (instanceMutated bool, err error) {
	...
	for _, ip := range ipsToMark {
		scopedLog.Debug("Marking IP for release", logfields.IPAddr, ip)
	...
	if len(ipsToRelease) > 0 {
		...
		scopedLog.Warn("Unable to unassign IPs from interface",
			"selectedInterface", a.release.InterfaceID,
...
func (n *Node) handleIPAllocation(ctx context.Context, a *maintenanceAction) (instanceMutated bool, err error) {
	...
	if a.allocation.IPv4.AvailableForAllocation > 0 {
		...
		scopedLog.Warn("Unable to assign additional IPs to interface, will create new interface",
			"selectedInterface", a.allocation.InterfaceID,
...
func (n *Node) syncToAPIServer() error {
	...
	for retry := 0; retry < maxRetries; retry++ {
		...
		if err == nil {
			...
			scopedLog.Info("Failed to update CiliumNode status, will retry", logfields.Error, err)
		} else {
			scopedLog.Warn("Unable to update CiliumNode status", logfields.Error, err)
			return err
		}
	}
	...
	for retry := 0; retry < maxRetries; retry++ {
		...
		scopedLog.Debug("Updating node in apiserver", "poolSize", len(node.Spec.IPAM.Pool))
		...
		if err == nil {
			...
			scopedLog.Info("Failed to update CiliumNode spec, will retry", logfields.Error, err)
		} else {
			scopedLog.Warn("Unable to update CiliumNode spec", logfields.Error, err)
			return err
		}
	}
