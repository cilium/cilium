...
func (n *Node) getPreAllocate() int {
	...
	if n.resource.Spec.IPAM.PreAllocate != 0 {
		// Apply instance limits to the configured value
		value := n.resource.Spec.IPAM.PreAllocate
		if value > minInstanceLimit {
			n.logger.Load().Warn(
				fmt.Sprintf("pre-allocate (%d) is higher than the instance type limits (%d)",
					value, minInstanceLimit),
			)
	...
	// Apply instance limits to the default value
...
func (n *Node) getMinAllocate() int {
	minInstanceLimit := n.ops.GetMinimumAllocatableIPv4()
	value := n.resource.Spec.IPAM.MinAllocate
