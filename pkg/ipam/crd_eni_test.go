...
func (s *IPAMSuite) TestCiliumNodeENIRulesAndRoutes(c *check.C) {
	...
	eniLinkByMac := linkMap{
		// ENI number 0
		"0a:c0:d6:f1:72:a3": &netlink.Dummy{
			LinkAttrs: netlink.LinkAttrs{
				Index: 2,
			},
		},
		// ENI number 1
		"0a:c5:98:de:c6:5d": &netlink.Dummy{
			LinkAttrs: netlink.LinkAttrs{
				Index: 3,
			},
		},
	...
	for _, tc := range []struct {
		...
		obtainedRules, obtainedRoutes := ciliumNodeENIRulesAndRoutes(node, eniLinkByMac, tc.options)
