...
func (ipam *IPAM) ConfigureAllocator(ctx context.Context) error {
	...
	case ipamOption.IPAMCRD, ipamOption.IPAMAlibabaCloud:
		...
		if ipam.config.IPv6Enabled() {
			allocator := newCRDAllocator(ipam.logger, IPv6, ipam.config, ipam.nodeDiscovery, ipam.localNodeStore, ipam.clientset, ipam.k8sEventReg, ipam.mtuConfig, ipam.sysctl)
		...
		if ipam.config.IPv4Enabled() {
			allocator := newCRDAllocator(ipam.logger, IPv4, ipam.config, ipam.nodeDiscovery, ipam.localNodeStore, ipam.clientset, ipam.k8sEventReg, ipam.mtuConfig, ipam.sysctl)
