...
// observer has determined the native routing CIDR. It returns an error if the
// operator has not reported the VPC CIDR within
// waitForENINativeRoutingCIDRTimeout, or if ctx is cancelled before that.
func waitForENINativeRoutingCIDR(ctx context.Context, logger *slog.Logger, ready <-chan struct{}) error {
	...
	for {
		...
		case <-ready:
			return nil
		case <-deadline:
			return fmt.Errorf("timed out after %s waiting for the ENI VPC CIDR to be reported in CiliumNode status. %s",
				waitForENINativeRoutingCIDRTimeout, operatorHelpMessage)
		case <-ctx.Done():
			return fmt.Errorf("waiting for ENI VPC CIDR to be reported in CiliumNode status: %w", ctx.Err())
		case <-time.After(5 * time.Second):
			logger.Info(
				"Waiting for ENI VPC CIDR to be reported in CiliumNode status",
