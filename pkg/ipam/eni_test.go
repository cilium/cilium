...
func TestDeriveENIVpcCIDRs(t *testing.T) {
	...
	t.Run("returns the secondary CIDR block associations", func(t *testing.T) {
		node := &ciliumv2.CiliumNode{}
		node.Status.ENI.ENIs = map[string]awsTypes.ENI{
			"eni-1": {
				VPC: awsTypes.AwsVPC{
...
func TestAutoDetectENINativeRoutingCIDR(t *testing.T) {
	t.Run("auto-detects VPC CIDR when not configured", func(t *testing.T) {
		...
		conf := &option.DaemonConfig{}
		autoDetectENINativeRoutingCIDR(logger, primaryCIDR, nil, localNodeStore, conf)
	...
	t.Run("accepts a native routing CIDR that is a secondary VPC CIDR", func(t *testing.T) {
		// Regression test for the standard EKS secondary-CIDR layout: pods
		// live in subnets carved out of a secondary CIDR block association,
		// which by construction neither contains nor is contained by the VPC
		// primary CIDR. Validating against the primary alone would call
		// logging.Fatal and crash the agent on startup.
		logger := hivetest.Logger(t)
		localNodeStore := node.NewTestLocalNodeStore(node.LocalNode{})
	...
	t.Run("accepts a native routing CIDR that is a subnet of a secondary VPC CIDR", func(t *testing.T) {
		logger := hivetest.Logger(t)
		localNodeStore := node.NewTestLocalNodeStore(node.LocalNode{})
		...
		secondaryCIDRs := []netip.Prefix{netip.MustParsePrefix("100.64.0.0/16")}
		conf := &option.DaemonConfig{
			IPv4NativeRoutingCIDR: cidr.MustParseCIDR("100.64.32.0/19"),
		}
		autoDetectENINativeRoutingCIDR(logger, primaryCIDR, secondaryCIDRs, localNodeStore, conf)
