...
func TestDeriveVpcCIDRsFromENI(t *testing.T) {
	t.Run("returns VPC CIDRs from first ENI", func(t *testing.T) {
		node := &ciliumv2.CiliumNode{}
		node.Status.ENI.ENIs = map[string]awsTypes.ENI{
			"eni-1": {
				VPC: awsTypes.AwsVPC{
...
func TestAutoDetectENINativeRoutingCIDR(t *testing.T) {
	...
	t.Run("accepts a native routing CIDR that overlaps a secondary VPC CIDR", func(t *testing.T) {
		logger := hivetest.Logger(t)
		localNodeStore := node.NewTestLocalNodeStore(node.LocalNode{})
