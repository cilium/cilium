...
// NewSemaphoredMutex returns a new SemaphoredMutex that will allow up to n
// readers depending on the 'readers' argument.
func NewSemaphoredMutex(readers int64) *SemaphoredMutex {
	return &SemaphoredMutex{
		readers:  readers,
		Weighted: semaphore.NewWeighted(readers),
	}
}

func (i *SemaphoredMutex) Lock() {
	i.Acquire(context.Background(), i.readers)
}
