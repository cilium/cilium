...
const (
	...
	// defaultCAUnixAddr is the default Unix socket address for the CA server.
	defaultCAUnixAddr = "/var/run/cilium/ca.sock"
)
...
func newServer(log *slog.Logger, epManager endpointmanager.EndpointManager, caUnixAddr string) *Server {
	return &Server{
		log:        log,
		epManager:  epManager,
		caUnixAddr: caUnixAddr,
	}
}
...
func (s *Server) Serve() error {
	...
	if err = os.Remove(s.caUnixAddr); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("failed to remove existing unix socket %s: %w", s.caUnixAddr, err)
	}

	s.l, err = net.Listen("unix", s.caUnixAddr)
	if err != nil {
		return fmt.Errorf("failed to listen on unix socket %s: %w", s.caUnixAddr, err)
	}

	s.log.Info("zTunnel CA server started", "socket", s.caUnixAddr)
	go func() {
