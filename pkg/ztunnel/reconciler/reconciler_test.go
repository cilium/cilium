...
type MockEndpointEnroller struct {
	...
	enrollErr             error
	disenrollErr          error
	initialSnapshotErr    error
	initialSnapshotCalled bool
}
...
// InitialSnapshot sends an initial snapshot of endpoints to ztunnel.
func (m *MockEndpointEnroller) InitialSnapshot(endpoints ...*endpoint.Endpoint) error {
	m.initialSnapshotCalled = true
	if m.initialSnapshotErr != nil {
		return m.initialSnapshotErr
	}
...
func TestEnrollmentReconciler_Update(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			endpoints: []*endpoint.Endpoint{
				createTestEndpoint(1, "test-ns", "pod-1", "/var/run/netns/test1"),
				createTestEndpoint(2, "test-ns", "ztunnel-abc", "/var/run/netns/test2"),
			},
			expectedEnrolled: 1,
			expectedError:    false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			db, tbl, mockEpMgr, mockEnroller, ops := setupTest(t)
			...
			err := ops.Update(context.Background(), db.WriteTxn(tbl), 1, ns)
			if tt.expectedError {
				assert.Error(t, err)
			} else {
			...
			assert.Equal(t, tt.expectedEnrolled, len(mockEnroller.enrolledEndpoints))
		})
	}
}
...
func TestEnrollmentReconciler_Delete(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			endpoints: []*endpoint.Endpoint{
				createTestEndpoint(1, "test-ns", "pod-1", "/var/run/netns/test1"),
				createTestEndpoint(2, "test-ns", "ztunnel-xyz", "/var/run/netns/test2"),
			},
			expectedDisenrolled: 1,
			expectedError:       false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedError {
				...
				assert.NoError(t, err)
				assert.Equal(t, tt.expectedDisenrolled, len(mockEnroller.disenrolledIDs))
			}
		})
	}
}
...
func TestEnrollmentReconciler_EndpointCreated(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			endpoint:         createTestEndpoint(1, "test-ns", "ztunnel-abc", "/var/run/netns/test1"),
			enrolledNs:       []string{"test-ns"},
			expectedEnrolled: false,
		},
		{
...
func TestEnrollmentReconciler_EndpointDeleted(t *testing.T) {
	tests := []struct {
		...
		{
			name:                "skip ztunnel endpoint",
			endpoint:            createTestEndpoint(1, "test-ns", "ztunnel-xyz", "/var/run/netns/test1"),
			enrolledNs:          []string{"test-ns"},
			expectedDisenrolled: false,
		},
	}
...
// TestEnrollmentReconciler_InitialSnapshot tests the initial snapshot logic
// during reconciler startup, which sends all eligible endpoints to ztunnel.
func TestEnrollmentReconciler_InitialSnapshot(t *testing.T) {
	tests := []struct {
		name             string
		endpoints        []*endpoint.Endpoint
		enrolledNs       []string
		...
		{
			name: "initial snapshot with enrolled endpoints",
			endpoints: []*endpoint.Endpoint{
				createTestEndpoint(1, "test-ns", "pod-1", "/var/run/netns/test1"),
				createTestEndpoint(2, "test-ns", "pod-2", "/var/run/netns/test2"),
				createTestEndpoint(3, "other-ns", "pod-3", "/var/run/netns/test3"),
			},
			enrolledNs:       []string{"test-ns"},
			expectedEnrolled: 2,
			expectedError:    false,
		...
		{
			name: "skip endpoints without netns",
			endpoints: []*endpoint.Endpoint{
				createTestEndpoint(1, "test-ns", "pod-1", "/var/run/netns/test1"),
				createTestEndpoint(2, "test-ns", "pod-2", ""),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockEnroller := NewMockEndpointEnroller()
			mockEnroller.initialSnapshotErr = tt.snapshotErr

			// Create a promise that resolves immediately with a mock restorer.
			restorer, restorerPromise := promise.New[endpointstate.Restorer]()
			restorer.Resolve(&MockRestorer{})

			// Initialize table and add enrolled namespaces BEFORE creating reconciler.
			// This ensures the reconciler's Start hook doesn't block waiting for initialization.
			txn := db.WriteTxn(tbl)
			complete := tbl.RegisterInitializer(txn, "test")
			for _, ns := range tt.enrolledNs {
				tbl.Insert(txn, &table.EnrolledNamespace{
			...
			if tt.expectedError {
				// For error cases, manually create and start the reconciler without hivetest.Lifecycle.
				// hivetest.Lifecycle calls tb.Fatal on start hook errors, which would fail the test.
				reconcilerOps := &EnrollmentReconciler{
					logger:                 logger,
					db:                     db,
					enrolledNamespaceTable: tbl,
					endpointManager:        mockEpMgr,
				...
				err := reconcilerOps.Start(ctx)

				assert.Error(t, err)
				assert.Contains(t, err.Error(), "snapshot failed")
				assert.True(t, mockEnroller.initialSnapshotCalled)
			} else {
				lc := hivetest.Lifecycle(t)

				// Create the reconciler - hivetest.Lifecycle.Append will call Start() immediately.
				ops := NewEnrollmentReconciler(params{
					DB:                     db,
					EnrolledNamespaceTable: tbl,
					Logger:                 logger,
				...
				select {
				case <-reconcilerOps.initialized:
					// Initialization completed successfully.
				case <-time.After(5 * time.Second):
					t.Fatal("Timeout waiting for reconciler initialization")
				}
