...
func (x *Server) CreateCertificate(ctx context.Context, csr *v1alpha1.IstioCertificateRequest) (*v1alpha1.IstioCertificateResponse, error) {
	...
	id := path.Base(uri.Path)
	secID, err := strconv.Atoi(id)
	if err != nil {
		return nil, fmt.Errorf("failed to parse security ID from URI: %w", err)
	}

	// we must confirm at least one endpoint with the provided security label
	// in the CSR exists on the node.
	//
	// this is a security measure ensuring we do not issue certificates for
	// pods that are not available on the host.
	eps := x.epLookup.GetEndpointsBySecurityIdentifier(identity.NumericIdentity(secID))
	if len(eps) == 0 {
		return nil, fmt.Errorf("no endpoints found for security ID %d", secID)
	}
	...
	x.log.Debug("created certificate for security ID", logfields.Identity, secID)
