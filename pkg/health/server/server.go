...
type Server struct {
	...
	*ciliumPkg.Client // Client to "GET /healthz" on cilium daemon
	...
	tcpServers []*probeserver.Server // Servers for external pings
	startTime  time.Time
...
// Actively probing other hosts and endpoints over ICMP and HTTP,
...
func NewServer(config Config) (*Server, error) {
	...
	for port := range PortToPaths {
		srv := probeserver.NewServer(port)
