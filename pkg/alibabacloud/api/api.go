...
func (c *Client) describeNetworkInterfacesFromInstances(ctx context.Context) ([]ecs.NetworkInterfaceSet, error) {
	...
	instanceIds := []string{}
	instanceIdMap := make(map[string]struct{})
	...
	for _, t := range tagResouces {
		if _, exists := instanceIdMap[t.ResourceId]; !exists {
			instanceIdMap[t.ResourceId] = struct{}{}
		}
	}
	...
	for i := 0; i < len(interfaceIds); i += 100 {
		idx := i
		endIdx := idx + 100
		if len(interfaceIds) < endIdx {
			endIdx = len(interfaceIds)
		}
		g.Go(func() error {
			req := ecs.CreateDescribeNetworkInterfacesRequest()
	...
	result := []ecs.NetworkInterfaceSet{}
