...
import (
	...
	zdsapi "github.com/cilium/cilium/pkg/zds/proto"
)
...
func (zc *ztunnelConn) sendMsg(req *zdsapi.WorkloadRequest, fd *int) error {
	var rights []byte
	if fd != nil {
...
func (zc *ztunnelConn) sendAndWaitForAck(req *zdsapi.WorkloadRequest, fd *int) error {
	if err := zc.sendMsg(req, fd); err != nil {
		return err
	}
...
type Config struct {
	UnixAddr string `mapstructure:"ztunnel-zds-unix-addr"`
}

var defaultConfig = Config{
	UnixAddr: defaultZtunnelUnixAddress,
}

func (def Config) Flags(flags *pflag.FlagSet) {
	flags.String("ztunnel-zds-unix-addr", def.UnixAddr, "Unix address for zds server")
}
...
func newZDSServer(p serverParams) (*Server, error) {
	...
	p.Lifecycle.Append(cell.Hook{
		OnStart: func(hc cell.HookContext) error {
			err := os.RemoveAll(p.Config.UnixAddr)
			if err != nil {
				return err
			}

			resolvedAddr, err := net.ResolveUnixAddr("unixpacket", p.Config.UnixAddr)
			if err != nil {
...
func (s *Server) handleConn(ctx context.Context, zc *ztunnelConn) error {
	...
	for {
		...
		case update := <-s.updates:
			if err := zc.sendAndWaitForAck(update.request, update.fd); err != nil {
...
func (s *Server) sendSnapshot(zc *ztunnelConn) error {
	...
	for id, ep := range s.endpointCache {
		...
		if err := zc.sendAndWaitForAck(req, &fd); err != nil {
