...
func (elf *ELF) copy(w io.Writer, r io.ReadSeeker, intOptions map[string]uint32, strOptions map[string]string) error {
	...
	for _, symbol := range elf.symbols.sort() {
		...
		case symbolUint32:
			...
			if exists {
				...
				elf.ByteOrder.PutUint32(value, v)
			}
		case symbolString:
		...
		if err := binary.Write(w, elf.ByteOrder, value); err != nil {
...
func (elf *ELF) Write(path string, intOptions map[string]uint32, strOptions map[string]string) error {
	...
	err = elf.copy(f, elf.file, intOptions, strOptions)
	if err == nil {
		err = f.Sync()
	}
	if _, err2 := elf.file.Seek(0, io.SeekStart); err2 != nil {
		log.WithField(
			logfields.Path, elf.path,
	...
	log.WithField(logfields.Path, path).WithError(err).Debugf("Finished copying ELF from %s", elf.path)
	return err
}
