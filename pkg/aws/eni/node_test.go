...
func (e *ENISuite) TestGetMaximumAllocatableIPv6(c *check.C) {
	n := &Node{}

	// With no k8sObj defined, it should return 0
	c.Assert(n.GetMaximumAllocatableIPv6(), check.Equals, 0)
	...
	n.k8sObj = newCiliumNode("node", withInstanceType("m5.large"), withFirstInterfaceIndex(0))
	c.Assert(n.GetMaximumAllocatableIPv6(), check.Equals, 1728)
	...
	n.k8sObj = newCiliumNode("node", withInstanceType("m5.large"), withFirstInterfaceIndex(1))
	c.Assert(n.GetMaximumAllocatableIPv6(), check.Equals, 1152)
...
// TestGetUsedIPWithPrefixes tests the logic computing used IPv4 addresses
...
// on a node when prefix delegation is enabled.
/*func (e *ENISuite) TestGetUsedIPv6WithPrefixes(c *check.C) {
	cn := newCiliumNode("node1", withInstanceType("m5a.large"))
	n := &Node{k8sObj: cn}
	eniName := "eni-1"
	...
	c.Assert(n.GetUsedIPv6WithPrefixes(), check.Equals, 32)
}*/
