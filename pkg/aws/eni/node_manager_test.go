...
func TestGetNodeNames(t *testing.T) {
	...
	instances, err := NewInstancesManager(hivetest.Logger(t), ec2api, metadataMockapi, false)
...
func TestNodeManagerManyNodes(t *testing.T) {
	...
	instancesManager, err := NewInstancesManager(hivetest.Logger(t), ec2api, metadataMockapi, false)
	require.NoError(t, err)
...
func benchmarkAllocWorker(b *testing.B, workers int64, delay time.Duration, rateLimit float64, burst int) {
	...
	instances, err := NewInstancesManager(hivetest.Logger(b), ec2api, metadataMockapi, false)
