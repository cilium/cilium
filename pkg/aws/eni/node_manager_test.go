...
func newCiliumNode(node, instanceID, instanceType, az, vpcID string, preAllocate, minAllocate, available, used int) *v2.CiliumNode {
	cn := &v2.CiliumNode{
		...
		Status: v2.NodeStatus{
			IPAM: v2.IPAMStatus{
				InUse: map[string]v2.AllocationIP{},
			},
		},
	}
...
func updateCiliumNode(cn *v2.CiliumNode, available, used int) *v2.CiliumNode {
	cn.Spec.IPAM.Available = map[string]v2.AllocationIP{}
	for i := 0; i < used; i++ {
		cn.Spec.IPAM.Available[fmt.Sprintf("1.1.1.%d", i)] = v2.AllocationIP{Resource: "foo"}
	}

	cn.Status.IPAM.InUse = map[string]v2.AllocationIP{}
	for ip, ipAllocation := range cn.Spec.IPAM.Available {
		if used > 0 {
			delete(cn.Spec.IPAM.Available, ip)
			cn.Status.IPAM.InUse[ip] = ipAllocation
			used--
		}
	}
