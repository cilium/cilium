...
func (e *ENISuite) TestNodeManagerManyNodes(c *check.C) {
	...
	subnets := []*types.Subnet{
		&types.Subnet{ID: "s-1", AvailabilityZone: "us-west-1", VpcID: "vpc-1", AvailableAddresses: 400},
		&types.Subnet{ID: "s-2", AvailabilityZone: "us-west-1", VpcID: "vpc-1", AvailableAddresses: 400},
		&types.Subnet{ID: "s-3", AvailabilityZone: "us-west-1", VpcID: "vpc-1", AvailableAddresses: 400},
	}
...
func (e *ENISuite) TestNodeManagerInstanceNotRunning(c *check.C) {
	...
	mngr, err := NewNodeManager(ec2api, ec2api, k8sapi, metricsMock, 10)
	c.Assert(err, check.IsNil)
	c.Assert(mngr, check.Not(check.IsNil))
