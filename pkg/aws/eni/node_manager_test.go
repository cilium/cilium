...
func TestGetNodeNames(t *testing.T) {
	...
	instances, err := NewInstancesManager(hivetest.Logger(t), ec2api, metadataMockapi, true)
...
func TestNodeManagerManyNodes(t *testing.T) {
	...
	instancesManager, err := NewInstancesManager(hivetest.Logger(t), ec2api, metadataMockapi, true)
	require.NoError(t, err)
...
func benchmarkAllocWorker(b *testing.B, workers int64, delay time.Duration, rateLimit float64, burst int) {
	...
	instances, err := NewInstancesManager(hivetest.Logger(b), ec2api, metadataMockapi, true)
