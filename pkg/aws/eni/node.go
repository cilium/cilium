...
func (n *Node) loggerLocked() *logrus.Entry {
	logger := log.WithField("name", n.name)
...
func (n *Node) updatedResource(resource *v2.CiliumNode) bool {
	...
	if allocationNeeded {
		n.logger().Infof("Triggering deficit")
...
func (n *Node) recalculateLocked() bool {
	...
	n.stats.usedIPs = len(n.resource.Status.IPAM.InUse)
...
func (n *Node) determineAllocationAction() (*allocatableResources, error) {
	...
	for _, e := range n.enis {
		scopedLog.Debugf("Considering ENI %s for allocation, need index %d >= %d, addresses %d < %d",
			e.ID, e.Number, n.resource.Spec.ENI.FirstInterfaceIndex, len(e.Addresses), a.limits.IPv4)
		...
		scopedLog.Debugf("ENI has %d IPs available, max allocate is %d", availableOnENI, maxAllocate)
		maxAllocateOnENI := math.IntMin(availableOnENI, maxAllocate)

		if subnet := n.manager.instancesAPI.GetSubnet(e.Subnet.ID); subnet != nil {
			scopedLog.Debugf("Subnet %s has %d IPs available", e.Subnet.ID, subnet.AvailableAddresses)
...
func (n *Node) prepareENICreation(a *allocatableResources) (*types.Subnet, error) {
	...
	if a.remainingInterfaces == 0 {
		...
		if time.Since(n.lastMaxAdapterWarning) > warningInterval {
			log.Warningf("Instance %s is out of ENIs %d/%d",
				n.resource.Spec.ENI.InstanceID, a.totalENIs, a.limits.Adapters)
...
func (n *Node) resolveIPDeficit() error {
	...
	if a.subnet != nil {
		...
		scopedLog.WithError(err).Warningf("Unable to assign %d additional private IPs to ENI %s, will create new ENI",
			a.availableOnSubnet, a.eni.ID)
	}
...
func (n *Node) SyncToAPIServer() (err error) {
	...
	scopedLog := n.logger()
	scopedLog.Debugf("Refreshing node")
	...
	for retry := 0; retry < 2; retry++ {
		...
		scopedLog.Debugf("Updating status of node in apiserver %d ENIs %d used IPs",
			len(node.Status.ENI.ENIs), len(node.Status.IPAM.InUse))
	...
	if err != nil {
		scopedLog.WithError(err).Warningf("Unable to update CiliumNode %s status", origNode.Name)
		return err
	}
	...
	for retry := 0; retry < 2; retry++ {
		...
		for k, allocationIP := range n.available {
			node.Spec.IPAM.Available[k] = allocationIP
		}

		scopedLog.Debugf("Updating node in apiserver %d available IPs", len(node.Spec.IPAM.Available))
	...
	if err != nil {
		scopedLog.WithError(err).Warningf("Unable to update CiliumNode %s spec", origNode.Name)
	}
