...
func (n *Node) logSubnetRouteTableMismatch(subnet *ipamTypes.Subnet, matchType string) {
	...
	n.loggerLocked().WithField("subnet-id", subnet.ID).
		Warnf("%s subnet %s is not in the same route table as the node's subnet %s, this may cause traffic to be routed incorrectly",
			matchType, subnet.ID, n.k8sObj.Spec.ENI.NodeSubnetID)
}
