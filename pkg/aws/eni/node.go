...
func (n *Node) SyncToApiServer() (err error) {
	...
	for retry := 0; retry < 2; retry++ {
		...
		scopedLog.Debugf("Updating node in apiserver %d ENIs %d available IPs %d used IPs",
			len(node.Status.ENI.ENIs), len(node.Spec.IPAM.Available), len(node.Status.IPAM.InUse))

		node, err = n.manager.k8sAPI.Update(node, origNode)
		if err == nil || node == nil {
			break
		}
	}
	...
	if err != nil {
		scopedLog.WithError(err).Warningf("Unable to update CiliumNode %s", origNode.Name)
	}
