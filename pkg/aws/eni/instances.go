...
func (m *InstancesManager) resync(ctx context.Context, instanceID string) time.Time {
	...
	if instanceID == "" {
		...
		m.logger.Info(
			...
			logfields.NumVPCs, len(m.vpcs),
			logfields.NumSubnets, len(m.subnets),
			logfields.NumRouteTables, len(m.routeTables),
			logfields.NumSecurityGroups, len(m.securityGroups),
		)
		m.mutex.Lock()
		defer m.mutex.Unlock()
		m.instances = instances
	} else {
		instance, err := m.ec2api.GetInstance(ctx, m.vpcs, m.subnets, instanceID)
		if err != nil {
			m.logger.Warn("Unable to synchronize EC2 interface list", logfields.Error, err)
			return time.Time{}
		}
