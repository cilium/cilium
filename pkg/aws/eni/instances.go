...
func (m *InstancesManager) resync(ctx context.Context, instanceID string) time.Time {
	...
	if err != nil {
		log.Warn("Unable to synchronize EC2 VPC list", slog.Any(logfields.Error, err))
		return time.Time{}
	}

	subnets, err := m.api.GetSubnets(ctx)
	if err != nil {
		log.Warn("Unable to retrieve EC2 subnets list", slog.Any(logfields.Error, err))
		return time.Time{}
	}

	securityGroups, err := m.api.GetSecurityGroups(ctx)
	if err != nil {
		log.Warn("Unable to retrieve EC2 security group list", slog.Any(logfields.Error, err))
		return time.Time{}
	}
	...
	if instanceID == "" {
		...
		if err != nil {
			log.Warn("Unable to synchronize EC2 interface list", slog.Any(logfields.Error, err))
			return time.Time{}
		}
	...
	if operatorOption.Config.UpdateEC2AdapterLimitViaAPI {
		if err := limits.UpdateFromEC2API(ctx, m.api); err != nil {
			log.Warn("Unable to update instance type to adapter limits from EC2 API", slog.Any(logfields.Error, err))
			return time.Time{}
		}
	}
