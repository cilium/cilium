...
func (c config) Flags(flags *pflag.FlagSet) {
	...
	flags.String("identity-seed-salt", c.IdentitySeedSalt, "Shared secret to make identities less predictable")
	flags.MarkHidden("identity-seed-salt")
}
...
func newIdentityAllocator(params identityAllocatorParams) identityAllocatorOut {
	...
	if option.NetworkPolicyEnabled(option.Config) {
		...
		allocatorConfig := cache.AllocatorConfig{
			...
			IdentitySeedSalt:         []byte(params.Config.IdentitySeedSalt),
		}
