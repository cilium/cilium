...
func GetMinimalAllocationIdentity(clusterID uint32) NumericIdentity {
	if clusterID > 0 {
		...
		return NumericIdentity((1 << cmtypes.DefaultClusterInfo.GetClusterIDShift()) * clusterID)
	}
...
func GetMaximumAllocationIdentity(clusterID uint32) NumericIdentity {
	return NumericIdentity((1<<cmtypes.DefaultClusterInfo.GetClusterIDShift())*(clusterID+1) - 1)
}
...
func (id NumericIdentity) ClusterID() uint32 {
	return (uint32(id) >> uint32(cmtypes.DefaultClusterInfo.GetClusterIDShift())) & cmtypes.ClusterIDMax
}
