...
const (
	...
	vtepIP1 = "1.2.3.4"
	rmac1   = "00:11:22:33:44:55"

	vtepIP2 = "1.2.3.5"
	rmac2   = "00:11:22:33:44:56"
...
func TestVtepPolicyCVPParser(t *testing.T) {
...
func TestVtepPolicyManager(t *testing.T) {
...
func parseVtepRule(sourceIP, destCIDR, vtepIP, rmac string) parsedVtepRule {
	sip := net.ParseIP(sourceIP)
	dc := cidr.MustParseCIDR(destCIDR)
	vip := net.ParseIP(vtepIP)
	mac, _ := mac.ParseMAC(rmac)
...
func tryAssertVtepRules(policyMap *vtep_policy.VtepPolicyMap, rules []vtepRule) error {
	...
	for _, r := range rules {
		parsedRules = append(parsedRules, parseVtepRule(r.sourceIP, r.destCIDR, r.vtepIP, r.rmac))
	}
	...
	for _, r := range parsedRules {
		...
		if !val.Match(r.vtepIP, r.rmac) {
			return fmt.Errorf("mismatched val, wanted: %s %s, got: %s", r.vtepIP, r.rmac, val)
		}
	}
	...
	policyMap.IterateWithCallback(
		func(key *vtep_policy.VtepPolicyKey, val *vtep_policy.VtepPolicyVal) {
			for _, r := range parsedRules {
				if key.Match(r.sourceIP, r.destCIDR) && val.Match(r.vtepIP, r.rmac) {
					return
				}
			}
