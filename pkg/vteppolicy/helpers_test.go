...
func newCVP(params *policyParams) (*v2alpha1.CiliumVtepPolicy, *PolicyConfig) {
	parsedDestinationCIDRs := make([]*cidr.CIDR, 0, len(params.destinationCIDRs))
	for _, destCIDR := range params.destinationCIDRs {
		parsedDestinationCIDR, _ := cidr.ParseCIDR(destCIDR)
		parsedDestinationCIDRs = append(parsedDestinationCIDRs, parsedDestinationCIDR)
	}

	parsedVtepIp := net.ParseIP(params.vtepIP)
	parsedRmac, _ := mac.ParseMAC(params.rmac)
