...
type PolicyConfig struct {
	...
	podSelectors []*policyTypes.LabelSelector
...
func (config *PolicyConfig) matchesPodLabels(podLabels map[string]string) bool {
	labelsToMatch := labels.K8sSet(podLabels)
	for i := range config.podSelectors {
		if policyTypes.Matches(config.podSelectors[i], labelsToMatch) {
			return true
		}
	}
	return false
...
func ParseCVP(cvp *v2alpha1.CiliumVtepPolicy) (*PolicyConfig, error) {
	var podSelectorList []*policyTypes.LabelSelector
	...
	for _, vtepRule := range cvp.Spec.Selectors {
		if vtepRule.NamespaceSelector != nil {
			...
			podSelectorList = append(
				podSelectorList,
				policyTypes.NewLabelSelector(api.NewESFromK8sLabelSelector("", vtepRule.PodSelector)))
		} else {
