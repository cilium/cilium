...
func getAsPolicyLabelSelectors(k8sLss []api.EndpointSelector) (lss []*policyTypes.LabelSelector) {
	for _, ls := range k8sLss {
		lss = append(lss, policyTypes.NewLabelSelector(
			api.NewESFromK8sLabelSelector(labels.LabelSourceK8sKeyPrefix, ls.LabelSelector)),
		)
	}
	return lss
}
...
func TestPolicyConfig_updateMatchedEndpointIDs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			config := &PolicyConfig{
				id:               tt.fields.id,
				podSelectors:     getAsPolicyLabelSelectors(tt.fields.podSelectors),
