...
func (m *LBMockMap) UpsertService(p *datapathTypes.UpsertServiceParams) error {
	...
	if !found {
		frontend := lb.NewL3n4AddrID(lb.NONE, cmtypes.NewIPCluster(p.IP.String(), 0), p.Port, p.Scope, lb.ID(p.ID))
...
func (m *LBMockMap) AddBackend(b *lb.Backend, ipv6 bool) error {
	...
	be := lb.NewBackendWithState(id, b.Protocol, ipCluster, port, b.State, false)
	m.BackendByID[id] = be
...
func (m *LBMockMap) UpdateBackendWithState(b *lb.Backend) error {
	...
	if b.ID != be.ID || b.Port != be.Port || !b.IPCluster.Equal(be.IPCluster) {
