	...
	if !pp.isStatic && pp.nRedirects == 0 && pp.releaseCancel == nil {
		...
		go func() {
			...
			case <-time.After(portReuseWait):
				...
				if pp.nRedirects == 0 {
					pp.releaseCancel = nil
					p.logger.Debug("Delayed release of proxy port %d", fieldProxyRedirectID, name, logfields.ProxyPort, pp.ProxyPort)
					p.reset(pp)
...
func (p *ProxyPorts) FindByTypeWithReference(l7Type types.ProxyType, listener string, ingress bool) (string, *ProxyPort) {
	...
	case types.ProxyTypeCRD:
		...
		p.logger.Debug("findProxyPortByType: can not find crd listener %s from %v", logfields.Listener, listener, logfields.ProxyPort, p.proxyPorts)
		return "", nil
