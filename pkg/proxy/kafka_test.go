...
func (s *proxyTestSuite) SetUpSuite(c *C) {
	Allocator = cache.NewCachingIdentityAllocator(&testutils.IdentityAllocatorOwnerMock{})
...
func (s *proxyTestSuite) TestKafkaRedirect(c *C) {
	...
	// the EP ID by the IP.
	ep := endpoint.NewEndpointWithState(s, &endpoint.FakeEndpointProxy{}, &testutils.FakeIdentityAllocator{}, uint16(localEndpointMock.GetID()), endpoint.StateReady)
