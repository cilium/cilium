...
func findProxyPortByType(l7Type types.ProxyType, listener string, ingress bool) (string, *ProxyPort) {
	portType := l7Type
	switch l7Type {
	case types.ProxyTypeCRD:
		// CRD proxy ports are dynamically created, look up by name
		// 'ingress' is always false for CRD type
		if pp, ok := proxyPorts[listener]; ok && pp.ProxyType == types.ProxyTypeCRD && !pp.Ingress {
			return listener, pp
		}
...
func (p *Proxy) restoreProxyPortsFromFile() error {
	...
	for name, pp := range portsMap {
		if existing := proxyPorts[name]; existing != nil {
...
func GetProxyPort(name string) (port uint16, isStatic bool, err error) {
...
func (p *Proxy) AllocateCRDProxyPort(name string) (uint16, error) {
	...
	if pp.ProxyPort == 0 {
		...
		if pp.rulesPort != 0 && !allocatedPorts[pp.rulesPort] {
...
func (p *Proxy) CreateOrUpdateRedirect(ctx context.Context, l4 policy.ProxyPolicy, id string, localEndpoint endpoint.EndpointUpdater,
	...
	if redir, ok := p.redirects[id]; ok {
		...
		if redir.listener.configured && redir.listener.ProxyType == types.ProxyType(l4.GetL7Parser()) {
	...
	ppName, pp := findProxyPortByType(types.ProxyType(l4.GetL7Parser()), l4.GetListener(), l4.GetIngress())
	if pp == nil {
		err = proxyTypeNotFoundError(types.ProxyType(l4.GetL7Parser()), l4.GetListener(), l4.GetIngress())
		return 0, err, nil, nil
	}
	...
	for nRetry := 0; nRetry < redirectCreationAttempts; nRetry++ {
		...
		if !pp.configured && (nRetry > 0 || pp.ProxyPort == 0) {
			...
			proxyPort, err := allocatePort(pp.ProxyPort, p.rangeMin, p.rangeMax)
			if err != nil {
				return 0, err, nil, nil
			}
		...
		default:
			if pp.ProxyType == types.ProxyTypeCRD {
