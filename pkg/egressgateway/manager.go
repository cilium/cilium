...
func (manager *Manager) addMissingEgressRules() (stale sets.Set[egressmap.EgressPolicyKey4]) {
	...
	// Stale collects all the keys existing in the map, but that should not be
	// present accorging to the current state, so that they can be subsequently
	// garbage collected.
...
func (manager *Manager) removeUnusedEgressRules(stale sets.Set[egressmap.EgressPolicyKey4]) {
...
func (manager *Manager) reconcileLocked() {
	...
	stale := manager.addMissingEgressRules()
	manager.removeUnusedEgressRules(stale)
