...
func TestMultigatewayPolicy(t *testing.T) {
	...
	assignEndpoints := func(endpoints []testEndpoints, gateways []testNodes, ipv4 bool) []egressRule {
		...
		for _, endpoint := range endpoints {
			h := fnv.New32a()
			h.Write([]byte(endpoint.ep.UID))
			gw := gateways[h.Sum32()%uint32(len(gateways))]
	...
	// Remove one endpoint and check that the mapping was updated.
	deleteEndpoint(t, k.endpoints, eps[0].ep)
	reconciliationEventsCount = waitForReconciliationRun(t, egressGatewayManager, reconciliationEventsCount)
	assertEgressRules4(t, policyMap4, assignEndpoints(eps[1:], nodes, true))
	assertEgressRules6(t, policyMap6, assignEndpoints(eps[1:], nodes, false))
	...
	// Remove two gateways to ensure the endpoints are added to the correct gateway.
