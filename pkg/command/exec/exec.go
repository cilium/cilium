...
func warnToLog(cmd *exec.Cmd, out []byte, scopedLog logging.FieldLogger, err error) {
	scopedLog.Error("Command execution failed", slog.Any(logfields.Error, err), slog.Any("cmd", cmd.Args))
...
func combinedOutput(ctx context.Context, cmd *exec.Cmd, scopedLog logging.FieldLogger, verbose bool) ([]byte, error) {
...
func output(ctx context.Context, cmd *exec.Cmd, scopedLog logging.FieldLogger, verbose bool) ([]byte, error) {
	out, err := cmd.Output()
	if ctx.Err() != nil {
		if !errors.Is(ctx.Err(), context.Canceled) {
			scopedLog.Error("Command execution failed", slog.Any(logfields.Error, err), slog.Any("cmd", cmd.Args))
...
func (c *Cmd) CombinedOutput(scopedLog logging.FieldLogger, verbose bool) ([]byte, error) {
...
//
// Logs any errors that occur to the specified logger.
func (c *Cmd) Output(scopedLog logging.FieldLogger, verbose bool) ([]byte, error) {
