...
func (o *orchestrator) reconciler(ctx context.Context, health cell.Health) error {
	localNodeConfig, _, _, err := newLocalNodeConfig(
		option.Config,
		node.LocalNode{},
		o.params.MTU,
		o.params.DB.ReadTxn(),
	...
	// We have to reinitialize the host device before we wait for the daemon config.
	// That is because the daemon startup expects the host device to be initialized
	// before it will populate the config with the values we are looking for.
	if err := o.params.Loader.ReinitializeHostDev(ctx, &localNodeConfig); err != nil {
	...
	if agentConfig.DirectRoutingDeviceRequired() {
		// Wait until the device has been detected and the direct-routing device has been set.
		health.OK("Waiting for direct-routing device")
		for {
			dev, _, changed, ok := o.params.Devices.GetWatch(o.params.DB.ReadTxn(), tables.DeviceNameIndex.Query(option.Config.DirectRoutingDevice))
			if !ok {
				<-changed
				continue
			}
			...
			for _, addr := range dev.Addrs {
				if addr.Addr.Is4() {
					foundV4 = true
					o.params.Log.Debug("Found IPv4 address for direct-routing device", "addr", addr.String())
				} else if addr.Addr.Is6() {
					foundV6 = true
					o.params.Log.Debug("Found IPv6 address for direct-routing device", "addr", addr.String())
				}
			}

			if (!foundV4 && option.Config.EnableIPv4) ||
				(!foundV6 && option.Config.EnableIPv6) {
				<-changed
				continue
			}
			break
		...
		o.params.Log.Debug("Direct routing is disabled, skipping device check")
	}
	...
	for {
		localNodeConfig, devsWatch, addrsWatch, err := newLocalNodeConfig(
			option.Config,
			localNode,
			o.params.MTU,
			o.params.DB.ReadTxn(),
