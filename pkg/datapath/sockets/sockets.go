	...
	case unix.IPPROTO_UDP:
		err := filterAndDestroyUDPSockets(family, func(sock netlink.SocketID, err error) {
			...
			if filter.MatchSocket(sock) {
				d.Logger.Info("", logfields.Socket, sock)
				if err := destroySocket(d.Logger, sock, family, unix.IPPROTO_UDP, 0xffff, true); err != nil {
					errs = errors.Join(errs, fmt.Errorf("destroying UDP socket with filter [%v]: %w", filter, err))
					failed++
					return
				}
				d.Logger.Debug("Destroyed socket", logfields.Socket, sock)
				success++
			}
		})
		if err != nil {
...
func (sd *BPFSocketDestroyer) Destroy(f SocketFilter) error {
	...
	if sd.prog == nil {
		prog, filterMap, err := loader.LoadSockTerm(sd.Logger, "")
		if err != nil {
			return err
		}
		sd.prog = prog
		sd.filterMap = filterMap
	}

	if err := sd.filterMap.SetFilter(f.Family, f.DestIp, f.DestPort); err != nil {
	...
	for err == nil {
		...
		if n != len(cookie) {
			sd.Logger.Warn("Unexpected number of bytes", logfields.Value, n)
			continue
		}

		sd.Logger.Debug("Destroyed socket",
			logfields.Filter, f,
	...
	if count > 0 {
		sd.Logger.Info("Forcefully terminated sockets",
			logfields.Filter, f,
...
func (d *BPFSocketDestroyer) IsSupported() (supported bool, _ error) {
	prog, filterMap, err := loader.LoadSockTerm(d.Logger, "")
