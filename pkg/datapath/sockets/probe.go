...
// and attempts to both list and
// This is sufficient for both ip4 and ip6.
func probeForSockDestroy(logger *slog.Logger, tcp, udp bool) error {
	...
	if tcp {
		tcpSock, port, err := createProbeTCPSocket(context.TODO())
		if err != nil {
			return err
		}
		defer tcpSock.Close()
...
func InetDiagDestroyEnabled(logger *slog.Logger, probeTCP, probeUDP bool) error {
	var err error
	probeOnce.Do(func() {
		err = probeForSockDestroy(logger, probeTCP, probeUDP)
	})
	return err
}
