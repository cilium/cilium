...
func setupTunnelDevice(logger *slog.Logger, sysctl sysctl.Sysctl, mode tunnel.EncapProtocol, port, srcPortLow, srcPortHigh uint16, mtu int, bigtcp *bigtcp.Configuration) error {
	switch mode {
	case tunnel.Geneve:
...
func setupTunnelGSOGRO(logger *slog.Logger, device string, bigtcp *bigtcp.Configuration) error {
	...
	if bigtcp.GetGROIPv6MaxSize() != 0 && bigtcp.GetGSOIPv6MaxSize() != 0 {
		...
		err = bigtcp.setGROGSOIPv6MaxSize(logger, bigtcp, device, bigtcp.GetGROIPv6MaxSize(), bigtcp.GetGSOIPv6MaxSize())
		if err != nil {
	...
	if bigtcp.GetGROIPv4MaxSize() != 0 && bigtcp.GetGSOIPv4MaxSize() != 0 {
		...
		err = bigtcp.setGROGSOIPv4MaxSize(logger, bigtcp, device, bigtcp.GetGROIPv4MaxSize(), bigtcp.GetGSOIPv4MaxSize())
		if err != nil {
