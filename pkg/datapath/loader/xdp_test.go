...
func TestMaybeUnloadObsoleteXDPPrograms(t *testing.T) {
	...
	ns.Do(func() error {
		...
		newTestLoader(t, testLoaderContext()).maybeUnloadObsoleteXDPPrograms(
...
func TestAttachXDPWithExistingLink(t *testing.T) {
	...
	ns.Do(func() error {
		...
		// Detach the program.
		err = newTestLoader(t, testLoaderContext()).DetachXDP(veth, basePath, "test")
		require.NoError(t, err)
...
func TestDetachXDPWithPreviousAttach(t *testing.T) {
	...
	ns.Do(func() error {
		...
		err = newTestLoader(t, testLoaderContext()).DetachXDP(veth, basePath, "foo")
		require.NoError(t, err)
		require.True(t, getLink(t, veth).Attrs().Xdp.Attached)
