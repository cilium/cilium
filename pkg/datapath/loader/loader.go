...
func compileDatapath(ep endpoint, dirs *InfoDirectories, debug bool) error {
	...
	if debug {
		for _, p := range debugProgs {
			if err := compile(p, ctx, dirs, debug); err != nil {
	...
	if err := compile(datapathProg, ctx, dirs, debug); err != nil {
		scopedLog := epLog.WithFields(logrus.Fields{
...
func CompileAndLoad(ep endpoint, cfg config) error {
	if ep == nil {
	...
	epdir := ep.StateDir()
	dirs := &InfoDirectories{Library: libdir, Runtime: rundir, Output: epdir}
