...
func netdevRewrites(cfg *datapath.LocalNodeConfiguration, ep datapath.Endpoint, link netlink.Link) (*config.BPFHost, map[string]string) {
	...
	// In that case, leave the MAC address empty and set the ethernet header
	// length to 0.
	em := ep.GetNodeMAC()
	if em != nil {
	...
	hcfg.PolicyVerdictLogFilter = ep.GetPolicyVerdictLogFilter()
...
func ciliumHostRewrites(ep datapath.Endpoint) (*config.BPFHost, map[string]string) {
	strings := ELFMapSubstitutions(ep)
	...
	em := ep.GetNodeMAC()
	if em == nil {
		panic("nil MAC address for cilium_host")
	}
...
func ciliumNetRewrites(ep datapath.Endpoint, link netlink.Link) (*config.BPFHost, map[string]string) {
	strings := ELFMapSubstitutions(ep)
...
func endpointRewrites(ep datapath.Endpoint) (*config.BPFLXC, map[string]string) {
	...
	em := ep.GetNodeMAC()
	if em == nil {
		panic(fmt.Sprintf("nil MAC address for endpoint %s", ep.InterfaceName()))
	}
