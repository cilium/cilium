...
func (l *loader) reinitializeIPSec(lnc *datapath.LocalNodeConfiguration) error {
	...
	// by configuration.
	if !lnc.EnableIPSec || option.Config.AreDevicesRequired(lnc.LBConfig.KubeProxyReplacement, lnc.EnableWireguard, lnc.EnableIPSec) {
		return nil
	}
...
func (l *loader) Reinitialize(ctx context.Context, lnc *datapath.LocalNodeConfiguration, tunnelConfig tunnel.Config, iptMgr datapath.IptablesManager, p datapath.Proxy) error {
	...
	if lnc.LBConfig.EnableSocketLB {
		...
		if err := socketlb.Enable(l.logger, l.sysctl, lnc.LBConfig); err != nil {
			return err
		}
	} else {
