...
func (l *loader) reinitializeIPSec() error {
	...
	for _, iface := range interfaces {
		...
		if err := attachSKBProgram(l.logger, device, obj.FromNetwork, symbolFromNetwork, bpffsDeviceLinksDir(bpf.CiliumPath(), device), netlink.HANDLE_MIN_INGRESS, option.Config.EnableTCX); err != nil {
...
func reinitializeOverlay(ctx context.Context, logger *slog.Logger, tunnelConfig tunnel.Config) error {
	...
	if err := replaceOverlayDatapath(ctx, logger, opts, link); err != nil {
...
func reinitializeWireguard(ctx context.Context, logger *slog.Logger) (err error) {
	...
	if err := replaceWireguardDatapath(ctx, logger, link); err != nil {
		return fmt.Errorf("failed to load wireguard programs: %w", err)
	}
	return
}

func reinitializeXDPLocked(ctx context.Context, logger *slog.Logger, extraCArgs []string, devices []string, xdpConfig xdp.Config) error {
	...
	for _, dev := range devices {
		...
		if err := compileAndLoadXDPProg(ctx, logger, dev, xdpConfig.Mode(), extraCArgs); err != nil {
...
func (l *loader) ReinitializeXDP(ctx context.Context, cfg *datapath.LocalNodeConfiguration, extraCArgs []string) error {
	...
	return reinitializeXDPLocked(ctx, l.logger, extraCArgs, devices, cfg.XDPConfig)
}
...
func (l *loader) Reinitialize(ctx context.Context, cfg *datapath.LocalNodeConfiguration, tunnelConfig tunnel.Config, iptMgr datapath.IptablesManager, p datapath.Proxy) error {
	...
	hostDev1, _, err := setupBaseDevice(l.logger, l.sysctl, cfg.DeviceMTU)
	if err != nil {
		return fmt.Errorf("failed to setup base devices: %w", err)
	}
	...
	if err := setupTunnelDevice(l.logger, l.sysctl, tunnelConfig.Protocol(), tunnelConfig.Port(),
	...
	if err := reinitializeXDPLocked(ctx, l.logger, extraArgs, devices, cfg.XDPConfig); err != nil {
	...
	if err := reinitializeOverlay(ctx, l.logger, tunnelConfig); err != nil {
		return err
	}

	if err := reinitializeWireguard(ctx, l.logger); err != nil {
		return err
	}
