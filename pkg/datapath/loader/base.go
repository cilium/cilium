...
func addENIRules(logger logging.FieldLogger, sysSettings []tables.Sysctl) ([]tables.Sysctl, error) {
...
func (l *loader) reinitializeIPSec() error {
	...
	spec, err := bpf.LoadCollectionSpec(log, networkObj)
	if err != nil {
...
func (l *loader) Reinitialize(ctx context.Context, cfg *datapath.LocalNodeConfiguration, tunnelConfig tunnel.Config, iptMgr datapath.IptablesManager, p datapath.Proxy) error {
	...
	if option.Config.IPAM == ipamOption.IPAMENI {
		var err error
		if sysSettings, err = addENIRules(log, sysSettings); err != nil {
