...
func HashDatapath(dp datapath.Datapath, nodeCfg *datapath.LocalNodeConfiguration, netdevCfg datapath.DeviceConfiguration, epCfg datapath.EndpointConfiguration) *Hash {
	...
	if epCfg != nil {
		_ = dp.WriteTemplateConfig(h, epCfg)
	}
...
func (h *Hash) Copy() *Hash {
	newHash := sha1.New()
	newHash.Write(h.Data())
	return &Hash{
		Hash: newHash,
	}
}

// Data fetches the underlying data in the hash.
func (h *Hash) Data() []byte {
...
func (h *Hash) SumEndpoint(dp datapath.Datapath, epCfg datapath.EndpointConfiguration, staticData bool) string {
	result := h.Copy()
	if staticData {
