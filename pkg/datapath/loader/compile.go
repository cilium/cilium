...
var (
	...
	// compilationSem dictates the number of concurrent compilations that can
	// be performed at a given time
	compilationSem = semaphore.NewWeighted(int64(numCompilations()))
)

// numCompilations returns the number of threads which can be used to compile
// BPF programs, with a minimum of 2.
func numCompilations() int {
	ncpu := runtime.NumCPU()
	minWorkerThreads := 2
...
func compile(ctx context.Context, prog *progInfo, dir *directoryInfo, debug bool) (err error) {
	...
	err = compilationSem.Acquire(ctx, 1)
	if err != nil {
		return err
	}
	defer compilationSem.Release(1)
