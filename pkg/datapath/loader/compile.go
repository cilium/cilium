...
// rusageFields returns the kernel's accounting for a finished child, to tell a
// compiler that was starved of cpu, memory or i/o from one that was just slow.
func rusageFields(state *os.ProcessState) []any {
	// Cmd.Wait() populates this even for a signalled child, but Cmd.Run() leaves
	// it nil if Cmd.Start() failed.
	if state == nil {
		return nil
	}

	fields := []any{
		logfields.UserTime, state.UserTime(),
		logfields.SystemTime, state.SystemTime(),
	}
	...
	return append(fields,
		logfields.MaxRSSKBytes, usage.Maxrss,
		logfields.MinorFaults, usage.Minflt,
		logfields.MajorFaults, usage.Majflt,
		logfields.BlockReads, usage.Inblock,
		logfields.BlockWrites, usage.Oublock,
		logfields.VoluntaryCtxSwitches, usage.Nvcsw,
		logfields.InvoluntaryCtxSwitches, usage.Nivcsw,
	)
}
...
func compile(ctx context.Context, logger *slog.Logger, prog *progInfo, dir *directoryInfo) (string, error) {
	...
	if err := compileCmd.Run(); err != nil {
		...
		// In linux/unix based implementations, terminating the context for a cmd.Run()
		// returns "context canceled" or "context deadline exceeded" if the context was
		// done before the process started, and "signal: killed" if it was already
		// running. A bare "signal: killed" hides that the kill was ours, so join the
		// context error in to name it.
		if ctx.Err() != nil &&
		...
		if !errors.Is(err, context.Canceled) {
			logger.Error(
				...
				}, rusageFields(compileCmd.ProcessState)...)...,
			)
		}
...
func compileDatapath(ctx context.Context, logger *slog.Logger, dirs *directoryInfo, isHost bool) error {
	...
	// This probe is charged to the same context as the compilation below, so
	// time it separately to tell a slow compiler start from a slow codegen.
	...
	if err != nil {
		if !errors.Is(err, context.Canceled) {
			scopedLog.Error(
				"Failed to probe compiler version",
				logfields.Error, err,
				logfields.Duration, time.Since(started),
			)
		}
