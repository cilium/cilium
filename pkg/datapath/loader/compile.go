...
func getBPFCPU(logger logging.FieldLogger) string {
...
func compile(ctx context.Context, logger logging.FieldLogger, prog *progInfo, dir *directoryInfo) (string, error) {
	...
	log.Debug(
		"Launching compiler",
		slog.String("target", compiler),
		slog.Any("args", compileArgs),
	)
	...
	if err := compileCmd.Run(); err != nil {
		...
		if !errors.Is(err, context.Canceled) {
			log.Error(
				err.Error(),
				slog.String("compiler-pid", pidFromProcess(compileCmd.Process)),
			)
		}
	...
	if usage, ok := compileCmd.ProcessState.SysUsage().(*syscall.Rusage); ok {
		log.Debug(
			...
			slog.Int("compiler-pid", compileCmd.Process.Pid),
			slog.String("output", output.Name()),
			slog.Int64("rss-bytes", usage.Maxrss),
		)
	}
...
func compileDatapath(ctx context.Context, dirs *directoryInfo, isHost bool, logger logging.FieldLogger) error {
	...
	scopedLog.Debug(
		"Compiling datapath",
		slog.String(compiler, string(compilerVersion)),
	)
		...
		if _, err := compile(ctx, logger, &debugProg, dirs); err != nil {
			...
			if !errors.Is(err, context.Canceled) {
				scopedLog.Debug(
					...
					slog.Any(logfields.Params, debugProg),
				)
			}
			return err
		}
	...
	if _, err := compile(ctx, logger, prog, dirs); err != nil {
		...
		if !errors.Is(err, context.Canceled) {
			scopedLog.Warn(
				"JoinEP: Failed to compile",
				slog.Any(logfields.Error, err),
				slog.Any(logfields.Params, prog),
			)
		}
		return err
	}
...
func compileWithOptions(ctx context.Context, src string, out string, opts []string) error {
	...
	_, err := compile(ctx, log, &prog, &dirs)
	return err
}
...
func compileNetwork(ctx context.Context) error {
	...
	scopedLog := log.With(slog.Bool(logfields.Debug, true))
	...
	if _, err := compile(ctx, log, networkTcProg, &dirs); err != nil {
		scopedLog.Warn(
			"Failed to compile",
			slog.Any(logfields.Error, err),
			slog.Any(logfields.Params, networkTcProg),
		)
		return err
	}
	return nil
...
func compileOverlay(ctx context.Context, opts []string) error {
	...
	// Write out assembly and preprocessing files for debugging purposes
	if _, err := compile(ctx, log, prog, dirs); err != nil {
		scopedLog.Warn(
			"Failed to compile",
			slog.Any(logfields.Error, err),
			slog.Any(logfields.Params, prog),
