...
func compileAndLoadXDPProg(ctx context.Context, logger *slog.Logger, lnc *datapath.LocalNodeConfiguration, xdpDev string, xdpMode xdp.Mode, extraCArgs []string) error {
	...
	if err := loadAssignAttach(logger, xdpMode, iface, spec, lnc, cfg); err != nil {
		return err
	}
...
func loadAssignAttach(logger *slog.Logger, xdpMode xdp.Mode, iface netlink.Link, spec *ebpf.CollectionSpec, lnc *datapath.LocalNodeConfiguration, cfg *config.BPFXDP) error {
	var (
		obj    xdpObjects
		commit func() error
		err    error
