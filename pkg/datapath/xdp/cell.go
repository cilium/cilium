...
var Cell = cell.Module(
	...
	cell.Provide(
		...
		func(dcfg *option.DaemonConfig) (EnablerOut, error) {
			xdpMode, ok := map[string]Mode{
				"":                                    ModeDisabled,
				option.NodePortAccelerationDisabled:   ModeDisabled,
				option.NodePortAccelerationGeneric:    ModeGeneric,
				option.NodePortAccelerationBestEffort: ModeBestEffort,
				option.NodePortAccelerationNative:     ModeNative,
			}[dcfg.NodePortAcceleration]

			if !ok {
				return NewEnabler(ModeDisabled), fmt.Errorf("Invalid value for --%s: %s", option.NodePortAcceleration, dcfg.NodePortAcceleration)
			}
	...
	cell.Invoke(func(c Config, l *slog.Logger) {
		l.Info("Determined final XDP mode", "mode", c.Mode(), "tcmode", c.TCMode())
	}),
)
