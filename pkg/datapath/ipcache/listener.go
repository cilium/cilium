...
type regenerator interface {
	TriggerReload(context string) (*sync.WaitGroup, error)
}

func newListener(m *ipcacheMap.Map, r regenerator) *BPFListener {
	return &BPFListener{
		bpfMap:      m,
		regenerator: r,
	}
...
func NewListener(r regenerator) *BPFListener {
...
func (l *BPFListener) garbageCollect() error {
	...
	if ipcacheMap.SupportsDelete() {
		...
		wg, err := l.regenerator.TriggerReload("datapath ipcache")
		if err != nil {
