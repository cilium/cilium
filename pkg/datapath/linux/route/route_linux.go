...
func Upsert(logger logging.FieldLogger, route Route) error {
...
func lookupDefaultRoute(logger logging.FieldLogger, family int) (netlink.Route, error) {
	...
	logger.Debug("Found default route on node", slog.Any("route", routes[0]))
	return routes[0], nil
}
...
func NodeDeviceWithDefaultRoute(logger logging.FieldLogger, enableIPv4, enableIPv6 bool) (netlink.Link, error) {
	linkIndex := 0
	if enableIPv4 {
