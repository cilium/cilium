...
func TestRefreshLinuxNodesOnce(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		return found && first.Statuses.Get(linuxNodeReconcilerName).Kind ==
	...
	require.Equal(t, reconciler.StatusKindDone,
		second.Statuses.Get(linuxNodeReconcilerName).Kind)
	...
	require.Eventually(t, func() bool {
		...
		return found && second.Statuses.Get(linuxNodeReconcilerName).Kind ==
			reconciler.StatusKindRefreshing
	}, time.Second, time.Millisecond)
