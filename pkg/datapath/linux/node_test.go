...
func TestCreateNodeRoute(t *testing.T) {
	...
	nodeHandler := newNodeHandler(log, dpConfig, nil, kpr.KPRConfig{}, &fakeipsec.Agent{}, fakeipsec.Config{}, lns, newNodePolicy(), nil, nil)
...
func TestClassifyRemoteCIDRs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "auto-direct disabled - subnet grouping still applies",
			localPodCIDR: "10.244.0.0/24",
			remoteNode:   makeNodeWithPodCIDRs("10.0.0.10", "10.244.1.0/24"),
			entries: []subnetmap.SubnetTableEntry{
				subnetmap.NewSubnetEntry(netip.MustParsePrefix("10.244.0.0/16"), 1),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := newNodeHandler(log, DatapathConfiguration{}, nil, kpr.KPRConfig{}, &fakeipsec.Agent{}, fakeipsec.Config{}, lns, newNodePolicy(), db, table)
