...
func HaveManagedNeighbors(logger logging.FieldLogger) error {
	managedNeighborOnce.Do(func() {
		...
		if managedNeighborResult != nil && !errors.Is(managedNeighborResult, ErrNotSupported) {
			logging.Fatal(logger, "failed to probe managed neighbor support", slog.Any(logfields.Error, managedNeighborResult))
		}
	})
