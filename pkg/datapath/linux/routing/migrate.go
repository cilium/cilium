...
func (m *migrator) MigrateENIDatapath(compat bool) (int, int) {
	...
	if err != nil {
		log.Error("Failed to migrate ENI datapath due to a failure in listing the existing rules. "+
	...
	if isUpgrade {
		for _, r := range v1Rules {
			if routes, err := m.upgradeRule(r); err != nil {
				log.Warn("Failed to migrate endpoint to new ENI datapath. "+
		...
		for _, r := range v2Rules {
			if routes, err := m.downgradeRule(r); err != nil {
				log.Warn("Failed to downgrade endpoint to original ENI datapath. "+
					"Previous datapath is still intact and endpoint connectivity is not affected.",
					slog.Any(logfields.Error, err),
					slog.Any("rule", r),
				)
...
func (m *migrator) upgradeRule(rule netlink.Rule) ([]netlink.Route, error) {
	...
	// skip.
	if len(routes) == 0 {
		log.Debug("Skipping migration of egress rule due to no routes found", logAttr)
		return nil, nil
	}
	...
	if err != nil {
		...
		if revErr := stack.Revert(); revErr != nil {
			log.Warn(upgradeRevertWarning, slog.Any(logfields.Error, err), slog.Any("revertError", revErr), logAttr)
		}
...
func (m *migrator) downgradeRule(rule netlink.Rule) ([]netlink.Route, error) {
	...
	if err != nil {
		if revErr := stack.Revert(); revErr != nil {
			log.Warn(downgradeRevertWarning, slog.Any(logfields.Error, err), slog.Any("revertError", revErr), logAttr)
		}
	...
	if err := m.rpdb.RuleDel(&rule); err != nil {
		...
		// migration in MigrateENIDatapath().
		log.Warn(downgradeFailedRuleDeleteWarning, slog.Any(logfields.Error, err), logAttr)
		return nil, nil
	}
