...
func (mgr *endpointRulesManager) EndpointDeleted(ep *endpoint.Endpoint, _ endpoint.DeleteConfig) {
	...
	mgr.handleEvent(endpointAddrs(ep), endpointRulesOwner(ep), true)
}
...
func (mgr *endpointRulesManager) handleEvent(
	...
	var err error
	if deleted {
		err = mgr.handleEndpointDeletion(txn, addresses, owner)
	} else {
...
func (mgr *endpointRulesManager) handleEndpointDeletion(
	...
	for _, address := range addresses {
		...
		// A different owner means that a replacement endpoint
