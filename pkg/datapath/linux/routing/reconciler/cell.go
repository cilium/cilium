...
func registerEndpointRulesReconciler(p params) error {
	...
	if ipamMode == ipamOption.IPAMAlibabaCloud && p.DaemonConfig.EnableIPv6 {
		...
		err := errors.New("routing metadata for IPv6 is not supported by AlibabaCloud IPAM")
		const message = "Endpoint routing rule reconciliation is disabled"
		p.Logger.Error(message, logfields.Error, err)
		return nil
	}
