...
func NewNodeHandler(datapathConfig DatapathConfiguration, nodeAddressing types.NodeAddressing, wgAgent datapath.WireguardAgent, ipsecManager ipsec.ManagerInterface) datapath.NodeHandler {
...
func (n *linuxNodeHandler) enableSubnetIPsec(v4CIDR, v6CIDR []*net.IPNet) {
	...
	for _, cidr := range v4CIDR {
		...
		spi, err = n.ipsecManager.UpsertIPsecEndpoint(ipsecIPv4Wildcard, cidr, wildcardIP, cidr.IP, ipsec.IPSecDirOut, zeroMark)
		...
		if n.nodeConfig.EncryptNode {
			...
			spi, err := n.ipsecManager.UpsertIPsecEndpoint(cidr, ipsecIPv4Wildcard, linkAddr.IP, wildcardIP, ipsec.IPSecDirIn, zeroMark)
	...
	for _, cidr := range v6CIDR {
		...
		n.replaceNodeIPSecOutRoute(cidr)
		spi, err := n.ipsecManager.UpsertIPsecEndpoint(ipsecIPv6Wildcard, cidr, wildcardIP, cidr.IP, ipsec.IPSecDirOut, zeroMark)
		...
		if n.nodeConfig.EncryptNode {
			...
			spi, err := n.ipsecManager.UpsertIPsecEndpoint(cidr, ipsecIPv6Wildcard, linkAddr.IP, wildcardIP, ipsec.IPSecDirIn, zeroMark)
...
func (n *linuxNodeHandler) encryptNode(newNode *nodeTypes.Node) {
	...
	if n.nodeConfig.EnableIPv4 {
		...
		if newNode.IsLocal() {
			...
			spi, err = n.ipsecManager.UpsertIPsecEndpoint(ipsecLocal, ipsecIPv4Wildcard, internalIPv4, wildcardIP, ipsec.IPSecDirIn, false)
			...
			if remoteIPv4 := newNode.GetNodeIP(false); remoteIPv4 != nil {
				...
				spi, err = n.ipsecManager.UpsertIPsecEndpoint(ipsecLocal, ipsecRemote, internalIPv4, remoteIPv4, ipsec.IPSecDirOutNode, false)
			...
			if remoteIPv4 != nil {
				...
				if remoteIPv4T := newNode.GetNodeIP(false); remoteIPv4T != nil {
					err = n.ipsecManager.UpsertIPsecEndpointPolicy(ipsecWildcard, ipsecRemote, internalIPv4, remoteIPv4T, ipsec.IPSecDirOutNode)
				}
	...
	if n.nodeConfig.EnableIPv6 {
		...
		if newNode.IsLocal() {
			...
			ipsecIPv6Wildcard := &net.IPNet{IP: wildcardIP, Mask: net.CIDRMask(0, 0)}
			n.replaceNodeIPSecInRoute(ipsecLocal)
			spi, err = n.ipsecManager.UpsertIPsecEndpoint(ipsecLocal, ipsecIPv6Wildcard, internalIPv6, wildcardIP, ipsec.IPSecDirIn, false)
			...
			if remoteIPv6 := newNode.GetNodeIP(true); remoteIPv6 != nil {
				...
				n.replaceNodeExternalIPSecOutRoute(ipsecRemote)
				spi, err = n.ipsecManager.UpsertIPsecEndpoint(ipsecLocal, ipsecRemote, internalIPv6, remoteIPv6, ipsec.IPSecDirOut, false)
			...
			if remoteIPv6 != nil {
				...
				if remoteIPv6T := newNode.GetNodeIP(true); remoteIPv6T != nil {
					err = n.ipsecManager.UpsertIPsecEndpointPolicy(ipsecWildcard, ipsecRemote, internalIPv6, remoteIPv6T, ipsec.IPSecDirOutNode)
				}
	...
	if n.nodeConfig.EnableIPv4 && newNode.IPv4AllocCIDR != nil {
		...
		if newNode.IsLocal() {
			...
			if localIP := newNode.GetCiliumInternalIP(false); localIP != nil {
				...
				spi, err = n.ipsecManager.UpsertIPsecEndpoint(localCIDR, wildcardCIDR, localIP, wildcardIP, ipsec.IPSecDirIn, false)
			...
			if remoteIP := newNode.GetCiliumInternalIP(false); remoteIP != nil {
				...
				spi, err = n.ipsecManager.UpsertIPsecEndpoint(localCIDR, remoteCIDR, localIP, remoteIP, ipsec.IPSecDirOut, false)
