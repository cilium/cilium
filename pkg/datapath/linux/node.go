...
func (n *linuxNodeHandler) enableIPsec(newNode *node.Node) {
	upsertIPsecLog := func(err error, spec string, loc, rem *net.IPNet) {
		if err != nil {
			log.WithError(err).Errorf("Upsert %s local %s remote %s", spec, loc, rem)
		} else {
			log.Debugf("Upsert %s local %s remote %s", spec, loc, rem)
		}
	}
	...
	if n.nodeConfig.EnableIPv4 {
		...
		if newNode.IsLocal() {
			...
			if ciliumInternalIPv4 != nil {
				...
				upsertIPsecLog(err, "(local node)", ipsecLocal, ipsecIPv4Wildcard)
			}
		} else {
			if ciliumInternalIPv4 := newNode.GetCiliumInternalIP(false); ciliumInternalIPv4 != nil {
				...
				upsertIPsecLog(err, "", ipsecLocal, ipsecRemote)
			}
		}
	}
	...
	if n.nodeConfig.EnableIPv6 {
		...
		if newNode.IsLocal() {
			...
			if ciliumInternalIPv6 != nil {
				...
				upsertIPsecLog(err, "(local node)", ipsecLocal, ipsecIPv6Wildcard)
			}
		} else {
			if ciliumInternalIPv6 := newNode.GetCiliumInternalIP(true); ciliumInternalIPv6 != nil {
				...
				if !ipsecHost.IP.Equal(ipsecLocal.IP) {
					...
					upsertIPsecLog(err, "(host != local)", ipsecHost, ipsecRemote)
				}
			}
		}
	}
