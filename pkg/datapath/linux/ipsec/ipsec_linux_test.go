...
func TestUpsertIPSecEndpointOut(t *testing.T) {
	...
	derivedAuthKey := computeNodeIPsecKey([]byte("auth"), authKey, local.IP, remote.IP, []byte(localBootID), []byte(remoteBootID))
	require.Equal(t, derivedAuthKey, state.Auth.Key)
	require.NotNil(t, state.Crypt)
	require.Equal(t, "cbc(aes)", state.Crypt.Name)
	derivedCryptKey := computeNodeIPsecKey([]byte("crypt"), cryptKey, local.IP, remote.IP, []byte(localBootID), []byte(remoteBootID))
...
func TestUpsertIPSecEndpointIn(t *testing.T) {
	...
	require.NotNil(t, state.Auth)
	require.Equal(t, "hmac(sha256)", state.Auth.Name)
	derivedAuthKey := computeNodeIPsecKey([]byte("auth"), authKey, remote.IP, local.IP, []byte(remoteBootID), []byte(localBootID))
	require.Equal(t, derivedAuthKey, state.Auth.Key)
	require.NotNil(t, state.Crypt)
	require.Equal(t, "cbc(aes)", state.Crypt.Name)
	derivedCryptKey := computeNodeIPsecKey([]byte("crypt"), cryptKey, remote.IP, local.IP, []byte(remoteBootID), []byte(localBootID))
	require.Equal(t, derivedCryptKey, state.Crypt.Key)
	// ESN bit is not set, so ReplayWindow should be 0
	require.Equal(t, 0, state.ReplayWindow)
