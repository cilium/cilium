...
var (
	...
	dfltKeysDat    = []byte("hmac(sha256) abcdefghijklmnopqrstuvwzyzABCDEF cbc(aes) abcdefghijklmnopqrstuvwzyzABCDEF\n")
	scopedKeysDat  = []byte("hmac(sha256) abcdefghijklmnopqrstuvwzyzABCDEF cbc(aes) abcdefghijklmnopqrstuvwzyzABCDEF foobar\n")
...
func (p *IPSecSuitePrivileged) TestLoadKeysNoFile(c *C) {
	...
	c.Assert(err, ErrorMatches, "failed to load IPSec Keys File ipsec_keys_test: open ipsec_keys_test: no such file or directory")
}
...
func (p *IPSecSuitePrivileged) TestLoadKesyFile(c *C) {
	...
	c.Assert(err, IsNil)
	defer f.Close()
	_, err = f.Write(dfltKeysDat)
	c.Assert(err, IsNil)
	_, err = f.Write(scopedKeysDat)
	c.Assert(err, IsNil)
	f.Sync()
	err = LoadIPSecKeysFile(path)
	c.Assert(err, IsNil)
