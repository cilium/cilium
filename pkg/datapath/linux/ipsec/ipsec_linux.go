...
// DeleteXFRM will remove XFRM OUT policies by their node ID and destination subnet.
func DeleteXfrmPolicyOut(log *slog.Logger, dst *net.IPNet) error {
	if dst.IP.To4() != nil {
		return deleteXfrmPolicyOutFamily(log, dst, netlink.FAMILY_V4)
	} else {
		return deleteXfrmPolicyOutFamily(log, dst, netlink.FAMILY_V6)
	}
}

func deleteXfrmPolicyOutFamily(log *slog.Logger, dst *net.IPNet, family int) error {
