...
func ipSecReplacePolicyIn(src, dst *net.IPNet) error {
	ipSecReplacePolicyInFwd(src, dst, netlink.XFRM_DIR_IN)
...
func UpsertIPSecEndpoint(local, remote *net.IPNet) error {
	...
	if !local.IP.Equal(remote.IP) {
		ipSecReplaceState(local.IP, remote.IP)
		ipSecReplaceState(remote.IP, local.IP)
		ipSecReplacePolicyOut(local, remote)
		ipSecReplacePolicyIn(remote, local)
	}
	return nil
}
