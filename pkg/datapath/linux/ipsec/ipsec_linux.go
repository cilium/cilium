...
type IPSecParameters struct {
	// The BootID for the local host, used for bookkeeping.
	LocalBootID string
	// The BootID for the remote host, used for bookkeeping.
	RemoteBootID string
...
func ipSecReplaceStateIn(log *slog.Logger, params *IPSecParameters) (uint8, error) {
	key := getNodeIPsecKey(*params.SourceTunnelIP, *params.DestTunnelIP, params.LocalBootID, params.RemoteBootID, netlink.XFRM_DIR_IN)
	if key == nil {
...
func ipSecReplaceStateOut(log *slog.Logger, params *IPSecParameters) (uint8, error) {
	key := getNodeIPsecKey(*params.SourceTunnelIP, *params.DestTunnelIP, params.LocalBootID, params.RemoteBootID, netlink.XFRM_DIR_OUT)
	if key == nil {
		return 0, fmt.Errorf("IPSec key missing")
	}
	key.ReqID = params.ReqID
