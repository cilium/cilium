...
//
//  1. Refresh node reconcilers to update the IPSec policies and states for all
//     endpoints. We do this before advertising the new SPI to ensure our
//     ingress XFRM states are ready before peers start sending traffic encrypted
//     with the new key.
//
...
func (a *agent) publishCurrentSPI(ctx context.Context, currentSPI uint8) error {
	if err := a.nodeWriter.Refresh(ctx); err != nil {
