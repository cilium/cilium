...
func (h *HeaderfileWriter) WriteNodeConfig(w io.Writer, cfg *datapath.LocalNodeConfiguration) error {
	...
	// Enabling NetfilterCompatibleMode increases BPF instruction memory size, and may
	// cause issues in kernels have lower memory. Hence enabling, this feature
	// only in kernels supports higher instruction size.
