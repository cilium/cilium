...
func (h *HeaderfileWriter) WriteNodeConfig(w io.Writer, cfg *datapath.LocalNodeConfiguration) error {
	...
	if option.Config.EnableNodePort {
		...
		if option.Config.EnableNat46X64Gateway {
			...
			cDefinesMap["NAT_46X64_PREFIX_0"] = fmt.Sprintf("%d", int(option.Config.IPv6NAT46x64CIDRBase[0]))
			cDefinesMap["NAT_46X64_PREFIX_1"] = fmt.Sprintf("%d", int(option.Config.IPv6NAT46x64CIDRBase[1]))
			cDefinesMap["NAT_46X64_PREFIX_2"] = fmt.Sprintf("%d", int(option.Config.IPv6NAT46x64CIDRBase[2]))
			cDefinesMap["NAT_46X64_PREFIX_3"] = fmt.Sprintf("%d", int(option.Config.IPv6NAT46x64CIDRBase[3]))
		} else {
