...
func (h *HeaderfileWriter) WriteNodeConfig(w io.Writer, cfg *datapath.LocalNodeConfiguration) error {
	...
	cDefinesMap["CILIUM_LB_PINNING_MAP_MAX_ENTRIES"] = fmt.Sprintf("%d", cfg.LBConfig.LBPinningEntries)
	...
	cDefinesMap["CT_SIP_SESSION_LIFETIME"] = fmt.Sprintf("%d", int64(option.Config.CTMapEntriesTimeoutSIP.Seconds()))
