...
func (s *linuxPrivilegedBaseTestSuite) TestUpdateNodeRoute(c *check.C) {
	...
	linuxNodeHandler := NewNodeHandler(dpConfig, s.nodeAddressing, nodemapfake.NewFakeNodeMap(), nil, nil)
...
func (s *linuxPrivilegedBaseTestSuite) TestSingleClusterPrefix(c *check.C) {
	...
	linuxNodeHandler := NewNodeHandler(dpConfig, s.nodeAddressing, nil, nil, nil)
	c.Assert(linuxNodeHandler, check.Not(check.IsNil))
...
func (s *linuxPrivilegedBaseTestSuite) TestNodeUpdateIDs(c *check.C) {
	...
	dpConfig := DatapathConfiguration{HostDevice: dummyHostDeviceName}
	linuxNodeHandler := NewNodeHandler(dpConfig, s.nodeAddressing, nodeMap, nil, nil)
	c.Assert(linuxNodeHandler, check.Not(check.IsNil))
...
func (s *linuxPrivilegedIPv6OnlyTestSuite) TestArpPingHandlingForMultiDevice(c *check.C) {
	...
	// FIXME: use hive and the DeviceTableCell here.
	devices, _ := statedb.NewTable[*tables.Device]("devices", tables.DeviceIDIndex, tables.DeviceNameIndex, tables.DeviceSelectedIndex)
	db, _ := statedb.NewDB([]statedb.TableMeta{devices}, statedb.NewMetrics())
	txn := db.WriteTxn(devices)
	devices.Insert(txn, &tables.Device{Name: "veth0", Index: 2})
	devices.Insert(txn, &tables.Device{Name: "veth2", Index: 3})
	txn.Commit()
	...
	linuxNodeHandler := NewNodeHandler(dpConfig, s.nodeAddressing, nodemapfake.NewFakeNodeMap(), db, devices)
	c.Assert(linuxNodeHandler, check.Not(check.IsNil))
