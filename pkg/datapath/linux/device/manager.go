...
type ManagerOperations interface {
	...
	UpsertDeviceWait(device DesiredDevice) error
...
func (m *manager) GetOrRegisterOwner(name string) DeviceOwner {
	if owner, ok := m.owners.Load(name); ok {
		return owner
	}
	owner := DeviceOwner{
		Name: name,
	}
	m.owners.Store(name, owner)
	return owner
}
...
func (m *manager) UpsertDeviceWait(device DesiredDevice) error {
	err := m.UpsertDevice(device)
	if err != nil {
		return err
	}

	return m.waitForReconciliation(device.GetKey())
}
...
func (m *manager) waitForReconciliation(deviceKey DesiredDeviceKey) error {
	t := time.NewTimer(1 * time.Second) // reconciliation timeout
	defer t.Stop()
	...
	for {
		...
		case reconciler.StatusKindError:
			return errors.New(obj.status.GetError())
		}
