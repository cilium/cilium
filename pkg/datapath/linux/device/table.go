...
func (k *DesiredDeviceKey) UnmarshalBinary(data []byte) error {
	...
	k.Name = string(data[1:]) // potentially check length, device size should not be > 16 bytes.
	return nil
}
...
func (dd *DesiredDevice) GetOwnerlessKey() DesiredDeviceKey {
	return DesiredDeviceKey{
		Name: dd.Name,
	}
}
...
func (dd *DesiredDevice) Validate() (err error) {
	if dd.Owner.Name == "" {
