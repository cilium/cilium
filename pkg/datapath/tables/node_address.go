...
func (n *nodeAddressController) updateWildcardDevice(txn statedb.WriteTxn, dev *Device, deleted bool) {
	...
	n.Log.Info(
		...
		slog.String("node-addresses", showAddresses(newAddrs)),
		slog.String(logfields.Device, WildcardDeviceName),
	)
}
...
func (n *nodeAddressController) update(txn statedb.WriteTxn, new []NodeAddress, reporter cell.Health, device string) {
	...
	if updated {
		...
		n.Log.Info(
			...
			slog.String("node-addresses", addrs),
			slog.String(logfields.Device, device),
		)
		if reporter != nil {
			reporter.OK(addrs)
		}
