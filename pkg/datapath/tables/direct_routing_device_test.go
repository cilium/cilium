...
func TestDirectRoutingDevice(t *testing.T) {
	...
	dev, _ := directRoutingDev.Get(db.ReadTxn(), tctx)
	cancel()
	require.Nil(t, dev)
	...
	tctx, cancel = context.WithTimeout(ctx, time.Millisecond)
	got, watch := directRoutingDev.Get(db.ReadTxn(), tctx)
	cancel()
	...
	tctx, cancel = context.WithTimeout(ctx, time.Millisecond)
	dev, watch = directRoutingDev.Get(db.ReadTxn(), tctx)
	cancel()
	require.Nil(t, dev)
	select {
	case <-watch:
