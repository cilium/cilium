...
// This is all well and good, but given a list of arbitrary config objects,
// how do we determine the schema/type for each one? Now we're just
// reimplementing Any and protobuf.
type Config interface {
	MarshalProto() (proto.Message, error)
	UnmarshalProto(msg proto.Message) error
}
...
// allowed.
func Map(obj Config) (map[string]any, error) {
...
// We need a function to marshal to proto.Message
// and to unmarshal from proto.Message
//
// marshaling:
// myStructMessage := map[string]*structpb.Value
// for each field in my struct
//   if the type is uint64/int64:
//      convert to a string before encoding
//   else if the type is a [n]byte:
//      convert to a []byte before encoding
