...
stop:
	for {
		...
		case req, ok := <-params.proxies:
			...
			log.Debug("iptables proxy rules incremental update")
			...
			if err := updateProxyRules(req.info.port, req.info.isLocalOnly, req.info.name); err != nil {
				...
				health.OK("iptables proxy rules incremental update completed")
				close(req.updated)
			}
		case req, ok := <-params.addNoTrackPod:
			...
			log.Debug("iptables no track rules incremental install")
			...
			if err := installNoTrackRules(req.info.ip, req.info.port); err != nil {
				...
				health.OK("iptables no track rules incremental install completed")
				close(req.updated)
			}
		case req, ok := <-params.delNoTrackPod:
			...
			log.Debug("iptables no track rules incremental removal")
			...
			if err := removeNoTrackRules(req.info.ip, req.info.port); err != nil {
				...
				health.OK("iptables no track rules incremental removal completed")
				close(req.updated)
			}
		case <-ticker.C:
			if !stateChanged {
			...
			log.Debug("iptables rules full reconciliation")
