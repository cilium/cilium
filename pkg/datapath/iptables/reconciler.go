...
func reconciliationLoop(
	...
	setNoTrackHostPorts func(currentState noTrackHostPortsByPod, podName podAndNameSpace, ports []string) error,
...
stop:
	for {
		...
		case req, ok := <-params.setNoTrackHostPorts:
			if !ok {
				break stop
			}
			...
			if err := setNoTrackHostPorts(state.noTrackHostPorts, req.info.podKey, req.info.ports); err != nil {
				if partialLogLimiter.Allow() {
		...
		case req, ok := <-params.removeNoTrackHostPorts:
			if !ok {
				break stop
			}
	...
	for range params.setNoTrackHostPorts {
	}
	for range params.removeNoTrackHostPorts {
	}
