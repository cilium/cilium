	...
	for {
		...
		case req, ok := <-params.proxies:
			...
			if err := updateProxyRules(req.info.port, req.info.name); err != nil {
				if partialLogLimiter.Allow() {
					log.Error("iptables proxy rules incremental update failed, will retry a full reconciliation", slog.Any(logfields.Error, err))
				}
		...
		case req, ok := <-params.addNoTrackPod:
			...
			if err := installNoTrackRules(req.info.ip, req.info.port); err != nil {
				if partialLogLimiter.Allow() {
					log.Error("iptables no track rules incremental install failed, will retry a full reconciliation", slog.Any(logfields.Error, err))
				}
				// incremental rules update failed, schedule a full iptables reconciliation
				stateChanged = true
				updatedChs = append(updatedChs, req.updated)
		...
		case req, ok := <-params.delNoTrackPod:
			...
			if err := removeNoTrackRules(req.info.ip, req.info.port); err != nil {
				if partialLogLimiter.Allow() {
					log.Error("iptables no track rules incremental removal failed, will retry a full reconciliation", slog.Any(logfields.Error, err))
				}
				// incremental rules update failed, schedule a full iptables reconciliation
				stateChanged = true
				updatedChs = append(updatedChs, req.updated)
		...
		case <-ticker.C():
			...
			if err := updateRules(state, firstInit); err != nil {
				if fullLogLimiter.Allow() {
					log.Error("iptables rules full reconciliation failed, will retry another one later", slog.Any(logfields.Error, err))
				}
