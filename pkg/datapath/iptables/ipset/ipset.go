...
func (m *manager) onStart(ctx cell.HookContext) error {
	if !m.enabled {
		...
		if err := m.ipset.remove(ctx, CiliumNodeIPSetV4); err != nil {
			m.logger.WithError(err).Infof("Unable to remove stale ipset %s. This is usually due to a stale iptables rule referring to it. "+
				"The set will not be removed. This is harmless and it will be removed at the next Cilium restart, when the stale iptables rule has been removed.", CiliumNodeIPSetV4)
		}
		if err := m.ipset.remove(ctx, CiliumNodeIPSetV6); err != nil {
			m.logger.WithError(err).Infof("Unable to remove stale ipset %s. This is usually due to a stale iptables rule referring to it. "+
				"The set will not be removed. This is harmless and it will be removed at the next Cilium restart, when the stale iptables rule has been removed.", CiliumNodeIPSetV6)
		}
		return nil
	}
