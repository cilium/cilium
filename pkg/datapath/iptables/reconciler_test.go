...
func TestReconciliationLoop(t *testing.T) {
	...
	h := hive.New(
		...
		cell.Invoke(func(
			...
			params = &reconcilerParams{
				...
				setNoTrackHostPorts:    make(chan reconciliationRequest[noTrackHostPortsPodInfo]),
				removeNoTrackHostPorts: make(chan reconciliationRequest[podAndNameSpace]),
			}
		}),
	)
	...
	close(params.setNoTrackHostPorts)
	close(params.removeNoTrackHostPorts)
	cancel()
