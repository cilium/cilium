...
func AddToNodeIpset(nodeIP net.IP) {
	...
	log.WithField(logfields.IPAddr, nodeIP.String()).Debugf("Adding IP to %s ipset", ciliumNodeIpset)
	createIpset(ciliumNodeIpset, ip.IsIPv6(nodeIP))
	progArgs := []string{"add", ciliumNodeIpset, nodeIP.String(), "-exist"}
	ipset.runProg(progArgs, false)
}
...
func RemoveFromNodeIpset(nodeIP net.IP) {
	...
	log.WithField(logfields.IPAddr, nodeIP.String()).Debugf("Removing IP to %s ipset", ciliumNodeIpset)
