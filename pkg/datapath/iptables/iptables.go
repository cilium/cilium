...
type reconcilerParams struct {
	...
	setNoTrackHostPorts    chan reconciliationRequest[noTrackHostPortsPodInfo]
	removeNoTrackHostPorts chan reconciliationRequest[podAndNameSpace]
}
...
func (m *Manager) Stop(ctx cell.HookContext) error {
	...
	close(m.reconcilerParams.setNoTrackHostPorts)
	close(m.reconcilerParams.removeNoTrackHostPorts)
	return nil
}
...
func (m *Manager) SetNoTrackHostPorts(namespace, name string, ports []string) {
	...
	reconciled := make(chan struct{})
	m.reconcilerParams.setNoTrackHostPorts <- reconciliationRequest[noTrackHostPortsPodInfo]{noTrackHostPortsPodInfo{podName, ports}, reconciled}
	<-reconciled
}
...
func (m *Manager) RemoveNoTrackHostPorts(namespace, name string) {
	...
	reconciled := make(chan struct{})
	m.reconcilerParams.removeNoTrackHostPorts <- reconciliationRequest[podAndNameSpace]{podName, reconciled}
	<-reconciled
}
...
func (m *Manager) installRules(state desiredState) error {
	...
	if err := m.installHostNoTrackRules(state.noTrackHostPorts.flatten().AsSlice()); err != nil {
		return err
	}
...
func (m *Manager) replaceNoTrackHostPortRules(oldPorts, newPorts set.Set[lb.L4Addr]) error {
	if oldPorts.Equal(newPorts) {
		return nil
	}

	if err := m.installHostNoTrackRules(newPorts.AsSlice()); err != nil {
		return err
	}
	if err := m.cleanupHostNoTrackRules(oldPorts.AsSlice()); err != nil {
		return err
	}
...
func (m *Manager) installHostNoTrackRules(p []lb.L4Addr) error {
	byProto := groupL4AddrsByProto(p)

	for proto, ports := range byProto {
		if err := m.hostNoTrackMultiPorts(ip4tables, "-A", proto, ports); err != nil {
			return err
		}

		if m.sharedCfg.EnableIPv6 {
			if err := m.hostNoTrackMultiPorts(ip6tables, "-A", proto, ports); err != nil {
				return err
			}
		}
	}
...
func (m *Manager) cleanupHostNoTrackRules(p []lb.L4Addr) error {
	byProto := groupL4AddrsByProto(p)

	for proto, ports := range byProto {
		if err := m.hostNoTrackMultiPorts(ip4tables, "-D", proto, ports); err != nil {
			return err
		}

		if m.sharedCfg.EnableIPv6 {
			if err := m.hostNoTrackMultiPorts(ip6tables, "-D", proto, ports); err != nil {
				return err
			}
		}
	}
...
func (m *Manager) removeNoTrackHostPorts(currentState noTrackHostPortsByPod, podName podAndNameSpace) error {
	oldPorts := currentState.flatten()
	delete(currentState, podName)
	newPorts := currentState.flatten()
