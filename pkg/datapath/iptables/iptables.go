...
func (m *Manager) installHostNoTrackRules(p []lb.L4Addr) error {
	...
	for proto, ports := range byProto {
		if m.sharedCfg.EnableIPv4 {
			if err := m.hostNoTrackMultiPorts(m.ip4tables, "-A", proto, ports); err != nil {
				return err
			}
		}

		if m.sharedCfg.EnableIPv6 {
			if err := m.hostNoTrackMultiPorts(m.ip6tables, "-A", proto, ports); err != nil {
				return err
			}
		}
	}
...
func (m *Manager) cleanupHostNoTrackRules(p []lb.L4Addr) error {
	...
	for proto, ports := range byProto {
		if m.sharedCfg.EnableIPv4 {
			if err := m.hostNoTrackMultiPorts(m.ip4tables, "-D", proto, ports); err != nil {
				return err
			}
		}

		if m.sharedCfg.EnableIPv6 {
			if err := m.hostNoTrackMultiPorts(m.ip6tables, "-D", proto, ports); err != nil {
				return err
			}
		}
	}
