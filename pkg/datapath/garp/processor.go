...
func newGARPProcessor(p processorParams) (*processor, error) {
	...
	if p.Config.L2PodAnnouncementsInterface != "" && p.Config.L2PodAnnouncementsInterfaces != "" {
		return nil, fmt.Errorf("only one of '--%s' and '--%s' can be set", L2PodAnnouncementsInterface, L2PodAnnouncementsInterfaces)
	}
	...
	if p.Config.L2PodAnnouncementsInterface != "" {
		...
		// See
		p.Logger.Warn("The '--" + L2PodAnnouncementsInterface + "' flag is deprecated and will be removed in a future release. Please use '--" + L2PodAnnouncementsInterfaces + "' instead.")

	} else if p.Config.L2PodAnnouncementsInterfaces != "" {
		devicesRegex, err = regexp.Compile(p.Config.L2PodAnnouncementsInterfaces)
		if err != nil {
			return nil, fmt.Errorf("failed to compile devices regex: %w", err)
		}

	} else {
		return nil, fmt.Errorf("'--%s' or '--%s' must be set, when --%s=true", L2PodAnnouncementsInterface, L2PodAnnouncementsInterfaces, EnableL2PodAnnouncements)
	}
