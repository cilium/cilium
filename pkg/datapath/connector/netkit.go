...
func SetupNetkitWithNames(lxcIfName, peerIfName string, mtu, groIPv6MaxSize, gsoIPv6MaxSize, groIPv4MaxSize, gsoIPv4MaxSize int, l2Mode bool, ep *models.EndpointChangeRequest, sysctl sysctl.Sysctl) (*netlink.Netkit, netlink.Link, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("unable to create netkit pair: %s", err)
	}
	defer func() {
		if err != nil {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("unable to lookup netkit peer just created: %s", err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("unable to lookup netkit just created: %s", err)
	}
	...
	if err = netlink.LinkSetUp(netkit); err != nil {
		return nil, nil, fmt.Errorf("unable to bring up netkit pair: %s", err)
	}
