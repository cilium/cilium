...
func SetupNetkit(defaultLogger logging.FieldLogger, id string, mtu, groIPv6MaxSize, gsoIPv6MaxSize, groIPv4MaxSize, gsoIPv4MaxSize int, l2Mode bool, ep *models.EndpointChangeRequest, sysctl sysctl.Sysctl) (*netlink.Netkit, netlink.Link, string, error) {
	if id == "" {
...
func SetupNetkitWithNames(defaultLogger logging.FieldLogger, lxcIfName, peerIfName string, mtu, groIPv6MaxSize, gsoIPv6MaxSize, groIPv4MaxSize, gsoIPv4MaxSize int, l2Mode bool, ep *models.EndpointChangeRequest, sysctl sysctl.Sysctl) (*netlink.Netkit, netlink.Link, error) {
	logger := defaultLogger.With(subsysLogAttr)
	...
	defer func() {
		if err != nil {
			if err = netlink.LinkDel(netkit); err != nil {
				logger.Warn("failed to clean up netkit", slog.Any(logfields.Error, err), slog.Any(logfields.Netkit, netkit.Name))
			}
		}
	}()

	logger.Debug("Created netkit pair", slog.Any(logfields.NetkitPair, []string{peerIfName, lxcIfName}))
	...
	if nk, ok := peer.(*netlink.Netkit); !ok {
		logger.Debug("peer does not appear to be a Netkit device", slog.Any(logfields.NetkitPair, []string{peerIfName, lxcIfName}))
	} else if !nk.SupportsScrub() {
		logger.Warn("kernel does not support IFLA_NETKIT_SCRUB, some features may not work with netkit", slog.Any(logfields.Netkit, netkit.Name))
	}
