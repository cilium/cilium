...
// something fails.
func SetupVeth(defaultLogger logging.FieldLogger, id string, mtu, groIPv6MaxSize, gsoIPv6MaxSize, groIPv4MaxSize, gsoIPv4MaxSize int, ep *models.EndpointChangeRequest, sysctl sysctl.Sysctl) (*netlink.Veth, netlink.Link, string, error) {
	if id == "" {
...
func SetupVethWithNames(defaultLogger logging.FieldLogger, lxcIfName, peerIfName string, mtu, groIPv6MaxSize, gsoIPv6MaxSize, groIPv4MaxSize, gsoIPv4MaxSize int, ep *models.EndpointChangeRequest, sysctl sysctl.Sysctl) (*netlink.Veth, netlink.Link, error) {
	...
	defer func() {
		if err != nil {
			if err = netlink.LinkDel(veth); err != nil {
				logger.Warn("failed to clean up veth", slog.Any(logfields.Error, err), slog.Any(logfields.Veth, veth.Name))
			}
		}
	}()

	logger.Debug("Created veth pair", slog.Any(logfields.VethPair, []string{veth.PeerName, lxcIfName}))
