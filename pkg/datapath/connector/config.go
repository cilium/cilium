...
func newConnectorConfig(p connectorParams) (*ConnectorConfig, error) {
	...
	if cc.UserConfig.EnableTunedBufferMargins {
		...
		//
		// The right approach requires another change in the upstream netlink
		// library, like nk.SupportsScrub().
		//
		// A hack might be to just try create a dummy netkit interface with
		// some magic headroom value, and check we can read it back.
