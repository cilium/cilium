...
func TestRemoveDisabledMaps_RemovesNatExclusionMapV4(t *testing.T) {
	testEPManager := newTestEPManager([]string{nat.NatExclusionMapNameIPv4})
	bwManager := newTestBWManager()
	sweeper := NewMapSweeper(hivetest.Logger(t), testEPManager, bwManager, loadbalancer.DefaultConfig, kpr.KPRConfig{})
	...
	sweeper.RemoveDisabledMaps()
	require.Equal(t, []string{nat.NatExclusionMapNameIPv4}, testEPManager.removedPaths)
}

func TestRemoveDisabledMaps_RemovesNatExclusionMapV6(t *testing.T) {
	testEPManager := newTestEPManager([]string{nat.NatExclusionMapNameIPv6})
	bwManager := newTestBWManager()
	sweeper := NewMapSweeper(hivetest.Logger(t), testEPManager, bwManager, loadbalancer.DefaultConfig, kpr.KPRConfig{})
	...
	sweeper.RemoveDisabledMaps()
	require.Equal(t, []string{nat.NatExclusionMapNameIPv6}, testEPManager.removedPaths)
}
