...
func newDesiredNeighborCalculator(p desiredNeighborCalculatorParams) (*desiredNeighborCalculator, error) {
	if !p.Config.EnableL2NeighDiscovery {
		if !p.XDPConfig.Disabled() {
			return nil, fmt.Errorf(
				"XDP is enabled but L2 neighbor discovery is disabled, which is required for forwarding traffic with XDP."+
					" Please enable L2 neighbor discovery by setting `--%s=true`.", option.EnableL2NeighDiscovery)
		}
