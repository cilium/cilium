...
func (ops *ops) Update(ctx context.Context, rx statedb.ReadTxn, _ statedb.Revision, neighbor *DesiredNeighbor) error {
	...
	if ops.daemonConfig.ARPPingKernelManaged {
...
// Prune gets called with all the desired neighbors. Any neighbor entries in the kernel not in `desiredNeighbors`
// should be deleted.
	...
	jobGroup.Add(
		job.OneShot("neighbor-refresher", func(ctx context.Context, health cell.Health) error {
			...
			for {
				...
				// Get inserted or updated neighbors since the last revision processed
