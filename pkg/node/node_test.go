...
func (s *NodeSuite) TestGetNodeIP(c *C) {
	n := Node{
		Name: "node-1",
		IPAddresses: []Address{
			{IP: net.ParseIP("192.0.2.3"), AddressType: NodeExternalIP},
		},
	}
	...
	n.IPAddresses = append(n.IPAddresses, Address{IP: net.ParseIP("192.0.2.3"), AddressType: NodeExternalIP})
	ip = n.GetNodeIP(false)
	// The next priority should be NodeExternalIP
	c.Assert(ip.Equal(net.ParseIP("192.0.2.3")), Equals, true)

	n.IPAddresses = append(n.IPAddresses, Address{IP: net.ParseIP("198.51.100.2"), AddressType: NodeInternalIP})
	ip = n.GetNodeIP(false)
	// The next priority should be NodeInternalIP
	c.Assert(ip.Equal(net.ParseIP("198.51.100.2")), Equals, true)

	n.IPAddresses = append(n.IPAddresses, Address{IP: net.ParseIP("2001:DB8::1"), AddressType: NodeExternalIP})
	ip = n.GetNodeIP(true)
	// The next priority should be NodeExternalIP and IPv6
	c.Assert(ip.Equal(net.ParseIP("2001:DB8::1")), Equals, true)

	n.IPAddresses = append(n.IPAddresses, Address{IP: net.ParseIP("2001:DB8::2"), AddressType: NodeInternalIP})
	ip = n.GetNodeIP(true)
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				...
				IPAddresses:   []Address{{IP: net.ParseIP("1.1.1.1"), AddressType: NodeHostName}},
		...
		},
		{
			...
			args: args{
				o: &Node{
					...
					ClusterID:     1,
					Source:        FromKubernetes,
					IPAddresses:   []Address{{IP: net.ParseIP("1.1.1.0"), AddressType: NodeHostName}},
					IPv6AllocCIDR: &net.IPNet{IP: net.ParseIP("fd00::1"), Mask: net.CIDRMask(64, 128)},
				},
			},
			want: false,
