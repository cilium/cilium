...
// InitDefaultNodeInternalIP initializes the node address with default values derived
// from the system. device can be set to the primary network device of the system
// in which case the first address with global scope will be regarded as the
// system's node address.
func InitDefaultNodeInternalIP(device string) {
	localNode.Update(func(n *LocalNode) {
		DeriveDefaultNodeInternalIP(option.Config, device, n)
	})
}

func DeriveDefaultNodeInternalIP(cfg *option.DaemonConfig, device string, node *LocalNode) {
	if cfg.EnableIPv4 {
		isIPv6 := false
...
func AutoComplete() error {
	...
	InitDefaultNodeInternalIP(option.Config.DirectRoutingDevice)
	return nil
}
