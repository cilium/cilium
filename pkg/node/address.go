...
func InitDefaultPrefix(n *types.Node, device string) {
	if option.Config.EnableIPv4 {
		ip, err := firstGlobalV4Addr(device, n.GetCiliumInternalIP(false), preferPublicIP)
		if err != nil {
			return
		}

		if n.GetNodeIP(false) == nil {
		...
		if n.IPv4AllocCIDR == nil {
			...
			if n.IPv6AllocCIDR != nil {
				ip = net.IPv4(
					n.IPv6AllocCIDR.IP[8],
					n.IPv6AllocCIDR.IP[9],
					n.IPv6AllocCIDR.IP[10],
					n.IPv6AllocCIDR.IP[11])
			}
			...
			n.IPv4AllocCIDR = cidr.NewCIDR(ip4net)
			log.WithField(logfields.V4Prefix, n.IPv4AllocCIDR).Info("Using autogenerated IPv4 allocation range")
		}
	}

	if option.Config.EnableIPv6 {
		if n.GetNodeIP(true) == nil {
			// Find a IPv6 node address first
			addr, _ := firstGlobalV6Addr(device, n.GetCiliumInternalIP(true), preferPublicIP)
			if addr == nil {
				addr = makeIPv6HostIP()
			}
			n.SetNodeInternalIP(addr)
		}

		if n.IPv6AllocCIDR == nil && n.IPv4AllocCIDR != nil {
			// The IPv6 allocation should be derived from the IPv4 allocation.
			ip := n.IPv4AllocCIDR.IP
			...
			n.IPv6AllocCIDR = cidr.NewCIDR(ip6net)
			log.WithField(logfields.V6Prefix, n.IPv6AllocCIDR).Info("Using autogenerated IPv6 allocation range")
		}
	}
}
...
func AutoComplete(n *types.Node) error {
	if option.Config.EnableHostIPRestore {
		...
		restoreCiliumHostIPsFromFS(n)
	}

	InitDefaultPrefix(n, option.Config.DirectRoutingDevice)

	if option.Config.EnableIPv6 && n.IPv6AllocCIDR == nil {
...
func restoreCiliumHostIPsFromFS(n *types.Node) {
	// Read the previous cilium_host IPs from node_config.h for backward
	// compatibility.
	router4, router6 := getCiliumHostIPs()
	if option.Config.EnableIPv4 {
		n.SetCiliumInternalIP(router4)
	}
	if option.Config.EnableIPv6 {
		n.SetCiliumInternalIP(router6)
	}
}
