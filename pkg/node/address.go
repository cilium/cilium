...
var (
	...
	// localNode holds the current state of the local node.
...
func InitDefaultPrefix(enableIPv4, enableIPv6 bool, device string, localNode *types.Node) {
	if enableIPv4 {
		ip, err := firstGlobalV4Addr(device, localNode.GetCiliumInternalIP(false), preferPublicIP)
		if err != nil {
			return
		}

		if localNode.GetNodeIP(false) == nil {
			localNode.IPAddresses = append(
		...
		if localNode.IPv4AllocCIDR == nil {
			...
			ipv6AllocCIDR := localNode.IPv6AllocCIDR
			if ipv6AllocCIDR != nil {
				ip = net.IPv4(
					ipv6AllocCIDR.IP[8],
					ipv6AllocCIDR.IP[9],
					ipv6AllocCIDR.IP[10],
			...
			log.WithField(logfields.V4Prefix, localNode.IPv4AllocCIDR).Info("Using autogenerated IPv4 allocation range")
		}
	}

	if enableIPv6 {
		ip, err := firstGlobalV6Addr(device, localNode.GetCiliumInternalIP(true), preferPublicIP)
		if err != nil {
			return
		}
		...
		if localNode.IPv6AllocCIDR == nil && localNode.IPv4AllocCIDR != nil {
			...
			log.WithField(logfields.V6Prefix, localNode.IPv6AllocCIDR).Info("Using autogenerated IPv6 allocation range")
		}
	}
}
...
func InitNodePortAddrs(devices []string, inheritIPAddrFromDevice string) error {
	...
	if option.Config.EnableIPv4 {
		if inheritIPAddrFromDevice != "" {
			inheritedIP, err = firstGlobalV4Addr(inheritIPAddrFromDevice, k8sNodeIP, !preferPublicIP)
			if err != nil {
		...
		for _, device := range devices {
			if inheritIPAddrFromDevice != "" {
				...
				ip, err := firstGlobalV4Addr(device, k8sNodeIP, !preferPublicIP)
				if err != nil {
	...
	if option.Config.EnableIPv6 {
		if inheritIPAddrFromDevice != "" {
			inheritedIP, err = firstGlobalV6Addr(inheritIPAddrFromDevice, k8sNodeIP, !preferPublicIP)
			if err != nil {
		...
		for _, device := range devices {
			if inheritIPAddrFromDevice != "" {
				...
				ip, err := firstGlobalV6Addr(device, k8sNodeIP, !preferPublicIP)
				if err != nil {
...
func SetInternalIPv4Router(ip net.IP) {
	localNode.Update(func(localNode *types.Node) {
		localNode.SetCiliumInternalIP(ip)
	})
}
...
func GetInternalIPv4Router() net.IP {
	n := localNode.Get()
	return clone(n.GetCiliumInternalIP(false))
}
...
func RestoreHostIPs(ipv6 bool, fromK8s net.IP, cidrs []*cidr.CIDR) net.IP {
	if !option.Config.EnableHostIPRestore {
		return nil
	}

	// Get the router IPs (`cilium_host`) from a previous Cilium run.
...
func GetK8sExternalIPv6() net.IP {
	n := localNode.Get()
	return clone(n.GetExternalIP(true))
}
