...
type addresses struct {
	ipv4Loopback      *netip.Addr
	ipv4NodePortAddrs map[string]netip.Addr // iface name => ip addr
	ipv4MasqAddrs     map[string]netip.Addr // iface name => ip addr
	ipv6NodePortAddrs map[string]netip.Addr // iface name => ip addr
...
func makeIPv6HostIP() *netip.Addr {
	ipstr := "fc00::10CA:1"
...
func InitDefaultPrefix(device string) {
	if option.Config.EnableIPv4 {
		addr, err := firstGlobalV4Addr(device, GetInternalIPv4Router(), preferPublicIP)
		if err != nil {
			return
		}
		...
		if ipv4range == nil {
			...
			if ipv6range != nil {
				v4Fromv6 := net.IPv4(
					...
					ipv6range.IP[10],
					ipv6range.IP[11])
				v4Addr, ok := netip.AddrFromSlice(v4Fromv6)
				if !ok {
					return
				}
				addr = &v4Addr
			...
			v4range := fmt.Sprintf(defaults.DefaultIPv4Prefix+"/%d",
				addr.As4()[3], defaults.DefaultIPv4PrefixLen)
			v4Net, err := netip.ParsePrefix(v4range)
			if err != nil {
				log.WithError(err).WithField(logfields.V4Prefix, v4range).Panic("BUG: Invalid default IPv4 prefix")
			}

			SetIPv4AllocRange(cidr.NewCIDRFromPrefix(&v4Net))
...
func InitNodePortAddrs(devices []string, inheritIPAddrFromDevice string) error {
	...
	if option.Config.EnableIPv4 {
		...
		addrs.ipv4NodePortAddrs = make(map[string]netip.Addr, len(devices))
	...
	if option.Config.EnableIPv6 {
		...
		addrs.ipv6NodePortAddrs = make(map[string]netip.Addr, len(devices))
		for _, device := range devices {
			if inheritIPAddrFromDevice != "" {
...
func InitBPFMasqueradeAddrs(devices []string) error {
	...
	if option.Config.EnableIPv4 {
		addrs.ipv4MasqAddrs = make(map[string]netip.Addr, len(devices))
...
func clone(ip *netip.Addr) *netip.Addr {
	if ip == nil {
		return nil
	}
	ret := *ip
...
func GetIPv4Loopback() *netip.Addr {
	addrsMu.RLock()
...
func SetIPv4Loopback(ip *netip.Addr) {
	addrsMu.Lock()
...
// for tunnel mode).
func SetInternalIPv4Router(ip *netip.Addr) {
	localNode.Update(func(n *LocalNode) {
...
func GetInternalIPv4Router() *netip.Addr {
	n := localNode.Get()
...
func SetK8sExternalIPv4(ip *netip.Addr) {
	localNode.Update(func(n *LocalNode) {
...
func GetK8sExternalIPv4() *netip.Addr {
	n := localNode.Get()
...
func GetHostMasqueradeIPv4() *netip.Addr {
...
func GetNodePortIPv4Addrs() []netip.Addr {
	addrsMu.RLock()
	defer addrsMu.RUnlock()
	addrs4 := make([]netip.Addr, 0, len(addrs.ipv4NodePortAddrs))
	for _, addr := range addrs.ipv4NodePortAddrs {
		clonedIP := clone(&addr)
		addrs4 = append(addrs4, *clonedIP)
	}
	return addrs4
}

// GetNodePortIPv4AddrsWithDevices returns the map iface => NodePort IPv4.
func GetNodePortIPv4AddrsWithDevices() map[string]netip.Addr {
	addrsMu.RLock()
	defer addrsMu.RUnlock()
...
func GetNodePortIPv6Addrs() []netip.Addr {
	addrsMu.RLock()
	defer addrsMu.RUnlock()
	addrs6 := make([]netip.Addr, 0, len(addrs.ipv6NodePortAddrs))
	for _, addr := range addrs.ipv6NodePortAddrs {
		clonedIP := clone(&addr)
		addrs6 = append(addrs6, *clonedIP)
	}
	return addrs6
}

// GetNodePortIPv6AddrsWithDevices returns the map iface => NodePort IPv6.
func GetNodePortIPv6AddrsWithDevices() map[string]netip.Addr {
	addrsMu.RLock()
	defer addrsMu.RUnlock()
...
func GetMasqIPv4AddrsWithDevices() map[string]netip.Addr {
	addrsMu.RLock()
	defer addrsMu.RUnlock()
...
func RestoreHostIPs(ipv6 bool, fromK8s, fromFS *netip.Addr, cidrs []*cidr.CIDR) *netip.Addr {
	if !option.Config.EnableHostIPRestore {
		return nil
	}

	var (
		setter func(*netip.Addr)
	)
	if ipv6 {
...
func chooseHostIPsToRestore(ipv6 bool, fromK8s, fromFS *netip.Addr, cidrs []*cidr.CIDR) (ip *netip.Addr, err error) {
	switch {
	...
	case fromK8s != nil && fromFS != nil:
		if fromK8s.Compare(*fromFS) == 0 {
			...
			for _, cidr := range cidrs {
				if cidr != nil && cidr.Contains(ip.AsSlice()) {
					return
				} else if cidr != nil && cidr.Contains(fromK8s.AsSlice()) {
					ip = fromK8s
					return
				}
			}
...
// of cilium_host device.
func SetIPv6Router(ip *netip.Addr) {
	localNode.Update(func(n *LocalNode) {
		n.SetCiliumInternalIP(ip)
	})
}
...
func GetK8sExternalIPv6() *netip.Addr {
	n := localNode.Get()
...
// on the network as well as the internet.
func SetK8sExternalIPv6(ip *netip.Addr) {
	localNode.Update(func(n *LocalNode) {
		n.SetNodeExternalIP(ip)
	})
}
...
func getCiliumHostIPsFromFile(nodeConfig string) (ipv4GW, ipv6Router *netip.Addr) {
	...
	default:
		...
		for scanner.Scan() {
			...
			case !hasIPv6 && strings.Contains(txt, defaults.RestoreV6Addr):
				...
				parsedV6, ok := netip.AddrFromSlice(ipv6)
				if !ok {
					continue
				}
				ipv6Router = &parsedV6
			...
			case !hasIPv4 && strings.Contains(txt, defaults.RestoreV4Addr):
				...
				parsedV4, ok := netip.AddrFromSlice(ipv4)
				if !ok {
					continue
				}
				ipv4GW = &parsedV4
			...
			case !hasIPv4 && strings.Contains(txt, "IPV4_GATEWAY"):
				...
				if ipv4GWUint64 != 0 {
					...
					v4IP := net.IPv4(bs[0], bs[1], bs[2], bs[3])
					parasedIP, ok := ip.AddrFromIP(v4IP)
					if !ok || !ip.IsAddrV4(&parasedIP) {
						continue
					}
					ipv4GW = &parasedIP
					hasIPv4 = true
			...
			case !hasIPv6 && strings.Contains(txt, " ROUTER_IP "):
				...
				v6addr, ok := netip.AddrFromSlice(ipv6)
				if !ok || !ip.IsAddrV6(&v6addr) {
					continue
				}
				ipv6Router = &v6addr
				hasIPv6 = true
...
func getCiliumHostIPs() (ipv4GW, ipv6Router *netip.Addr) {
...
func SetEndpointHealthIPv4(ip *netip.Addr) {
	localNode.Update(func(n *LocalNode) {
		n.IPv4HealthIP.IP = *ip
	})
}

// GetEndpointHealthIPv4 returns the IPv4 cilium-health endpoint address.
func GetEndpointHealthIPv4() *netip.Addr {
	return &localNode.Get().IPv4HealthIP.IP
}

// SetEndpointHealthIPv6 sets the IPv6 cilium-health endpoint address.
func SetEndpointHealthIPv6(ip *netip.Addr) {
	localNode.Update(func(n *LocalNode) {
		n.IPv6HealthIP = &types.Address{IP: *ip}
	})
}

// GetEndpointHealthIPv6 returns the IPv6 cilium-health endpoint address.
func GetEndpointHealthIPv6() *netip.Addr {
	return &localNode.Get().IPv6HealthIP.IP
}

// SetIngressIPv4 sets the local IPv4 source address for Cilium Ingress.
func SetIngressIPv4(ip *netip.Addr) {
	localNode.Update(func(n *LocalNode) {
		n.IPv4IngressIP = &types.Address{IP: *ip}
	})
}

// GetIngressIPv4 returns the local IPv4 source address for Cilium Ingress.
func GetIngressIPv4() *netip.Addr {
	return &localNode.Get().IPv4IngressIP.IP
}

// SetIngressIPv6 sets the local IPv6 source address for Cilium Ingress.
func SetIngressIPv6(ip *netip.Addr) {
	localNode.Update(func(n *LocalNode) {
		n.IPv6IngressIP = &types.Address{IP: *ip}
	})
}

// GetIngressIPv6 returns the local IPv6 source address for Cilium Ingress.
func GetIngressIPv6() *netip.Addr {
	return &localNode.Get().IPv6IngressIP.IP
}
...
func copyStringToNetIPMap(in map[string]netip.Addr) map[string]netip.Addr {
	out := make(map[string]netip.Addr, len(in))
	for iface, addr := range in {
		out[iface] = addr
	}
	return out
}
