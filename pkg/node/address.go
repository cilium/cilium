...
func getLocalNode() LocalNode {
	...
	if err != nil {
		...
		logging.Fatal(log, "getLocalNode: unexpected error", slog.Any(logfields.Error, err))
	}
	return n
}
...
func makeIPv6HostIP() net.IP {
	...
	if ip == nil {
		logging.Fatal(log, "Unable to parse IP", slog.String(logfields.IPAddr, ipstr))
	}
...
func setDefaultPrefix(cfg *option.DaemonConfig, device string, node *LocalNode) {
	if cfg.EnableIPv4 {
		...
		if ipv4range == nil {
			...
			if err != nil {
				logging.Panic(log, "BUG: Invalid default IPv4 prefix", slog.Any(logfields.Error, err), slog.String(logfields.V4Prefix, v4range))
			}

			node.IPv4AllocCIDR = cidr.NewCIDR(ip4net)
			log.Info(
				"Using autogenerated IPv4 allocation range",
				slog.Any(logfields.V4Prefix, node.IPv4AllocCIDR),
			)
		}
	}
	...
	if cfg.EnableIPv6 {
		...
		if ipv6range == nil && ipv4range != nil {
			...
			if err != nil {
				logging.Panic(log, "BUG: Invalid default IPv6 prefix", slog.Any(logfields.Error, err), slog.String(logfields.V4Prefix, v6range))
			}

			node.IPv6AllocCIDR = cidr.NewCIDR(ip6net)
			log.Info(
				"Using autogenerated IPv6 allocation range",
				slog.Any(logfields.V6Prefix, node.IPv6AllocCIDR),
			)
		}
	}
}
