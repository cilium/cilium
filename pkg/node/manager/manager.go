...
func NewManager(name string, datapath datapath.NodeHandler) (*Manager, error) {
	...
	if err := metrics.Register(m.metricEventsReceived); err != nil {
	...
	if err := metrics.Register(m.metricNumNodes); err != nil {
	...
	if err := metrics.Register(m.metricDatapathValidations); err != nil {
		metrics.Unregister(m.metricEventsReceived)
...
func overwriteAllowed(oldSource, newSource node.Source) bool {
	...
	case node.FromKubernetes:
		return oldSource == node.FromKubernetes
