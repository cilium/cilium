...
func TestNodeLifecycle(t *testing.T) {
	...
	mngr, err := New(logger, &option.DaemonConfig{}, tunnel.Config{}, ipcacheMock, newIPSetMock(), nil, NewNodeMetrics(), h, nil, nil, nil, fakeTypes.WireguardConfig{}, nil)
	mngr.Subscribe(dp)
...
func TestNodeEncryption(t *testing.T) {
	...
	mngr, err := New(logger, &option.DaemonConfig{
		EncryptNode: true,
		EnableIPSec: true,
	}, tunnel.Config{}, ipcacheMock, newIPSetMock(), nil, NewNodeMetrics(), h, nil, nil, nil, fakeTypes.WireguardConfig{}, nil)
	require.NoError(t, err)
	mngr.Subscribe(dp)
	defer mngr.Stop(context.TODO())
...
func TestNodeWithSameInternalIP(t *testing.T) {
	...
	}, tunnel.Config{}, ipcache, newIPSetMock(), nil, NewNodeMetrics(), h, nil, nil, nil, fakeTypes.WireguardConfig{}, nil)
	require.NoError(t, err)
	mngr.Subscribe(dp)
	defer mngr.Stop(context.TODO())
...
func TestNodeIpset(t *testing.T) {
	...
	}, tunnel.Config{}, newIPcacheMock(), newIPSetMock(), filter, NewNodeMetrics(), h, nil, nil, nil, fakeTypes.WireguardConfig{}, nil)
	mngr.Subscribe(dp)
	require.NoError(t, err)
	defer mngr.Stop(context.TODO())
