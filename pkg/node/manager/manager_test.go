...
func (s *managerTestSuite) TestNodeLifecycle(c *check.C) {
	...
	mngr, err := New("test", &configMock{}, ipcacheMock, nil)
	mngr.Subscribe(dp)
...
func (s *managerTestSuite) TestRemoteNodeIdentities(c *check.C) {
	ipcacheMock := newIPcacheMock()
	dp := newSignalNodeHandler()
	mngr, err := New("test", &configMock{RemoteNodeIdentity: true}, ipcacheMock, nil)
	c.Assert(err, check.IsNil)
	mngr.Subscribe(dp)
	defer mngr.Stop(context.TODO())
...
func (s *managerTestSuite) TestNodeEncryption(c *check.C) {
	ipcacheMock := newIPcacheMock()
	dp := newSignalNodeHandler()
	mngr, err := New("test", &configMock{NodeEncryption: true, Encryption: true}, ipcacheMock, nil)
	c.Assert(err, check.IsNil)
	mngr.Subscribe(dp)
	defer mngr.Stop(context.TODO())
