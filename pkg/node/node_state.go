	...
	for _, rt := range ns.realized.routes {
		if ns.desired.findRoute(rt) == nil {
			if err := rt.delete(); err != nil {
				log.WithError(err).WithField(logfields.Route, rt.String()).
					Warning("Unable to delete route")
			} else {
				// success, do not keep route in realized list
				continue
			}
		}

		newRealized = append(newRealized, rt)
	}

	ns.realized.routes = newRealized

	for _, rt := range ns.desired.routes {
		if ns.realized.findRoute(rt) == nil {
			if err := rt.add(); err != nil {
				log.WithError(err).WithField(logfields.Route, rt.String()).
					Warning("Unable to replace route")
			} else {
				ns.realized.routes = append(ns.realized.routes, rt)
			}
		}
	}
}
...
func (n *Node) getDatapathConfiguration() datapathConfiguration {
	...
	if n.EncapsulationEnabled() {
		...
		if n.Routing.Encapsulation != localMode {
			...
			if n.IPv4AllocCIDR != nil && n.GetIPv4() != nil {
				cfg.tunnelDestinations[n.GetIPv4().String()] = tunnelConfiguration{
					prefix:    n.IPv4AllocCIDR,
					encapType: n.Routing.Encapsulation,
				}
			}

			if n.IPv6AllocCIDR != nil && n.GetIPv6() != nil {
				cfg.tunnelDestinations[n.GetIPv6().String()] = tunnelConfiguration{
