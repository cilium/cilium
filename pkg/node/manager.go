...
func createNodeIPSecInRoute(ip *net.IPNet) routeUtils.Route {
	return routeUtils.Route{
		Nexthop: nil,
		Device:  option.IPSecRouteConfig.TunnelDevice,
		Prefix:  *ip,
		Table:   option.IPSecRouteConfig.RouteTable,
		Proto:   option.IPSecRouteConfig.RouteProtocol,
		Type:    unix.RTN_LOCAL,
	}
}
...
func createNodeIPSecOutRoute(ip *net.IPNet) routeUtils.Route {
	...
	return routeUtils.Route{
		Nexthop: &nexthop,
		Device:  option.IPSecRouteConfig.HostDevice,
		Prefix:  *ip,
		Table:   option.IPSecRouteConfig.RouteTable,
	}
}
...
func (cc *clusterConfiguation) replaceHostRules() error {
	err := routeUtils.ReplaceRule(option.IPSecRouteConfig.RouteDecrypt,
		option.IPSecRouteConfig.RouteTable)
	if err != nil {
		log.WithError(err).Error("Replace route rule failed")
		return err
	}
	err = routeUtils.ReplaceRule(option.IPSecRouteConfig.RouteEncrypt,
		option.IPSecRouteConfig.RouteTable)
	if err != nil {
		log.WithError(err).Error("Replace route rule failed")
	}
