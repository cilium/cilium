...
func (s *VersionedSuite) TestSyncComparableMap_DoLocked(c *check.C) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				iterate: func(key UUID, value Object) {
					wantValue := m.Map[key]
					c.Assert(wantValue, check.DeepEquals, value,
						check.Commentf("%s", "provided key is different than given value"))
	...
	for _, tt := range tests {
		...
		for _, v := range tt.functionsCalled {
			...
			case "iterate":
				fooObj, _ := sm.Get("foo")
				c.Assert(fooObj.Version, check.Equals, Version(1),
					check.Commentf("%s", "iterate modified the object and it shouldn't"))
			}
		}
	}
}
