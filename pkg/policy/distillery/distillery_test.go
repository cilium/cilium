...
func (repo *testPolicyRepo) ResolvePolicyLocked(*identityPkg.Identity) (*policy.IdentityPolicy, error) {
	return policy.NewIdentityPolicy(), repo.err
}
...
func (s *DistilleryTestSuite) TestCacheManagement(c *C) {
	...
	// TODO: If we implement deferred IdentityPolicy expiry, then
...
func (s *DistilleryTestSuite) TestCachePopulation(c *C) {
	...
	updated, err := cache.updateIdentityPolicy(repo, ep1)
	c.Assert(err, IsNil)
	...
	c.Assert(err, IsNil)
	c.Assert(updated, Equals, true)
	idp3 := policy3.(*cachedIdentityPolicy).getPolicy()
