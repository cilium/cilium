...
func (d *policyDistillery) distillPolicy(epLabels labels.LabelArray) (MapState, error) {
	...
	// Handle ingress from each identity in the cache to the endpoint.
	io.WriteString(d.log, "[distill] Producing filter keys\n")
	for _, l4 := range *l4IngressPolicy {
		io.WriteString(d.log, fmt.Sprintf("[distill] Processing Filter (l3: %+v), (l4: %d/%s), (l7: %+v)\n", l4.Endpoints, l4.Port, l4.Protocol, l4.L7RulesPerEp))
		for _, key := range l4.ToKeys(0, d.identityCache) {
			io.WriteString(d.log, fmt.Sprintf("[distill] ingress allow %+v (parser=%s, redirect=%t)\n", key, l4.L7Parser, l4.IsRedirect()))
			if l4.IsRedirect() {
