...
func TestCreateL4Filter(t *testing.T) {
	...
	for _, selector := range selectors {
		...
		filter, err := createL4IngressFilter(td.testPolicyContext, eps, nil, nil, portrule, tuple, tuple.Protocol, labels.Empty)
		require.NoError(t, err)
		...
		filter, err = createL4EgressFilter(td.testPolicyContext, eps, nil, portrule, tuple, tuple.Protocol, labels.Empty, nil)
		require.NoError(t, err)
		require.Len(t, filter.PerSelectorPolicies, 1)
		for _, r := range filter.PerSelectorPolicies {
			explicit, authType := r.getAuthType()
...
func TestCreateL4FilterAuthRequired(t *testing.T) {
	...
	for _, selector := range selectors {
		...
		// a single L7 rule whether the selector is wildcarded
		// or if it is based on specific labels.
		filter, err := createL4IngressFilter(td.testPolicyContext, eps, auth, nil, portrule, tuple, tuple.Protocol, labels.Empty)
		require.NoError(t, err)
		require.Len(t, filter.PerSelectorPolicies, 1)
		for _, r := range filter.PerSelectorPolicies {
			explicit, authType := r.getAuthType()
		...
		filter, err = createL4EgressFilter(td.testPolicyContext, eps, auth, portrule, tuple, tuple.Protocol, labels.Empty, nil)
		require.NoError(t, err)
		require.Len(t, filter.PerSelectorPolicies, 1)
		for _, r := range filter.PerSelectorPolicies {
			explicit, authType := r.getAuthType()
...
func TestCreateL4FilterMissingSecret(t *testing.T) {
	...
	for _, selector := range selectors {
		...
		// a single L7 rule whether the selector is wildcarded
		// or if it is based on specific labels.
		_, err := createL4IngressFilter(td.testPolicyContext, eps, nil, nil, portrule, tuple, tuple.Protocol, labels.Empty)
		require.Error(t, err)

		_, err = createL4EgressFilter(td.testPolicyContext, eps, nil, portrule, tuple, tuple.Protocol, labels.Empty, nil)
		require.Error(t, err)
	}
}
