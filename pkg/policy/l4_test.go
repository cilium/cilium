...
func (s *PolicyTestSuite) TestCreateL4Filter(c *C) {
	...
	for _, selector := range selectors {
		...
		filter := createL4IngressFilter(eps, nil, portrule, tuple, tuple.Protocol, nil, testSelectorCache)
	...
	filter := createL4IngressFilter(nil, nil, portrule, tuple, tuple.Protocol, nil, testSelectorCache)
	c.Assert(len(filter.L7RulesPerEp), Equals, 1)
