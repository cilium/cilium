...
func (ds *PolicyTestSuite) TestComputePolicyEnforcementAndRules(c *C) {
	...
	dummyEndpointID := uint16(7777)

	convertedFooIngressRule1 := &rule{
		Rule: fooIngressRule1,
		metadata: &ruleMetadata{
			EndpointsSelected: map[uint16]*identity.Identity{
				dummyEndpointID: fooIdentity,
			},
	...
	convertedFooIngressRule2 := &rule{
		Rule: fooIngressRule2,
		metadata: &ruleMetadata{
			EndpointsSelected: map[uint16]*identity.Identity{
				dummyEndpointID: fooIdentity,
			},
	...
	_, _, err := repo.Add(fooIngressRule1, []IdentityConsumer{})
	...
	_, _, err = repo.Add(fooIngressRule2, []IdentityConsumer{})
	c.Assert(err, IsNil, Commentf("unable to add rule to policy repository"))
	ing, egr, matchingRules = repo.computePolicyEnforcementAndRules(dummyEndpointID, fooIdentity)
	c.Assert(ing, Equals, true, Commentf("ingress policy enforcement should apply since ingress rule selects"))
	c.Assert(egr, Equals, false, Commentf("egress policy enforcement should not apply since no egress rules select"))
	...
	_, _, err = repo.Add(fooEgressRule1, []IdentityConsumer{})
	c.Assert(err, IsNil, Commentf("unable to add rule to policy repository"))
	ing, egr, matchingRules = repo.computePolicyEnforcementAndRules(dummyEndpointID, fooIdentity)
	...
	ingress, egress, matchingRules := repo.computePolicyEnforcementAndRules(dummyEndpointID, initIdentity)
	...
	SetPolicyEnabled(option.AlwaysEnforce)
	ingress, egress, _ = repo.computePolicyEnforcementAndRules(dummyEndpointID, initIdentity)
	c.Assert(ingress, Equals, true)
	c.Assert(egress, Equals, true)
...
func (ds *PolicyTestSuite) TestAddSearchDelete(c *C) {
	...
	rev, _, err := repo.Add(api.Rule{}, []IdentityConsumer{})
	...
	// add rule1,rule2
	rev, _, err = repo.Add(rule1, []IdentityConsumer{})
	c.Assert(err, IsNil)
	c.Assert(rev, Equals, nextRevision)
	nextRevision++
	rev, _, err = repo.Add(rule2, []IdentityConsumer{})
	c.Assert(err, IsNil)
	c.Assert(rev, Equals, nextRevision)
	nextRevision++
	...
	// add rule3
	rev, _, err = repo.Add(rule3, []IdentityConsumer{})
	c.Assert(err, IsNil)
	c.Assert(rev, Equals, nextRevision)
	nextRevision++
...
func (ds *PolicyTestSuite) TestContainsAllRLocked(c *C) {
	...
	_, _, err := repoA.Add(rule1, []IdentityConsumer{})
	c.Assert(err, IsNil)
	_, _, err = repoA.Add(rule2, []IdentityConsumer{})
	c.Assert(err, IsNil)
	_, _, err = repoA.Add(rule3, []IdentityConsumer{})
	c.Assert(err, IsNil)
	...
	_, _, err = repoB.Add(rule4, []IdentityConsumer{})
	c.Assert(err, IsNil)
	_, _, err = repoB.Add(rule5, []IdentityConsumer{})
	c.Assert(err, IsNil)
	...
	_, _, err = repoEmpty.Add(rule6, []IdentityConsumer{})
	c.Assert(err, IsNil)
...
func (ds *PolicyTestSuite) TestCanReachIngress(c *C) {
	...
	c.Assert(err, IsNil)
	_, _, err = repo.Add(rule2, []IdentityConsumer{})
	c.Assert(err, IsNil)
	_, _, err = repo.Add(rule3, []IdentityConsumer{})
	c.Assert(err, IsNil)
...
func (ds *PolicyTestSuite) TestWildcardL3RulesIngress(c *C) {
	...
	l3Rule.Sanitize()
	_, _, err := repo.Add(l3Rule, []IdentityConsumer{})
	c.Assert(err, IsNil)
	...
	kafkaRule.Sanitize()
	_, _, err = repo.Add(kafkaRule, []IdentityConsumer{})
	c.Assert(err, IsNil)
	...
	_, _, err = repo.Add(httpRule, []IdentityConsumer{})
	c.Assert(err, IsNil)
	...
	_, _, err = repo.Add(l7Rule, []IdentityConsumer{})
	c.Assert(err, IsNil)
...
func (ds *PolicyTestSuite) TestWildcardL4RulesIngress(c *C) {
	...
	l49092Rule.Sanitize()
	_, _, err := repo.Add(l49092Rule, []IdentityConsumer{})
	c.Assert(err, IsNil)
	...
	l480Rule.Sanitize()
	_, _, err = repo.Add(l480Rule, []IdentityConsumer{})
	c.Assert(err, IsNil)
...
`
	...
	_, _, err := repo.Add(l4rule, []IdentityConsumer{})
	c.Assert(err, IsNil)
