...
func (in *PolicyEntry) DeepEqual(other *PolicyEntry) bool {
	...
	if len(in.L3) != len(other.L3) {
		return false
	}
	for i := range in.L3 {
		switch v1 := in.L3[i].(type) {
		case api.EndpointSelector:
			if v2, ok := other.L3[i].(api.EndpointSelector); !ok || !v1.DeepEqual(&v2) {
				return false
			}
		case api.FQDNSelector:
			if v2, ok := other.L3[i].(api.FQDNSelector); !ok || !v1.DeepEqual(&v2) {
				return false
			}
		case api.CIDR:
			if v2, ok := other.L3[i].(api.CIDR); !ok || v1 != v2 {
				return false
			}
		case api.CIDRRule:
			if v2, ok := other.L3[i].(api.CIDRRule); !ok || !v1.DeepEqual(&v2) {
				return false
			}
		default:
			return false
