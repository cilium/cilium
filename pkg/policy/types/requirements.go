...
func MatchesRequirement[T labels.LabelMatcher](logger *slog.Logger, r *Requirement, ls T) bool {
...
// overhead. When the compiler specializes the generic type to a concrete type (not an interface),
// it can perform correct escape analysis and avoid unnecessary heap allocations.
func MatchesRequirements[T labels.LabelMatcher](logger *slog.Logger, reqs Requirements, ls T) bool {
	for i := range reqs {
		if !MatchesRequirement(logger, &reqs[i], ls) {
			return false
		}
	}
	return true
