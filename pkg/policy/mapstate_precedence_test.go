...
func TestOrderedPolicyValidation(t *testing.T) {
	...
	for _, tt := range tests {
		// Run each test for every possible order of rules
		// to make sure that policy application is truly commutative
		i := 0
		Perm(tt.entries, func(entries []*types.PolicyEntry) {
			t.Run(fmt.Sprintf("%s/perm-%d", tt.name, i), func(t *testing.T) {
				logger := hivetest.Logger(t)
