...
func reportCNPChangeMetrics(op string, err error) {
	if err != nil {
		metrics.PolicyChangeTotal.WithLabelValues(op, string(source.Directory), metrics.LabelValueOutcomeFail).Inc()
	} else {
		metrics.PolicyChangeTotal.WithLabelValues(op, string(source.Directory), metrics.LabelValueOutcomeSuccess).Inc()
	}
}
