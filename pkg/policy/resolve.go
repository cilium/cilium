...
func (p *selectorPolicy) Delete() {
	if p.L4Policy != nil {
		if p.SelectorCache == nil {
			panic("nil selector cache with L4 policy")
		}
		p.L4Policy.Delete(p.SelectorCache)
	}
}
...
func (p *selectorPolicy) DistillPolicy(policyOwner PolicyOwner) *EndpointPolicy {
	...
	if !p.IngressPolicyEnabled {
		calculatedPolicy.PolicyMapState.AllowAllIdentities(p.SelectorCache, trafficdirection.Ingress)
	}

	if !p.EgressPolicyEnabled {
		calculatedPolicy.PolicyMapState.AllowAllIdentities(p.SelectorCache, trafficdirection.Egress)
	}
