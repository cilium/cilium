...
// AllowAllIdentities fills in this EndpointPolicy's map state with wildcard
// allow entries for the given directions, mirroring what DistillPolicy does
// for a SelectorPolicy with policy enforcement disabled.
//
// This is used to repair an empty realized EndpointPolicy stub (as created by
// NewEndpointPolicy) so that it accurately reflects a "policy enforcement
// disabled" BPF policy map, rather than an empty one. Without this, reverting
// to such a stub after a first-regeneration failure would cause
// syncPolicyMapWith to delete the wildcard entries a pre-compile sync had
// already written to the BPF map, leaving the datapath dropping all traffic.
func (p *EndpointPolicy) AllowAllIdentities(ingress, egress bool) {
	p.policyMapState.allowAllIdentities(ingress, egress)
}
