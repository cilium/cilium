...
func mergeL4IngressPort(ctx *SearchContext, endpoints []api.EndpointSelector, endpointsWithL3Override []labels.LabelArray, r api.PortRule, p api.PortProtocol,
...
func mergeL4Ingress(ctx *SearchContext, rule api.IngressRule, ruleLabels labels.LabelArray, resMap L4PolicyMap) (int, error) {
	...
	// proxy
	endpointsWithL3Override := []labels.LabelArray{}
	if option.Config.AlwaysAllowLocalhost() {
		hostLabels := labels.LabelArray{labels.NewLabel(labels.IDNameHost, "", labels.LabelSourceReserved)}
		endpointsWithL3Override = append(endpointsWithL3Override, hostLabels)
		if option.Config.HostAllowsWorld {
			worldLabels := labels.LabelArray{labels.NewLabel(labels.IDNameWorld, "", labels.LabelSourceReserved)}
			endpointsWithL3Override = append(endpointsWithL3Override, worldLabels)
		}
	}
