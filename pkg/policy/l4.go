...
func (l4 *L4Filter) IdentitySelectionSync(handleFunc GetHandleFunc) {
	...
	l4Policy := l4.policy.Load()
	if l4Policy != nil {
		l4Policy.SyncMapChanges(l4, handleFunc)
	}
}
...
func (l4Policy *L4Policy) SyncMapChanges(l4 *L4Filter, handleFunc GetHandleFunc) {
	// SelectorCache may not be called into while holding this lock!
	l4Policy.mutex.RLock()
	for epPolicy := range l4Policy.users {
		// Skip if endpoint has no policy maps
		...
		if option.Config.Debug {
			log.Debug("SyncMapChanges")
		}
		epPolicy.policyMapChanges.SyncMapChanges(handleFunc)
	}
	l4Policy.mutex.RUnlock()
}
