...
func (l4 *L4Filter) toMapState(p *EndpointPolicy, features policyFeatures, changes ChangeState) {
	...
	var wildcardEntry *mapStateEntry
	if l4.wildcard != nil {
	...
	for cs, currentRule := range l4.PerSelectorPolicies {
		...
		if wildcardEntry != nil && port != 0 && entry.MapStateEntry == wildcardEntry.MapStateEntry {
			logger.WithField(logfields.EndpointSelector, cs).Debug("ToMapState: Skipping L3/L4 key due to existing identical L4-only key")
			continue
		}
