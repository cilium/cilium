...
// AWSGroup is the structure that can be used to whitelisting information from AWS integration
...
func (group *ToGroups) GetCidrSet() ([]CIDRRule, error) {
	...
	if group.AWS != nil {
		...
		if !ok {
			return emptyResult, fmt.Errorf("Provider callback for %s is not a valid instance", AWSProvider)
		}
