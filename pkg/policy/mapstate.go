...
func (keys MapState) AddVisibilityKeys(e PolicyOwner, redirectPort uint16, visMeta *VisibilityMetadata, oldMapState, insertedMapState MapState) {
	...
	entry := NewMapStateEntry(nil, derivedFrom, true, false)
	...
	for k, v := range keys {
		...
		if k.DestPort == key.DestPort && k.Nexthdr == key.Nexthdr {
			...
			//
			k.DestPort = key.DestPort
			k.Nexthdr = key.Nexthdr
