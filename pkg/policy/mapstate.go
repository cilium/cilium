...
func (ms *mapState) Get(k Key) (bool, MapStateEntry) {
	if ms == nil || ms.keys == nil {
		return false, MapStateEntry{}
	}
	v, ok := ms.keys[k]
	return ok, v
}
...
// ForEach iterates over every Key MapStateEntry and
// stops when the function argument returns false
func (ms *mapState) ForEach(f func(Key, MapStateEntry) bool) {
	if ms == nil || ms.keys == nil {
		return
	}
...
func (msA *mapState) Equals(msB MapState) bool {
	for kA, vA := range msA.keys {
		if ok, vB := msB.Get(kA); ok {
	...
	msB.ForEach(func(kB Key, vB MapStateEntry) bool {
		if vA, ok := msA.keys[kB]; ok {
			if !(&vA).DatapathEqual(&vB) {
				res = false
				return false
			}
		} else {
