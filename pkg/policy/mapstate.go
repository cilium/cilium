...
type mapState struct {
	logger *slog.Logger
	// whether or not semantic identity aggregation is enabled.
...
func (ms *mapState) forCoveredIDs(agg identity.NumericIdentity, k Key, idSet IDSet, f func(Key, mapStateEntry) bool) bool {
	for id := range idSet {
		if aggregates(ms.enableAggregation, agg, id) {
			k.Identity = id
			if !ms.forKey(k, f) {
				return false
			}
...
func (ms *mapState) CoveringBroaderOrEqualKeys(key Key) iter.Seq2[Key, mapStateEntry] {
	agg := aggregateFor(ms.enableAggregation, key.Identity)
...
func (ms *mapState) NarrowerOrEqualKeys(key Key) iter.Seq2[Key, mapStateEntry] {
	wc := aggregateFor(ms.enableAggregation, key.Identity)
	return func(yield func(Key, mapStateEntry) bool) {
		iter := ms.trie.DescendantIterator(key.PrefixLength(), key.LPMKey)
		for ok, lpmKey, idSet := iter.Next(); ok; ok, lpmKey, idSet = iter.Next() {
			k := Key{LPMKey: lpmKey}
...
func emptyMapState(logger *slog.Logger) mapState {
	return newMapState(logger, true /* always enable aggregation for tests */, nil, 0)
}
...
func newMapState(logger *slog.Logger, enableAggregation bool, old *mapState, features policyFeatures) mapState {
	var nEntries int
...
func (ms *mapState) pruneCoveredNarrowerKey(k Key, v mapStateEntry, key Key, entry mapStateEntry, precedence types.Precedence, changes ChangeState) {
	...
	deleteEntry = deleteEntry || (key.LPMKey == k.LPMKey && aggregates(ms.enableAggregation, key.Identity, k.Identity) && v.equivalent(entry))
...
func (sp *keySlice) collectNarrowerPasses(aggregation bool, tierMaxPrecedence types.Precedence, k Key, v mapStateEntry, key Key, doneKeys *Keys) {
	// k has narrower L4, but the narrower identity may be on 'key'
	if k.Identity == aggregateFor(aggregation, key.Identity) {
...
func (ms *mapState) insertWithPasses(tierMaxPrecedence types.Precedence, key Key, entry mapStateEntry, changes ChangeState) {
	...
	var keys keySlice
	aggregateID := aggregateFor(ms.enableAggregation, key.Identity)
	...
	for k, v := range ms.NarrowerOrEqualKeys(key) {
		...
		keys.collectNarrowerPasses(ms.enableAggregation, tierMaxPrecedence, k, v, key, &doneKeys)
	}
...
func (ms *mapState) aggregateIsEquivalent(newKey Key, newEntry mapStateEntry) bool {
	agg := aggregateFor(ms.enableAggregation, newKey.Identity)
...
func (ms *mapState) pruneAggregated(newKey Key, newEntry mapStateEntry, changes ChangeState) {
	...
	if !isAggregate(ms.enableAggregation, newKey.Identity) {
		return
	}
	...
	for id := range idSet {
		if !aggregates(ms.enableAggregation, newKey.Identity, id) {
