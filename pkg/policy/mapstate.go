...
func (ms *mapState) authPreferredInsert(newKey Key, newEntry mapStateEntry, features policyFeatures, changes ChangeState) {
	...
	var shouldBail, mustKeep, derived bool
	...
	for k, v := range ms.CoveringKeysWithSameID(newKey) {
		...
		// explicit auth type
		if derived {
			// already derived but may still find a covering entry with higher proxy
			// port priority to propagate
			continue
		}
		if !newEntryHasExplicitAuth && !k.PortProtoIsEqual(newKey) && v.AuthRequirement.IsExplicit() {
	...
	for k, v := range ms.SubsetKeysWithSameID(newKey) {
		...
		if newEntryHasExplicitAuth && !k.PortProtoIsEqual(newKey) {
