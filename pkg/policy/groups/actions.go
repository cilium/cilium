...
func UpdateDerivativeCNPIfNeeded(newCNP *cilium_v2.CiliumNetworkPolicy, oldCNP *cilium_v2.CiliumNetworkPolicy) bool {
	if !newCNP.HasDerivatives() && oldCNP.HasDerivatives() {
		...
		DeleteDerivativeCNPIfNeeded(oldCNP)
		return false
	}
...
// DeleteDerivativeCNPIfNeeded if the given policy has any derivatives policy will be
// deleted from the repo and the cache.
func DeleteDerivativeCNPIfNeeded(cnp *cilium_v2.CiliumNetworkPolicy) error {
