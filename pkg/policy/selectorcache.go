...
// The caller is responsible for making sure the same identity is not
// present in both 'added' and 'deleted'.
func (s *fqdnSelector) notifyUsers(added, deleted []identity.NumericIdentity) {
	for user := range s.users {
		// pass 's' to the user as '*fqdnSelector'
...
// The caller is responsible for making sure the same identity is not
// present in both 'added' and 'deleted'.
func (s *labelIdentitySelector) notifyUsers(added, deleted []identity.NumericIdentity) {
	for user := range s.users {
		// pass 's' to the user as '*labelIdentitySelector'
		user.IdentitySelectionUpdated(s, s.GetSelections(), added, deleted)
	}
}
