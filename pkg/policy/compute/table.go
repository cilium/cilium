...
var (
	PolicyComputationNameIndex = statedb.Index[Result, identity.NumericIdentity]{
		...
		FromObject: func(r Result) index.KeySet {
			return index.NewKeySet(index.Uint32(uint32(r.Identity.ID)))
		},
...
func newTable(db *statedb.DB) (statedb.RWTable[Result], error) {
	tbl, _, err := NewPolicyComputationTable(db)
	if err != nil {
		return nil, err
	}
