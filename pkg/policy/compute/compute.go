...
type PolicyRecomputer interface {
	...
	RecomputeIdentityPolicyForAllIdentities(toRev uint64) (<-chan struct{}, error)
...
// RecomputeIdentityPolicy recomputes the policy for a specific identity.
func (r *IdentityPolicyRecomputer) RecomputeIdentityPolicyForAllIdentities(toRev uint64) (<-chan struct{}, error) {
	...
	for _, id := range r.idmanager.GetAll() {
		r.RecomputeIdentityPolicy(id, toRev)
	}
	// TODO: Make this return real objects.
	return nil, nil
}
