...
func (td *testData) checkL4PolicyMapEqual(expected, actual L4PolicyMap, availableIDs ...identity.NumericIdentity) bool {
	...
	expected.ForEach(func(l4 *L4Filter) bool {
		...
		// cached selector is used for both l4PolicyMaps to validate.
		...
		if l4.Port != l4B.Port ||
			l4.EndPort != l4B.EndPort ||
			l4.PortName != l4B.PortName ||
			l4.Protocol != l4B.Protocol ||
			l4.Ingress != l4B.Ingress ||
			l4.wildcard != l4B.wildcard {
			equal = false
			return equal
		}

		if len(l4.PerSelectorPolicies) != len(l4B.PerSelectorPolicies) {
			equal = false
			return equal
		}
		...
		for k, v := range l4.PerSelectorPolicies {
			...
			for bK, bV := range l4B.PerSelectorPolicies {
				if k.String() == bK.String() && bV.Equal(v) {
					...
					if maps.Equal(selExpected, selActual) {
						verified = true
					}
				}
			}
...
func (td *testData) validateResolvedPolicy(t *testing.T, id *identity.Identity, expectedIn, expectedOut L4PolicyMap) {
	...
	if expectedIn != nil {
		require.True(t, td.checkL4PolicyMapEqual(expectedIn, pol.L4Policy.Ingress.PortRules, td.idSet.AsSlice()...))
	}

	if expectedOut != nil {
		require.True(t, td.checkL4PolicyMapEqual(expectedOut, pol.L4Policy.Egress.PortRules, td.idSet.AsSlice()...))
	}
}
