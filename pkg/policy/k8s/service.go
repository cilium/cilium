...
func (p *policyWatcher) onServiceEvent(event k8s.ServiceNotification) {
	err := p.updateMatchedServicesPolicies(event.ID, event.Service, event.OldService)
	if err != nil {
...
// updateMatchedServicesPolicies is to be invoked when a service has changed (i.e. it was
...
func (p *policyWatcher) updateMatchedServicesPolicies(svcID k8s.ServiceID, newSVC, oldSVC *k8s.Service) error {
	var errs []error
	...
	candidatePolicyKeys := p.matchedServicesPolicies
