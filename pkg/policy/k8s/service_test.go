...
func TestPolicyWatcher_updateToServicesPolicies(t *testing.T) {
	...
	assert.Equal(t, p.matchedServicesPolicies, map[resource.Key]struct{}{
		svcByNameKey:  {},
		svcByLabelKey: {},
	})
	...
	err = p.updateMatchedServicesPolicies(fooSvcID, fooSvc, nil)
	assert.NoError(t, err)
	rules = <-policyAdd
	assert.Len(t, rules, 2)
	...
	err = p.updateMatchedServicesPolicies(barSvcID, barSvc, nil)
	assert.NoError(t, err)
	...
	err = p.updateMatchedServicesPolicies(fooSvcID, fooSvc, fooSvc)
	assert.NoError(t, err)
	...
	barSvc.Labels = nil
	err = p.updateMatchedServicesPolicies(barSvcID, barSvc, oldBarSvc)
	assert.NoError(t, err)
	...
	err = p.updateMatchedServicesPolicies(bazSvcID, bazSvc, nil)
	assert.NoError(t, err)
	rules = <-policyAdd
	assert.Len(t, rules, 1)
	// Check that Spec was translated
...
func TestPolicyWatcher_updateToServicesPoliciesTransformToEndpoint(t *testing.T) {
	...
	err = p.updateMatchedServicesPolicies(fooSvcID, fooSvc, oldFooSvc)
	assert.NoError(t, err)
	rules = <-policyAdd
	assert.Len(t, rules, 1)
	assert.Len(t, rules[0].Egress, 1)
...
func TestPolicyWatcher_updateFromServicesPolicies(t *testing.T) {
	policyAdd := make(chan api.Rules, 3)
	policyManager := &fakePolicyManager{
		OnPolicyAdd: func(rules api.Rules, opts *policy.AddOptions) (newRev uint64, err error) {
			policyAdd <- rules
	...
	// Upsert policies. No services are known, so generated FromCIDRSet should be empty
	err := p.onUpsert(svcByNameCNP, svcByNameKey, k8sAPIGroupCiliumNetworkPolicyV2, svcByNameResourceID)
	assert.NoError(t, err)
	rules := <-policyAdd
	assert.Len(t, rules, 2)
	assert.Len(t, rules[0].Ingress, 1)
	assert.Empty(t, rules[0].Ingress[0].FromCIDRSet)
	assert.Len(t, rules[1].Ingress, 1)
	assert.Empty(t, rules[1].Ingress[0].FromCIDRSet)
	...
	// Check that policies are recognized as FromServices policies
	assert.Equal(t, p.matchedServicesPolicies, map[resource.Key]struct{}{
		svcByNameKey:  {},
		svcByLabelKey: {},
	})
	...
	assert.Len(t, rules[0].Ingress, 1)
	assert.Contains(t, rules[0].Labels, svcByNameLbl)
	assert.Equal(t, svcByNameCNP.Spec.Ingress[0].FromServices, rules[0].Ingress[0].FromServices)
	assert.Equal(t, sortCIDRSet(rules[0].Ingress[0].FromCIDRSet), api.CIDRRuleSlice{
		addrToCIDRRule(fooEpAddr1.Addr()),
		addrToCIDRRule(fooEpAddr2.Addr()),
	})
	...
	assert.Len(t, rules[1].Ingress, 1)
	assert.Contains(t, rules[1].Labels, svcByNameLbl)
	assert.Equal(t, svcByNameCNP.Specs[0].Ingress[0].FromServices, rules[1].Ingress[0].FromServices)
	assert.Equal(t, sortCIDRSet(rules[1].Ingress[0].FromCIDRSet), api.CIDRRuleSlice{
		addrToCIDRRule(fooEpAddr1.Addr()),
		addrToCIDRRule(fooEpAddr2.Addr()),
	})
	...
	// Check that svcByNameCNP Spec (matching foo and bar) was translated
	assert.Len(t, byNameRules, 2)
	assert.Len(t, byNameRules[0].Ingress, 1)
	assert.Contains(t, byNameRules[0].Labels, svcByNameLbl)
	assert.Equal(t, svcByNameCNP.Spec.Ingress[0].FromServices, byNameRules[0].Ingress[0].FromServices)
	assert.Equal(t, sortCIDRSet(byNameRules[0].Ingress[0].FromCIDRSet), api.CIDRRuleSlice{
		addrToCIDRRule(fooEpAddr1.Addr()),
		addrToCIDRRule(fooEpAddr2.Addr()),
		addrToCIDRRule(barEpAddr.Addr()),
	})
	...
	// Check that svcByNameCNP Specs (matching only foo) was translated
	assert.Len(t, byNameRules[1].Ingress, 1)
	assert.Contains(t, byNameRules[1].Labels, svcByNameLbl)
	assert.Equal(t, svcByNameCNP.Specs[0].Ingress[0].FromServices, byNameRules[1].Ingress[0].FromServices)
	assert.Equal(t, sortCIDRSet(byNameRules[1].Ingress[0].FromCIDRSet), api.CIDRRuleSlice{
		addrToCIDRRule(fooEpAddr1.Addr()),
		addrToCIDRRule(fooEpAddr2.Addr()),
	})
	...
	// Check that svcByLabelCNP Spec (matching only bar) was translated
	assert.Len(t, byLabelRules, 1)
	assert.Len(t, byLabelRules[0].Ingress, 1)
	assert.Contains(t, byLabelRules[0].Labels, svcByLabelLbl)
	assert.Equal(t, svcByLabelCNP.Spec.Ingress[0].FromServices, byLabelRules[0].Ingress[0].FromServices)
	assert.Equal(t, byLabelRules[0].Ingress[0].FromCIDRSet, api.CIDRRuleSlice{
		addrToCIDRRule(barEpAddr.Addr()),
	})
	...
	// Check that svcByNameCNP has not changed
	assert.Equal(t,
		sortCIDRSet(byNameRules[0].Ingress[0].FromCIDRSet),
		sortCIDRSet(oldByNameRules[0].Ingress[0].FromCIDRSet))
	assert.Equal(t,
		sortCIDRSet(byNameRules[1].Ingress[0].FromCIDRSet),
		sortCIDRSet(oldByNameRules[1].Ingress[0].FromCIDRSet))
	...
	assert.Contains(t, byLabelRules[0].Labels, svcByLabelLbl)
	assert.Equal(t, svcByLabelCNP.Spec.Ingress[0].FromServices, byLabelRules[0].Ingress[0].FromServices)
	assert.Empty(t, byLabelRules[0].Ingress[0].FromCIDRSet)
...
func Test_hasMatchingFromServices(t *testing.T) {
	type args struct {
		spec  *api.Rule
		svcID k8s.ServiceID
		svc   *k8s.Service
	...
	tests := []struct {
		...
		},
		{
			name: "by name and namespace",
			args: args{
				spec: &api.Rule{Ingress: []api.IngressRule{
					{
						IngressCommonRule: api.IngressCommonRule{
							FromServices: []api.Service{
								{K8sService: &api.K8sServiceNamespace{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equalf(t, tt.want, hasMatchingFromServices(tt.args.spec, tt.args.svcID, tt.args.svc), "hasMatchingFromServices(%v, %v, %v)", tt.args.spec, tt.args.svcID, tt.args.svc)
		})
	}
}
