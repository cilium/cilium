...
func TestPolicyWatcher_updateToServicesPolicies(t *testing.T) {
	...
	assert.Equal(t, p.matchServicesPolicies, map[resource.Key]struct{}{
		svcByNameKey:  {},
		svcByLabelKey: {},
	})
	...
	err = p.updateMatchServicesPolicies(fooSvcID, fooSvc, nil)
	assert.NoError(t, err)
	rules = <-policyAdd
	assert.Len(t, rules, 2)
	...
	err = p.updateMatchServicesPolicies(barSvcID, barSvc, nil)
	assert.NoError(t, err)
	...
	err = p.updateMatchServicesPolicies(fooSvcID, fooSvc, fooSvc)
	assert.NoError(t, err)
	...
	barSvc.Labels = nil
	err = p.updateMatchServicesPolicies(barSvcID, barSvc, oldBarSvc)
	assert.NoError(t, err)
	...
	err = p.updateMatchServicesPolicies(bazSvcID, bazSvc, nil)
	assert.NoError(t, err)
