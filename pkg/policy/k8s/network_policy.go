...
func (p *policyWatcher) addK8sNetworkPolicyV1(k8sNP *slim_networkingv1.NetworkPolicy, apiGroup string, dc chan uint64) error {
	...
	if err != nil {
		...
		p.log.Error(
			...
			slog.String(logfields.K8sAPIVersion, k8sNP.TypeMeta.APIVersion),
			slog.Any(logfields.CiliumNetworkPolicy, k8sNP),
		)
		return err
	}
	...
	p.log.Info(
		...
		slog.String(logfields.K8sNetworkPolicyName, k8sNP.ObjectMeta.Name),
		slog.String(logfields.K8sAPIVersion, k8sNP.TypeMeta.APIVersion),
	)
	return nil
}
...
func (p *policyWatcher) deleteK8sNetworkPolicyV1(k8sNP *slim_networkingv1.NetworkPolicy, apiGroup string, dc chan uint64) error {
	...
	p.log.Info(
		...
		slog.String(logfields.K8sNetworkPolicyName, k8sNP.ObjectMeta.Name),
		slog.String(logfields.K8sNamespace, k8sNP.ObjectMeta.Namespace),
		slog.String(logfields.K8sAPIVersion, k8sNP.TypeMeta.APIVersion),
