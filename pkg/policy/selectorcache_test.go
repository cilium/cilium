...
func (csu *cachedSelectionUser) AddFQDNSelector(sel api.FQDNSelector) CachedSelector {
	notifications := csu.notifications
	cached, added := csu.sc.AddFQDNSelector(csu, sel.String())
	csu.c.Assert(cached, Not(Equals), nil)
...
func (ds *SelectorCacheTestSuite) TestFQDNSelectorUpdates(c *C) {
	...
	googleIdentities := []identity.NumericIdentity{456, 987, 321}
	ciliumIdentities := []identity.NumericIdentity{123, 456, 789}

	sc.UpdateFQDNSelector(ciliumSel.String(), ciliumIdentities)
	sc.UpdateFQDNSelector(googleSel.String(), googleIdentities)
	...
	c.Assert(selections2[0], Equals, identity.NumericIdentity(321))
	...
	_, added := sc.AddFQDNSelector(user1, yahooSel.String())
