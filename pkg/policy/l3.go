...
func (cp *CIDRPolicy) Validate() error {
	...
	if l := len(cp.Egress.IPv6PrefixCount); l > v3.MaxCIDRPrefixLengths {
		return fmt.Errorf("too many egress CIDR prefix lengths %d/%d", l, v3.MaxCIDRPrefixLengths)
	}
	if l := len(cp.Ingress.IPv6PrefixCount); l > v3.MaxCIDRPrefixLengths {
		return fmt.Errorf("too many ingress CIDR prefix lengths %d/%d", l, v3.MaxCIDRPrefixLengths)
	}
	return nil
}
