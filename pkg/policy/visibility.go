	...
	for i := range anSplit {
		...
		if res, ok := dvp[pp]; ok {
			if res.Parser != l7Protocol {
				return nil, fmt.Errorf("duplicate annotations with different L7 protocols %s and %s for %s", res, l7Protocol, pp)
			}
		}
...
/*func (ms MapState) AddKeysFromVisibilityPolicy(ingress bool, d DirectionalVisibilityPolicy, owner PolicyOwner) {
	var (
	...
	for _, vis := range d {
		proxyPort := owner.LookupRedirectPortByMetadata(ingress, vis.Proto.String(), vis.Port)
		keyToAdd := Key{
			DestPort:         vis.Port,
			Nexthdr:          uint8(vis.Proto),
			TrafficDirection: direction.Uint8(),
		}
		ms[keyToAdd] = MapStateEntry{
			ProxyPort: proxyPort,
		}
	}
}*/
