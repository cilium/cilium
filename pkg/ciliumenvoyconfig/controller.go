...
func (bs *backendProcessor) process(wtxn statedb.WriteTxn, closedWatches []<-chan struct{}, allWatches *statedb.WatchSet) {
	for res := range bs.envoyResources.List(wtxn, EnvoyResourceByOrigin(EnvoyResourceOriginBackendSync)) {
		...
		if found {
			...
			ws.Add(watchBes)
			newEndpoints[svc.Name.String()] = computeLoadAssignments(
				svc.Name,
		...
		endpointsEqual := slices.EqualFunc(slices.Collect(maps.Values(prevEndpoints)), slices.Collect(maps.Values(newEndpoints)), claEqual)
