...
// with Origin=cec for the resouces coming from [CEC] and it upserts an EnvoyResource with
...
func (bs *backendProcessor) process(wtxn statedb.WriteTxn, closedWatches []<-chan struct{}, allWatches *statedb.WatchSet) {
	for res := range bs.envoyResources.List(wtxn, EnvoyResourceByOrigin(EnvoyResourceOriginBackendSync)) {
		...
		if found {
			...
			// Forget the old watches.
			ws.Clear()
		}

		// Look up the EnvoyResource to grab the watch channel. A future improvement would be to add a
		// "ListWithWatches" to StateDB that includes the watch channels in the results.
		res, _, watchResource, _ := bs.envoyResources.GetWatch(wtxn, EnvoyResourceByName(res.Name))
		...
		bs.envoyResources.Insert(wtxn, res)
