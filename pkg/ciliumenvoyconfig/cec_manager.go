...
func newCiliumEnvoyConfigManager(logger logging.FieldLogger,
	policyUpdater *policy.Updater,
	serviceManager service.ServiceManager,
	xdsServer envoy.XDSServer,
	backendSyncer *envoyServiceBackendSyncer,
...
func (r *cecManager) addK8sServiceRedirects(resourceName service.L7LBResourceName, spec *ciliumv2.CiliumEnvoyConfigSpec, resources envoy.Resources) error {
	...
	for _, svc := range spec.Services {
		...
		if proxyPort == 0 {
			...
			if svc.Listener == "" {
				r.logger.Info("Skipping L7LB k8s service redirect for service. No Listener found in CEC resources", slog.String("namespace", svc.Namespace), slog.String("name", svc.Name))
				continue
			}
