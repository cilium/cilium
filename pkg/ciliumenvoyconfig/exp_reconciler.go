...
func (ops *envoyOps) Update(ctx context.Context, txn statedb.ReadTxn, res *EnvoyResource) error {
	...
	if err == nil {
		...
		if len(res.Listener.Redirects) > 0 || len(res.Listener.ReconciledRedirects) > 0 {
			wtxn := ops.writer.WriteTxn()
			for name, redirect := range res.Listener.Redirects {
				svc, _, found := ops.writer.Services().Get(wtxn, experimental.ServiceByName(name))
			...
			for name := range res.Listener.ReconciledRedirects {
				if _, found := res.Listener.Redirects[name]; !found {
					svc, _, found := ops.writer.Services().Get(wtxn, experimental.ServiceByName(name))
					if found {
						svc = svc.Clone()
						svc.ProxyRedirect = nil
			...
			wtxn.Commit()
			res.Listener.ReconciledRedirects = res.Listener.Redirects
		}
