...
// RuleWithEntry combines a policy key with its entry for allocation input.
type RuleWithEntry struct {
	Key   policyTypes.Key
	Entry policyTypes.MapStateEntry
}

// GetOrAllocate returns the rule_set_id for the given rules.
func (a *RuleSetAllocator) GetOrAllocate(rules []RuleWithEntry) (uint32, error) {
...
func (a *RuleSetAllocator) UpdateEndpointRules(epID uint16, rules []RuleWithEntry) (uint32, bool, error) {
	...
	if existingID, ok := a.rulesets[newHash]; ok {
		...
		if err := updateOverlayPolicyEntry(epID, existingID); err != nil {
			return 0, false, err
		}
	...
	if hadOld && a.refcount[oldRuleSetID] == 1 {
		...
		if oldRules != nil {
			...
			if len(added) > 0 || len(removed) > 0 || len(modified) > 0 {
				...
				_ = updateOverlayPolicyEntry(epID, oldRuleSetID)
...
func (a *RuleSetAllocator) allocateNewRuleSetLocked(epID uint16, rules []RuleWithEntry, hash ruleSetKey, hadOld bool, oldRuleSetID uint32) (uint32, bool, error) {
	ruleSetID, err := a.nextIDLocked()
	if err != nil {
		return 0, false, err
	}
	...
	// Update overlay map in eBPF datapath first before releasing old ruleset
	if err := updateOverlayPolicyEntry(epID, ruleSetID); err != nil {
		for _, r := range sharedRules {
			_ = m.Delete(&r.Key)
		}
		a.freeIDs = append(a.freeIDs, ruleSetID)
		return 0, false, fmt.Errorf("failed to update overlay entry: %w", err)
	}
...
func (a *RuleSetAllocator) convertToSharedRules(ruleSetID uint32, rules []RuleWithEntry) []SharedRule {
	sharedRules := make([]SharedRule, len(rules))
	for i, r := range rules {
		key := NewSharedKey(ruleSetID, r.Key)
		entry := NewSharedEntry(key, r.Entry)
...
func (a *RuleSetAllocator) applyIncrementalUpdate(ruleSetID uint32, added, removed []SharedRule, modified []modifiedRule) error {
	...
	// Add new rules first to prevent transient drops
	for _, r := range added {
		if err := m.Update(&r.Key, &r.Entry); err != nil {
...
func ComputeRuleSetHashFromEntries(rules []RuleWithEntry) ruleSetKey {
	sorted := make([]RuleWithEntry, len(rules))
	copy(sorted, rules)

	slices.SortFunc(sorted, func(a, b RuleWithEntry) int {
