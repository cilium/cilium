...
func (r *MapRegistry) Start(cell.HookContext) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	r.started = true

	// Make a deep copy of the map specs to prevent callers modifying a retained
	// MapSpec after Start() has been called.
	cpy := make(map[string]*ebpf.MapSpec, len(r.specs))
...
func (r *MapRegistry) Stop(cell.HookContext) error {
	return nil
}
