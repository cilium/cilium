...
func newActiveConnectionTrackingMap(in struct {
	...
	err = in.SpecRegistry.Modify(ActiveConnectionTrackingMapName, func(spec *ebpf.MapSpec) error {
	...
	err = in.SpecRegistry.Modify(FailedConnectionTrackingMapName, func(spec *ebpf.MapSpec) error {
		spec.MaxEntries = uint32(size)
		return nil
	})
	if err != nil {
...
func (m actMap) Start(context cell.HookContext) error {
	var err error
	m.m, err = m.specRegistry.NewMap(ActiveConnectionTrackingMapName, &ActiveConnectionTrackerKey{}, &ActiveConnectionTrackerValue{})
	if err != nil {
		return err
	}

	m.f, err = m.specRegistry.NewMap(FailedConnectionTrackingMapName, &ActiveConnectionTrackerKey{}, &FailedConnectionTrackerValue{})
	if err != nil {
		return err
	}
