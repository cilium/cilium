	...
	go func() {
		...
		for {
			...
			if initialScan {
				...
				signal.RegisterChannel(signal.SignalGC, wakeup)
				signal.SetupSignalListener()
				signal.MuteChannel(signal.SignalGC)
			}

			signal.UnmuteChannel(signal.SignalGC)
			select {
			case x := <-wakeup:
				ipv4 = false
				ipv6 = false
				if x == signal.SignalGCV4 {
					ipv4 = true
				} else if x == signal.SignalGCV6 {
					ipv6 = true
				}
