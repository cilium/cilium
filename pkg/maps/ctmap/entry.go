...
func (c *CtEntry) String() string {
	return fmt.Sprintf("expires=%d TxPackets=%d TxBytes=%d TxFlagsSeen=%#02x LastTxReport=%d TxPackets=%d TxBytes=%d TxFlagsSeen=%#02x LastTxReport=%d Flags=%#04x RevNAT=%d Slave=%d SourceSecurityID=%d \n",
		c.Lifetime,
		c.RxPackets,
		c.RxBytes,
		c.RxFlagsSeen,
