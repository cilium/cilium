...
func CreateCtKey(remoteAddr, localAddr string, proto u8proto.U8proto, ingress bool) (CtKey, error) {
	...
	if sIP.To4() != nil {
		key := &CtKey4{
			SourcePort: byteorder.HostToNetwork(uint16(sport)).(uint16),
			DestPort:   byteorder.HostToNetwork(uint16(dport)).(uint16),
			NextHeader: proto,
			Flags:      TUPLE_F_OUT,
		}
...
func Lookup(mapname string, remoteAddr, localAddr string, proto u8proto.U8proto, ingress bool) (*CtEntry, error) {
...
func maps(e CtEndpoint, ipv4, ipv6 bool) []*Map {
	...
	if e == nil {
		...
		idStr := fmt.Sprintf("%05d", int(e.GetID()))
		if ipv4 {
			result = append(result, NewMap(MapNameTCP4+idStr, MapTypeIPv4TCPLocal))
			result = append(result, NewMap(MapNameAny4+idStr, MapTypeIPv4AnyLocal))
		}
		if ipv6 {
			result = append(result, NewMap(MapNameTCP6+idStr, MapTypeIPv6TCPLocal))
			result = append(result, NewMap(MapNameAny6+idStr, MapTypeIPv6AnyLocal))
		}
	}
	return result
}
