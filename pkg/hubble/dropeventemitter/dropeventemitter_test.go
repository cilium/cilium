...
func TestExpireHistory(t *testing.T) {
	now := time.Now()
	tests := []struct {
		name   string
		...
		{
			name: "one has expired",
			input: map[string]time.Time{
				"1.2.3.4/5.6.7.8": now.Add(-time.Second * 40),
				"2.3.4.5/6.7.8.9": now.Add(-time.Second * 130),
			},
			expect: map[string]time.Time{
				"1.2.3.4/5.6.7.8": now.Add(-time.Second * 40),
			},
		...
		{
			name: "none have expired",
			input: map[string]time.Time{
				"1.2.3.4/5.6.7.8": now.Add(-time.Second * 40),
				"2.3.4.5/6.7.8.9": now.Add(-time.Second * 60),
			},
			expect: map[string]time.Time{
				"1.2.3.4/5.6.7.8": now.Add(-time.Second * 40),
				"2.3.4.5/6.7.8.9": now.Add(-time.Second * 60),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			now := time.Now()
			e.expireHistory(now)
			assert.Equal(t, e.history, tt.expect)
		})
	}
}
...
func TestProcessFlow(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			history: map[string]time.Time{
				"1.2.3.4/5.6.7.8": now.Add(-time.Minute * 3),
			},
			flow: &flowpb.Flow{
			...
			expect: "Incoming packet dropped from unknown (1.2.3.4) TCP/443",
		},
		{
			name:    "duplicate ingress drop event",
			history: map[string]time.Time{"1.2.3.4/5.6.7.8": now.Add(-time.Minute * 1)},
			flow: &flowpb.Flow{
				Verdict:          flowpb.Verdict_DROPPED,
				TrafficDirection: flowpb.TrafficDirection_INGRESS,
				IP:               &flowpb.IP{Source: "1.2.3.4", Destination: "5.6.7.8"},
		...
		},
		{
			...
			expect: "Outgoing packet dropped to remote-node (5.6.7.8) UDP/512",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
