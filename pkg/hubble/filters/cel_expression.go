...
func filterByCELExpression(ctx context.Context, log logging.FieldLogger, exprs []string) (FilterFunc, error) {
	...
	return func(ev *v1.Event) bool {
		for _, prg := range programs {
			...
			if err != nil {
				log.Error("error running CEL program", slog.Any(logfields.Error, err))
				return false
			}

			v, err := out.ConvertToNative(goBoolType)
			if err != nil {
				log.Error("invalid conversion in CEL program", slog.Any(logfields.Error, err))
				return false
			}
			b, ok := v.(bool)
			if ok && b {
