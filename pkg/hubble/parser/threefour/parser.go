...
type packet struct {
	...
	decLayerL2     *gopacket.DecodingLayerParser
	decLayerL3IPv4 *gopacket.DecodingLayerParser
	decLayerL3IPv6 *gopacket.DecodingLayerParser
...
func New(
	...
	packet.decLayerL2 = gopacket.NewDecodingLayerParser(layers.LayerTypeEthernet, decoders...)
	packet.decLayerL3IPv4 = gopacket.NewDecodingLayerParser(layers.LayerTypeIPv4, decoders...)
	packet.decLayerL3IPv6 = gopacket.NewDecodingLayerParser(layers.LayerTypeIPv6, decoders...)
	...
	packet.decLayerL2.IgnoreUnsupported = true
	packet.decLayerL3IPv4.IgnoreUnsupported = true
	packet.decLayerL3IPv6.IgnoreUnsupported = true
...
func (p *Parser) Decode(data []byte, decoded *pb.Flow) error {
	...
	if len(data[packetOffset:]) > 0 {
		...
		switch {
		case !isL3Device:
			err = p.packet.decLayerL2.DecodeLayers(data[packetOffset:], &p.packet.Layers)
		case isIPv6:
			err = p.packet.decLayerL3IPv6.DecodeLayers(data[packetOffset:], &p.packet.Layers)
		default:
			err = p.packet.decLayerL3IPv4.DecodeLayers(data[packetOffset:], &p.packet.Layers)
		}
