...
func (r *EndpointResolver) ResolveEndpoint(ip net.IP, datapathSecurityIdentity uint32, context DatapathContext) *pb.Endpoint {
	...
	resolveIdentityConflict := func(userspaceID identity.NumericIdentity, isLocalEndpoint bool) uint32 {
		...
		if datapathID != userspaceID {
			if context.TraceObservationPoint == pb.TraceObservationPoint_TO_OVERLAY &&
				ip.Equal(context.SrcIP) && datapathID.Uint32() == context.SrcLabelID &&
				...
				userspaceID == identity.ReservedIdentityWorld {
				// Ignore
				//
				...
				ip.Equal(context.SrcIP) && datapathID.Uint32() == context.SrcLabelID &&
				datapathID == identity.ReservedIdentityWorld && userspaceID == identity.ReservedIdentityKubeAPIServer {
				// Ignore
				//
