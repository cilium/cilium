...
func (f *exporterFactory) Create(ctx context.Context, config ExporterConfig) (FlowLogExporter, error) {
...
func (f *exporterFactory) create(ctx context.Context, config *FlowLogConfig) (*exporter, error) {
	exporterOpts := []Option{
	...
	return NewExporter(ctx, f.logger.With(logfields.FlowLogName, config.Name), exporterOpts...)
}
...
func (f *FlowLogConfig) equals(other *FlowLogConfig) bool {
	...
	if time.Duration(f.AggregationInterval) != time.Duration(other.AggregationInterval) {
		return false
	}
