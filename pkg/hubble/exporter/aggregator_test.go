...
func TestGenerateAggregationKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			aggregator := NewAggregatorWithFields(fieldAgg)
...
func TestAggregateAdd(t *testing.T) {
	t.Run("basic aggregation", func(t *testing.T) {
		...
		// Verify aggregation
		assert.Equal(t, 1, len(aggregator.m))
	...
	t.Run("nil event", func(t *testing.T) {
		...
		assert.Equal(t, 0, len(aggregator.m))
	})
...
func (ewc *errorWriteCloser) Write(p []byte) (int, error) {
	if ewc.shouldError {
		return 0, fmt.Errorf("simulated encoder error")
	}
	return len(p), nil
}

func (ewc *errorWriteCloser) Close() error { return nil }
