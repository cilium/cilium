...
func NewExporter(logger logging.FieldLogger, options ...Option) (*exporter, error) {
...
func newExporter(logger logging.FieldLogger, opts Options) (*exporter, error) {
...
func (e *exporter) Export(ctx context.Context, ev *v1.Event) error {
	...
	for _, f := range e.opts.OnExportEvent {
		...
		if err != nil {
			e.logger.Warn("OnExportEvent failed", slog.Any(logfields.Error, err))
		}
		if stop {
