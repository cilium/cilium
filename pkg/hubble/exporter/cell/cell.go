...
type Config struct {
	...
	// ExportAggregationInterval specifies the interval at which to aggregate before exporting Hubble flows.
...
var DefaultConfig = Config{
	...
	ExportAggregationInterval: exporter.DefaultAggregationInterval,
}
...
func (def Config) Flags(flags *pflag.FlagSet) {
	...
	flags.StringSlice("hubble-export-fieldaggregate", def.ExportFieldAggregate, "Specify list of fields to use for aggregation in Hubble exporter.")
	flags.Duration("hubble-export-aggregation-interval", def.ExportAggregationInterval, "Interval at which to aggregate before exporting Hubble flows.")
}
...
func newHubbleStaticExporter(params hubbleExportersParams) (hubbleExportersOut, error) {
	...
	builder := &FlowLogExporterBuilder{
		...
		Build: func() (exporter.FlowLogExporter, error) {
			...
			// Use context.Background() for static exporter - lifecycle is managed via OnStop hook
			staticExporter, err := exporter.NewExporter(context.Background(), params.Logger, exporterOpts...)
			if err != nil {
