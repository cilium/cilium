...
type FlowLogExporterBuilder struct {
	Name     string
	Replaces string
	Build    func() (FlowLogExporter, error)
}
...
func ResolveExporters(builders []*FlowLogExporterBuilder) ([]FlowLogExporter, error) {
...
func replaceBuilders(builders []*FlowLogExporterBuilder) ([]*FlowLogExporterBuilder, error) {
	...
	for _, builder := range builders {
		...
		if _, ok := buildersByName[builder.Replaces]; !ok {
			return nil, fmt.Errorf("exporter builder %q not found when trying to replace with %q", builder.Replaces, builder.Name)
		}
...
func buildExporters(builders []*FlowLogExporterBuilder) ([]FlowLogExporter, error) {
	exporters := make([]FlowLogExporter, 0, len(builders))
	for _, builder := range builders {
