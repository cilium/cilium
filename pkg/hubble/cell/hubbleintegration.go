...
type hubbleIntegration struct {
	...
	// launchWarning is used to report the cell status to the daemon when probed using Status().
	// A nil launchWarning means hubble has been successfully initialized.
	launchWarning atomic.Pointer[string]
...
func new(
	...
	startingMsg := "Hubble starting"
	hi.launchWarning.Store(&startingMsg)
...
func (h *hubbleIntegration) Launch(ctx context.Context) error {
	...
	if err != nil {
		h.log.WithError(err).Error("Failed to launch hubble")
		errStr := err.Error()
		h.launchWarning.Store(&errStr)
		return err
	}
	h.observer = observer
	h.launchWarning.Store(nil)
	return nil
}
...
func (h *hubbleIntegration) Status(ctx context.Context) *models.HubbleStatus {
	...
	launchErr := h.launchWarning.Load()
	if launchErr != nil {
	...
	status, err := h.observer.ServerStatus(ctx, req)
	if err != nil {
...
func (h *hubbleIntegration) launch(ctx context.Context) (*observer.LocalObserverServer, error) {
	...
	if addr := h.config.ListenAddress; addr != "" {
		...
		if err != nil {
			// TODO: bubble up the error and/or set cell health as degraded
			h.log.WithError(err).WithField("address", addr).Warn("Hubble server will not pass port information in change notifications on exposed Hubble peer service")
		} else {
