...
// trySendLostEvent tries to send a lost event as needed. If it succeeds, it clears the
// lost event counter, otherwise it does nothing so we keep the existing count.
func (c *consumer) trySendLostEvent(ts time.Time) {
	// check if we should send a lost event using a read lock first
	c.lostLock.RLock()
	shouldSend := c.lostEventCounter.IsElapsed(ts)
	c.lostLock.RUnlock()

	// bail out early if we don't need to
	if !shouldSend {
		return
	}

	// otherwise take a write lock
	c.lostLock.Lock()
	defer c.lostLock.Unlock()

	// re-evaluate the condition, as it may have changed
...
// incrementLostEvent increments the lost event counter. It also logs a warning
// message if the counter was previously empty and the log limiter allows it.
func (c *consumer) incrementLostEvent(ts time.Time) {
	c.lostLock.Lock()
	defer c.lostLock.Unlock()
