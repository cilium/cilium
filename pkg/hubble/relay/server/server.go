...
func (s *Server) Serve() error {
	...
	if s.metricsServer != nil {
		eg.Go(func() error {
			s.opts.log.Info("Starting metrics server...", slog.Any("address", s.opts.metricsListenAddress))
			return s.metricsServer.ListenAndServe()
		})
	}

	eg.Go(func() error {
		s.opts.log.Info("Starting gRPC server...", slog.Any("options", fmt.Sprintf("%+v", s.opts)))
		s.pm.Start()
	...
	eg.Go(func() error {
		s.opts.log.Info("Starting gRPC health server...", slog.Any("addr", s.opts.healthListenAddress))
...
func (s *Server) Stop() {
	...
	if s.metricsServer != nil {
		if err := s.metricsServer.Shutdown(context.Background()); err != nil {
			s.opts.log.Info("Failed to gracefully stop metrics server", slog.Any(logfields.Error, err))
		}
	}
	s.pm.Stop()
	s.healthServer.stop()
...
func copyObserverOptionsWithLogger(log logging.FieldLogger, options []observer.Option) []observer.Option {
