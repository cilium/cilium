...
func TestHealthServer(t *testing.T) {
	...
	t.Run("initially unavailable", func(t *testing.T) {
		eventualServingStatus(t, hs.svc, healthpb.HealthCheckResponse_NOT_SERVING)
	})
	t.Run("updated available", func(t *testing.T) {
		...
		eventualServingStatus(t, hs.svc, healthpb.HealthCheckResponse_SERVING)
	})
...
func eventualServingStatus(t *testing.T, svc healthpb.HealthServer, status healthpb.HealthCheckResponse_ServingStatus) {
	t.Helper()
	assert.eventualWithT(t, func(c *assert.CollectT) {
