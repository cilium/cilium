...
func NewRedirectPolicyManager(svc svcManager, lpr agentK8s.LocalPodResource, epM endpointManager) *Manager {
	...
	if err != nil {
		log.Warnf("failed to init cilium_skip_lb maps: %v"+
			"Check the local redirect policy documentation if you are using"+
			"CiliumLocalRedirectPolicy where it's required to"+
			"skip load-balancing in certain cases", err)
	}
	return &Manager{
		svcManager:            svc,
		epManager:             epM,
...
func (rpm *Manager) AddRedirectPolicy(config LRPConfig) (bool, error) {
	...
	if config.skipRedirectFromBackend {
		...
		if rpm.noNetnsCookieSupport {
			err := fmt.Errorf("policy with skipRedirectFromBackend set not applied" +
...
func (rpm *Manager) EndpointCreated(ep *endpoint.Endpoint) {
	...
	log.Infof("debug-aditi EndpointCreated %v %v %v", podID, ep.IPv4, ep.NetNsCookie)
	rpm.mutex.Lock()
	defer rpm.mutex.Unlock()
...
func (rpm *Manager) EndpointRestored(ep *endpoint.Endpoint) {
	...
	log.Infof("debug-aditi EndpointRestored %v %v %v", podID, ep.IPv4, ep.NetNsCookie)
	rpm.mutex.Lock()
	defer rpm.mutex.Unlock()
	if policyIDs, found := rpm.policyEndpoints[podID]; found {
		for _, id := range policyIDs.UnsortedList() {
			config := rpm.policyConfigs[id]
			// Track policies that need pod netns cookie for applying the policies.
			if config.skipRedirectFromBackend {
				for _, mapping := range config.frontendMappings {
					log.Infof("debug-aditi endpoint restore dump %v %v", *mapping.feAddr, mapping.podBackends)
				}
			}
		}
	}
	...
	if config.skipRedirectFromBackend {
		if err := rpm.plumbSkipLBEntries(feMapping); err != nil {
			log.WithFields(logrus.Fields{
				...
				logfields.K8sNamespace: config.id.Namespace,
				logfields.LRPName:      config.id.Name,
			}).Errorf("LRP not applied as skipRedirectFromBackend not supported: %e", err)
			return
		}
	}
