...
import (
	...
	envoy_service_discovery_v3 "github.com/cilium/proxy/go/envoy/service/discovery/v3"
...
var (
	...
	nodes                    = map[string]*envoy_config_core_v3.Node{
...
func (c *ResponseMatchesChecker) Check(params []interface{}, names []string) (result bool, error string) {
	response, ok := params[0].(*envoy_service_discovery_v3.DiscoveryResponse)
	if !ok {
		return false, "response must be an *envoy_service_discovery_v3.DiscoveryResponse"
	}
	if response == nil {
...
var resources = []*envoy_config_route_v3.RouteConfiguration{
	{Name: "resource0"},
	{Name: "resource1"},
	{Name: "resource2"},
}
...
func (s *ServerSuite) TestRequestAllResources(c *C) {
	...
	var err error
	var req *envoy_service_discovery_v3.DiscoveryRequest
...
func (s *ServerSuite) TestRequestSomeResources(c *C) {
	...
	c.Assert(rsrc.(*envoy_config_route_v3.RouteConfiguration), checker.DeepEquals, resources[2])
