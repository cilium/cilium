...
func getXDSRequestFields(req *envoy_service_discovery_v3.DiscoveryRequest) logrus.Fields {
...
func (s *Server) processRequestStream(ctx context.Context, streamLog *logrus.Entry, stream Stream,
	reqCh <-chan *envoy_service_discovery_v3.DiscoveryRequest, defaultTypeURL string) error {
	...
	for {
		...
		case reqChIndex: // Request received from the stream.
			...
			req := recv.Interface().(*envoy_service_discovery_v3.DiscoveryRequest)
