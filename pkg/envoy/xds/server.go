...
func (s *Server) processRequestStream(ctx context.Context, streamLog *slog.Logger, stream Stream,
	...
	for {
		...
		case reqChIndex: // Request received from the stream.
			...
			// Preserve the legacy SotW behavior that recognizes a NACK from
			// version_info lagging response_nonce.
			isNACK := state.clientReceivedFirstResponse && lastAckedVersion < lastReceivedVersion
			...
			if state.clientReceivedFirstResponse {
				...
				if ackObserver != nil {
					...
					if !state.responseAcked {
						...
						ackObserver.HandleResourceVersionAck(nodeIP, 0, lastReceivedVersion, isNACK, detail, typeURL, state.resourceNames)
					} else {
						ackObserver.HandleResourceVersionAck(nodeIP, lastAckedVersion, lastReceivedVersion, isNACK, detail, typeURL, state.resourceNames)
					}
				} else {
			...
			if isNACK {
				...
				// versions after lastAckedVersion, upto and including lastReceivedVersion are NACKed
				requestLog.Warn(
...
type perDeltaTypeStreamState struct {
	...
	// interest in. "*" is an explicit wildcard marker that must be present
	// for a wildcard subscription and may coexist with  named subscriptions.
	// A pending watch may hold a read-only reference to this set, so this
	// must not be mutated until the pending watch has finished.
...
func (s *Server) processDeltaRequestStream(ctx context.Context, streamLog *slog.Logger, stream DeltaStream,
	...
	for {
		...
		case reqChIndex: // Request received from the stream.
			...
			// Apply subscription updates before inspecting the response nonce because
			// subscriptions and ACKs are independent Delta request roles. A stale ACK
			// must not prevent valid subscription changes from taking effect.
			...
			// normalize legacy wildcard
			if initialRequestForType && len(subscribe) == 0 {
				subscribe = []string{"*"}
			}

			var forceResponseNames set.Set[string]
			// Delta subscription updates are additive. Apply subscriptions first so
			// an unsubscription wins if a name appears in both lists.
			// Delta xDS does not specify this order, but go-control-plane applies
			// subscriptions first, so we do the same.
			for _, name := range subscribe {
				state.subscriptions.Insert(name)
				forceResponseNames.Insert(name)
			}

			wildcardSubscribed := state.subscriptions.Has("*") && !slices.Contains(unsubscribe, "*")
			for _, name := range unsubscribe {
				explicitlySubscribed := state.subscriptions.Has(name)
				state.subscriptions.Remove(name)
				state.deferredForceResponseNames.Remove(name)

				// When a named subscription is removed while wildcard interest
				// remains, resolve whether the wildcard still covers that resource.
				if name != "*" && explicitlySubscribed && wildcardSubscribed {
					forceResponseNames.Insert(name)
				} else {
					forceResponseNames.Remove(name)
					if explicitlySubscribed {
			...
			if responseNonce == "" {
			...
			} else if responseNonce != state.nonce {
				receivedNonce, err := strconv.ParseUint(responseNonce, 10, 64)
				receivedNonceIsValid := err == nil && receivedNonce != 0 &&
					strconv.FormatUint(receivedNonce, 10) == responseNonce
				if state.nonce == "" {
					if !receivedNonceIsValid || state.pendingResponse == nil || receivedNonce > streamNonce {
					...
					expectedNonce, expectedErr := strconv.ParseUint(state.nonce, 10, 64)
					if !receivedNonceIsValid || expectedErr != nil || receivedNonce >= expectedNonce {
				...
				// Subscription updates and ACKs are independent Delta request
				// roles.
				// Keep the subscription changes above, but ignore an ACK for an
				// older response. If no response is outstanding, the subscription
				// update below starts a new watch immediately.
				requestLog.Debug("ignoring stale response nonce in delta xDS request",
					logfields.ResponseNonce, responseNonce,
					logfields.Expected, state.nonce,
				)
			} else { // ACK/NACK for the outstanding response
				...
				if ackObserver != nil {
					...
					ackObserver.HandleResourceVersionAck(nodeIP, state.lastAckedVersion, lastReceivedVersion, errorDetail != nil, detail, typeURL, observerNames)
				} else {
				...
				if errorDetail != nil {
					s.metrics.IncreaseNACK(typeURL)
					requestLog.Warn(
						"delta xDS NACK received; waiting for a version update before sending again",
						logfields.XDSDetail, detail,
					)
				}
			}
		...
		default: // Pending watch response.
			...
			resp := recv.Interface().(*VersionedResources)
			streamNonce++
			state.nonce = strconv.FormatUint(streamNonce, 10)
