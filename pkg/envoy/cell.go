...
func (r ProxyConfig) Flags(flags *pflag.FlagSet) {
	...
	flags.Duration("policy-restore-timeout", 3*time.Minute, "Maxiumum time to wait for enpoint policy restoration before starting serving resources to Envoy")
}
...
func newEnvoyXDSServer(params xdsServerParams) (XDSServer, error) {
	...
	xdsServer := newXDSServer(
		...
		xdsServerConfig{
			...
			policyRestoreTimeout:          params.EnvoyProxyConfig.PolicyRestoreTimeout,
