...
func getXDSSocketPath(proxySocketDir string) string {
	return filepath.Join(proxySocketDir, "xds.sock")
}
...
func StartXDSServer(ipcache IPCacheEventSource, proxySocketDir string) *XDSServer {
	...
	return &XDSServer{
		...
		accessLogPath:          getAccessLogSocketPath(proxySocketDir),
	...
	return &envoy_config_http.HttpFilter{
		...
		ConfigType: &envoy_config_http.HttpFilter_TypedConfig{
			TypedConfig: toAny(&cilium.L7Policy{
				AccessLogPath:  getAccessLogSocketPath(option.Config.ProxySocketDir),
