	...
	go func() {
		...
		for {
			...
			if err := cmd.Start(); err != nil {
				o.logger.Warn("Envoy: Failed to start proxy", logfields.Error, err)
				select {
				case started <- false:
				default:
				}
			...
			o.logger.Info("Envoy: Proxy started", logfields.PID, cmd.Process.Pid)
			...
			go func() {
				if err := cmd.Wait(); err != nil {
					...
					o.logger.Info("Envoy: Proxy terminated", logfields.PID, cmd.Process.Pid)
				}
				close(crashCh)
			}()
			...
			case <-crashCh:
				...
				continue
			case <-envoy.stopCh:
				o.logger.Info("Envoy: Stopping embedded Envoy proxy", logfields.PID, cmd.Process.Pid)
				if err := envoy.admin.quit(); err != nil {
					...
					if err := cmd.Process.Kill(); err != nil {
						o.logger.Error("Envoy: Stopping Envoy failed", logfields.Error, err)
						envoy.errCh <- err
					}
				}
				close(envoy.errCh)
...
func (o *onDemandXdsStarter) newEnvoyLogPiper() io.WriteCloser {
	...
	go func() {
		...
		if err := scanner.Err(); err != nil {
			o.logger.Error("Error while parsing Envoy logs", logfields.Error, err)
		}
		reader.Close()
	}()
	return writer
...
func (o *onDemandXdsStarter) writeBootstrapConfigFile(config bootstrapConfig) {
	...
	o.logger.Debug("Envoy: Writing Bootstrap config", "config", bs)
	data, err := proto.Marshal(bs)
	if err != nil {
		o.logger.Error("Envoy: Error marshaling Envoy bootstrap", logfields.Error, err)
		return
	}
	if err := os.WriteFile(config.filePath, data, 0644); err != nil {
		o.logger.Error("Envoy: Error writing Envoy bootstrap file", logfields.Error, err)
	}
}
