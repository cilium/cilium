...
func upsertADSPolicyForTest(t *testing.T, cache xdsnew.Cache, nodeID, name string, policy *cilium.NetworkPolicy) {
	t.Helper()
	updated, rollback, err := cache.UpsertNetworkPolicy(t.Context(), nodeID, name, policy, nil, nil)
...
func updateTrackedADSSnapshot(t *testing.T, server *adsServer, resources xds.Resources, wg *completion.WaitGroup, typeURL string) {
	...
	server.mutex.Lock()
	_, update, err := server.cache.ApplyResources(t.Context(), localNodeID,
...
func TestSnapshotRevertGeneration(t *testing.T) {
	...
	upsert := func(server *adsServer, endpointID uint64, wg *completion.WaitGroup) error {
		_, rollback, err := server.cache.UpsertNetworkPolicy(t.Context(), localNodeID, "policy", policyForID(endpointID), wg, nil)
		finalizeRollback(rollback)
		return err
	}
	...
	t.Run("current generation is reverted", func(t *testing.T) {
		...
		ctx := t.Context()
		_, _, err := server.cache.UpsertNetworkPolicy(ctx, localNodeID, "policy", policyForID(1), nil, nil)
		require.NoError(t, err)
		cache.rollbacks = nil

		wg := completion.NewWaitGroup(ctx)
		t.Cleanup(wg.Cancel)
		_, _, err = server.cache.UpsertNetworkPolicy(ctx, localNodeID, "policy", policyForID(2), wg, nil)
		require.NoError(t, err)
		require.Len(t, cache.rollbacks, 1)

		_, reverted := cache.rollbacks[0].RevertGeneration(revertCurrentGeneration)
	...
	t.Run("updates share a global generation space without coupling node reverts", func(t *testing.T) {
		...
		publish := func(nodeID string, endpointID uint64) {
			server.mutex.Lock()
			err := func() error {
				_, _, err := server.cache.UpsertNetworkPolicy(ctx, nodeID, "policy", policyForID(endpointID), nil, nil)
				return err
			}()
			server.mutex.Unlock()
			require.NoError(t, err)
		...
		generation, reverted := revertNodeA.RevertGeneration(revertCurrentGeneration)
		require.True(t, reverted)
	...
	t.Run("replayed revert lets a NACK continue through coalesced generations", func(t *testing.T) {
		...
		revertedGeneration, reverted := newerRevert.RevertGeneration(revertCurrentGeneration)
		require.True(t, reverted)
		...
		replayedGeneration, reverted := newerRevert.RevertGeneration(responseGeneration)
		require.False(t, reverted)
		require.Equal(t, revertedGeneration, replayedGeneration)

		_, reverted = olderRevert.RevertGeneration(replayedGeneration)
		require.True(t, reverted)
	...
	t.Run("coalesced ABA generations revert to the last accepted state", func(t *testing.T) {
		...
		listener := server.getListenerConf("npds-listener", policy.ParserTypeHTTP, 12345, false, false)
		_, listenerRollback, err := cache.Cache.UpsertListener(ctx, localNodeID, listener.Name, listener, nil, nil)
		require.NoError(t, err)
		finalizeRollback(listenerRollback)
	...
	t.Run("NACK preserves a newer version of the same resource", func(t *testing.T) {
		...
		apply := func(values map[string]uint64, wg *completion.WaitGroup) {
			...
			for name, endpointID := range values {
				updated, _, err := cache.UpsertNetworkPolicy(ctx, localNodeID, name,
...
func TestStrictADSEndpointNACKKeepsCluster(t *testing.T) {
	...
	_, exists := cache.GetResource(localNodeID, typeurl.Cluster, "cluster")
	require.True(t, exists, "an EDS NACK must not roll back an accepted CDS cluster")
	_, exists = cache.GetResource(localNodeID, typeurl.Endpoint, "cluster")
...
func TestUpdateEnvoyResourcesAddressChangeSupersedesOlderRollback(t *testing.T) {
	...
	_, reverted := staleRollback.RevertGeneration(revertCurrentGeneration)
	require.False(t, reverted)
...
func TestUpdateNetworkPolicyReusesIdempotentCacheRevert(t *testing.T) {
	...
	generationAfterNACK, reverted := cache.rollbacks[0].RevertGeneration(revertCurrentGeneration)
	require.True(t, reverted)
	require.Same(t, previousPolicy, cachedNetworkPolicy(t, cache, localNodeID, "1"))

	require.NoError(t, revertible.Revert())
	replayedGeneration, replayed := cache.rollbacks[0].RevertGeneration(revertCurrentGeneration)
...
func TestLastNPDSListenerRemovalKeepsSentPolicyNACKRollback(t *testing.T) {
	...
	defer wg.Cancel()
	callbackCount := 0
	updated, policyRollback, err := cache.UpsertNetworkPolicy(ctx, localNodeID, "policy", &cilium.NetworkPolicy{EndpointId: 1}, wg, func(err error) {
		require.NoError(t, err)
		callbackCount++
	})
	require.NoError(t, err)
...
func TestUpdateNetworkPolicyNoOpWaitsForCurrentACKWithoutPublishing(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(os.Stderr, nil))
	cache := &countingADSCache{Cache: newADSCache(logger, true)}
	server := newADSServerWithCache(cache, logger, nil, GetLocalEndpointStoreForTest(), xdsServerConfig{}, certificatemanager.NewMockSecretManagerInline(), nil)
	ctx, cancel := context.WithTimeout(t.Context(), time.Second)
	t.Cleanup(cancel)
