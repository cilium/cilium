...
import (
	...
	_ "google.golang.org/grpc/encoding/proto" // Register the production gRPC protobuf codec.
...
type benchmarkEnvoy interface {
	reset()
	report(*testing.B)
	startControlledServerWork()
	waitForServerWork(context.Context, <-chan struct{}, bool) (bool, error)
	resumeServerWork(context.Context) (bool, error)
	stopControlledServerWork()
	err() error
	close()
}
...
type benchmarkEnvoyState struct {
	...
	serverWorkMu         lock.Mutex
	serverWorkControlled bool
	serverWorkReady      chan struct{}
	serverWorkProceed    chan struct{}
	serverWorkDone       chan bool
	skipClientDelay      chan struct{}
...
func newBenchmarkEnvoyState(ctx context.Context) benchmarkEnvoyState {
	...
	clientTimer := time.NewTimer(time.Hour)
	if !clientTimer.Stop() {
		<-clientTimer.C
	}
	return benchmarkEnvoyState{
		...
		serverWorkReady:   make(chan struct{}),
		serverWorkProceed: make(chan struct{}),
		serverWorkDone:    make(chan bool),
		skipClientDelay:   make(chan struct{}, 1),
	}
}

func (e *benchmarkEnvoyState) reset() {
	e.responses.Store(0)
	e.encodedBytes.Store(0)
}

func (e *benchmarkEnvoyState) report(b *testing.B) {
	b.ReportMetric(float64(e.responses.Load())/float64(b.N), "xds-responses")
	b.ReportMetric(float64(e.encodedBytes.Load())/float64(b.N), "xds-response-bytes")
}
...
func (e *benchmarkEnvoyState) recordResponse(encodedBytes int) {
	e.responses.Add(1)
	e.encodedBytes.Add(uint64(encodedBytes))
}

// encodeResponse accounts for the server-side encoding normally performed by
// gRPC SendMsg. Releasing the buffers models transport completion; decoding and
// otherwise processing the bytes belongs to the deliberately excluded client.
func (e *benchmarkEnvoyState) encodeResponse(response any) error {
	codec := grpcencoding.GetCodecV2("proto")
	if codec == nil {
		return errors.New("gRPC protobuf codec is not registered")
	}
	buffers, err := codec.Marshal(response)
	if err != nil {
		return err
	}
	e.recordResponse(buffers.Len())
	buffers.Free()
	return nil
}
...
func (e *benchmarkEnvoyState) processResponse() bool {
	e.serverWorkMu.Lock()
	controlled := e.serverWorkControlled
	e.serverWorkMu.Unlock()
	if controlled {
		return true
	}

	e.clientTimer.Reset(benchmarkEnvoyProcessTime)
	select {
	case <-e.clientTimer.C:
		return true
	case <-e.skipClientDelay:
		if !e.clientTimer.Stop() {
			select {
			case <-e.clientTimer.C:
			default:
			}
...
// beginServerWork serializes mock-client handoffs with the benchmark goroutine.
// It returns with serverWorkMu held so startControlledServerWork can establish
// a clean accounting boundary after any already-running server work finishes.
func (e *benchmarkEnvoyState) beginServerWork() bool {
	e.serverWorkMu.Lock()
	if !e.serverWorkControlled {
		return true
	}
	e.serverWorkMu.Unlock()

	select {
	case e.serverWorkReady <- struct{}{}:
	case <-e.ctx.Done():
		return false
	}
	select {
	case <-e.serverWorkProceed:
	case <-e.ctx.Done():
		return false
	}
	e.serverWorkMu.Lock()
...
// endServerWork reports whether this server-side step produced a response
// which still needs the simulated Envoy processing delay and a subsequent ACK.
func (e *benchmarkEnvoyState) endServerWork(responsePending bool) {
	controlled := e.serverWorkControlled
	e.serverWorkMu.Unlock()
	if controlled {
		select {
		case e.serverWorkDone <- responsePending:
		case <-e.ctx.Done():
		}
	}
}

func (e *benchmarkEnvoyState) startControlledServerWork() {
	e.serverWorkMu.Lock()
	e.serverWorkControlled = true
	e.serverWorkMu.Unlock()
	select {
	case e.skipClientDelay <- struct{}{}:
	default:
	}
}

func (e *benchmarkEnvoyState) stopControlledServerWork() {
	e.serverWorkMu.Lock()
	e.serverWorkControlled = false
	e.serverWorkMu.Unlock()
	select {
	case <-e.skipClientDelay:
	default:
...
// waitForServerWork waits with the benchmark timer stopped. When required is
// false, a completed identity update wins over an idle mock client.
func (e *benchmarkEnvoyState) waitForServerWork(ctx context.Context, updateDone <-chan struct{}, required bool) (bool, error) {
	if required {
		select {
		case <-e.serverWorkReady:
			return true, nil
		case <-ctx.Done():
...
// resumeServerWork releases one prepared mock-client request and waits for all
// synchronous Cilium/go-control-plane work caused by it.
func (e *benchmarkEnvoyState) resumeServerWork(ctx context.Context) (bool, error) {
	select {
	case e.serverWorkProceed <- struct{}{}:
	case <-ctx.Done():
		return false, ctx.Err()
	}
	select {
	case responsePending := <-e.serverWorkDone:
...
func newBenchmarkADSEnvoy(b *testing.B, xdsCache xdsnew.Cache) *benchmarkADSEnvoy {
	...
	initialVersion, initialNonce, err := envoy.observeResponse(initial)
	require.NoError(b, err)
	envoy.request.VersionInfo = initialVersion
	envoy.request.ResponseNonce = initialNonce
	require.NoError(b, envoy.openWatch())
...
func (e *benchmarkADSEnvoy) openWatch() error {
	if err := e.cache.GetCompletionCallbacks().OnStreamRequest(e.streamID, &e.request); err != nil {
		return err
	}
	cancel, err := e.cache.CreateWatch(&e.request, &e.subscription, e.responses)
	if err != nil {
		return err
	}
...
func (e *benchmarkADSEnvoy) observeResponse(response cache.Response) (string, string, error) {
	...
	if err := e.encodeResponse(discoveryResponse); err != nil {
		return "", "", err
	}
	return discoveryResponse.VersionInfo, discoveryResponse.Nonce, nil
}
...
func (e *benchmarkADSEnvoy) run() {
	...
	for {
		if !responsePending {
			...
			if !e.beginServerWork() {
				return
			}
			var err error
			version, nonce, err = e.observeResponse(response)
			if err != nil {
		...
		// Constructing and populating the request models work done by Envoy, so
		// do it before handing control back to the timed server-side path.
		e.request.VersionInfo = version
		e.request.ResponseNonce = nonce
		if !e.beginServerWork() {
			return
		}
		err := e.openWatch()
		...
		if err == nil {
			// SnapshotCache delivers an already-available response synchronously
			// from CreateWatch. Observe it before reporting this server step done.
			select {
			case response := <-e.responses:
				version, nonce, err = e.observeResponse(response)
				responsePending = err == nil
			default:
			}
		}
...
func newBenchmarkLegacyEnvoy(b *testing.B, xdsCache *xds.Cache, ackObserver xds.ResourceVersionAckObserver, delta bool) *benchmarkLegacyEnvoy {
	b.Helper()
	...
	envoy := &benchmarkLegacyEnvoy{
		...
		delta:               delta,
		subscriptions:       set.NewSet("*"),
	}
	initial := envoy.getResources(0, true)
	require.NotNil(b, initial)
	require.NoError(b, envoy.observeResponse(initial))
	envoy.ack(initial)
...
func (e *benchmarkLegacyEnvoy) getResources(lastVersion uint64, initial bool) *xds.VersionedResources {
	if e.delta {
		return e.cache.GetDeltaResources(
			NetworkPolicyTypeURL,
			lastVersion,
			e.subscriptions,
			e.ackedResourceNames,
			set.Set[string]{},
			initial,
		)
	}
	return e.cache.GetResources(NetworkPolicyTypeURL, lastVersion, nil)
}

func (e *benchmarkLegacyEnvoy) observeResponse(response *xds.VersionedResources) error {
	var out any
	if e.delta {
		e.streamNonce++
		discoveryResponse, err := response.DeltaDiscoveryResponse(
			NetworkPolicyTypeURL,
			strconv.FormatUint(e.streamNonce, 10),
		)
		if err != nil {
			return err
		}
		...
		discoveryResponse, err := response.DiscoveryResponse(NetworkPolicyTypeURL)
		if err != nil {
			return err
		}
		out = discoveryResponse
	}
	if err := e.encodeResponse(out); err != nil {
		return fmt.Errorf("encoding xDS response: %w", err)
	}

	e.resourceNames = make([]string, 0, len(response.VersionedResources))
	for _, resource := range response.VersionedResources {
		e.resourceNames = append(e.resourceNames, resource.Name)
	}
	if !e.delta {
		slices.Sort(e.resourceNames)
	}
	return nil
}
...
func (e *benchmarkLegacyEnvoy) ack(response *xds.VersionedResources) {
	...
	if e.delta {
		for _, name := range response.RemovedNames {
			e.ackedResourceNames.Remove(name)
		}
		for _, resource := range response.VersionedResources {
			e.ackedResourceNames.Insert(resource.Name)
		}
	}
}
...
func (e *benchmarkLegacyEnvoy) run() {
	defer close(e.done)
	var response *xds.VersionedResources
	for {
		if response == nil {
			...
			response = e.getResources(e.lastVersion, false)
			responsePending := response != nil
			if responsePending {
				if err := e.observeResponse(response); err != nil {
					e.endServerWork(false)
					e.fail(err)
					return
				}
		...
		// Consume a newer cache version as part of the same server-side step.
		// This is the response which used to escape the benchmark timer after
		// the simulated Envoy delay.
		currentVersion, _ := e.cache.VersionState()
		if currentVersion > e.lastVersion {
			response = e.getResources(e.lastVersion, false)
			if response != nil {
				if err := e.observeResponse(response); err != nil {
...
func (s *benchmarkObservedXDSServer) UpdateNetworkPolicy(ctx context.Context, ep proxyendpoint.EndpointUpdater, epp *policy.EndpointPolicy, wg *completion.WaitGroup) (error, revert.Revertible) {
	s.networkPolicies.Add(1)
	err, revertible := s.XDSServer.UpdateNetworkPolicy(ctx, ep, epp, wg)
	s.networkPoliciesCompleted.Add(1)
	select {
	case s.networkPolicyCompleted <- struct{}{}:
	default:
	}
	return err, revertible
}
...
func newBenchmarkXDSBackend(b *testing.B, logger *slog.Logger, mode envoyconfig.XDSMode) benchmarkXDSBackend {
	...
	if mode.IsADS() {
		...
		backend = benchmarkXDSBackend{
			...
			startEnvoy: func(b *testing.B) benchmarkEnvoy {
				return newBenchmarkLegacyEnvoy(
					b,
					server.networkPolicyCache,
					ackObserver,
					mode == envoyconfig.EnvoyXDSModeDeltaSplit,
				)
			},
		}
	}
...
var _ interface {
	UpdateNetworkPolicy(context.Context, proxyendpoint.EndpointUpdater, *policy.EndpointPolicy, *completion.WaitGroup) (error, revert.Revertible)
...
func benchmarkRemoteIdentities(count int) identity.IdentityMap {
	...
	remoteLabels := labels.ParseLabels("k8s:role=remote")
	for i := range count {
...
// cilium.NetworkPolicy construction, its production cache update, response
// construction, and the delivered response's gRPC protobuf encoding.
//
...
// ADS uses a real go-control-plane SotW watch and materializes its lazy
// DiscoveryResponse. The legacy modes use their respective SotW and Delta cache
// queries and construct the corresponding protocol response. Every delivered
// response is encoded with gRPC's production protobuf codec, so response
// construction, resource marshaling, and wire encoding are included. HTTP/2
// framing and socket I/O are intentionally omitted along with the mock client.
// Once all endpoint UpdateNetworkPolicy calls return, the benchmark switches
// the mock client to a controlled handoff. The timer is stopped for simulated
// Envoy response processing and client-side ACK preparation, then restarted for
// Cilium/go-control-plane ACK handling, watch creation, and any immediately
// generated final response. The remaining one-second delay is short-circuited
// once producers finish, since it can no longer affect update coalescing.
//
...
// calibration sees only the producer and server-side response time and can
// therefore schedule many iterations despite the excluded one-second Envoy
// processing delays which may run while producers are still active.
//
// Run with:
//
//	GOMAXPROCS=2 go test ./pkg/envoy -run '^$' \
	...
	workloads := []benchmarkPolicyWorkload{
		{
			...
			identityLabels: labels.ParseLabels("k8s:role=unrelated"),
		},
		{
			name:              "network-policy-churn",
			identityLabels:    labels.ParseLabels("k8s:role=remote"),
			...
			for _, benchmarkMode := range modes {
				b.Run(benchmarkMode.name, func(b *testing.B) {
					...
					for b.Loop() {
						...
						// the response. Once every endpoint has returned from the
						// production xDS update, gate subsequent mock-client handoffs so
						// only Cilium/go-control-plane response work is timed.
						...
						fixture.envoy.startControlledServerWork()
						b.StopTimer()

						// A policy-changing batch must drain at least the response which
						// was current when its producers finished. Every server step tells
						// us whether it produced another response needing a final ACK.
						responsePending := workload.expectPolicyChurn
						for {
							ready, err := fixture.envoy.waitForServerWork(b.Context(), updateDone, responsePending)
							require.NoError(b, err)
							if !ready {
								break
							}
							b.StartTimer()
							responsePending, err = fixture.envoy.resumeServerWork(b.Context())
							b.StopTimer()
							require.NoError(b, err)
						}
						fixture.envoy.stopControlledServerWork()
						b.StartTimer()
						add = !add
					}
