...
func getPortNetworkPolicyRule(sel policy.CachedSelector, l7Parser policy.L7ParserType, l7Rules *policy.PerSelectorPolicy, useFullTLSContext bool) (*cilium.PortNetworkPolicyRule, bool) {
	...
	l4Policy.ForEach(func(l4 *policy.L4Filter) bool {
		...
		if port == 0 {
			...
			for sel, l7 := range l4.PerSelectorPolicies {
				rule, cs := getPortNetworkPolicyRule(sel, l4.L7Parser, l7, useFullTLSContext)
				if rule != nil {
					if !cs {
						canShortCircuit = false
					}
