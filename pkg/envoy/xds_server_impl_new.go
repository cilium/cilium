...
func newADSServerWithCache(cache xdsnew.Cache, logger *slog.Logger, ipCache IPCacheEventSource, localEndpointStore *LocalEndpointStore, config adsServerConfig, secretManager certificatemanager.SecretManager) *adsServer {
	adsServer := &adsServer{
		logger:             logger,
		cache:              cache,
		ipCache:            ipCache,
...
func (s *adsServer) getMetricsListenerConfig(port uint16) *envoy_config_listener.Listener {
	...
	listenerConf := &envoy_config_listener.Listener{
		...
		Address: xdsnew.GetPublicListenerAddress(port, option.Config.IPv4Enabled(), option.Config.IPv6Enabled()),
...
func (s *adsServer) addListener(ctx context.Context, name string, listenerConf func() *envoy_config_listener.Listener, cb func(err error)) error {
	...
	s.logger.Debug("ADS server: Adding listener",
		logfields.ListenerName, listenerConfig.Name,
		logfields.ListenerConfig, listenerConfig,
	)
	...
	if resources == nil {
		s.logger.Warn("Failed to get existing resources for node %s, creating new one", logfields.NodeID, localNodeID)
		resources = &xds.Resources{
			Listeners: make(map[string]*envoy_config_listener.Listener),
		}
	}
	s.logger.Info("Failed to get existing resources for node %s, creating new one", logfields.NodeID, localNodeID)
	resources.Listeners[name] = listenerConfig

	return s.updateSnapshot(ctx, resources, localNodeID)
}
...
func (s *adsServer) getListenerConf(name string, kind policy.L7ParserType, port uint16, isIngress bool, mayUseOriginalSourceAddr bool) *envoy_config_listener.Listener {
	...
	listenerConf := &envoy_config_listener.Listener{
		...
		ListenerFilters: []*envoy_config_listener.ListenerFilter{
			...
			xdsnew.GetListenerFilter(isIngress, mayUseOriginalSourceAddr, port, lingerConfig),
		},
	}
...
func (s *adsServer) updateSnapshot(ctx context.Context, resources *xds.Resources, nodeId string) error {
	...
	if oldSnapshot == nil || s.cache.AreDifferentSnapshots(oldSnapshot, newSnapshot) {
...
func subtractResources(a *xds.Resources, b *xds.Resources) xds.Resources {
	...
	if a.Endpoints != nil && b.Endpoints != nil {
	...
	if a.Listeners != nil && b.Listeners != nil {
	...
	if a.Secrets != nil && b.Secrets != nil {
	...
	if a.NetworkPolicies != nil && b.NetworkPolicies != nil {
