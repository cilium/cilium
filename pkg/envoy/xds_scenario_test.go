...
func (r *realEnvoyScenarioRunner) waitForConfig(check realEnvoyScenarioCheck) error {
	...
	if lastErr != nil {
		return fmt.Errorf("Envoy config %q presence for %q did not become %t: %w", check.configType, check.needle, check.present, lastErr)
	}
	return fmt.Errorf("Envoy config %q presence for %q did not become %t", check.configType, check.needle, check.present)
}
