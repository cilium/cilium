...
func (s *AccessLogServer) start() error {
	...
	go func() {
		s.logger.Info("Envoy: Starting access log server listening", logfields.Address, socketListener.Addr())
		for {
			...
			if err != nil {
				...
				s.logger.Warn("Envoy: Failed to accept access log connection", logfields.Error, err)
				continue
			}
...
func (s *AccessLogServer) newSocketListener() (*net.UnixListener, error) {
	...
	if err = os.Chown(s.socketPath, -1, int(s.proxyGID)); err != nil {
		s.logger.Warn("Envoy: Failed to change the group of access log listen socket", "socketpath", s.socketPath, logfields.Error, err)
	}
	...
	for {
		...
		if err != nil {
			if !errors.Is(err, io.EOF) {
				s.logger.Error("Envoy: Error while reading from access log connection", logfields.Error, err)
			}
			break
		}
		if flags&unix.MSG_TRUNC != 0 {
			s.logger.Warn("Envoy: Discarded truncated access log message - increase buffer size via --envoy-access-log-buffer-size", "access_log_buffer_size", s.bufferSize)
			continue
		}
		pblog := cilium.LogEntry{}
		err = proto.Unmarshal(buf[:n], &pblog)
		if err != nil {
			s.logger.Warn("Envoy: Discarded invalid access log message", logfields.Error, err)
			continue
		}
