...
// to any L4 port.
var allowAllTCPPortNetworkPolicy = &cilium.PortNetworkPolicy{
...
func GetNetworkPolicy(ep endpoint.EndpointUpdater, selectors policy.SelectorSnapshot, names []string, l4Policy *policy.L4Policy,
	...
	p.IngressPerPortPolicies = GetDirectionNetworkPolicy(ep, selectors, ingressMap, ingressPolicyEnforced, useFullTLSContext, useSDS, "ingress", policySecretsNamespace, logger, l7RulesTranslator)
	p.EgressPerPortPolicies = GetDirectionNetworkPolicy(ep, selectors, egressMap, egressPolicyEnforced, useFullTLSContext, useSDS, "egress", policySecretsNamespace, logger, l7RulesTranslator)

	return p
}

func GetPortNetworkPolicyRule(ep endpoint.EndpointUpdater, selectors policy.SelectorSnapshot, sel policy.CachedSelector, l7Rules *policy.PerSelectorPolicy, useFullTLSContext, useSDS bool, policySecretsNamespace string,
	l7RulesTranslator envoypolicy.EnvoyL7RulesTranslator,
) (*cilium.PortNetworkPolicyRule, bool) {
	...
	case policy.ParserTypeHTTP:
		...
		if len(l7Rules.HTTP) > 0 {
			...
			if l7Rules.EnvoyHTTPRules() != nil {
				...
				httpRules, canShortCircuit = l7RulesTranslator.GetEnvoyHTTPRules(&l7Rules.L7Rules, "")
			}
...
func GetWildcardNetworkPolicyRules(snapshot policy.SelectorSnapshot, selectors policy.L7DataMap, logger *slog.Logger) (rules []*cilium.PortNetworkPolicyRule) {
	...
	for sel, l7 := range selectors {
		...
		if l7.IsRedirect() {
			...
			logger.Warn("L3-only rule for selector surprisingly requires proxy redirection!", logfields.Selector, sel)
		}
...
func GetDirectionNetworkPolicy(ep endpoint.EndpointUpdater, selectors policy.SelectorSnapshot, l4Policy policy.L4PolicyMaps, policyEnforced bool, useFullTLSContext, useSDS bool, dir string, policySecretsNamespace string, logger *slog.Logger, l7RulesTranslator envoypolicy.EnvoyL7RulesTranslator) []*cilium.PortNetworkPolicy {
	...
	addWildcardRules := func(l4 *policy.L4Filter) {
		...
		wildcardRules := GetWildcardNetworkPolicyRules(selectors, l4.PerSelectorPolicies, logger)
	...
	if !wildcardDenyAll {
		for l4 := range l4Policy.Filters() {
			...
			for sel, l7 := range l4.PerSelectorPolicies {
				rule, cs := GetPortNetworkPolicyRule(ep, selectors, sel, l7, useFullTLSContext, useSDS, policySecretsNamespace, l7RulesTranslator)
				if rule != nil {
					if !cs {
			...
			if denyAllRule == nil && canShortCircuit {
				if wildcardAllowAll {
					logger.Debug("Short circuiting HTTP rules due to wildcard allowing all and no other rules needing attention",
						logfields.EndpointID, ep.GetID(),
						logfields.TrafficDirection, dir,
						logfields.Port, port,
					)
				...
				if allowAllRule != nil {
					logger.Debug("Short circuiting HTTP rules due to rule allowing all and no other rules needing attention",
						logfields.EndpointID, ep.GetID(),
						logfields.TrafficDirection, dir,
						logfields.Port, port,
					)
