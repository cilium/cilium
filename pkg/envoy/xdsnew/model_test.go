...
func TestGetWildcardNetworkPolicyRules(t *testing.T) {
	...
	obtained := GetWildcardNetworkPolicyRules(version, perSelectorPoliciesWithWildcard, logger)
	...
	obtained = GetWildcardNetworkPolicyRules(version, perSelectorPolicies, logger)
...
func TestGetPortNetworkPolicyRule(t *testing.T) {
	...
	obtained, canShortCircuit := GetPortNetworkPolicyRule(ep, version, cachedSelector1, L7Rules12, false, false, "", envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	require.Equal(t, ExpectedPortNetworkPolicyRule12, obtained)
	require.True(t, canShortCircuit)

	obtained, canShortCircuit = GetPortNetworkPolicyRule(ep, version, cachedSelector1, L7Rules12Deny, false, false, "", envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	require.Equal(t, ExpectedPortNetworkPolicyRule12Deny, obtained)
	require.False(t, canShortCircuit)

	obtained, canShortCircuit = GetPortNetworkPolicyRule(ep, version, cachedSelector1, L7Rules12HeaderMatch, false, false, "", envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	require.Equal(t, ExpectedPortNetworkPolicyRule122HeaderMatch, obtained)
	require.False(t, canShortCircuit)

	obtained, canShortCircuit = GetPortNetworkPolicyRule(ep, version, cachedSelector2, L7Rules1, false, false, "", envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
...
func TestGetDirectionNetworkPolicy(t *testing.T) {
	...
	obtained := GetDirectionNetworkPolicy(ep, selectors, L4PolicyMap1, true, false, false, "ingress", "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	require.Equal(t, ExpectedPerPortPolicies12, obtained)

	// L4+L7 with header mods
	obtained = GetDirectionNetworkPolicy(ep, selectors, L4PolicyMap1HeaderMatch, true, false, false, "ingress", "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	require.Equal(t, ExpectedPerPortPolicies122HeaderMatch, obtained)

	// L4+L7
	obtained = GetDirectionNetworkPolicy(ep, selectors, L4PolicyMap2, true, false, false, "ingress", "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	require.Equal(t, ExpectedPerPortPolicies1, obtained)

	// L4+L7 with Deny L3
	obtained = GetDirectionNetworkPolicy(ep, selectors, L4PolicyMap1Deny2, true, false, false, "ingress", "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	require.Equal(t, ExpectedPerPortPolicies1Deny2, obtained)

	// L4-only
	obtained = GetDirectionNetworkPolicy(ep, selectors, L4PolicyMap4, true, false, false, "ingress", "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	require.Equal(t, ExpectedPerPortPolicies, obtained)

	// L4-only
	obtained = GetDirectionNetworkPolicy(ep, selectors, L4PolicyMap5, true, false, false, "ingress", "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	require.Equal(t, ExpectedPerPortPoliciesWildcard, obtained)

	// L4-only with SNI
	obtained = GetDirectionNetworkPolicy(ep, selectors, L4PolicyMapSNI, true, false, false, "ingress", "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
...
func TestGetNetworkPolicy(t *testing.T) {
	logger := hivetest.Logger(t)
	selectors := testSelectorCache.GetSelectorSnapshot()
	obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, L4Policy1, true, true, false, false, "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
...
func TestGetNetworkPolicyWildcard(t *testing.T) {
	logger := hivetest.Logger(t)
	selectors := testSelectorCache.GetSelectorSnapshot()
	obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, L4Policy2, true, true, false, false, "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	expected := &cilium.NetworkPolicy{
		EndpointIps:            []string{IPv4Addr},
		EndpointId:             uint64(ep.GetID()),
...
func TestGetNetworkPolicyDeny(t *testing.T) {
	logger := hivetest.Logger(t)
	selectors := testSelectorCache.GetSelectorSnapshot()
	obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, L4Policy1RequiresV2, true, true, false, false, "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	expected := &cilium.NetworkPolicy{
		EndpointIps:            []string{IPv4Addr},
		EndpointId:             uint64(ep.GetID()),
...
func TestGetNetworkPolicyNil(t *testing.T) {
	logger := hivetest.Logger(t)
	selectors := testSelectorCache.GetSelectorSnapshot()
	obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, nil, true, true, false, false, "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	expected := &cilium.NetworkPolicy{
		EndpointIps:            []string{IPv4Addr},
		EndpointId:             uint64(ep.GetID()),
...
func TestGetNetworkPolicyIngressNotEnforced(t *testing.T) {
	logger := hivetest.Logger(t)
	selectors := testSelectorCache.GetSelectorSnapshot()
	obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, L4Policy2, false, true, false, false, "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	expected := &cilium.NetworkPolicy{
		EndpointIps:            []string{IPv4Addr},
		EndpointId:             uint64(ep.GetID()),
...
func TestGetNetworkPolicyEgressNotEnforced(t *testing.T) {
	logger := hivetest.Logger(t)
	selectors := testSelectorCache.GetSelectorSnapshot()
	obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, L4Policy1RequiresV2, true, false, false, false, "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	expected := &cilium.NetworkPolicy{
		EndpointIps:            []string{IPv4Addr},
		EndpointId:             uint64(ep.GetID()),
		IngressPerPortPolicies: ExpectedPerPortPolicies12RequiresV2,
...
func TestGetNetworkPolicyL7(t *testing.T) {
	logger := hivetest.Logger(t)
	selectors := testSelectorCache.GetSelectorSnapshot()
	obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, L4PolicyL7, true, true, false, false, "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	expected := &cilium.NetworkPolicy{
		EndpointIps:            []string{IPv4Addr},
		EndpointId:             uint64(ep.GetID()),
...
func TestGetNetworkPolicyKafka(t *testing.T) {
	logger := hivetest.Logger(t)
	selectors := testSelectorCache.GetSelectorSnapshot()
	obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, L4PolicyKafka, true, true, false, false, "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	expected := &cilium.NetworkPolicy{
		EndpointIps:            []string{IPv4Addr},
		EndpointId:             uint64(ep.GetID()),
...
func TestGetNetworkPolicyMySQL(t *testing.T) {
	logger := hivetest.Logger(t)
	selectors := testSelectorCache.GetSelectorSnapshot()
	obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, L4PolicyMySQL, true, true, false, false, "", logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
	expected := &cilium.NetworkPolicy{
		EndpointIps:           []string{IPv4Addr},
		EndpointId:            uint64(ep.GetID()),
...
func TestGetNetworkPolicyTLSInterception(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			selectors := testSelectorCache.GetSelectorSnapshot()
			logger := hivetest.Logger(t)
			obtained := GetNetworkPolicy(ep, selectors, []string{IPv4Addr}, tt.args.inputPolicy, true, true, tt.args.useFullTLSContext, tt.args.useSDS, tt.args.policySecretsNamespace, logger, envoypolicy.NewEnvoyL7RulesTranslator(logger, certificatemanager.NewMockSecretManagerInline()))
			expected := &cilium.NetworkPolicy{
				EndpointIps:            []string{IPv4Addr},
				EndpointId:             uint64(ep.GetID()),
