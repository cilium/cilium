...
A rollback lifecycle owns rollback state until exactly one terminal operation is selected:

- `Revert`, or
- `Finalize`.

Calling another terminal operation afterward is treated as a programming error, but it is deliberately harmless: the call emits a warning and does nothing.
...
`UpdateNetworkPolicy` returns both:

- a revert function;
- a finalize function.

The endpoint-regeneration caller must eventually invoke exactly one.

### Caller revert

If broader endpoint regeneration fails, the caller invokes the revert function.

The cache restores only resources that still belong to that update’s generation. Newer resources are preserved.

### Caller finalize

If endpoint regeneration succeeds, the caller invokes the finalize function.
...
2. Caller later invokes its revert function.
...
Normal endpoint regeneration with a policy-requiring Envoy listener waits for its proxy completion before the regeneration chooses caller revert or finalize. This typically eliminates the caller-finalize-before-NACK case, but correctness must not depend on it: initial policy publication and explicit
...
Duplicate terminal calls (revert/finalize) on the same lifecycle emit a warning and otherwise do nothing.
...
Caller-owned rollback is released when each caller invokes revert or finalize, even if Envoy is disconnected.
...
## Listener-derived side effects

Some ADS server state is derived from cached listeners rather than stored directly in the cache.

The ADS server tracks how many local-node listeners require NetworkPolicy updates. The cache reports the complete set of committed Listener transitions for one transaction to a single Listener observer while holding its mutation lock. The observer applies one aggregate count delta, so replacing one such listener with another cannot expose a transient zero count.

The observer may request a follow-up after the cache releases its mutex. The ADS server uses that follow-up when the transaction removed the last relevant listener to cancel NetworkPolicy completions which Envoy can no longer ACK. Forward mutations and cache-owned rollbacks use the same observer path, without scanning all cached listeners or reversing lock order.

`UpdateNetworkPolicy` does not register an ACK wait when the listener count is zero, because no NPDS client can consume and ACK the policy. It completes the caller callback locally instead. A transition from a positive count to zero cancels NetworkPolicy waits which were already registered.

The atomic listener count does not by itself serialize that decision with completion registration. The last listener can disappear after `UpdateNetworkPolicy` reads a positive count but before the cache registers the policy completion. The listener transition would then find no completion to cancel. To cover both orderings, `UpdateNetworkPolicy` checks the count again after the cache upsert: if removal and cancellation happened first, the post-upsert check cancels the newly registered wait; if removal happens after registration, the Listener observer cancels it. Thus a NetworkPolicy wait remains pending only while an NPDS listener exists that can cause Envoy to ACK it.
...
14. Duplicate terminal (revert/finalize) operations warn and otherwise do nothing.
