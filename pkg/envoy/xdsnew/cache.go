...
func (c *Cache) GetVersion(resources *xds.Resources) string {
	...
	if err != nil {
		c.logger.Error("failed to marshal resources for versioning %s", logfields.Error, err)
		return ""
	}
...
func (c *Cache) GenerateSnapshot(resources *xds.Resources, logger *slog.Logger) (*cache.Snapshot, error) {
	...
	if err = snapshot.Consistent(); err != nil {
		c.logger.Error("failed due to snapshot inconsistency: %q", logfields.Error, err)
		return nil, err
	}
	return snapshot, nil
}
...
func (c Cache) ClearSnapshotForType(nodeID string, resourceType envoy_resource.Type) {
	...
	if !exists {
		c.logger.Warn("No resources found for node %s when clearing snapshot for resource type %s", nodeID, resourceType)
		return
	}
	switch resourceType {
	...
	default:
		c.logger.Warn("Clearing snapshot for resource type %s is not supported", logfields.ResourceType, resourceType)
	}
	newSnapshot, err := c.GenerateSnapshot(resources, c.logger)
	if err != nil {
		c.logger.Error("Failed to generate snapshot after clearing resources of type %s for node %s: %v", logfields.ResourceType, resourceType, nodeID, err)
		return
	}
	if err = c.SetSnapshot(context.Background(), nodeID, newSnapshot); err != nil {
		c.logger.Error("Failed to set snapshot after clearing resources of type %s for node %s: %v", logfields.ResourceType, resourceType, nodeID, err)
		return
	}
}
