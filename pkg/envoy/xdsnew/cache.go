...
func NewRollback(revert RevertFunc, finalize FinalizeFunc) Rollback {
	return callbacks.NewRollback(revert, finalize)
}

// RevertFactory builds one generation-fenced rollback from the resource state
// before a coalesced group of updates to its latest desired state. The cache
// invokes it only when finalizing the staged snapshot, keeping both rollback
// work and retained state bounded while Envoy is not ready for another update.
type RevertFactory func(generation uint64, previous, current *xds.Resources, changedTypeURLs map[string]struct{}) Rollback
...
type Cache interface {
	...
	UpdateResources(ctx context.Context, nodeID string, generation uint64, resources *xds.Resources, changedTypeURLs map[string]struct{}, generator SnapshotGenerator, wg *completion.WaitGroup, updatedTypeURLs map[string]func(err error), completionRollbacks map[string]Rollback, revertFactory RevertFactory) error
	// AwaitCurrentVersion registers completions against the current snapshot
	// without publishing it again.
	AwaitCurrentVersion(nodeID string, generation uint64, wg *completion.WaitGroup, typeURLs map[string]func(err error)) error
...
type stagedSnapshot struct {
	...
	// rollbackPrevious is the desired state before the first mutation represented
	// by rollbackTypeURLs. A separate rollback is built lazily for each changed
	// type at finalization so a NACK cannot revert resources from another xDS
	// response.
	rollbackPrevious   *xds.Resources
	rollbackGeneration uint64
	rollbackTypeURLs   map[string]struct{}
	revertFactory      RevertFactory
}
...
func (c *cacheImpl) registerGenerationCompletions(nodeID string, generation uint64, newSnapshot, oldSnapshot cache.ResourceSnapshot, wg *completion.WaitGroup, typeURLs map[string]func(err error), rollbacks map[string]Rollback) ([]*completion.Completion, []immediateCompletion) {
	...
	if wg != nil && len(typeURLs) > 0 {
		for typeURL, completionCallback := range typeURLs {
			...
			registered, err := c.completionCbs.AddPreparedTypeGenerationCompletionWithRollback(
				comp, owner, version, versionChanged, rollback)
			if !registered {
				immediateCompletions = append(immediateCompletions, immediateCompletion{
					comp:                      comp,
					typeURL:                   typeURL,
...
func (c *cacheImpl) registerStagedGenerationCompletions(nodeID string, generation uint64, resources *xds.Resources, wg *completion.WaitGroup, typeURLs map[string]func(err error), rollbacks map[string]Rollback) ([]*completion.Completion, []immediateCompletion) {
	completions := make([]*completion.Completion, 0, len(typeURLs))
	immediateCompletions := make([]immediateCompletion, 0, 1)
	if wg == nil || len(typeURLs) == 0 {
		return completions, immediateCompletions
	...
	for typeURL, completionCallback := range typeURLs {
		...
		registered, err := c.completionCbs.AddPreparedTypeGenerationCompletionWithRollback(comp, owner, "", true, rollback)
		if !registered {
			immediateCompletions = append(immediateCompletions, immediateCompletion{
				comp:                      comp,
				typeURL:                   typeURL,
...
func (c *cacheImpl) completeImmediateCompletions(nodeID string, immediateCompletions []immediateCompletion) {
	for _, completion := range immediateCompletions {
		if completion.completeUnsentCompletions {
			c.completionCbs.CompleteWaitersThroughGeneration(nodeID, completion.typeURL, completion.generation, nil)
		}
		if completion.rollback != nil {
			completion.rollback.Finalize()
		}
...
func (c *cacheImpl) finalizeStagedSnapshotLocked(ctx context.Context, nodeID string) (bool, []finalizedCompletion, error) {
	...
	// Register response rollback before SetSnapshot can make the snapshot
	// visible to go-control-plane. OnStreamResponse may run synchronously while
	// SetSnapshot is delivering an already-open watch.
	for typeURL := range staged.rollbackTypeURLs {
		version := newSnapshot.GetVersion(typeURL)
		versionChanged := oldSnapshot == nil || oldSnapshot.GetVersion(typeURL) != version
		var rollback Rollback
		if staged.revertFactory != nil {
			rollback = staged.revertFactory(
				staged.generation,
				staged.rollbackPrevious,
				staged.resources,
				map[string]struct{}{typeURL: {}},
			)
		}
		registered, completeUnsent := c.completionCbs.AddTypeGenerationWithRollback(
			staged.rollbackGeneration, version, typeURL, nodeID, versionChanged, rollback)
		if !registered && rollback != nil {
			rollback.Finalize()
		}
		if completeUnsent {
			c.completionCbs.CompleteCompletionsThroughGeneration(
				nodeID, typeURL, staged.rollbackGeneration, nil)
		}
	}
	...
	if err != nil {
		...
		if !committed {
			...
			for typeURL := range staged.rollbackTypeURLs {
				c.completionCbs.RemoveTypeGeneration(nodeID, typeURL, staged.rollbackGeneration)
			}
			return false, nil, err
		}
...
func (c *cacheImpl) UpdateResources(ctx context.Context, nodeID string, generation uint64, resources *xds.Resources, changedTypeURLs map[string]struct{}, generator SnapshotGenerator, wg *completion.WaitGroup, updatedTypeURLs map[string]func(err error), completionRollbacks map[string]Rollback, revertFactory RevertFactory) error {
	...
	completions, immediateCompletions := c.registerStagedGenerationCompletions(
		nodeID, generation, resources, wg, updatedTypeURLs, completionRollbacks)
	// Keep one response-owned rollback for every direct resource mutation,
	// independently of whether a caller is waiting for its ACK or NACK.
	var rollbackTypeURLs map[string]struct{}
	if revertFactory != nil {
		// Rollback follows actual resource mutations, not dependent resource
		// groups which merely need their versions regenerated.
	...
	var rollbackPrevious *xds.Resources
	var rollbackGeneration uint64
	var mergedRollbackTypeURLs map[string]struct{}
	var stagedRevertFactory RevertFactory
	if oldStaged != nil {
		...
		rollbackPrevious = oldStaged.rollbackPrevious
		rollbackGeneration = oldStaged.rollbackGeneration
		mergedRollbackTypeURLs = maps.Clone(oldStaged.rollbackTypeURLs)
		stagedRevertFactory = oldStaged.revertFactory
	}
	for typeURL := range updatedTypeURLs {
		completionTypeURLs[typeURL] = struct{}{}
	}
	if len(rollbackTypeURLs) > 0 {
		newRollback := len(mergedRollbackTypeURLs) == 0
		if newRollback {
			rollbackPrevious = oldResources
			rollbackGeneration = generation
			mergedRollbackTypeURLs = make(map[string]struct{}, len(rollbackTypeURLs))
		}
		for typeURL := range rollbackTypeURLs {
			mergedRollbackTypeURLs[typeURL] = struct{}{}
		}
		if revertFactory != nil {
			stagedRevertFactory = revertFactory
		}
	}
	c.resourcesInSnapshot[nodeID] = resources
	c.stagedSnapshots[nodeID] = &stagedSnapshot{
		...
		generator:          generator,
		rollbackPrevious:   rollbackPrevious,
		rollbackGeneration: rollbackGeneration,
		rollbackTypeURLs:   mergedRollbackTypeURLs,
		revertFactory:      stagedRevertFactory,
	}
...
func (c *cacheImpl) AwaitCurrentVersion(nodeID string, generation uint64, wg *completion.WaitGroup, typeURLs map[string]func(err error)) error {
	if wg == nil || len(typeURLs) == 0 {
		return nil
	}
	...
	if staged != nil {
		...
		_, immediateCompletions := c.registerStagedGenerationCompletions(
			nodeID, generation, staged.resources, wg, stagedTypeURLs, nil)
		...
		_, publishedImmediate := c.registerGenerationCompletions(
			nodeID, generation, currentSnapshot, currentSnapshot, wg, publishedTypeURLs, nil)
		c.mutex.Unlock()
		c.completeImmediateCompletions(nodeID, immediateCompletions)
