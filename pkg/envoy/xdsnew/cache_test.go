...
func TestSetResources_OverwriteExisting(t *testing.T) {
	...
	storedResources := c.resourcesInSnapshot["node1"]
	require.NotNil(t, storedResources)
	assert.NotContains(t, storedResources.Listeners, "old-listener")
...
func TestClearSnapshotForType_Endpoints(t *testing.T) {
	mock := newMockSnapshotCache()
	c := newTestCacheWithHasher(mock)

	resources := emptyResources()
	resources.Endpoints["ep1"] = &envoy_config_endpoint.ClusterLoadAssignment{ClusterName: "ep1"}
	c.resourcesInSnapshot["node1"] = resources
	...
	// Verify SetSnapshot was called (ClearSnapshotForType generates + sets a new snapshot)
	require.NotEmpty(t, mock.setSnapshotCalls)
	assert.Equal(t, "node1", mock.setSnapshotCalls[len(mock.setSnapshotCalls)-1].nodeID)
}

func TestClearSnapshotForType_Clusters(t *testing.T) {
	mock := newMockSnapshotCache()
	c := newTestCacheWithHasher(mock)
...
func TestClearSnapshotForType_Routes(t *testing.T) {
	mock := newMockSnapshotCache()
	c := newTestCacheWithHasher(mock)
...
func TestClearSnapshotForType_Listeners(t *testing.T) {
	mock := newMockSnapshotCache()
	c := newTestCacheWithHasher(mock)
...
func TestClearSnapshotForType_Secrets(t *testing.T) {
	mock := newMockSnapshotCache()
	c := newTestCacheWithHasher(mock)
...
func TestClearSnapshotForType_ExtensionConfig_ClearsNetworkPolicies(t *testing.T) {
	mock := newMockSnapshotCache()
	c := newTestCacheWithHasher(mock)
