...
// revert-only function adapter used by production callers.
type finalizingTestRollback struct {
	RevertGenerationFunc
	finalize func()
}
...
func TestWaitCancellationRetainsResponseGenerationState(t *testing.T) {
	...
	registered, completeUnsent := cb.AddTypeGenerationWithRollback(
		2, "version-2", listenerTypeURL, "node-1", true,
		finalizingTestRollback{RevertGenerationFunc: func(expected uint64) (uint64, bool) {
			reverted = true
			return expected + 1, true
...
func TestDetachedPolicyWaitCompletesWithoutDiscardingNACKRollback(t *testing.T) {
	...
	registered, err := cb.AddTypeGenerationCompletion(otherComp, 2, "version-2", typeurl.NetworkPolicy, "node-2", true, nil)
	require.NoError(t, err)
	require.True(t, registered)

	reverted := false
	finalized := false
	registered, _ = cb.AddTypeGenerationWithRollback(1, "version-1", typeurl.NetworkPolicy, "node-1", true,
		finalizingTestRollback{RevertGenerationFunc: func(expected uint64) (uint64, bool) {
			reverted = true
			return expected + 1, true
		}, finalize: func() { finalized = true }})
