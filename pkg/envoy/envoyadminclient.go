...
func (a *EnvoyAdminClient) transact(query string) error {
	...
	a.logger.Debug("Envoy: Admin response", "query", query, "response", ret)
	return nil
}
...
func (a *EnvoyAdminClient) ChangeLogLevel(agentLogLevel logrus.Level) error {
	...
	if envoyLevel == a.currentLogLevel {
		a.logger.Debug("Envoy: Log level is already set", "level", envoyLevel)
		return nil
	}

	if err := a.transact("logging?level=" + envoyLevel); err != nil {
		a.logger.Warn("Envoy: Failed to set log level", "level", envoyLevel, logfields.Error, err)
		return err
	}
