...
func endpointsToServiceName(eps *k8s.Endpoints) lb.ServiceName {
	return lb.ServiceName{Scope: lb.ScopeSVC, Name: eps.ServiceID.Name, Namespace: eps.ServiceID.Namespace}
}
...
func serviceToFrontends(svc *slim_corev1.Service) []lb.FE {
	name := lb.ServiceName{Scope: lb.ScopeSVC, Name: svc.Name, Namespace: svc.Namespace}
	common := lb.CommonFE{
		Name: name,
	}
	switch svc.Spec.ExternalTrafficPolicy {
	case slim_corev1.ServiceExternalTrafficPolicyTypeLocal:
		common.TrafficPolicy = lb.SVCTrafficPolicyLocal
	default:
		common.TrafficPolicy = lb.SVCTrafficPolicyCluster
	}
