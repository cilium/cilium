...
func (m *APIEventTSHelper) ServeHTTP(r http.ResponseWriter, req *http.Request) {
	...
	if req != nil && req.URL != nil && req.URL.Path != "" {
		took := float64(duration.End(true).Total().Nanoseconds()) / float64(time.Second)
		m.Histogram.WithLabelValues(req.URL.Path, req.Method, strconv.Itoa(rw.code)).Observe(took)
	}
}
