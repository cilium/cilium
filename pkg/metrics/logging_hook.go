...
// LoggingHook is a hook for logrus which counts error, fatal, panic and warning
// messages as a Prometheus metric.
...
func NewLoggingHook(component string) *LoggingHook {
	return &LoggingHook{component: component}
}
...
// fatal, panic or warning message.
func (h *LoggingHook) Fire(entry *logrus.Entry) error {
	...
	metric.WithLabelValues(entry.Level.String(), subsystem).Inc()
