...
func (e *EtcdLockedSuite) TestGetLocked(c *C) {
	...
	for _, tt := range tests {
		...
		c.Assert(value, DeepEquals, want.value)
		err = tt.cleanup(args)
		c.Assert(err, IsNil)
	}
}
	...
	tests := []struct {
		...
		},
		{
			...
			cleanup: func(args args) error {
				...
				c.Assert(value.Kvs[0].Value, DeepEquals, []byte("bar"))
				return nil
			},
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			cleanup: func(args args) error {
				...
				c.Assert(gr.Kvs[0].Value, DeepEquals, []byte("newbar"))
		...
		},
		{
			...
			cleanup: func(args args) error {
				...
				c.Assert(err, IsNil)
				c.Assert(gr.Count, Equals, int64(1))
				c.Assert(gr.Kvs[0].Value, DeepEquals, []byte("bar"))
				return nil
			},
		},
		{
...
func (e *EtcdLockedSuite) TestListPrefixLocked(c *C) {
	...
	for _, tt := range tests {
		...
		for k, v := range kvPairs {
			...
			c.Assert(v.Data, DeepEquals, v1.Data)
		}
		err = tt.cleanup(args)
		c.Assert(err, IsNil)
	}
