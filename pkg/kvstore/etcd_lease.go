...
func newEtcdLeaseManager(cl *client.Client, ttl time.Duration, limit uint32, expired func(key string), log logging.FieldLogger) *etcdLeaseManager {
...
func (elm *etcdLeaseManager) newSession(ctx context.Context) (session *concurrency.Session, err error) {
	...
	elm.log.Info(
		...
		slog.Any("LeaseID", leaseID),
		slog.Duration("TTL", elm.ttl),
	)
	return session, nil
}
...
func (elm *etcdLeaseManager) waitForExpiration(session *concurrency.Session) {
	...
	elm.log.Warn(
		"Lease expired",
		slog.Any("LeaseID", session.Lease()),
	)
