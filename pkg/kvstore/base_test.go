...
func (s *BaseTests) TestGetPrefix(c *C) {
	...
	c.Assert(Client().Update(context.Background(), testKey, testValue(0), true), IsNil)
...
func (s *BaseTests) TestUpdate(c *C) {
	...
	// create
	c.Assert(Client().Update(context.Background(), testKey(prefix, 0), testValue(0), true), IsNil)
	...
	val, err = Client().Get(context.TODO(), testKey(prefix, 0))
	c.Assert(err, IsNil)
	c.Assert(stirng(val), checker.DeepEquals, testValue(0))
}
...
func (s *BaseTests) TestCreateOnly(c *C) {
	...
	success, err := Client().CreateOnly(context.Background(), testKey(prefix, 0), testValue(0), false)
	c.Assert(err, IsNil)
	...
	success, err = Client().CreateOnly(context.Background(), testKey(prefix, 0), testValue(1), false)
	c.Assert(err, IsNil)
	...
	c.Assert(Client().CreateIfExists(context.TODO(), testKey(prefix, 1), testKey(prefix, 2), testValue(2), false), Not(IsNil))
	...
	c.Assert(Client().CreateIfExists(context.TODO(), testKey(prefix, 0), testKey(prefix, 2), testValue(2), false), IsNil)
...
func (s *BaseTests) TestListAndWatch(c *C) {
	...
	success, err := Client().CreateOnly(context.Background(), key1, val1, false)
	c.Assert(err, IsNil)
	c.Assert(success, Equals, true)
	...
	success, err = Client().CreateOnly(context.Background(), key2, val2, false)
	c.Assert(err, IsNil)
	c.Assert(success, Equals, true)
