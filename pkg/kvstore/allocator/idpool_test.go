...
func (s *AllocatorSuite) TestSelectAvailableID(c *C) {
	minID, maxID := 1, 5
	p := newIDPool(ID(minID), ID(maxID))

	selectAllIDs(p, minID, maxID, c)
}
...
func (s *AllocatorSuite) TestRefresh(c *C) {
	...
	// Select all ids from current cache.
	for i := minID; i <= maxID; i++ {
		id := p.SelectAvailableID()
		c.Assert(id, Not(Equals), NoID)
	}
	...
	// Check we can only select the remaining
...
func (s *AllocatorSuite) TestReleaseID(c *C) {
	...
	// Select all ids and release them.
	for i := minID; i <= maxID; i++ {
		id := p.SelectAvailableID()
		c.Assert(id, Not(Equals), NoID)
	}
	...
	// Select all ids. This time, remove them before
	// releasing them.
	selectAllIDs(p, minID, maxID, c)
	for i := minID; i <= maxID; i++ {
		c.Assert(p.Remove(ID(i)), Equals, false)
...
func (s *AllocatorSuite) TestOperationsOnAvailableIDs(c *C) {
	...
	// Selecting available IDs should move its state to leased.
	p0 := newIDPool(ID(minID), ID(maxID))
	selectAllIDs(p0, minID, maxID, c)
	...
	selectAllIDs(p4, minID, minID-1, c)
	for i := minID; i <= maxID; i++ {
...
func (s *AllocatorSuite) TestOperationsOnLeasedIDs(c *C) {
	...
	selectAllIDs(p1, minID, minID-1, c)
	...
	selectAllIDs(p3, minID, minID-1, c)
}
...
func (s *AllocatorSuite) TestOperationsOnUnavailableIDs(c *C) {
	...
	selectAllIDs(p2, minID, minID-1, c)
...
func selectAllIDs(p *idPool, minID int, maxID int, c *C) {
	expected := make([]int, 0)
	actual := make([]int, 0)
	for i := minID; i <= maxID; i++ {
		id := p.SelectAvailableID()
	...
	// Unique ids must have been selected.
	sort.Ints(actual)
	c.Assert(actual, DeepEquals, expected)
}
