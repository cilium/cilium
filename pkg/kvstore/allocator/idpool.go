...
// simultaneously.
// Local usage of an ID becomes a two stage process of selecting
...
// In the case of SelectAvailableID() the ID is returned rather
// than provided as an input to the operation.
// All ID's begin in the available state.
/*
----------------------------------------------------------------------
|state\event   | SelectAvailableID | Release | Use | Insert | Remove |
----------------------------------------------------------------------
|1 available   |        2          |    *    |  *  |   *    |   3    |
----------------------------------------------------------------------
|2 leased      |        **         |    1    |  3  |   *    |   3    |
...
type idPool struct {
	...
	// min is the lower limit when selecting IDs. The pool will never
	// return an ID lesser than this value.
	minID ID

	// max is the upper limit when selecting IDs. The pool will never
...
func (p *idPool) SelectAvailableID() ID {
	...
	return p.idCache.selectAvailableID()
}

// Release returns a leased ID back to the pool.
// This operation accounts for IDs that were previously selected
...
// selectAvailableID returns a random available ID.
func (c *idCache) selectAvailableID() ID {
	if len(c.ids) == 0 {
		return NoID
	}
...
// to the cache if it is currently unavailable.
