			...
			controller.ControllerParams{
				...
				DoFunc: func(_ context.Context) error {
					...
					b.buffer.Compact(func(e interface{}) bool {
						event, ok := e.(*Event)
						if !ok {
							m.logger.Error("Failed to compact the event buffer", slog.Any(logfields.Error, wrongObjTypeErr(e)))
							return false
						}
...
func (eb *eventsBuffer) eventIsValid(e interface{}) bool {
	event, ok := e.(*Event)
	if !ok {
		eb.logger.Error("Could not dump contents of events buffer", slog.Any(logfields.Error, wrongObjTypeErr(e)))
		return false
	}
