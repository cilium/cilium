...
// NOTE: So far this function is used only to remove maps from old Cilium
// versions, that's why error is ignored here. If you want to use this function
// for the other purpose than upgrade, implementing the logic for error handling
// minght be needed.
func removeMap(m string) {
	os.Remove(m)
}

// RemoveAfterMount schedules a map to be opened/created after the BPF filesystem
// has been mounted. If the filesystem is already mounted, the map is removed
// immediately.
func RemoveAfterMount(m string) {
	mountMutex.Lock()
	defer mountMutex.Unlock()
	...
	log.WithField(logfields.BPFMapName, m).Debug("bpffs is not mounted yet; adding to list of maps to remove once it is mounted")
	delayedRemoves = append(delayedRemoves, m)
}
