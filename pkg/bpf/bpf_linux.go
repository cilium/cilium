...
func ObjPin(fd int, pathname string) error {
	...
	if err != nil {
		return fmt.Errorf("Unable to convert pathname %q to byte pointer: %v", pathname, err)
	}
...
func ObjGet(pathname string) (int, error) {
	pathStr, err := syscall.BytePtrFromString(pathname)
	if err != nil {
		return -1, fmt.Errorf("Unable to convert pathname %q to byte pointer: %v", pathname, err)
	}
