...
func getUnusedMaps(coll *ebpf.Collection) (usedMaps []string, unusedMaps []string, err error) {
	...
	for name, m := range coll.Maps {
		...
		// These maps are assumed to always be in use (by cilium code), even if they are not.
		// TODO: Encode this in the spec, instead of hardcoding it here.
		switch name {
		case "cilium_call_policy", "cilium_egresscall_policy":
			continue

		// We expect the maps config section to exists even if never referenced by the code.
		case config.Section:
			continue
		}
...
func RemoveUnusedMaps(spec *ebpf.CollectionSpec) error {
	mapsInUse := make(map[string]bool)

	// TODO: Encode un-prunable maps in the spec, instead of hardcoding them here.
	mapsInUse["cilium_call_policy"] = true
	mapsInUse["cilium_egresscall_policy"] = true
...
determinePredictableBranches:
	for _, block := range blocks {
		...
		//   LoadMapValue dst: Rx, fd: 0 off: 0 <{name of global data map}>
		//   LdXMem{B,H,W,DW} dst: Ry src: Rx off: {offset of variable in global data value}
