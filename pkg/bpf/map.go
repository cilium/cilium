...
var (
	...
	// supportedMapTypes contains information about supported BPF map types.
	supportedMapTypes *probes.MapTypes

	onceMapTypes sync.Once
)
...
func GetMapType(t MapType) (MapType, error) {
	var err error
	getMapTypes := func() {
		var probeManager *probes.ProbeManager
		probeManager, err = probes.NewProbeManager()
		if err != nil {
			return
		}
		supportedMapTypes = probeManager.GetMapTypes()
	}
	onceMapTypes.Do(getMapTypes)
	if err != nil {
		return -1, err
	}
