...
func tcPathFromMountInfo(logger logging.FieldLogger, name string) string {
	readMountInfo.Do(func() {
		mountInfos, err := mountinfo.GetMountInfo()
		if err != nil {
			logging.Fatal(logger, "Could not get mount info for map root lookup", slog.Any(logfields.Error, err))
		}
...
func MapPath(name string) string {
	...
	return tcPathFromMountInfo(logging.DefaultLogger, name)
}
...
func mountFS(logger logging.FieldLogger, printWarning bool) error {
	if printWarning {
		logger.Warn("================================= WARNING ==========================================")
	...
	logger.Info("Mounting BPF filesystem", slog.String("bpf-root-fs", bpffsRoot))
...
// given path.
func checkOrMountCustomLocation(logger logging.FieldLogger, bpfRoot string) error {
	...
	logger.Info("Detected mounted BPF filesystem", slog.String("bpf-root-fs", bpffsRoot))
...
func checkOrMountDefaultLocations(logger logging.FieldLogger) error {
...
func CheckOrMountFS(logger *slog.Logger, bpfRoot string) {
	mountOnce.Do(func() {
		if err := checkOrMountFS(logger, bpfRoot); err != nil {
			logging.Fatal(logger, "Unable to mount BPF filesystem", slog.Any(logfields.Error, err))
		}
	})
}
