...
// Decode populates 'r' based on the provided bytes array.
...
//
// The caller MUST eventually call Disable() to free event resources if this
...
func PerfEventFromMemory(page *PerfEventMmapPage, buf []byte) *PerfEvent {
	...
	if err := e.enable(); err != nil {
		log.Warningf("Cannot allocate temp buffer: %s", err)
		return nil
	}
	return e
}
