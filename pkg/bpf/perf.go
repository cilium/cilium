...
func (e *PerfEvent) Mmap(pagesize int, npages int) error {
	var datasize uint32 = uint32(pagesize) * uint32(npages)
...
func (e *PerfEvent) Read(receive ReceiveFunc, lostFn LostFunc) {
	...
	if unknown != 0 || missed != 0 || trunc != 0 {
		e.unknown += unknown
		e.missed += missed
		e.trunc += trunc
		log.Warning("Events missed/truncated/unknown: %v/%v/%v (total: %v/%v/%v)!",
			missed, trunc, unknown, e.missed, e.trunc, e.unknown)
	}
}
