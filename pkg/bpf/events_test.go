...
func TestEventsSubscribe(t *testing.T) {
	...
	eb.add(&Event{cacheEntries: cacheEntries{{Key: IntTestKey(123)}}})
	eb.add(&Event{cacheEntries: cacheEntries{{Key: IntTestKey(124)}}})
	eb.add(&Event{cacheEntries: cacheEntries{{Key: IntTestKey(125)}}})
	assert.Equal("key=123", (<-handle.C()).cacheEntries[0].Key.String())
	assert.Equal("key=124", (<-handle.C()).cacheEntries[0].Key.String())
	...
	for i := 0; i < eventSubChanBufferSize; i++ {
		...
		eb.add(&Event{cacheEntries: cacheEntries{{Key: IntTestKey(i)}}})
	}
