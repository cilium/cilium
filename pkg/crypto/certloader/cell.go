...
func NewWatchedServerConfigPromise(lc cell.Lifecycle, log *slog.Logger, cfg Config) (promise.Promise[*WatchedServerConfig], error) {
	if !cfg.TLS {
		return nil, nil
	}

	metricsTLSConfigChan, err := FutureWatchedServerConfig(
		log.With(logfields.Config, "certloader-tls"),
		cfg.TLSClientCAFiles, cfg.TLSCertFile, cfg.TLSKeyFile,
	)
	if err != nil {
		return nil, fmt.Errorf("failed to initialize certloader TLS configuration: %w", err)
	}
		...
		OnStart: func(_ cell.HookContext) error {
			...
			go func() {
				...
				for metricsTLSConfig == nil {
					...
					case <-ctx.Done():
						resolver.Reject(fmt.Errorf("timeout while waiting for certloader TLS certificate and key files to be created: %w", ctx.Err()))
						return
					}
				}
