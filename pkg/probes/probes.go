...
type Helpers struct {
	...
	LircMode2AvailableHelpers      []interface{} `json:"lirc_mode2_available_helpers"`
	...
	CgroupSysctlAvailableHelpers   []interface{} `json:"cgroup_sysctl_available_helpers"`
}
...
func Probes() error {
	...
	if err := json.Unmarshal(out, &features); err != nil {
		return fmt.Errorf("could not parse bpftool output: %s", err)
	}
