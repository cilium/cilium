...
func (lsp *LabelArrayList) InsertSorted(la LabelArray) {
	for j := 0; j < len(*lsp); j++ {
		if la.Less((*lsp)[j]) {
			*lsp = append((*lsp)[:j+1], (*lsp)[j:]...) // make space at merged[j]
			(*lsp)[j] = la
			return
		} else if la.Equals((*lsp)[j]) {
			return
		}
	}
	// 'la' is greater than anything in 'lsp', append to the end
	*lsp = append(*lsp, la)
}
