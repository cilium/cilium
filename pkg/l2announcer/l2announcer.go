...
func (l2a *L2Announcer) run(ctx context.Context, health cell.Health) error {
	...
	for {
		...
		if err := l2a.processLocalNodeEvent(ctx, event); err != nil {
			l2a.params.Logger.Warn("Error processing local node event", slog.Any(logfields.Error, err))
		}
	...
	for {
		...
		case event, more := <-svcChan:
			...
			if err := l2a.processSvcEvent(event); err != nil {
				l2a.params.Logger.Warn("Error processing service event", slog.Any(logfields.Error, err))
			}
		...
		case event, more := <-policyChan:
			...
			if err := l2a.processPolicyEvent(ctx, event); err != nil {
				l2a.params.Logger.Warn("Error processing policy event", slog.Any(logfields.Error, err))
			}
		...
		case event := <-l2a.leaderChannel:
			if err := l2a.processLeaderEvent(event); err != nil {
				l2a.params.Logger.Warn("Error processing leader event", slog.Any(logfields.Error, err))
			}
		...
		case <-watchDevices:
			...
			if err := l2a.processDevicesChanged(ctx); err != nil {
				l2a.params.Logger.Warn("Error processing devices changed signal", slog.Any(logfields.Error, err))
			}
		}
	}
	...
	if policy.Spec.NodeSelector != nil {
		...
		if err != nil {
			if err2 := l2a.updatePolicyStatus(ctx, policy, "io.cilium/bad-node-selector", err); err2 != nil {
				l2a.params.Logger.Warn("updating policy status failed", slog.Any(logfields.Error, err2))
			}
		...
		if err := l2a.updatePolicyStatus(ctx, policy, "io.cilium/bad-node-selector", nil); err != nil {
			l2a.params.Logger.Warn("updating policy status failed", slog.Any(logfields.Error, err))
		}
...
func (l2a *L2Announcer) leaseTimings() (leaseDuration, renewDeadline, retryPeriod time.Duration) {
	...
	if leaseDuration < 1*time.Second {
		log.Warn(
			...
			slog.Duration("leaseDuration", leaseDuration),
		)
	...
	if renewDeadline < 1 {
		log.Warn(
			...
			slog.Duration("renewDeadline", renewDeadline),
		)
	...
	if retryPeriod < 1 {
		log.Warn(
			...
			slog.Duration("retryPeriod", retryPeriod),
		)
