...
func (n *Node) PrepareIPAllocation(scopedLog *logrus.Entry) (a *ipam.AllocationAction, err error) {
	...
	for _, iface := range n.manager.GetInterfaces(n.instanceIdLocked()) {
		...
		if a.InterfaceID != "" {
			...
			poolID, available := n.manager.Allocator().FirstPoolWithAvailableQuota(preferredPoolIDs)
...
func (n *Node) AllocateIPs(ctx context.Context, a *ipam.AllocationAction) error {
	ips, err := n.manager.Allocator().AllocateMany(a.PoolID, a.AvailableForAllocation)
	if err != nil {
		return err
	}
	...
	if err != nil {
		n.manager.Allocator().ReleaseMany(a.PoolID, ips)
		return err
	}
