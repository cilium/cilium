...
func (e *IPAMSuite) TestGetVpcsAndSubnets(c *check.C) {
	...
	c.Assert(mngr.Allocator().PoolExists("subnet-1"), check.Equals, false)
	c.Assert(mngr.Allocator().PoolExists("subnet-2"), check.Equals, false)
	c.Assert(mngr.Allocator().PoolExists("subnet-3"), check.Equals, false)

	iteration1(api, mngr)

	c.Assert(mngr.Allocator().PoolExists("subnet-1"), check.Equals, true)
	c.Assert(mngr.Allocator().PoolExists("subnet-2"), check.Equals, true)
	c.Assert(mngr.Allocator().PoolExists("subnet-3"), check.Equals, false)
	...
	c.Assert(mngr.Allocator().PoolExists("subnet-1"), check.Equals, true)
	c.Assert(mngr.Allocator().PoolExists("subnet-2"), check.Equals, true)
	c.Assert(mngr.Allocator().PoolExists("subnet-3"), check.Equals, true)
}
