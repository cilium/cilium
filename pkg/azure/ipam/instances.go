...
//
// The returned subnet is immutable so it can be safely accessed
//func (m *InstancesManager) FindSubnetByTags(vpcID, availabilityZone string, required ipam.Tags) (bestSubnet *types.Subnet) {
//	m.mutex.RLock()
//	defer m.mutex.RUnlock()
//
...
func (m *InstancesManager) GetInterfaces(instanceID string) []*v2.AzureInterface {
	m.mutex.RLock()
	defer m.mutex.RUnlock()
