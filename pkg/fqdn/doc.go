...
// with matching ToCIDRSet sections (in the same rule, thus inheriting the same
// L4/L7 policy). Each CIDR is a fully qualified IP (i.e. a /32 or /128) and
// each IP returned in the DNS lookup creates a corresponding CIDR.  The
// package relies on the internal policy logic to return early/trigger no
