...
func doResolverLogic(lookupFunc func(string, string, uint16) (*dns.Msg, error), dnsNames []string) (DNSIPs map[string]*DNSIPRecords, DNSErrors map[string]error) {
	...
	perTypeToQuery:
		for _, dnsType := range []uint16{dns.TypeA, dns.TypeAAAA} { // the dns library doesn't use dns.Type

			// Try the servers in the order they were configured in resov.conf
		perServerToAttempt:
			for _, server := range dnsConfig.Servers {
				response, err := lookupFunc(server+":"+dnsConfig.Port, dnsName, dnsType)
