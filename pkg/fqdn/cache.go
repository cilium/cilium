...
type DNSCache struct {
	...
	// updated is a map tracking the other DNSCaches that have contributed to the given DNSCache
	// since last GC round
	updated map[*DNSCache]struct{}
}
...
func NewDNSCache(minTTL int) *DNSCache {
	c := &DNSCache{
		...
		perHostLimit: 0,
		minTTL:       minTTL,
		updated:      make(map[*DNSCache]struct{}),
	}
	return c
}
...
func (c *DNSCache) ReplaceFromCacheByNames(namesToUpdate []string, maybeDeleteThese map[netip.Addr][]string, updates ...*DNSCache) {
	c.mu.Lock()
	defer c.mu.Unlock()

	// merge the DNSCaches from the endpoints present during the start of the GC, with
	// the ones used for updating the DNSCache. This ensures that new endpoints created after
	// the start of the GC iteration are also accounted for when updating the global cache.
	for _, update := range updates {
		c.updated[update] = struct{}{}
	}
	...
	c.forceExpireByNames(time.Now(), maybeDeleteThese, namesToUpdate)
...
func (c *DNSCache) UpdateAndCompare(lookupTime time.Time, name string, ips []netip.Addr, ttl int, cache *DNSCache) bool {
	if c.minTTL > ttl {
		ttl = c.minTTL
	}
...
func (c *DNSCache) forceExpireByNames(expireLookupsBefore time.Time, maybeDeleteThese map[netip.Addr][]string, names []string) {
