...
// LookupByRegex returns all non-expired cache entries that match re as a map
// of name -> IPs
func (c *DNSCache) LookupByRegex(re *regexp.Regexp) (matches map[string][]net.IP) {
	c.RLock()
	defer c.RUnlock()

	return c.lookupByRegexByTime(time.Now(), re)
}

// lookupByRegexByTime takes a timestamp for expiration comparisions, and is
// only intended for testing.
func (c *DNSCache) lookupByRegexByTime(now time.Time, re *regexp.Regexp) (matches map[string][]net.IP) {
