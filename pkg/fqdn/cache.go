...
func (c *DNSCache) cleanupExpiredEntries(expires time.Time) (affectedNames []string, removed map[string][]*cacheEntry) {
	...
	for _, name := range common.DeduplicateStrings(toCleanNames) {
...
func (c *DNSCache) GC(now time.Time, zombies *DNSZombieMappings) (affectedNames []string) {
	...
	return common.DeduplicateStrings(append(expiredNames, overLimitNames...))
}
...
func (c *DNSCache) ForceExpire(expireLookupsBefore time.Time, nameMatch *regexp.Regexp) (namesAffected []string) {
	...
	return common.DeduplicateStrings(namesAffected)
}
...
func (zombies *DNSZombieMappings) Upsert(expiryTime time.Time, ipStr string, qname ...string) (updatedExisting bool) {
	...
	if !updatedExisting {
		zombie = &DNSZombieMapping{
			Names:           common.DeduplicateStrings(qname),
			IP:              net.ParseIP(ipStr),
			DeletePendingAt: expiryTime,
		}
		zombies.deletes[ipStr] = zombie
	} else {
		zombie.Names = common.DeduplicateStrings(append(zombie.Names, qname...))
