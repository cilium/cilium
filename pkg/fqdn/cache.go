...
func (c *DNSCache) cleanupOverLimitEntries() []string {
	...
	for dnsName := range c.overLimit {
		...
		sort.Slice(sortedEntries, func(i, j int) bool {
			return sortedEntries[i].entry.ExpirationTime.Unix() < sortedEntries[j].entry.ExpirationTime.Unix()
		})
		...
		toClean = append(toClean, dnsName)
	}
...
// all DNS names in update are used.
// When clean is true the data in c for each specific name is cleared before the update.
func (c *DNSCache) UpdateFromCache(update *DNSCache, namesToUpdate []string, clean bool) {
	if update == nil {
		return
	}
