...
type FQDNStateEvent struct {
	Rows []FQDNMapping `json:"rows"`
}

func (event FQDNStateEvent) MarshalBinary() ([]byte, error) {
	return json.Marshal(event)
}

func UnmarshalFQDNStateEvent(data []byte) (FQDNStateEvent, error) {
	var event FQDNStateEvent
...
var (
	QueryByName       = fqdnNameIndex.Query
	QueryByIP         = fqdnIPIndex.Query
	QueryByExpiration = fqdnExpirationIndex.Query
)
