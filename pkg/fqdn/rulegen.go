...
func NewRuleGen(config Config) *RuleGen {
	...
	if config.AddGeneratedRulesAndUpdateSelectors == nil {
		config.AddGeneratedRulesAndUpdateSelectors = func(generatedRules []*api.Rule, selectorIPMapping map[string][]net.IP, namesMissingIPs []string) error {
			return nil
		}
	}
...
func (gen *RuleGen) PrepareFQDNRules(sourceRules []*api.Rule) ([]*api.Rule, map[string][]net.IP) {
	gen.Lock()
	defer gen.Unlock()

	result := []*api.Rule{}
	selectorIPMapping := make(map[string][]net.IP)
	var namesMissingIPs []string
perRule:
	for _, sourceRule := range sourceRules {
		...
		var selectorIPMappingTmp map[string][]net.IP
		...
		_, namesMissingIPs, selectorIPMappingTmp = injectToCIDRSetRules(sourceRule, gen.cache, gen.sourceRules)
...
func (gen *RuleGen) ForceGenerateDNS(namesToRegen []string) error {
	log.Infof("ForceGenerateDNS for namesToRegen: %s", namesToRegen)
...
func (gen *RuleGen) GenerateRulesFromSources(sourceRules []*api.Rule) (generatedRules []*api.Rule, namesMissingIPs []string, selectorIPMapping map[string][]net.IP) {
	gen.Lock()
	defer gen.Unlock()
