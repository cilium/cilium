...
func BenchmarkNotifyOnDNSMsg(b *testing.B) {
	...
	ciliumDetails, err := dnsproxy.ExtractMsgDetails(ciliumMsg)
	require.NoError(b, err)
	ebpfDetails, err := dnsproxy.ExtractMsgDetails(ebpfMsg)
	require.NoError(b, err)
	...
	for b.Loop() {
		for _, ep := range endpoints {
			...
			go func() {
				...
				// net.IP to string.
				require.NoError(b, handler.NotifyOnDNSMsg(time.Now(), ep, "10.96.64.8:12345", 0, srvAddr, ciliumDetails, "udp", true, emptyPRCtx))
				require.NoError(b, handler.NotifyOnDNSMsg(time.Now(), ep, "10.96.64.4:54321", 0, srvAddr, ebpfDetails, "udp", true, emptyPRCtx))
			}()
		}
		wg.Wait()
	}
