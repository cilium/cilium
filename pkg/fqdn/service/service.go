...
func (s *FQDNDataServer) OnIPIdentityCacheChange(modType ipcache.CacheModification, cidr types.PrefixCluster, oldHostIP, newHostIP net.IP, oldID *ipcache.Identity, newID ipcache.Identity, encryptKey uint8, k8sMeta *ipcache.K8sMetadata, endpointFlags uint8) {
	...
	if cidr.ClusterID() != 0 {
		s.log.Debug("Ignoring IP identity cache change for non-local CIDR", logfields.CIDR, cidr.String())
	}

	ipNet := cidr.AsIPNet()
	if !cidr.IsSingleIP() {
		s.log.Debug("Ignoring IP identity cache change for non-single IP CIDR", logfields.CIDR, cidr.String())
	}
...
func (s *FQDNDataServer) deleteFromIdentityToIPLocked(identity *ipcache.Identity, ip net.IP) error {
	...
	if ips, ok := s.currentIdentityToIp[identity.ID]; ok {
		for i, currIp := range ips {
			if currIp.Equal(ip) {
				s.currentIdentityToIp[identity.ID] = slices.Delete(ips, i, i+1)
				break
			}
		}
		// If the slice becomes empty, delete the map entry.
		if len(s.currentIdentityToIp[identity.ID]) == 0 {
