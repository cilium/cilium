...
func (ds *FQDNTestSuite) TestDNSPollerRuleHandling(c *C) {
	...
	for _, testCase := range testCases {
		...
		fqdnMarkedRules, err := poller.StartPollForDNSName(testCase.rulesToAdd)
		c.Assert(err, IsNil, Commentf("Error adding ToFQDN rules during test"))
		c.Assert(len(fqdnMarkedRules), Equals, len(testCase.rulesToAdd), Commentf("Number of FQDN marked rules is not the same as original list"))
		...
		// rules
		markedRulesToDelete := make([]*api.Rule, 0, len(testCase.rulesToDelete))
		for _, toDeleteRule := range testCase.rulesToDelete {
...
func (ds *FQDNTestSuite) TestDNSPollerCIDRGeneration(c *C) {
	...
	c.Assert(err, IsNil, Commentf("Error adding ToFQDN rules during test"))
	c.Assert(len(fqdnMarkedRules), Equals, len(rulesToAdd), Commentf("Number of FQDN marked rules is not the same as original list"))

	// store original UUID
	uuidOrig := getUUIDFromRuleLabels(fqdnMarkedRules[0])
	...
	// poll DNS once, check that we only generate 1 rule (for 1 IP) and that we
	// still have 1 ToFQDN rule, and that the IP, now different, is correct
	generatedRules = nil
	pollCount++
	err = poller.LookupUpdateDNS()
	c.Assert(err, IsNil, Commentf("Error generating IP CIDR rules"))
