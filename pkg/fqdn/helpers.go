...
// are made into FQDNs
func injectToCIDRSetRules(rule *api.Rule, cache *DNSCache, reMap *regexpmap.RegexpMap) (emittedIPs map[string][]net.IP, namesMissingIPs []string, selectorIPMapping map[string][]net.IP) {
	...
	for egressIdx := range rule.Egress {
		...
		for _, ToFQDN := range egressRule.ToFQDNs {
			...
			selectorIPMapping[fqdnAsString] = ips
	...
	return emitted, namesMissingIPs, selectorIPMapping
}
