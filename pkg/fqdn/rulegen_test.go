...
func (ds *FQDNTestSuite) TestRuleGenCIDRGeneration(c *C) {
	...
	// add rules
	ids, existed := gen.StartManagerFQDNSelector(ciliumIOSel)
	c.Assert(existed, Equals, false)
...
func (ds *FQDNTestSuite) TestRuleGenMultiIPUpdate(c *C) {
	...
	for _, sel := range selectorsToAdd {
		gen.StartManagerFQDNSelector(sel)
	}
	...
	ids, exists := gen.StartManagerFQDNSelector(githubSel)
	c.Assert(ids, IsNil)
	c.Assert(exists, Equals, true)

	gen.StopManagerFQDNSelector(githubSel)
	_, exists = gen.allSelectors[githubSel]
	c.Assert(exists, Equals, false)

	gen.StopManagerFQDNSelector(ciliumIOSel)
