...
var udpOOBSize = func() int {
	...
	if len(oob4) > len(oob6) {
		return len(oob4) + unix.SizeofSockaddrInet4
	}

	return len(oob6) + unix.SizeofSockaddrInet6
}()
...
func transparentSetsockopt(fd int) error {
	...
	if err == nil {
		log.Printf("Set IPV6_TRANSPARENT on fd %d", fd)
		...
		if err == nil {
			log.Printf("Set IP_TRANSPARENT on fd %d", fd)
...
func (s *sessionUDPFactory) InitConn(conn *net.UDPConn) error {
	...
	transparentSetsockopt(int(UDPFile.Fd()))
	return err
}
...
func parseDstFromOOB(oob []byte) (*net.UDPAddr, error) {
	...
	for _, msg := range msgs {
		if msg.Header.Level == unix.SOL_IP && msg.Header.Type == unix.IP_ORIGDSTADDR {
			...
			offset := unsafe.Offsetof(pp.Family)
			family := *(*uint16)(unsafe.Pointer(&msg.Data[offset]))
