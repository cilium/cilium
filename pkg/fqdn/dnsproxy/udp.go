...
func transparentSetsockopt(fd int, ipv4, ipv6 bool) error {
	...
	if ipv4 {
		...
		// Only error out if both IPv4 and IPv6 fail, or if either fails and the
		// other is disabled (the !ip* bits).
		if (!ipv4 || err4 != nil) && (!ipv6 || err6 != nil) {
			return err4
		}
	}
	return nil
...
func (f *sessionUDPFactory) SetSocketOptions(conn *net.UDPConn) error {
	...
	udpOnce.Do(func() {
		rawconn4 = bindUDP("127.0.0.1", f.ipv4Enabled, f.ipv6Enabled) // raw socket for sending IPv4
		rawconn6 = bindUDP("::1", f.ipv4Enabled, f.ipv6Enabled)       // raw socket for sending IPv6
	})
