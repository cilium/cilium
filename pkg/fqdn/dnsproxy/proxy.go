...
// EndpointNotifyContext is a struct that stores the DNS request/response
// context related to an endpoint.
type EndpointNotifyContext struct {
	EpAddr string
	EpID   int64
}
...
// See DNSProxy.LookupEndpointIDByIP for usage.
type NotifyOnDNSMsgFunc func(lookupTime time.Time, epContext EndpointNotifyContext, serverAddr string, msg *dns.Msg, protocol string, allowed bool, stat ProxyRequestContext) error
...
func (p *DNSProxy) ServeDNS(w dns.ResponseWriter, request *dns.Msg) {
	...
	epContext := EndpointNotifyContext{}
	var err error

	scopedLog.Debug("Handling DNS query from endpoint")
	epContext.EpAddr, _, err = net.SplitHostPort(w.RemoteAddr().String())
	if err != nil {
		...
		p.NotifyOnDNSMsg(time.Now(), epContext, "", request, protocol, false, stat)
		p.sendRefused(scopedLog, w, request)
		return
	}
	epId, err := p.LookupEndpointIDByIP(net.ParseIP(epContext.EpAddr))
	if err != nil {
	...
	epContext.EpID, _ = strconv.ParseInt(epId, 0, 64)

	scopedLog = scopedLog.WithField(logfields.EndpointID, epContext.EpID)
	...
	if err != nil {
		...
		stat.ProcessingTime.End(false)
		p.NotifyOnDNSMsg(time.Now(), epContext, targetServerAddr, request, protocol, false, stat)
		p.sendRefused(scopedLog, w, request)
		return
	}
	...
	p.NotifyOnDNSMsg(time.Now(), epContext, targetServerAddr, request, protocol, true, stat)
	...
	if err != nil {
		...
		p.NotifyOnDNSMsg(time.Now(), epContext, targetServerAddr, response, protocol, true, stat)
	}
