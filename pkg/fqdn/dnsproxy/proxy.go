...
func bindToAddr(address string, port uint16) (*net.UDPConn, *net.TCPListener, error) {
	...
	listener, err = listenConfig(0xb00).Listen(context.Background(), "tcp", bindAddr)
	if err != nil {
		return nil, nil, err
	}

	conn, err = listenConfig(0xb00).ListenPacket(context.Background(), "udp", listener.Addr().String())
	if err != nil {
		return nil, nil, err
	}
