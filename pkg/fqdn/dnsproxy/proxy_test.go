...
func (s *DNSProxyTestSuite) SetUpTest(c *C) {
	...
	proxy, err := StartDNSProxy("", 0, true, 1000, // any address, any port, enable compression, max 1000 restore IPs
		...
		func(ip net.IP) (*endpoint.Endpoint, error) {
			...
			return endpoint.NewEndpointWithState(s, s, &endpoint.FakeEndpointProxy{}, testidentity.NewFakeIdentityAllocator(nil), uint16(epID1), endpoint.StateReady), nil
		},
		// LookupSecIDByIP
...
func (s *DNSProxyTestSuite) TestFullPathDependence(c *C) {
	...
	// Restore rules
	ep1 := endpoint.NewEndpointWithState(s, s, &endpoint.FakeEndpointProxy{}, testidentity.NewFakeIdentityAllocator(nil), uint16(epID1), endpoint.StateReady)
	...
	ep3 := endpoint.NewEndpointWithState(s, s, &endpoint.FakeEndpointProxy{}, testidentity.NewFakeIdentityAllocator(nil), uint16(epID3), endpoint.StateReady)
