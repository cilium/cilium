...
func setupDNSProxyTestSuite(tb testing.TB) *DNSProxyTestSuite {
	...
	proxy := NewDNSProxy(dnsProxyConfig,
		...
		func(lookupTime time.Time, ep *endpoint.Endpoint, epIPPort string, serverID identity.NumericIdentity, dstAddr netip.AddrPort, msg *dns.Msg, protocol string, stat *ProxyRequestContext) error {
			return nil
		},
		func(lookupTime time.Time, ep *endpoint.Endpoint, epIPPort string, serverID identity.NumericIdentity, dstAddr netip.AddrPort, msg *dns.Msg, protocol string, stat *ProxyRequestContext) error {
			return nil
