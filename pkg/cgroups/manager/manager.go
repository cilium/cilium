	...
	for _, c := range pod.Status.ContainerStatuses {
		...
		if !found || after == "" {
			m.logger.Error(
				...
				slog.String(logfields.K8sNamespace, pod.Namespace),
				slog.String("container-id", cId),
			)
			continue
		}
		cId = after
		...
		if err != nil {
			m.logger.Debug(
				...
				slog.String(logfields.K8sPodName, pod.Name),
				slog.String(logfields.K8sNamespace, pod.Namespace),
				slog.String("cgroup-path", cgrpPath),
			)
			continue
		}
	...
	for _, cm := range m.containerMetadataByCgrpId {
		...
		if !ok {
			m.logger.Debug(
				...
				slog.Uint64("container-cgroup-id", cm.cgroupId),
			)
			continue
		}
