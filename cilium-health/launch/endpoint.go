...
func LaunchAsEndpoint(owner endpoint.Owner, hostAddressing *models.NodeAddressing) error {
	...
	if _, _, err := plugins.SetupVethWithNames(vethName, vethPeerName, mtu.StandardMTU, info); err != nil {
		return fmt.Errorf("Error while creating cilium-health veth: %s", err)
	}
	...
	if err := cmd.Start(); err != nil {
		...
		return fmt.Errorf("Error spawning cilium-health endpoint (%q): %s", target, err)
	}

	// Create the endpoint
	ep, err := endpoint.NewEndpointFromChangeModel(info)
	if err != nil {
		return fmt.Errorf("Error while creating cilium-health endpoint: %s", err)
	}
	...
	for {
		if _, err := os.Stat(pidfile); err == nil {
			...
			return fmt.Errorf("Cilium endpoint failed to run: %s", err)
		} else {
	...
	if err = configureHealthRouting(info.ContainerName, vethPeerName, hostAddressing); err != nil {
		return fmt.Errorf("Error while configuring cilium-health routes: %s", err)
	}

	// Add the endpoint
	if err := endpointmanager.AddEndpoint(owner, ep, "Create cilium-health endpoint"); err != nil {
		return fmt.Errorf("Error while adding cilium-health endpoint: %s", err)
	}
