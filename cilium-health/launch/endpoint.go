...
func configureHealthRouting(netns, dev string, addressing *models.NodeAddressing) error {
	routes := []route.Route{}
	v4Routes, err := connector.IPv4Routes(addressing, mtu.StandardMTU)
	if err == nil {
		routes = append(routes, v4Routes...)
	} else {
		log.Debugf("Couldn't get IPv4 routes for health routing")
	}
	v6Routes, err := connector.IPv6Routes(addressing, mtu.StandardMTU)
	if err != nil {
...
func LaunchAsEndpoint(owner endpoint.Owner, hostAddressing *models.NodeAddressing) error {
	...
	if _, _, err := connector.SetupVethWithNames(vethName, vethPeerName, mtu.StandardMTU, info); err != nil {
