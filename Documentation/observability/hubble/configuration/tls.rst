...
.. tabs::
    ...
    .. group-tab:: Custom Per-Pod Certificates
        ...
        you can disable the default TLS certificate volumes and provide your
        own via ``extraVolumes``/``extraVolumeMounts``, and inject a
        certificate-fetching sidecar via ``extraInitContainers``.

        **How it works:**

        Setting ``disableDefaultVolumes`` to ``true`` removes the default
        projected TLS volume and its mount from the component. You then
        provide your own volume and mount via ``extraVolumes`` and
        ``extraVolumeMounts``.

        You should also set ``tls.auto.enabled=false`` to prevent the chart
        from generating certificates you don't need.
        ...
        **Configuration:**
        ...
        Disable the default TLS volumes for each component and provide your
        own via ``extraVolumes`` and ``extraVolumeMounts``:

        Hubble server TLS (on the cilium-agent DaemonSet):

        ::

            --set hubble.tls.disableDefaultVolumes=true

        The certificate agent sidecar for cilium-agent is configured via the
        top-level ``extraInitContainers``.
        ...
        The certificate agent sidecar for Hubble Relay is configured via
        ``hubble.relay.extraInitContainers``.
        ...
        The certificate agent sidecar for Hubble UI is configured via
        ``hubble.ui.extraInitContainers``.
