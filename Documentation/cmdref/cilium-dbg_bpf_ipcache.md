<!-- This file was autogenerated via cilium cmdref, do not edit manually-->

## cilium-dbg bpf ipcache

Manage the IPCache mappings for IP/CIDR <-> Identity

### Options

```
  -h, --help   help for ipcache
```

### Options inherited from parent commands

```
      --config string   Config file (default is $HOME/.cilium.yaml)
  -D, --debug           Enable debug messages
  -H, --host string     URI to server-side API
```

### SEE ALSO

* [cilium-dbg bpf](cilium-dbg_bpf.md)	 - Direct access to local BPF maps
* [cilium-dbg bpf ipcache delete](cilium-dbg_bpf_ipcache_delete.md)	 - Delete an entry for an ip
* [cilium-dbg bpf ipcache get](cilium-dbg_bpf_ipcache_get.md)	 - Retrieve identity for an ip
* [cilium-dbg bpf ipcache list](cilium-dbg_bpf_ipcache_list.md)	 - List endpoint IPs (local and remote) and their corresponding security identities
* [cilium-dbg bpf ipcache update](cilium-dbg_bpf_ipcache_update.md)	 - Update an entry for an ip

