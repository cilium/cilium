...
```
  ...
  -h, --help                                                      help for hive
      ...
      --only-masquerade-default-pool                              When using multi-pool IPAM, only masquerade flows from the default IP pool. This will preserve source IPs for pods from non-default IP pools. Useful when combining multi-pool IPAM with BGP control plane and tunneling. This option must be combined with enable-bpf-masquerade.
