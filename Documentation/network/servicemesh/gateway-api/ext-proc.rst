...
rules via ``ExtensionRef``. Installing Cilium with Gateway API support creates
the CRD.

.. Note::

    Creating the CRD does not permit Routes to reference it. You must explicitly
    enable Route references with the Helm value
...
.. Note::
    ...
    This example is not runnable as-is because it includes a placeholder image
    for the ext_proc server Deployment. Before applying the manifest, replace the
    placeholder image in the ``ext-proc-service`` Deployment with an
...
After replacing the placeholder image and deploying the resources, requests to
``/api`` pass through the ext_proc filter before reaching the backend. The
external processor can inspect or modify headers, and the response reflects any
changes it makes.
...
.. Note::
    ...
    Cilium places ``ext_proc`` **before** ``ext_authz`` in the Envoy HTTP filter
    chain. Header mutations performed by the external processor—including
    ``clear_route_cache``—are therefore visible to the ``ext_authz`` filter, and
    authorization decisions see the request in its fully-processed form.

ExtensionRef ordering and validity
==================================

Cilium preserves the declaration order of ext_proc ``ExtensionRef`` filters
within a rule. Every ext_proc reference must appear before ``ExternalAuth``.
The supported form is zero or more ext_proc references followed by the optional
``ExternalAuth`` filter. A rule using ``ExternalAuth`` before ext_proc, or
interleaving the two, is rejected fail-closed. Repeating the same
``cilium.io/CiliumEnvoyExtProcFilter`` reference within one rule is also
rejected; reusing a filter from different rules or Routes is supported.

When only some rules are invalid, the Route remains ``Accepted=True`` and gets
``PartiallyInvalid=True``. Its message starts with ``Dropped Rule`` and names
each invalid rule and cause. Each dropped rule returns a synthetic HTTP 500
with ext_proc and ``ext_authz`` disabled. If every rule in the attachment is
invalid, the Route is ``Accepted=False`` with ``OrderingConflict`` for an
aggregate ordering conflict or ``IncompatibleFilters`` for declaration-order
and duplicate violations. These conditions are scoped to the affected Gateway,
ListenerSet, or GAMMA Service parent; other parents are reconciled separately.

The ext_proc ``backendRef`` must resolve to an existing Kubernetes Service and
its declared numeric port must be present in ``Service.spec.ports``. A missing
Service or port sets ``ResolvedRefs=False`` with ``BackendNotFound`` and the
affected rule fails closed. EndpointSlice existence and endpoint readiness are
runtime concerns and do not change reference resolution.

.. Note::

    Fail-closed semantics apply at two distinct levels. At configuration time,
    if Cilium cannot resolve the ``ExtensionRef`` (for example, the
    ``CiliumEnvoyExtProcFilter`` resource does not exist or a cross-namespace
    ``ReferenceGrant`` is missing), the affected route is programmed to return
    HTTP 500 until the reference is resolved. At request time, if the ext_proc
    service is unreachable, requests are always rejected. Cilium does not expose
    Envoy's per-request fail-open option.
...
resource identity using the format ``ceepf.<namespace>.<name>.``. Within the
namespace and name components, ``-`` is encoded as ``_2d`` and ``.`` as
``_2e``. For a filter named ``my-ext-proc`` in namespace ``default``, the
stat_prefix is ``ceepf.default.my_2dext_2dproc.``. This injective encoding keeps
valid resource identities such as ``foo-bar`` and ``foo.bar`` distinct while
allowing multiple ext_proc filters on the same Gateway to be distinguished in
Envoy metrics without adding a Cilium-specific metric API.
...
``message_timeouts``, ``streams_failed``, ``spurious_msgs_received``, and
...
``messageTimeout`` (optional)
    Timeout for an individual message exchange with the ext_proc service. The
    value uses GEP-2257 duration syntax and must not exceed ``1h``. A bare ``0``
    is invalid because the value requires a unit. Setting ``0s`` starts an
    immediate timeout and fails every message. If omitted, Envoy uses its 200 ms
    default. Neither ``0s`` nor omission disables the timeout.
