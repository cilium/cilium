...
.. Note::
    ...
    service is unreachable, requests are rejected by default. Per-request
    fail-open—where Envoy forwards the request even when the ext_proc service
    cannot be reached—is controlled by the ``failureModeAllow`` field on the
    ``CiliumEnvoyExtProcFilter`` resource.
