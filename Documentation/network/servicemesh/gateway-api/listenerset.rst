...
Send a request to the listener after the Gateway is programmed and the
backend is ready:

.. code-block:: shell-session

    $ kubectl wait gateway/shared-gateway --for=condition=Programmed --timeout=120s
    $ kubectl -n listenerset-demo rollout status deployment/echo-1
