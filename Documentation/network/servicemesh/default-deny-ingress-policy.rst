...
Let's apply clusterwide network policy to deny all the traffic by default.
...
With this policy applied, the request to ``/details`` endpoint will be denied for external and in-cluster traffic.
...
Let's check if the in-cluster traffic to the same endpoint is being denied:
