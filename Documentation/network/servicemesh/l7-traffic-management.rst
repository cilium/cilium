...
* ``CiliumEnvoyConfig`` and ``CiliumClusterwideEnvoyConfig`` resources
  are implementation details. Both should be treated as cluster admin
  resources and permissions for their configuration should not be delegated to
  other users.
