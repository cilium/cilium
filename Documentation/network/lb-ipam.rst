...
LB IPAM work in conjunction with features like the :ref:`bgp_control_plane`. Where
...
LB IPAM is implicitly enabled when dependant features are enabled.
...
A basic IP Pools with both an IPv4 and IPv6 range looks like.
...
.. note::
    The amount of available IPs in the pool is less than the actual sum of all
    usable IPs in the CIDRs because the allocation logic is limited to 65536 IPs
    per CIDR. CIDRs containing more than 65536 IPs can be broken up into multiple
...
Therefor administrators should always check the status of all pools after making
modifications.

If we for example add 2 pools(``blue-pool`` and ``red-pool``) both with the same
...
the amount of used and available IPs. A machine parsable output can be gotten like so.
...
When LB IPAM is enabled it assumed that it is supposed to allocate and assign IPs for services with 
no load balancer class set.

LB IPAM only does IP allocation and doesn't provide load balancing services by itself. Therefor
...
Services can request specific IPs. The legacy wait of doing so is via ``.spec.loadBalancerIP``
...
if the services doesn't match the pool's selector.
