...
a. set a lower MTU value in the Cilium ConfigMap to account for the overhead.
   ...
   fragmentation. However, this is strongly discouraged as it would have a relevant
...
b. modify the KataPod deployment by adding an ``initContainer`` (with NET_ADMIN)
   ...
   either manually computed or retrieved by issuing ``ip route`` on a Cilium Node
           ...
           - name: set-mtu
             ...
             command:
               [
                 "sh",
                 "-c",
                 "DEFAULT=$(ip r | grep default); ip route replace $DEFAULT mtu 1450",
               ]
             securityContext:
               capabilities:
                 add: ["NET_ADMIN"]
