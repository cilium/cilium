...
GKE Metadata Server with OverrideIP
------------------------------------
The GKE metadata server is a pod running in the host network namespace. The new version of GKE metadata server only
listens a specific secondary address from the loopback interface. GKE uses the IP table rule to redirect the traffic
...
Assume you have the following helm values for Cilium:
...
The GKE metadata server pod is deployed and bind to the ``169.254.169.252:988``.
...
You can check local redirect policy to see the policy has been applied from cilium agent through the following commands:
