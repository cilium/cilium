...
This is useful when the backend pod is running in the host network namespace where it could only listen on the specific address from loopback interface(such as GKE metadata server).
...
.. note::
    ...
    Even though you can configure the ``overrideIP`` field with ``redirectFrontend`` in the policy, it will not be used.