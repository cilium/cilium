...
.. code-block:: yaml
    ...
    spec:
      description: "Allow x-wing deployed in the local cluster to contact rebel-base in cluster2"
      endpointSelector:
        matchLabels:
          name: x-wing
      egress:
...
To restrict endpoint selection to the local cluster by default you can enable the option ``--enable-default-restrict-local-cluster-policy``
via the ConfigMap option ``enable-default-restrict-local-cluster-policy`` or the Helm value ``clustermesh.enableDefaultRestrictLocalClusterPolicy``.
...
.. code-block:: yaml
    ...
    spec:
      description: "Allow x-wing deployed in the local cluster to contact rebel-base in any clusters"
      endpointSelector:
        matchLabels:
          name: x-wing
      egress:
