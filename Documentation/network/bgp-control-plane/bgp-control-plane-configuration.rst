...
.. note::
   ...
   To peer with a specific IPv6 link-local address instead of discovering it, set
   ``peerAddress`` to that address and ``peerInterface`` to the interface it is reachable
   on, without using ``autoDiscovery``.
