...
can inspect installed BGP policies using cilium-dbg CLI from the Cilium agent pod.
...
Cilium node on which BGP control plane is enabled based on `CiliumBGPClusterConfig` node selector gets associated
