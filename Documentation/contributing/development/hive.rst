...
This is helped by having a help system in place (``help``) and the ability to interactively try things out
with the ``break`` statement. Here's what would happen if you'd add ``break`` as the last command in ``example.txtar``:
