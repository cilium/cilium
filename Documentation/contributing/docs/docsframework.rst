...
- The build includes running ``check-build.sh``. This script:
  ...
  e. For ``html`` and ``dirhtml`` targets, generates Lunr search index files
     under ``_static/lunr/`` via ``Documentation/_scripts/build_lunr_index.py``

- The Lunr search index is generated by
  ``Documentation/_scripts/build_lunr_index.py`` from a built ``dirhtml``
  output tree.

  - Docker/CI builds that call ``check-build.sh`` now generate Lunr index files
    automatically for ``html`` and ``dirhtml`` outputs.
  - ReadTheDocs runs this script automatically via
    ``Documentation/.readthedocs.yaml`` ``post_build``.
  - For local development, contributors can run ``make -C Documentation
    dirhtml-with-index`` to build ``dirhtml`` and regenerate Lunr index data.
...
The documentation site uses a Lunr.js-based modal search UI.

- Front-end assets live in ``Documentation/_static/lunr/``.
- The search runtime uses a Web Worker (``lunr-worker.js``) to keep query work
  off the main UI thread.
- Search data files are generated at build time from rendered HTML:

  - ``_static/lunr/lunr_index.json``
  - ``_static/lunr/docs.json``
  - ``_static/lunr/manifest.json``

The indexer script is ``Documentation/_scripts/build_lunr_index.py``.
