...
For overview of ClusterLoader2, please refer to the `Readme <CL2_README_>`_ and `Getting Started <CL2_GETTING_STARTED_>`_.
...
* FQDN performance test - ``/fqdn-perf`` `Workflow <FQDN_PERF_WORKFLOW_>`_
  a simple two-node test that deploys pods with FQDN policies
  and measures the time it takes to resolve FQDNs from client point of view.
...
* Egress Gateway scale test - ``/scale-egw``. `Workflow <EGW_WORKFLOW_>`_ tests Egress Gateway on a small cluster,
  but with synthetically created Endpoints and Nodes to simulate large cluster.

Whenever developing new tests, you should consider if you want to add a test to already existing workflow,
create a new one or maybe extend the existing test.
If you are unsure, you can always ask in the ``#sig-scalabilty`` `Slack channel <SLACK_CHANNEL_>`_.
For example, if you want to run test on a large cluster,
you might consider adding it as a separate test scenario to already existing 100 nodes scale test
to reduce cost of CI, as spinning up a new cluster and tearing it down is quite a long process.
For some use-cases, it might be better to simulate large cluster and execute test on a small cluster,
like in the case of Egress Gateway scale test or ClusterMesh scale test.
...
For example, let's run network policy scale test in local kind cluster.
First, setup a kind cluster with Cilium as documented in :ref:`dev_env`.
Build ClusterLoader2 binary from `perf-tests repository <CL2_>`_.
Then you can run:
...
* ``--tear-down-prometheus-server=false`` - Leaves Prometheus and Grafana running after the test finishes, this helps with speeding up the test run
...
At the end of output, test should end successfully with:
...
* ``'GenericPrometheusQuery NetPol Average CPU Usage_netpol_.*.json'`` - ``GenericPrometheusQuery`` test results contain results of the prometheus queries executed during test.
  In this example, it contains CPU usage of the Cilium agents. All of the Prometheus Queries will be automatically
  visualized in :ref:`perfdash <perfdashdocs>`.
* ``'PodPeriodicCommand.*Profiles-stdout.*'`` - Contains memory and CPU profiles gathered during the test run. To see how to read them, please refer to the :ref:`profiling`


Accessing Grafana and Prometheus during test run
""""""""""""""""""""""""""""""""""""""""""""""""

During test execution, ClusterLoader2 deploys Prometheus and Grafana to the cluster.
...
Then you just need to pass additional argument to ClusterLoader2:
...
Now you can use the additional metrics in your test, by levaraging regular ``GenericPrometheusQuery`` measurement.
For example, Egress Gateway ensures that various percentiles of masquarade latency observed by client are
`below specific thresholds <_EGW_MASQ_METRICS>`_. This can be achiveved by following measurement in ClusterLoader2:
...
You can base your Github workflow on the existing tests, or add test scenario to already existing workflow.
...
After test run, all of the results will be saved in GS bucket.
In workflow run, you will see a link to the test results, for example in `test run <TEST_RUN_>`_:
...
Currently, we do not host publicly available instance of perfdash.
To visualize the results, please check the `Scaffoling repository <PERFDASH_>`_.
As an example, you can check how average CPU usage of the Cilium agent changed recently:

.. image:: /images/perfdash.png
    :align: center

Note that you can click on the graph and it will redirect you to Google Storage containing all of the results
...
during test run and can be used to further improve alerting in CI based on existing metrics.

For example, snapshot can be found in directory
``gs://cilium-scale-results/logs/scale-100-main/1745287079/artifacts/prometheus_snapshot.tar.gz``.
You just need to extract it and run Prometheus locally:
...
Simplest way to visualize them is to leverage `pprof-merge <PPROF_MERGE_>`_.
Example commands to aggregate CPU and memory profiles from whole test run:
...
.. code-block:: bash
    ...
    rm -rf cilium-bugtool* PodPeriodicCommand*
...
.. _EGW_MASQ_METRICS: https://github.com/isovalent/cilium/blob/main-ce/.github/actions/cl2-modules/egw/modules/masq-metrics.yaml
...
.. _TEST_RUN: https://github.com/cilium/cilium/actions/runs/14584355333
