...
external, peers. It allows tracking changing IPs that may not be known a
priori. IP information is collected into a shared cilium-agent cache (i.e. it
is shared by all containers and pods managed by Cilium on a node), and then
inserted into generated L3 `CIDR based` rules. Multiple matching ``toFQDNs``
selectors can be included in a single egress rule.
...
.. note:: DNS based rules are intended for external connections. See `Services
          based`_ and `Labels based`_ for cluster-internal traffic.
...
DNS Proxy
  ...
  Intercepting DNS requests allows Cilium to generate the IPs allowed by policy
  from the IPs received in DNS responses to applications. It is also required
  to implement wildcard policies in ``matchPattern``, as they cannot be polled.
