...
``toFQDNs`` simplifies specifying Layer 3 egress policy to IPs of remote,
external, peers. It allows tracking changing IPs or sets of IPs that may not be
known a priori. IP information is collected into a shared cilium-agent cache
(i.e. it is shared by all containers and pods managed by cilium on a node), and
then inserted into generated L3 `CIDR based` rules. Multiple matching
``toFQDNs`` selectors can be included in a single egress rule.
...
IPs to be inserted are selected via:
...
Obtaining DNS Data
~~~~~~~~~~~~~~~~~~
IPs are obtained via DNS polling or intercepting DNS requests via a proxy, and
matching names are inserted irrespective of how the data is obtained.
...
DNS Proxy
  ...
  policy on DNS requests see `Layer 7 Examples`_.

  Intercepting DNS requests allows ``cilium`` to match the IPs allowed by
  policy to the IPs received in DNS responses to applications. It also required
  to implement wildcard policies in ``matchPattern``.
...
DNS Polling
  ...
  ``cilium-agent``. The result is used to regenerate endpoint policy.  Despite
  the name, the ``matchName`` field does not have to be a fully-qualified
  domain name. In cases where search domains are configured for
  ``cilium-agent``, the DNS lookups from ``cilium`` will not be qualified and
  will utilize the search list. Unqualified names must be matched as-is by
  ``matchPattern`` in order to insert related IPs.

  DNS lookups are repeated with an interval of 5 seconds, and are made for
  A(IPv4) and AAAA(IPv6) addresses. Should a lookup fail, the most recent IP
  data is used instead. An IP change will trigger a regeneration of the
  ``cilium`` policy for each endpoint, and the updated IPs can be seen in the
  response from ``cilium policy get``. Each update will also increment the per
  ``cilium-agent`` policy repository revision.
  ...
  #. When using `DNS Polling`_ with DNS responses that return a new IP on every
...
considered). This policy is effected via a DNS proxy, and is also used to
collect IPs to be used in L3 `DNS based`_ ``toFQDNs`` rules.

.. note::  While Layer 7 DNS policy can be applied without any other Layer 3
           rules, the presence of a Layer 7 rule (with it's Layer 3 and 4
...
allowed but connections to the returned IPs are not, as there is no L3 toFQDNs
rule selecting them. L4 and L7 policy may also be applied (see `DNS based`_),
restricting connections to TCP port 80 in this case.
