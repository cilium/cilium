...
As a beta feature, the Cilium bandwidth manager is disabled by default for new deployments via Helm.
The option is controllable through ``bandwidthManager`` with a
possible setting of ``true`` and ``false`` (default).
