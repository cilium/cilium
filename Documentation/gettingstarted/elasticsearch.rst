...
* ``index: troop_logs``: Stormtroopers performance logs collected from every outpost and used to identify and eliminate weak performers!
* ``index: spaceship_diagnostics``: Spaceships diagnostics data collected from every spaceship and used for R&D and improvement of the spaceships.

Every outpost has an Elasticsearch client service to upload the Stormtroopers logs. And every spaceship has a service to upload diagnostics. Similarly, the Empire headquarters has a service to search and analyze the troop logs and spaceship diagnostics data. Before we look into the security concerns, lets first create this application scenario in minikube.
...
Testing the security policies, firstly, the search access is blocked for both outpost and spaceship. So from a compromised outpost, rebels will not be able to search and obtain knowledge about troops and spaceship diagnostics. Secondly, the outpost clients don't have access to create or update the ``index: spaceship_diagnostics``. 
