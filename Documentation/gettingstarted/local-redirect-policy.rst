...
The Cilium Local Redirect Policy feature is enabled by default on new deployments
via Helm:
...
``CustomResource`` in order to match the traffic that needs to be redirected.
...
for which traffic needs to be redirected.
...
.. code-block:: bash
    ...
    $ kubectl exec mediabot -- curl -I -s http://172.20.0.51:80/index.html
    HTTP/1.1 200 OK
    Server: nginx/1.19.2
...
Limitations
===========
When you create a CiliumLocalRedirectPolicy, traffic for all the new connections
