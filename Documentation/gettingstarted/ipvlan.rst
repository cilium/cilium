...
networking device such as ``"eth0"`` or ``"bond0"``, for example. For
deployment, this device must have consistent naming on each host.

The ipvlan datapath only supports direct routing mode, therefore
tunneling is disabled through setting ``tunnel`` to ``"disabled"``.
