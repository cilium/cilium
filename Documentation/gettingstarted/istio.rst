...
While the upstream `Istio Bookinfo
Applicatio example for Kubernetes
<https://istio.io/docs/examples/bookinfo/#if-you-are-running-on-kubernetes>`_ deploys multiple versions of the Bookinfo application at the same time (using ``${ISTIO_VERSION}/samples/bookinfo/platform/kube/bookinfo.yaml``), here we first deploy only the version 1.
