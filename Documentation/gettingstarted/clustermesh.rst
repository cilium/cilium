...
=======
 ...
 #. Validate that the ``cilium-xxx`` as well as the ``cilium-operator-xxx` pods
    are healthy and ready. Is is important that the ``cilium-operator`` is
    healthy as well as it is responsible to synchronize state from the local
    cluster into the kvstore. If this fails, check the logs of these pods to
    track the reason for failure.

 #. Validate that the ClusterMesh subsystem is initialized by looking for a log
    message like this:
...
==========================
 ...
 #. Validate that the configuration for remote clusters is picked up correctly.
    ...
    configuration`` along with the remote cluster name must be logged.
 ...
 #. Validate that the connection to the remote cluster could be established.
    You will see a log message like this for each remote cluster:
