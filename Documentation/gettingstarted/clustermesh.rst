...
The clustermesh control plane performs TLS based authentication and encryption.
...
2. Edit the ``cilium`` DaemonSet in each cluster and add the above section to
   the template spec:
...
automatically span across clusters. Note that Kubernetes security policies are
