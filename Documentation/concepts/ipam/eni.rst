...
``spec.eni.security-group-tags``
  ...
  If unspecified, the security groups passed in
  ``spec.eni.security-groups`` will be used.
...
The security groups attached to the ENI are computed in the following order:
 ...
 3. Finally if none of the above fields are set then the newly created ENI will
    inherit the security groups of ``eth0`` of the machine.
