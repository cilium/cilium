...
+-------------------------+---------------------------------------------------+
| reserved:non-local-node | The collection of all non-local cluster hosts.    |
...
.. note::
   ...
   default ConfigMap is used. The non-local node identity can be enabled via
   the option ``enable-non-local-node-identity``.
