...
#. The default value for the NAT table size parameter ``bpf-nat-global-max`` in
   ...
   .. code:: bash
      ...
      sudo grep -R SNAT_MAPPING_IPV[46]_SIZE /var/run/cilium/state/templates/

   If the maximum number is 524288, no action is required. If the number is
   different, ``bpf-nat-global-max`` needs to be adjusted in the `ConfigMap`
   to the value shown by the command above (841429 in the example below):
