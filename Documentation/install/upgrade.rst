...
Since Cilium might need more, or less permissions to access Kubernetes
...
``RBAC`` rules and the ``DaemonSet`` file in separate, which makes sure the
`ConfigMap` initially set up by ``cilium.yaml`` and already stored in Kubernetes
will not be affected by the upgrade.

It is also recommended to upgrade the `ConfigMap`, but is a process that
...
Simply pick your Kubernetes version and run the ``kubectl apply`` for the
