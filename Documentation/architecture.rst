...
By default Cilium operates in veth-based datapath mode which allows for more
flexibility in that all BPF programs are managed by Cilium out of the host
...
For a more latency optimized datapath, Cilium also supports ipvlan L3 mode with
a number of restrictions. In order to support older kernel's without ipvlan's
...
The ipvlan mode in Cilium can be enabled by running the Cilium daemon with
e.g. `--device eth0 --datapath-mode ipvlan` where `--device` specifies the
physical device used in Cilium's direct routing mode that then also acts as
