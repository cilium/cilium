...
* The Helm value of ``enableIPv4Masquerade`` in ``eni`` mode will change from ``true`` to ``false`` by default from 1.18.
  If you would like to keep the ``enableIPv4Masquerade`` to be ``true``, you could explicitly set the value or use ``upgradeCompatibility`` 
  lower than 1.18.
