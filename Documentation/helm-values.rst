...
.. list-table::
   ...
   * - :spelling:ignore:`hubble.metrics.tls.server.mtls.name`
     ...
     - string
     - ``nil``
   * - :spelling:ignore:`hubble.networkPolicyEnrichment`
     - Enables network policy enrichment of Hubble flows, i.e. populating ``egress_allowed_by``\ , ``ingress_denied_by`` fields with policy information.
     - object
     - ``{"enabled":true}``
