...
.. list-table::
   ...
   * - :spelling:ignore:`nativeRoutingCIDRFromClusterPool`
     ...
     - bool
     - ``false``
   * - :spelling:ignore:`networkPolicy.aggregateIdentities`
     - Enable policy-map identity aggregation. This can significantly reduce policy map size when using supported entities. The default is disabled. Changing from enabled to disabled may cause brief connectivity interruptions.
     - bool
     - ``nil``
