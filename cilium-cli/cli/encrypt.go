...
func newCmdNewIPsecKey() *cobra.Command {
	...
	cmd.Flags().StringVarP(&params.IPsecKeyAuthAlgo, "auth-algo", "", "gcm-aes", "IPsec key authentication algorithm (optional parameter, if omitted the current settings will be used). One of: gcm-aes, hmac-sha256, hmac-sha512")
	cmd.Flags().DurationVar(&params.WaitDuration, "wait-duration", 1*time.Minute, "Maximum time to wait for result, default 1 minute")
	return cmd
}
