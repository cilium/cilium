...
func (k *K8sClusterMesh) helmUpgrade(ctx context.Context, client *k8s.Client, values map[string]any) error {
	// FIXME: force output to list for now. This can be removed conditionally
	//        with a version check once we would be able to set ReuseValues to
	//        false
...
func filterClusters(clusters map[string]any) (map[string]any, error) {
...
func getClustersFromValues(values map[string]any) (map[string]any, error) {
	...
	if ok {
		return filterClusters(clusterMap)
	}
