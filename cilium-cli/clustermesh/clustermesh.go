...
type PolicyDefaultLocalClusterPrepareResult struct {
...
func PolicyDefaultLocalClusterPrepare(ctx context.Context, k8sClient *k8s.Client, namespace string) (*PolicyDefaultLocalClusterPrepareResult, error) {
	logger := slog.New(slog.NewTextHandler(os.Stderr, nil))
	res := PolicyDefaultLocalClusterPrepareResult{
	...
	for _, np := range nps.Items {
		...
		res.CiliumNetworkPolicies[client.ObjectKeyFromObject(&np).String()] = !slices.EqualFunc(rulesAny, rulesLocal, func(a, b *api.Rule) bool { return a.DeepEqual(b) })
	}
	...
	if namespace != corev1.NamespaceAll {
		...
		for _, ccnp := range ccnps.Items {
			...
			res.CiliumNetworkPolicies[client.ObjectKeyFromObject(&ccnp).String()] = !slices.EqualFunc(rulesAny, rulesLocal, func(a, b *api.Rule) bool { return a.DeepEqual(b) })
		}
	}
...
func (res *PolicyDefaultLocalClusterPrepareResult) OutputPolicyDefaultLocalClusterPrepare(output string) error {
