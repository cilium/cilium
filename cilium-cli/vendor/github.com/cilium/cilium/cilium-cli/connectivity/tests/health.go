...
func CiliumHealth() check2.Scenario {
	return &ciliumHealth{}
}
...
func (s *ciliumHealth) Run(ctx context.Context, t *check2.Test) {
	for name, pod := range t.Context().CiliumPods() {
		pod := pod
		t.NewGenericAction(s, name).Run(func(_ *check2.Action) {
			runHealthProbe(ctx, t, &pod)
		})
	}
}

func runHealthProbe(ctx context.Context, t *check2.Test, pod *check2.Pod) {
...
func validateHealthStatus(t *check2.ConnectivityTest, pod *check2.Pod, out bytes.Buffer) error {
	var (
