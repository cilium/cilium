...
// in the test context.
func PodToHost() check2.Scenario {
	return &podToHost{}
}
...
func (s *podToHost) Run(ctx context.Context, t *check2.Test) {
	ct := t.Context()
		...
		for _, node := range ct.Nodes() {
			...
			t.ForEachIPFamily(func(ipFam features.IPFamily) {
				for _, addr := range node.Status.Addresses {
					...
					dst := check2.ICMPEndpoint("", addr.Address)
					ipFam := features.GetIPFamily(addr.Address)

					t.NewAction(s, fmt.Sprintf("ping-%s-%s", ipFam, addrType), &pod, dst, ipFam).Run(func(a *check2.Action) {
...
// in the test context.
func PodToControlPlaneHost() check2.Scenario {
...
func (s *podToControlPlaneHost) Run(ctx context.Context, t *check2.Test) {
	ct := t.Context()
		...
		for _, node := range ct.ControlPlaneNodes() {
			t.ForEachIPFamily(func(ipFam features.IPFamily) {
				for _, addr := range node.Status.Addresses {
					...
					t.NewAction(s, fmt.Sprintf("ping-%s-node-%s-from-pod-%s", ipFam, node.Name, pod.Name()), &pod, dst, ipFam).Run(func(a *check2.Action) {
						a.ExecInPod(ctx, ct.PingCommand(dst, ipFam))
...
func PodToHostPort() check2.Scenario {
...
func (s *podToHostPort) Run(ctx context.Context, t *check2.Test) {
	var i int
	ct := t.Context()
	...
	for _, client := range ct.ClientPods() {
		...
		for _, echo := range ct.EchoPods() {
			...
			baseURL := fmt.Sprintf("%s://%s:%d%s", echo.Scheme(), echo.Pod.Status.HostIP, check2.EchoServerHostPort, echo.Path())
			ep := check2.HTTPEndpoint(echo.Name(), baseURL)
			t.NewAction(s, fmt.Sprintf("curl-%d", i), &client, ep, features.IPFamilyAny).Run(func(a *check2.Action) {
...
func HostToPod() check2.Scenario {
	return &hostToPod{}
}
...
func (s *hostToPod) Run(ctx context.Context, t *check2.Test) {
	var i int
	ct := t.Context()
	...
	for _, src := range ct.HostNetNSPodsByNode() {
		...
		for _, dst := range ct.EchoPods() {
			t.ForEachIPFamily(func(ipFam features.IPFamily) {
				t.NewAction(s, fmt.Sprintf("curl-%s-%d", ipFam, i), &src, dst, ipFam).Run(func(a *check2.Action) {
