...
func WaitForEgressGatewayBpfPolicyEntries(ctx context.Context, t *check2.Test,
	targetEntriesCallback func(ciliumPod check2.Pod) []bpfEgressGatewayPolicyEntry,
) {
	ct := t.Context()
...
func getGatewayNodeInternalIP(ct *check2.ConnectivityTest, egressGatewayNode string) net.IP {
...
func EgressGateway() check2.Scenario {
...
func (s *egressGateway) Run(ctx context.Context, t *check2.Test) {
	ct := t.Context()
	...
	WaitForEgressGatewayBpfPolicyEntries(ctx, t, func(ciliumPod check2.Pod) []bpfEgressGatewayPolicyEntry {
	...
	for _, client := range ct.ClientPods() {
		...
		for _, dst := range ct.HostNetNSPodsByNode() {
			...
			t.NewAction(s, fmt.Sprintf("ping-%d", i), &client, &dst, features.IPFamilyV4).Run(func(a *check2.Action) {
	...
	for _, client := range ct.ClientPods() {
		...
		kubeDNSServicePeer := check2.Service{Service: kubeDNSService}

		t.NewAction(s, fmt.Sprintf("dig-%d", i), &client, kubeDNSServicePeer, features.IPFamilyV4).Run(func(a *check2.Action) {
	...
	for _, client := range ct.ClientPods() {
		...
		for _, externalEcho := range ct.ExternalEchoPods() {
			...
			t.NewAction(s, fmt.Sprintf("curl-external-echo-pod-%d", i), &client, externalEcho, features.IPFamilyV4).Run(func(a *check2.Action) {
	...
	for _, client := range ct.ExternalEchoPods() {
		...
		for _, node := range ct.Nodes() {
			for _, echo := range ct.EchoServices() {
				...
				t.NewAction(s, fmt.Sprintf("curl-echo-service-%d", i), &client, echo, features.IPFamilyV4).Run(func(a *check2.Action) {
	...
	if status, ok := ct.Feature(features.Tunnel); ok && !status.Enabled {
		...
		for _, client := range ct.ExternalEchoPods() {
			...
			for _, echo := range ct.EchoPods() {
				t.NewAction(s, fmt.Sprintf("curl-echo-pod-%d", i), &client, echo, features.IPFamilyV4).Run(func(a *check2.Action) {
					a.ExecInPod(ctx, ct.CurlCommand(echo, features.IPFamilyV4))
				})
				i++
			}
...
func EgressGatewayExcludedCIDRs() check2.Scenario {
...
func (s *egressGatewayExcludedCIDRs) Run(ctx context.Context, t *check2.Test) {
	ct := t.Context()
	...
	for _, client := range ct.ClientPods() {
		...
		for _, externalEcho := range ct.ExternalEchoPods() {
			...
			t.NewAction(s, fmt.Sprintf("curl-%d", i), &client, externalEcho, features.IPFamilyV4).Run(func(a *check2.Action) {
				a.ExecInPod(ctx, ct.CurlCommandWithOutput(externalEcho, features.IPFamilyV4))
				clientIP := extractClientIPFromResponse(a.CmdOutput())
