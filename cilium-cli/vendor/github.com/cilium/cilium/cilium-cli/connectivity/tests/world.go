...
func PodToWorld(opts ...RetryOption) check2.Scenario {
	cond := &retryCondition{}
	for _, op := range opts {
		op(cond)
	}
...
func (s *podToWorld) Run(ctx context.Context, t *check2.Test) {
	extTarget := t.Context().Params().ExternalTarget
	http := check2.HTTPEndpoint(extTarget+"-http", "http://"+extTarget)
	https := check2.HTTPEndpoint(extTarget+"-https", "https://"+extTarget)
	httpsindex := check2.HTTPEndpoint(extTarget+"-https-index", fmt.Sprintf("https://%s/index.html", extTarget))

	fp := check2.FlowParameters{
		DNSRequired: true,
		RSTAllowed:  true,
	}
	...
	for _, client := range ct.ClientPods() {
		...
		t.NewAction(s, fmt.Sprintf("http-to-%s-%d", extTarget, i), &client, http, features.IPFamilyAny).Run(func(a *check2.Action) {
		...
		t.NewAction(s, fmt.Sprintf("https-to-%s-%d", extTarget, i), &client, https, features.IPFamilyAny).Run(func(a *check2.Action) {
		...
		t.NewAction(s, fmt.Sprintf("https-to-%s-index-%d", extTarget, i), &client, httpsindex, features.IPFamilyAny).Run(func(a *check2.Action) {
...
// Pods.
func PodToWorld2() check2.Scenario {
	return &podToWorld2{}
}
...
func (s *podToWorld2) Run(ctx context.Context, t *check2.Test) {
	https := check2.HTTPEndpoint("cilium-io-https", "https://cilium.io.")
	...
	for _, client := range ct.ClientPods() {
		...
		// With https, over port 443.
		t.NewAction(s, fmt.Sprintf("https-cilium-io-%d", i), &client, https, features.IPFamilyAny).Run(func(a *check2.Action) {
...
func PodToWorldWithTLSIntercept(curlOpts ...string) check2.Scenario {
...
func (s *podToWorldWithTLSIntercept) Run(ctx context.Context, t *check2.Test) {
	extTarget := t.Context().Params().ExternalTarget
