...
func NoInterruptedConnections() check2.Scenario {
...
func (n *noInterruptedConnections) Run(ctx context.Context, t *check2.Test) {
	ct := t.Context()

	restartCount := make(map[string]string)
	for _, client := range ct.Clients() {
		pods, err := client.ListPods(ctx, ct.Params().TestNamespace, metav1.ListOptions{LabelSelector: "kind=" + check2.KindTestConnDisrupt})
		if err != nil {
