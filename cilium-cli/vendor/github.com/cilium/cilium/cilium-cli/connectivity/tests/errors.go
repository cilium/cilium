...
func NoErrorsInLogs(ciliumVersion semver.Version) check2.Scenario {
...
func (n *noErrorsInLogs) Run(ctx context.Context, t *check2.Test) {
	...
	for pod, info := range pods {
		...
		for _, container := range info.containers {
			...
			t.NewGenericAction(n, id).Run(func(a *check2.Action) {
...
// packet drops.
func NoUnexpectedPacketDrops(expectedDrops []string) check2.Scenario {
...
func (n *noUnexpectedPacketDrops) Run(ctx context.Context, t *check2.Test) {
	ct := t.Context()
	...
	for _, pod := range ct.CiliumPods() {
		pod := pod
		t.NewGenericAction(n, fmt.Sprintf("%s/%s", pod.K8sClient.ClusterName(), pod.NodeName())).Run(func(a *check2.Action) {
...
func (n *noErrorsInLogs) allCiliumPods(ctx context.Context, ct *check2.ConnectivityTest) (map[podID]podInfo, error) {
...
func (n *noErrorsInLogs) checkErrorsInLogs(id string, logs string, a *check2.Action) {
