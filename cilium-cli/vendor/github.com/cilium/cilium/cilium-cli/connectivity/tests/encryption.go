...
import (
	...
	sniff2 "github.com/cilium/cilium/cilium-cli/connectivity/sniff"
	"strings"
...
// for the encryption.
func getInterNodeIface(ctx context.Context, t *check2.Test,
	client, clientHost, server, serverHost *check2.Pod, ipFam features.IPFamily,
	wgEncap bool) string {
...
func getFilter(ctx context.Context, t *check2.Test, client, clientHost *check2.Pod,
	server, serverHost *check2.Pod,
...
func isWgEncap(t *check2.Test) bool {
...
func PodToPodEncryption(reqs ...features.Requirement) check2.Scenario {
...
func (s *podToPodEncryption) Run(ctx context.Context, t *check2.Test) {
	ct := t.Context()
	client := ct.RandomClientPod()

	var server check2.Pod
...
func testNoTrafficLeak(ctx context.Context, t *check2.Test, s check2.Scenario,
	client, server, clientHost *check2.Pod, serverHost *check2.Pod,
	...
	snifferMode := sniff2.ModeAssert
	if !assertNoLeaks {
		snifferMode = sniff2.ModeSanity
	}

	srcSniffer, err := sniff2.Sniff(ctx, s.Name(), clientHost, srcIface, srcFilter, snifferMode, t)
	if err != nil {
		t.Fatal(err)
	}

	var dstSniffer *sniff2.Sniffer
	if biDirCheck {
		...
		dstSniffer, err = sniff2.Sniff(ctx, s.Name(), serverHost, dstIface, dstFilter, snifferMode, t)
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	case requestHTTP:
		...
		t.NewAction(s, fmt.Sprintf("curl-%s", ipFam), client, server, ipFam).Run(func(a *check2.Action) {
	...
	case requestICMPEcho:
		...
		t.NewAction(s, fmt.Sprintf("ping-%s", ipFam), client, server, ipFam).Run(func(a *check2.Action) {
...
func NodeToNodeEncryption(reqs ...features.Requirement) check2.Scenario {
...
func (s *nodeToNodeEncryption) Run(ctx context.Context, t *check2.Test) {
