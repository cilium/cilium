...
func PodToService(opts ...Option) check2.Scenario {
...
func (s *podToService) Run(ctx context.Context, t *check2.Test) {
	var i int
	ct := t.Context()
	...
	for _, pod := range ct.ClientPods() {
		...
		for _, svc := range ct.EchoServices() {
			...
			t.NewAction(s, fmt.Sprintf("curl-%d", i), &pod, svc, features.IPFamilyAny).Run(func(a *check2.Action) {
...
func PodToIngress(opts ...Option) check2.Scenario {
	options := &labelsOption{}
	for _, opt := range opts {
		opt(options)
	}
...
func (s *podToIngress) Run(ctx context.Context, t *check2.Test) {
	var i int
	ct := t.Context()
...
func PodToRemoteNodePort() check2.Scenario {
...
func (s *podToRemoteNodePort) Run(ctx context.Context, t *check2.Test) {
	var i int
...
// the client Pods.
func PodToLocalNodePort() check2.Scenario {
...
func (s *podToLocalNodePort) Run(ctx context.Context, t *check2.Test) {
	var i int
...
func curlNodePort(ctx context.Context, s check2.Scenario, t *check2.Test,
	name string, pod *check2.Pod, svc check2.Service, node *corev1.Node,
	...
	t.ForEachIPFamily(func(ipFam features.IPFamily) {
		...
		for _, addr := range addrs {
			...
			ep := check2.HTTPEndpoint(name, fmt.Sprintf("%s://%s:%d%s", svc.Scheme(), addr.Address, np, svc.Path()))

			// Create the Action with the original svc as this will influence what the
			// flow matcher looks for in the flow logs.
			t.NewAction(s, name, pod, svc, features.IPFamilyAny).Run(func(a *check2.Action) {
...
func OutsideToNodePort() check2.Scenario {
...
func (s *outsideToNodePort) Run(ctx context.Context, t *check2.Test) {
...
// Cilium to NodePort services.
func OutsideToIngressService() check2.Scenario {
...
func (s *outsideToIngressService) Run(ctx context.Context, t *check2.Test) {
	clientPod := t.Context().HostNetNSPodsByNode()[t.NodesWithoutCilium()[0]]
	i := 0

	for _, svc := range t.Context().IngressService() {
		t.NewAction(s, fmt.Sprintf("curl-ingress-service-%d", i), &clientPod, svc, features.IPFamilyAny).Run(func(a *check2.Action) {
			for _, node := range t.Context().Nodes() {
				node := node
				a.ExecInPod(ctx, t.Context().CurlCommand(svc.ToNodeportService(node), features.IPFamilyAny))

				a.ValidateFlows(ctx, clientPod, a.GetEgressRequirements(check2.FlowParameters{
					DNSRequired: true,
					AltDstPort:  svc.Port(),
				}))
			}
