...
// Network Performance
func Netperf(n string) check2.Scenario {
	return &netPerf{
		name: n,
	}
}
...
func (s *netPerf) Run(ctx context.Context, t *check2.Test) {
	...
	for sample := 1; sample <= samples; sample++ {
		for _, c := range t.Context().PerfClientPods() {
			...
			for _, server := range t.Context().PerfServerPod() {
				scenarioName := ""
				if strings.Contains(c.Pod.Name, check2.PerfHostName) {
					scenarioName += "host"
				} else {
					scenarioName += "pod"
				}
				scenarioName += "-to-"
				if strings.Contains(server.Pod.Name, check2.PerfHostName) {
					scenarioName += "host"
				} else {
					scenarioName += "pod"
				}
				sameNode := true
				if strings.Contains(c.Pod.Name, check2.PerfOtherNode) {
					sameNode = false
				}
				...
				for _, test := range tests {
					...
					action.Run(func(a *check2.Action) {
						k := common.PerfTests{
							Test:     test,
...
func parseDuration(a *check2.Action, value string) time.Duration {
...
func parseFloat(a *check2.Action, value string) float64 {
...
func netperf(ctx context.Context, sip string, perfTest common.PerfTests, a *check2.Action) common.PerfResult {
