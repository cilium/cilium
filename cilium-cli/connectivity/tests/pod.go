...
func (s *podToPodMissingIPCache) Run(ctx context.Context, t *check.Test) {
	...
	for _, echo := range ct.EchoPods() {
		...
		for _, ciliumPod := range ct.CiliumPods() {
			restore, err := ipcacheDeleteAndRestore(ctx, ct, ciliumPod, echoIP, 32, s.useExactMatch)
			if err != nil {
	...
	if s.removePodCIDREntries {
		for _, ciliumPod := range ct.CiliumPods() {
			...
			for _, prefix := range prefixes {
				restore, err := ipcacheDeleteAndRestore(ctx, ct, ciliumPod, prefix.Addr().String(), prefix.Bits(), s.useExactMatch)
				if err != nil {
...
func ipcacheDeleteAndRestore(
	...
	ciliumPod check.Pod, prefixAddr string, prefixBits int, exactMatch bool,
) (func(), error) {
	subCmd := "get"
	prefix := prefixAddr
	if exactMatch {
		subCmd = "match"
		prefix = fmt.Sprintf("%s/%d", prefixAddr, prefixBits)
	}

	lookupCmd := []string{"cilium", "bpf", "ipcache", subCmd, prefix}
	...
	deleteCmd := []string{"cilium", "bpf", "ipcache", "delete", prefix}
	...
	return func() {
		updateCmd := []string{
			"cilium", "bpf", "ipcache", "update", prefix,
