...
func (t *ipsecKeyDerivationTest) Run(ctx context.Context, test *check.Test) {
	...
	for nodeName, pod := range ciliumPods {
		...
		// Check if the dump-xfrm command exists (it's a new command)
		checkCmd := []string{"cilium", "encrypt", "dump-xfrm", "--help"}
		_, err := pod.K8sClient.ExecInPod(ctx, pod.Pod.Namespace, pod.Pod.Name, defaults.AgentContainerName, checkCmd)
		if err != nil {
			test.Infof("cilium encrypt dump-xfrm command not available on %s (likely older Cilium version): %s", nodeName, err)
			test.Infof("Skipping IPsec key derivation validation test - requires newer Cilium version")
			return
		}
		...
		if !strings.HasPrefix(trimmedOutput, "[") && !strings.HasPrefix(trimmedOutput, "{") {
			test.Fatalf("XFRM command from %s returned non-JSON output: %q", nodeName, rawOutput)
			return
		}
