...
func (t *ipsecKeyDerivationTest) Run(ctx context.Context, test *check.Test) {
	...
	for nodeName, pod := range ciliumPods {
		...
		if err := json.Unmarshal(stdout.Bytes(), &states); err != nil {
			test.Fatalf("Failed to parse XFRM states from %s: %s", nodeName, err)
			return
		}

		if len(states) == 0 {
			test.Debugf("No XFRM states found on node %s", nodeName)
			continue
		}
	...
	// Check if we have enough nodes with XFRM states
	...
	if len(nodeIPs) < 2 {
		test.Logf("Not enough nodes with XFRM states for validation (have %d, need 2)", len(nodeIPs))
		return
	}
		...
		for node2, states2 := range nodeIndices {
			...
			for tunnelKey, state1 := range states1 {
				if state2, exists := states2[tunnelKey]; exists {
					if t.validateKeysWith(test, state1, state2, node1, node2, tunnelKey) {
						matchingPairs = true
					}
				}
			}
...
func (t *ipsecKeyDerivationTest) validateKeysWith(test *check.Test, state1, state2 types.XfrmStateInfo, node1, node2, tunnelKey string) bool {
