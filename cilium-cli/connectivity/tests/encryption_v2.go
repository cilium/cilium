...
// client->server traffic is encrypteda and the return traffic, server->client
...
func (s *podToPodEncryptionV2) resolveEgressDevice(ctx context.Context, srcHostNS *check.Pod, dst *check.Pod) (string, error) {
	...
	// issue ip route get for destination in client's host network namespace
	// and extract device.
	//
	// example string output:
	// "172.18.0.2 dev eth0 src 172.18.0.4 uid 0 \n    cache \n"
	out, err := s.clientHostNS.K8sClient.ExecInPod(ctx,
	...
	if err != nil {
		return "", fmt.Errorf("Failed to resolve egress device for client: %w", err)
	}
	...
	for i, val := range outArray {
		if val == "dev" {
			if i+1 > len(outArray)-1 {
				...
				return "", fmt.Errorf("Failed to find egress device for client")
			}
			dev = outArray[i+1]
			break
		}
...
func (s *podToPodEncryptionV2) startSniffers(ctx context.Context, t *check.Test) error {
	...
	if s.encryptMode.Enabled {
		t.Infof("Encryption is enabled: test will fail if plain-text packets are seen.")
	...
	t.Infof("started client tcpdump sniffer: [client: %s] [node: %s] [dev: %s] [filter: %s] [mode: %s]",
	...
	t.Infof("started server tcpdump sniffer: [server: %s] [node: %s] [dev: %s] [filter: %s] [mode: %s]",
	...
	if s.ipv6Enabled.Enabled {
		...
		t.Infof("started client tcpdump sniffer for IPv6: [client: %s] [node: %s] [dev: %s] [filter: %s] [mode: %s]",
		...
		t.Infof("started server tcpdump sniffer for IPv6: [server: %s] [node: %s] [dev: %s] [filter: %s] [mode: %s]",
...
// clientToServerTest creates and runs an check.Action which performs a curl
...
func (s *podToPodEncryptionV2) clientToServerTest(ctx context.Context, t *check.Test) error {
	...
	t.Infof("performing client->server curl: [client: %s] [server: %s] [family: ipv4]", s.client.Pod.Name, s.server.Pod.Name)
	...
	if s.ipv6Enabled.Enabled {
		t.Infof("performing client->server curl: [client: %s] [server: %s] [family: ipv6]", s.client.Pod.Name, s.server.Pod.Name)
