...
func (s *podMACAddress) Run(ctx context.Context, t *check.Test) {
	...
	pods, err := ct.K8sClient().ListPods(ctx, ct.Params().TestNamespace, metav1.ListOptions{
		LabelSelector: podMACAddressLabelSelector,
	})
	if err != nil {
	...
	t.NewGenericAction(s, fmt.Sprintf("check-mac-%s", pod.Name)).Run(func(a *check.Action) {
		...
		if err != nil {
			a.Fatalf(
				"failed to verify MAC address %q in pod %s/%s: %v\noutput: %s",
				macAddress, pod.Namespace, pod.Name, err, strings.TrimSpace(stdout.String()),
			)
		}
	})
}