...
func eastWestLoadbalancingWithL7PolicyTest(ct *check.ConnectivityTest, portRanges bool) {
	...
	newTest(testName+"/pod-to-local-nodeport", ct).
		WithCiliumVersion(">=1.21.0").
		WithFeatureRequirements(
			features.RequireEnabled(features.L7Proxy),
		).
		WithCiliumPolicy(policyYAML).
		WithScenarios(tests.PodToLocalNodePort())

	// Without KPR there are many cases a pod-to-remote-nodeport would not work
	// (see https://github.com/cilium/cilium/issues/46890).
	newTest(testName+"/pod-to-remote-nodeport", ct).
		WithCiliumVersion(">=1.21.0").
		WithFeatureRequirements(
			features.RequireEnabled(features.L7Proxy),
