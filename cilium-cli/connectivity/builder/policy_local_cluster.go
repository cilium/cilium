...
func (t policyLocalCluster) build(ct *check.ConnectivityTest, _ map[string]string) {
	newTest("policy-local-cluster-egress", ct).
		WithFeatureRequirements(features.RequireEnabled(features.PolicyDefaultLocalCLuster)).
