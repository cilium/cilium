...
const (
	...
	// else fails with its error code. Includes `timeout 30` to prevent orphaned
	...
	sniffScriptStopTmpl = `
{
	...
	# Exit with success code in case tcpdump is not running.
	if [ ! -f "{{ .PidPath }}" ]; then
		report
		exit 0
	fi
...
func (sniffer *Sniffer) stop() (err error) {
	...
	sniffer.once.Do(func() {
		...
		// NOTE: Context is not passed anymore to to this function to avoid premature cancellation.
