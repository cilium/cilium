...
cilium --context "${CONTEXT1}" install --cluster-name "${CLUSTER_NAME_1}" --cluster-id 1 --restart-unmanaged-pods=false --config monitor-aggregation=none --native-routing-cidr=10.0.0.0/9
cilium --context "${CONTEXT1}" hubble enable
cilium --context "${CONTEXT1}" clustermesh enable

cilium --context "${CONTEXT2}" install --cluster-name "${CLUSTER_NAME_2}" --cluster-id 2  --restart-unmanaged-pods=false --config monitor-aggregation=none --native-routing-cidr=10.0.0.0/9 --inherit-ca "${CONTEXT1}"
