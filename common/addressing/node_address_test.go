...
func (s *AddressingSuite) TestNewNodeAddress(c *C) {
	n, err := NewNodeAddress("b007::aaaa:bbbb:0", "10.1.0.0", "")
	c.Assert(err, Equals, nil)
	c.Assert(n.String(), Equals, "b007::aaaa:bbbb:0")
	c.Assert(n.IPv6Address.EndpointID(), Equals, uint16(0))
	c.Assert(n.IPv6Address.NodeID(), Equals, uint32(0xaaaabbbb))
	c.Assert(n.IPv4Address.EndpointID(), Equals, uint16(0))
	c.Assert(n.IPv4Address.NodeID(), Equals, uint32(0xa010000))

	n, err = NewNodeAddress("b007::", "20.2.0.0", "")
	c.Assert(err, Equals, nil)
	c.Assert(n.IPv6Address.EndpointID(), Equals, uint16(0))
	...
	_, err = NewNodeAddress("b007::aaaa:bbbb:1", "10.1.0.0", "")
	c.Assert(err, Equals, ErrNodeIPEndpointIDSet)

	_, err = NewNodeAddress("b007::aaaa:bbbb:0", "0.0.0.0", "")
	c.Assert(err, Equals, ErrNodeIDZero)

	_, err = NewNodeAddress("b007::aaaa:bbbb:0", "10.0.1.0", "")
	c.Assert(err, Equals, ErrNodeIPEndpointIDSet)
}
