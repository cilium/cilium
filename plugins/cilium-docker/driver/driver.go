...
func (driver *driver) updateRoutes(addressing *models.NodeAddressing) {
	...
	if driver.conf.Addressing.IPV6 != nil {
		if routes, err := connector.IPv6Routes(driver.conf.Addressing, mtu.StandardMTU); err != nil {
	...
	if driver.conf.Addressing.IPV4 != nil {
		if routes, err := connector.IPv4Routes(driver.conf.Addressing, mtu.StandardMTU); err != nil {
...
func (driver *driver) createEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	veth, _, _, err := connector.SetupVeth(create.EndpointID, mtu.StandardMTU, endpoint)
	if err != nil {
