...
func NewDriver(url string) (Driver, error) {
	...
	if err := plugins.SufficientAddressing(d.conf.Addressing); err != nil {
		scopedLog.WithError(err).Fatalf("Insufficient addressing")
	}

	d.routes = []api.StaticRoute{}
	if d.conf.Addressing.IPV6 != nil {
		if routes, err := plugins.IPv6Routes(d.conf.Addressing); err != nil {
			scopedLog.WithError(err).Fatalf("Unable to generate IPv6 routes")
		} else {
	...
	if d.conf.Addressing.IPV4 != nil {
		if routes, err := plugins.IPv4Routes(d.conf.Addressing); err != nil {
			scopedLog.WithError(err).Fatalf("Unable to generate IPv4 routes")
		} else {
			for _, r := range routes {
				d.routes = append(d.routes, newLibnetworkRoute(r))
			}
...
func notFound(w http.ResponseWriter, r *http.Request) {
	log.WithField("obj", logfields.Repr(r)).Warning("plugin Not found")
	http.NotFound(w, r)
}
...
func (driver *driver) createNetwork(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("req", logfields.Repr(&create)).Debug("Network Create Called")
	emptyResponse(w)
}
...
func (driver *driver) deleteNetwork(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("req", logfields.Repr(&delete)).Debug("Delete network request")
	emptyResponse(w)
}
...
func (driver *driver) createEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("req", logfields.Repr(&create)).Debug("Create endpoint request")

	if create.Interface.Address == "" {
		log.Warning("No IPv4 address provided in CreateEndpoint request")
	}
	...
	//				return
	//			}
	//			log.WithField("obj", logfields.Repr(portmap)).Debug("PortBinding:")
	//
	...
	//				return
	//			}
	//			log.WithFields("obj", logfields.Repr(&tp)).Debug("ExposedPorts")
	...
	log.WithField("resp", resp).Debug("Create endpoint response")
...
func (driver *driver) deleteEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("req", &del).Debug("Delete endpoint request")

	if err := plugins.DelLinkByName(plugins.Endpoint2IfName(del.EndpointID)); err != nil {
		log.WithError(err).Warning("Error while deleting link")
	}
...
func (driver *driver) infoEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("req", &info).Debug("Endpoint info request")
	objectResponse(w, &api.EndpointInfoResponse{Value: map[string]interface{}{}})
	log.WithField("resp", info.EndpointID).Debug("Endpoint info")
}
...
func (driver *driver) joinEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("req", &j).Debug("Join request")
	...
	log.WithField("obj", old).Debug("Existing endpoint")
	...
	defer func() {
		if err != nil {
			if err = netlink.LinkDel(veth); err != nil {
				log.WithError(err).WithField(logfields.Veth, veth.Name).Warning("failed to clean up veth")
			}
		}
	}()
	...
	log.WithField("resp", res).Debug("Join response")
...
func (driver *driver) leaveEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("req", &l).Debug("Leave request")

	if err := driver.client.EndpointDelete(endpointID(l.EndpointID)); err != nil {
		log.WithError(err).Warning("Leaving the endpoint failed")
	}
