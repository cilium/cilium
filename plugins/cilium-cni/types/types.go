...
type NetConf struct {
	...
	// PluginConfig holds the cilium-cni plugin block (with name and cniVersion injected
	// from the conflist envelope) parsed by libcni. It is the complete plugin configuration
	// handed to a delegated IPAM plugin's stdin, preserving plugin-specific fields not
	// modeled by NetConf.
