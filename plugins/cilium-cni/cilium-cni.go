...
func cmdDel(args *skel.CmdArgs) error {
	...
	log.WithField("args", args).Debugf("Processing CNI DEL request %+v", args)
	...
	if err != nil {
		log.WithError(err).Warningf("Unable to delet interface %s in namespace %q, will not delete interface", args.IfName, args.Netns)
		// We are not returning an error as this is very unlikely to be recoverable
	}
