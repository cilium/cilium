...
// containsInterface returns whether an `intf` is contained within a set of interfaces.
// we do not compare the list of peers, just the peer count and public key.
func containsInterface(intfList []*models.WireguardInterface, intf *models.WireguardInterface) bool {
	for _, i := range intfList {
		if i.Name == intf.Name && i.PeerCount == intf.PeerCount &&
			i.PublicKey == intf.PublicKey {
			return true
		}
	}
...
// an error if they are not the same (if there's a mismatch on the interfaces, peers, keys).
func validateWireguardState(expected, actual *models.WireguardStatus) error {
	if expected == nil {
		if actual != nil {
	...
	if actual == nil {
		return fmt.Errorf("%w: actual (kernel) state is empty", errWireguardStateMismatch)
	}

	if len(actual.Interfaces) != len(expected.Interfaces) {
		return fmt.Errorf(
			"%w: interface count did not match between agent state and kernel state. agent: %d, kernel: %d",
			errWireguardStateMismatch, len(expected.Interfaces), len(actual.Interfaces),
		)
	}

	// we need to go over both lists and make sure all
	// interfaces on one side are seen on the other side
	// and the other way around.
	// this is high complexity but it's not a critical code path
	for _, actualIntf := range actual.Interfaces {
		if !containsInterface(expected.Interfaces, actualIntf) {
			return fmt.Errorf("%w: interface %s not found or did not match in the actual set of interfaces", errWireguardStateMismatch, actualIntf.Name)
		}
	}

	for _, expectedIntf := range expected.Interfaces {
		if !containsInterface(actual.Interfaces, expectedIntf) {
			return fmt.Errorf("%w: interface %s not found or did not match in the expected set of interfaces", errWireguardStateMismatch, expectedIntf.Name)
		}
	}
...
func getEncryptionStatus() (models.EncryptionStatus, error) {
	...
	if err == nil {
		result = *resp.Payload.Encryption
		if result.Mode == models.EncryptionStatusModeWireguard {
			actualWgState, err := dumpWireGuardStatus()
			if err != nil {
				result.Msg = err.Error()
			}

			err = validateWireguardState(result.Wireguard, actualWgState)
			if err != nil {
				result.Msg += err.Error()
			}
		} else {
	...
	// if we get here, the agent failed to respond.
	// get state from the kernel and return it back
	result.Msg = err.Error()
