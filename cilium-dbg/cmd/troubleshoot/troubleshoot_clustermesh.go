...
// staticEtcdDbgDialerWithFallback perform a a static host resolution support
...
func newStaticEtcdDbgDialerWithFallback(dialHostAliases dial.HostAliases, dialer kvstore.EtcdDbgDialer) *staticEtcdDbgDialerWithFallback {
	hostAliases := make(map[string][]net.IP, len(dialHostAliases))
	for hostname, hostAlias := range dialHostAliases {
		hostAliases[hostname] = cslices.Map(hostAlias.IPs, func(in netip.Addr) net.IP { return in.AsSlice() })
	}
	return &staticEtcdDbgDialerWithFallback{
		hostAliases:           hostAliases,
		fallbackEtcdDbgDialer: dialer,
		dialer: dial.NewStaticContextDialerWithFallback(
			slog.New(slog.DiscardHandler), dialHostAliases, dialer.DialContext,
		),
	}
}
