...
func (m *Monitor) registerNewListener(parentCtx context.Context, conn net.Conn, version listener.ListenerVersion) {
	m.Lock()
	defer m.Unlock()
...
func (m *Monitor) connectionHandler1_0(parentCtx context.Context, server net.Listener) {
	for !isCtxDone(parentCtx) {
		...
		m.registerNewListener(parentCtx, conn, listener.ListenerVersion1_0)
	}
}
...
func (m *Monitor) connectionHandler1_2(parentCtx context.Context, server net.Listener) {
	for !isCtxDone(parentCtx) {
		...
		m.registerNewListener(parentCtx, conn, listener.ListenerVersion1_2)
	}
}
