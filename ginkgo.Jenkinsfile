        ...
        stage ("BDD-Test-PR"){
            ...
            parallel {
                ...
                stage('BDD-Test-PR-K8s-1.10-net-next') {
                    ...
                    steps {
                        sh 'cd ${TESTDIR}; K8S_VERSION=1.10 ginkgo --focus=" K8sPolicy*" -v --failFast=${FAILFAST} -- -cilium.provision=false'
                    }
                    post {
                        always {
                            sh 'cd ${TESTDIR}; ./post_build_agent.sh || true'
                ...
                stage('BDD-Test-PR-K8s-1.15') {
                    ...
                    steps {
                        sh 'cd ${TESTDIR}; K8S_VERSION=1.15 ginkgo --focus=" K8sPolicy*" -v --failFast=${FAILFAST} -- -cilium.provision=false'
                    }
                    post {
                        always {
                            sh 'cd ${TESTDIR}; ./post_build_agent.sh || true'
