        ...
        stage('BDD-Test-PR') {
            ...
            steps {
                script {
                    parallel(
                        "Runtime":{
                            sh 'cd ${TESTDIR}; ginkgo --focus=" XRuntime*" -v --failFast=${FAILFAST}'
                        },
                        "K8s-1.8":{
                            sh 'cd ${TESTDIR}; K8S_VERSION=1.8 ginkgo --focus=" K8sXFoo*" -v --failFast=${FAILFAST}'
                        },
                        "K8s-1.11":{
                            sh 'cd ${TESTDIR}; K8S_VERSION=1.11 ginkgo --focus=" K8sXFoo*" -v --failFast=${FAILFAST}'
                        },
