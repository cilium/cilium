        ...
        stage ("BDD-Test-PR"){
            ...
            parallel {
                stage('BDD-Test-PR-runtime') {
                    ...
                    steps {
                        sh 'cd ${TESTDIR}; ginkgo --focus=" RuntimeNothing*" -v --failFast=${FAILFAST} -- -cilium.provision=false -cilium.timeout=${GINKGO_TIMEOUT}'
                    }
                    post {
                        always {
                ...
                stage('BDD-Test-PR-K8s-1.11-net-next') {
                    ...
                    steps {
                        sh 'cd ${TESTDIR}; K8S_VERSION=1.11 ginkgo --focus=" K8sDatapathConfig*" -v --failFast=${FAILFAST} -- -cilium.provision=false -cilium.timeout=${GINKGO_TIMEOUT}'
                    }
                    post {
                        always {
                            sh 'cd ${TESTDIR}; ./post_build_agent.sh || true'
                ...
                stage('BDD-Test-PR-K8s-1.15') {
                    ...
                    steps {
                        sh 'cd ${TESTDIR}; K8S_VERSION=1.15 ginkgo --focus=" K8sDatapathConfig*" -v --failFast=${FAILFAST} -- -cilium.provision=false -cilium.timeout=${GINKGO_TIMEOUT}'
                    }
                    post {
                        always {
                            sh 'cd ${TESTDIR}; ./post_build_agent.sh || true'
