...
func testUpdateCNPNodeStatusK8s(integrationTest bool, k8sVersion string, c *C) {
	...
	// For k8s >=v1.11
	// the unit tests will perform 3 actions, A, B and C where:
	// A-1.11) update k8s1 node status
	//         this will make 1 attempt as it is the first node populating status
	// B-1.11) update k8s2 node status
	//         this will make 2 attempts
	// C-1.11) update k8s1 node status with revision=2 and enforcing=false
	//         this will make 2 attempts
	// the code paths for A-1.11, B-1.11 and C-1.11 can be found in the comments
			...
			func(action k8sTesting.Action) (bool, runtime.Object, error) {
				...
				case receivedJsonPatch[0].OP == "test" && receivedJsonPatch[0].Path == "/status":
					...
					case receivedJsonPatch[0].Value == nil:
						...
						if nodes["k8s1"] == nil {
							// codepath B-1.10) and B-1.11) 1st attempt
						...
						if n["localPolicyRevision"].(float64) == 2 {
							// codepath C-1.10) and C-1.11) 1st attempt
...
func (k *K8sSuite) Test_updateCNPNodeStatus_1_11(c *C) {
	testUpdateCNPNodeStatusK8s(os.Getenv("INTEGRATION") != "", "1.11", c)
}
...
func (k *K8sSuite) Benchmark_CNPNodeStatusController_1_11(c *C) {
	nNodes, err := strconv.Atoi(os.Getenv("NODES"))
	c.Assert(err, IsNil)
	...
	c.Logf("Running with %d parallel clients and %d nodes", nClients, nNodes)
	benchmarkCNPNodeStatusController(os.Getenv("INTEGRATION") != "", nNodes, nClients, "1.11", c)
}
...
func (k *K8sSuite) Benchmark_UpdateCNPNodeStatus_1_11(c *C) {
	nNodes, err := strconv.Atoi(os.Getenv("NODES"))
	c.Assert(err, IsNil)
	...
	c.Logf("Running with %d parallel clients and %d nodes", nClients, nNodes)
	benchmarkUpdateCNPNodeStatus(os.Getenv("INTEGRATION") != "", nNodes, nClients, "1.11", c)
}
