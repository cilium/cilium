...
func cleanupHealthEndpoint(d *Daemon) {
	...
	ip6 := node.GetIPv6HealthIP().String()
	ep, err := endpointmanager.Lookup(endpointid.NewID(endpointid.IPv6Prefix, ip6))
	if ep == nil || err != nil {
		log.WithError(err).WithField(logfields.IPv6, ip6).Debug("Didn't find existing cilium-health endpoint to delete")
	} else {
