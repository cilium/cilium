...
func newDaemon(ctx context.Context, cleaner *daemonCleanup, params daemonParams) (*Daemon, *endpointRestoreState, error) {
	...
	d := Daemon{
		...
		// is initialized here.
		identityAllocator:    params.CachingIdentityAllocator,
	...
	if err := d.initPolicy(params.EndpointManager, params.PolicyRepoPromise, params.PolicyUpdaterPromise); err != nil {
	...
	params.NodeManager.SetIPCache(params.IPCache)
	...
	d.ipcache.RegisterK8sSyncedChecker(params.K8sCacheSyncedChecker)
	...
	if params.Clientset.IsEnabled() {
		...
		go func() {
			<-d.k8sCachesSynced
			params.K8sCacheSyncedChecker.Synced()
		}()
