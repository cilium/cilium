...
func newDaemon(ctx context.Context, cleaner *daemonCleanup, params *daemonParams) (*Daemon, *endpointRestoreState, error) {
	...
	if option.Config.EnableIPSec && params.Wireguard.Enabled() {
		return nil, nil, fmt.Errorf("WireGuard (--%s) cannot be used with IPsec (--%s)", wireguard.EnableWireguardFlag, option.EnableIPSecName)
	}
	...
	if option.Config.EnableIPSec &&
		...
		!option.Config.AreDevicesRequired(params.KPRConfig, params.Wireguard.Enabled()) &&
	...
	// created.
	if err := initKubeProxyReplacementOptions(params.Logger, params.Sysctl, params.TunnelConfig, params.LBConfig, params.KPRConfig, params.Wireguard); err != nil {
	...
	if directRoutingDevice == nil {
		if option.Config.AreDevicesRequired(params.KPRConfig, params.Wireguard.Enabled()) {
