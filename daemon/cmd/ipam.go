...
func (d *Daemon) allocateHealthIPs() error {
	...
	if option.Config.EnableIPv4 {
		...
		log.Debug("IPv4 health endpoint address", slog.Any("address", result.IP))
		...
		if option.Config.IPAM == ipamOption.IPAMENI || option.Config.IPAM == ipamOption.IPAMAlibabaCloud {
			if d.healthEndpointRouting, err = parseRoutingInfo(result); err != nil {
				log.Warn("Unable to allocate health information for ENI", slog.Any(logfields.Error, err))
			}
		}
	}
	...
	if option.Config.EnableIPv6 {
		...
		log.Debug("IPv6 health endpoint address", slog.Any("address", result.IP))
	}
	return nil
}
	...
	if option.Config.EnableEnvoyConfig {
		if option.Config.EnableIPv4 {
			...
			if ingressIPv4 != nil {
				...
				if err != nil {
					log.Warn("unable to re-allocate ingress IPv4.", slog.Any(logfields.Error, err), slog.Any(logfields.SourceIP, ingressIPv4))
					result = nil
				}
			}
			...
			log.Info(fmt.Sprintf("  Ingress IPv4: %s", node.GetIngressIPv4()))
			...
			if option.Config.IPAM == ipamOption.IPAMENI || option.Config.IPAM == ipamOption.IPAMAlibabaCloud {
				if ingressRouting, err := parseRoutingInfo(result); err != nil {
					log.Warn("Unable to allocate ingress information for ENI", slog.Any(logfields.Error, err))
				} else {
					if err := ingressRouting.Configure(
						...
						log.Warn("Error while configuring ingress IP rules and routes.", slog.Any(logfields.Error, err))
					}
				}
			}
		}
		...
		if option.Config.EnableIPv6 {
			...
			if ingressIPv6 != nil {
				...
				if err != nil {
					log.Warn("unable to re-allocate ingress IPv6.", slog.Any(logfields.Error, err), slog.Any(logfields.SourceIP, ingressIPv6))
					result = nil
				}
			}
			...
			log.Info(fmt.Sprintf("  Ingress IPv6: %s", node.GetIngressIPv6()))
		}
	}
...
func (d *Daemon) allocateIPs(ctx context.Context, router restoredIPs) error {
	...
	log.Info(fmt.Sprintf("  Cluster-Name: %s", option.Config.ClusterName))
	log.Info(fmt.Sprintf("  Cluster-ID: %d", option.Config.ClusterID))
	log.Info(fmt.Sprintf("  Local node-name: %s", nodeTypes.GetName()))
	log.Info(fmt.Sprintf("  Node-IPv6: %s", node.GetIPv6()))
	...
	if option.Config.EnableIPv6 {
		log.Info(fmt.Sprintf("  IPv6 allocation prefix: %s", node.GetIPv6AllocRange()))

		if c := option.Config.IPv6NativeRoutingCIDR; c != nil {
			log.Info(fmt.Sprintf("  IPv6 native routing prefix: %s", c.String()))
		}

		log.Info(fmt.Sprintf("  IPv6 router address: %s", node.GetIPv6Router()))

		log.Info("  Local IPv6 addresses:")
		for _, addr := range addrs {
			if addr.Addr.Is6() {
				log.Info(fmt.Sprintf("  - %s", addr.Addr))
			}
		}
	}

	log.Info(fmt.Sprintf("  External-Node IPv4: %s", node.GetIPv4()))
	log.Info(fmt.Sprintf("  Internal-Node IPv4: %s", node.GetInternalIPv4Router()))

	if option.Config.EnableIPv4 {
		log.Info(fmt.Sprintf("  IPv4 allocation prefix: %s", node.GetIPv4AllocRange()))

		if c := option.Config.IPv4NativeRoutingCIDR; c != nil {
			log.Info(fmt.Sprintf("  IPv4 native routing prefix: %s", c.String()))
		}
		...
		log.Info(fmt.Sprintf("  Loopback IPv4: %s", node.GetIPv4Loopback().String()))
...
func (d *Daemon) configureIPAM() {
	...
	if err := node.AutoComplete(device); err != nil {
		logging.Fatal(log, "Cannot autocomplete node addresses", slog.Any(logfields.Error, err))
	}
}
