...
func (d *Daemon) allocateRouterIPv4(n *nodeTypes.Node) (net.IP, error) {
	if option.Config.LocalRouterIPv4 != "" {
		...
		if n.IPv4AllocCIDR.Contains(routerIP) {
			log.Warn("Specified router IP is within IPv4 podCIDR.")
		}
		return routerIP, nil
	} else {
		return d.allocateDatapathIPs(false, n)
	}
}

func (d *Daemon) allocateRouterIPv6(n *nodeTypes.Node) (net.IP, error) {
	if option.Config.LocalRouterIPv6 != "" {
		...
		if n.IPv6AllocCIDR.Contains(routerIP) {
			log.Warn("Specified router IP is within IPv6 podCIDR.")
		}
		return routerIP, nil
	} else {
		return d.allocateDatapathIPs(true, n)
	}
}
...
func (d *Daemon) allocateDatapathIPs(ipv6 bool, n *nodeTypes.Node) (routerIP net.IP, err error) {
	nodeIP := n.GetNodeIP(ipv6)

	// Blacklist allocation of the external IP
	d.ipam.BlacklistIP(nodeIP, "node-ip")
	...
	routerIP = n.GetCiliumInternalIP(ipv6)
	if routerIP != nil {
	...
	if routerIP == nil {
		family := ipam.DeriveFamily(nodeIP)
...
func (d *Daemon) allocateHealthIPs(n *nodeTypes.Node) error {
		...
		if option.Config.EnableIPv6 {
			...
			if err != nil {
				if n.IPv4HealthIP != nil {
					d.ipam.ReleaseIP(n.IPv4HealthIP)
					n.IPv4HealthIP = nil
				}
				return fmt.Errorf("unable to allocate health IPs: %s, see https://cilium.link/ipam-range-full", err)
			}
...
func (d *Daemon) allocateIngressIPs(n *nodeTypes.Node) error {
	...
	if option.Config.EnableEnvoyConfig {
		if option.Config.EnableIPv4 {
			...
			if n.IPv4IngressIP != nil {
				result, err = d.ipam.AllocateIPWithoutSyncUpstream(n.IPv4IngressIP, "ingress")
				if err != nil {
					log.WithError(err).WithField(logfields.SourceIP, n.IPv4IngressIP).Warn("unable to re-allocate ingress IPv4.")
					result = nil
				}
			}
			...
			n.IPv4IngressIP = result.IP
			log.Infof("  Ingress IPv4: %s", n.IPv4IngressIP)
		...
		if option.Config.EnableIPv6 {
			...
			if n.IPv6IngressIP != nil {
				result, err = d.ipam.AllocateIPWithoutSyncUpstream(n.IPv6IngressIP, "ingress")
				if err != nil {
					log.WithError(err).WithField(logfields.SourceIP, n.IPv6IngressIP).Warn("unable to re-allocate ingress IPv6.")
					result = nil
				}
			}
			...
			if result == nil {
				...
				if err != nil {
					if n.IPv4IngressIP != nil {
						d.ipam.ReleaseIP(n.IPv4IngressIP)
						n.IPv4IngressIP = nil
					}
					return fmt.Errorf("unable to allocate ingress IPs: %s, see https://cilium.link/ipam-range-full", err)
				}
			...
			n.IPv6IngressIP = result.IP
			log.Infof("  Ingress IPv6: %s", n.IPv6IngressIP)
		}
	}
...
func (d *Daemon) allocateIPs(localNodeStore node.LocalNodeStore) error {
	var updateErr error
	localNodeStore.Update(func(n *nodeTypes.Node) {
		bootstrapStats.ipam.Start()
		if option.Config.EnableIPv4 {
			routerIP, err := d.allocateRouterIPv4(n)
			if err != nil {
				updateErr = err
				return
			}
			if routerIP != nil {
				n.SetCiliumInternalIP(routerIP)
			}
		}

		if option.Config.EnableIPv6 {
			routerIP, err := d.allocateRouterIPv6(n)
			if err != nil {
				updateErr = err
				return
			}
		...
		log.Infof("  Node-IPv6: %s", n.GetNodeIP(true))
		log.Infof("  Node-IPv4: %s", n.GetNodeIP(false))
		log.Infof("  Internal-Node IPv4: %s", n.GetCiliumInternalIP(false))

		bootstrapStats.ipam.End(true)

		if option.Config.EnableEnvoyConfig {
			if err := d.allocateIngressIPs(n); err != nil {
				updateErr = err
				return
			}
		}
		...
		if option.Config.EnableIPv4 {
			log.Infof("  IPv4 allocation prefix: %s", n.IPv4AllocCIDR)
			log.Infof("  IPv4 router address: %s", n.GetCiliumInternalIP(false))
		...
		if option.Config.EnableIPv6 {
			log.Infof("  IPv6 allocation prefix: %s", n.IPv6AllocCIDR)
			log.Infof("  IPv6 router address: %s", n.GetCiliumInternalIP(true))
		...
		updateErr = d.allocateHealthIPs(n)
...
func (d *Daemon) configureIPAM(localNodeStore node.LocalNodeStore) {
	...
	localNodeStore.Update(func(n *nodeTypes.Node) {
		...
		if err := node.AutoComplete(n); err != nil {
