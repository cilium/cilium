...
func (d *Daemon) startAgentHealthHTTPService() {
	...
	mux.Handle("/healthz", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		withoutK8sConnectivity := false
		if v := r.Header.Get("without-k8s-connectivity"); v != "" {
			res, err := strconv.ParseBool(v)
			if err != nil {
				log.WithError(err).WithFields(logrus.Fields{"value": v}).Warn("without-k8s-connectivity should be bool")
			} else {
				withoutK8sConnectivity = res
			}
		}
		...
		sr := d.getStatus(true, withoutK8sConnectivity)
		if isUnhealthy(&sr) {
