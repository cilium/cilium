...
func (d *Daemon) allocateIPsLocked(ep *endpoint.Endpoint) error {
	...
	if option.Config.EnableIPv6 && ep.IPv6 != nil {
		_, err = d.ipam.AllocateIPWithoutSyncUpstream(ep.IPv6.IP(), ep.HumanStringLocked()+" [restored]", ipam.PoolDefault)
		if err != nil {
		...
		defer func() {
			if err != nil {
				d.ipam.ReleaseIP(ep.IPv6.IP(), ipam.PoolDefault)
			}
		}()
	}

	if option.Config.EnableIPv4 && ep.IPv4 != nil {
		_, err = d.ipam.AllocateIPWithoutSyncUpstream(ep.IPv4.IP(), ep.HumanStringLocked()+" [restored]", ipam.PoolDefault)
		switch {
