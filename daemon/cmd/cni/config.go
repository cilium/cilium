...
func (c *cniConfigManager) GetCustomNetConf() *cnitypes.NetConf {
	...
	if err != nil {
		c.log.Warn(
			...
			slog.String("path", c.config.ReadCNIConf),
		)
		return nil
	}
	return conf
		...
		if err != nil {
			...
			if err := c.watcher.Add(c.cniConfDir); err != nil {
				c.log.Warn(
					...
					slog.Any(logfields.Error, err),
					slog.String("configuration-directory", c.cniConfDir),
				)
				c.watcher = nil
			}
		}
	...
	for {
		...
		case _, ok := <-c.watcher.Events:
			...
			c.log.Info(
				...
				slog.String("cni-configuration", c.cniConfDir),
			)
...
func (c *cniConfigManager) renderCNIConf() (cniConfig []byte, err error) {
	if c.config.CNIChainingTarget != "" {
		...
		c.log.Info(
			...
			slog.String("mode", c.config.CNIChainingMode),
		)
...
func (c *cniConfigManager) mergeExistingCNIConfig(pluginConfig []byte) ([]byte, error) {
	...
	c.log.Info(
		...
		slog.String("target", c.config.CNIChainingTarget),
	)
	return out, nil
}
...
func (c *cniConfigManager) cleanupOtherCNI() error {
	...
	for _, f := range files {
		...
		c.log.Info(
			...
			slog.String("destination", name+".cilium_bak"),
		)
