...
func (d *Daemon) restoreLocalIdentities() error {
	...
	if err2 != nil {
		log.Warn("Failed to restore existing identities from the previous ipcache. This may cause policy interruptions during restart.", slog.Any(logfields.Error, err2))
...
func (d *Daemon) dumpOldIPCache() (map[netip.Prefix]identity.NumericIdentity, error) {
	...
	log.Debug("dumping ipache found local identities", slog.Int("len-local-prefixes", len(localPrefixes)))
...
func (d *Daemon) restoreIPCache(localPrefixes map[netip.Prefix]identity.NumericIdentity, restoredIdentities map[identity.NumericIdentity]*identity.Identity) {
	...
	for prefix, nid := range localPrefixes {
		...
		if nid == identity.ReservedIdentityIngress {
			...
			log.Info("Restored ingress IP", slog.Any(logfields.Ingress, prefix))
			continue
		}
		...
		if id == nil {
			...
			log.Debug("ipache prefix not found in allocator cache, requesting identity", slog.Any(logfields.Ingress, prefix))
	...
	log.Info("restored prefixes in the ipcache",
...
func (d *Daemon) releaseRestoredIdentities() {
	...
	log.Info("Removing identity reservations for restored identities", slog.Int(logfields.Count, len(d.restoredCIDRs)))
