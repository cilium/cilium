...
func InitGlobalFlags(logger logging.FieldLogger, cmd *cobra.Command, vp *viper.Viper) {
	flags := cmd.Flags()
...
func initEnv(logger *slog.Logger, vp *viper.Viper) {
	...
	for _, grp := range option.Config.DebugVerbose {
		...
		default:
			logger.Warn("Unknown verbose debug group", slog.String("group", grp))
		}
	}
	...
	if err := os.MkdirAll(option.Config.RunDir, defaults.RuntimePathRights); err != nil {
		...
		logging.Fatal(logger, "Could not create runtime directory", logAttrs...)
	}

	if option.Config.RunDir != defaults.RuntimePath {
		if err := os.MkdirAll(defaults.RuntimePath, defaults.RuntimePathRights); err != nil {
			logAttrs = append(logAttrs, slog.Any(logfields.Error, err))
			logging.Fatal(logger, "Could not create default runtime directory", logAttrs...)
		}
	}
	...
	if err := os.MkdirAll(option.Config.StateDir, defaults.StateDirRights); err != nil {
		logAttrs = append(logAttrs, slog.Any(logfields.Error, err))
		logging.Fatal(logger, "Could not create state directory", logAttrs...)
	}

	if err := os.MkdirAll(option.Config.LibDir, defaults.RuntimePathRights); err != nil {
		logAttrs = append(logAttrs, slog.Any(logfields.Error, err))
		logging.Fatal(logger, "Could not create library directory", logAttrs...)
	}
	// Restore permissions of executable files
	if err := restoreExecPermissions(option.Config.LibDir, `.*\.sh`); err != nil {
		logAttrs = append(logAttrs, slog.Any(logfields.Error, err))
		logging.Fatal(logger, "Unable to restore agent asset permissions", logAttrs...)
	}
	...
	if err := os.MkdirAll(envoy.GetSocketDir(option.Config.RunDir), defaults.RuntimePathRights); err != nil {
		logAttrs = append(logAttrs, slog.Any(logfields.Error, err))
		logging.Fatal(logger, "Could not create envoy sockets directory", logAttrs...)
	}

	if option.Config.MaxControllerInterval < 0 {
		logging.Fatal(logger, fmt.Sprintf("Invalid %s value %d", option.MaxCtrlIntervalName, option.Config.MaxControllerInterval), logAttrs...)
	}

	// set rlimit Memlock to INFINITY before creating any bpf resources.
	if err := rlimit.RemoveMemlock(); err != nil {
		logging.Fatal(logger, "unable to set memory resource limits", slog.Any(logfields.Error, err))
	}

	globalsDir := option.Config.GetGlobalsDir()
	if err := os.MkdirAll(globalsDir, defaults.StateDirRights); err != nil {
		logging.Fatal(logger, "Could not create runtime directory", slog.Any(logfields.Error, err), slog.String(logfields.Path, globalsDir))
	}
	if err := os.Chdir(option.Config.StateDir); err != nil {
		logging.Fatal(logger, "Could not change to runtime directory", slog.Any(logfields.Error, err), slog.String(logfields.Path, option.Config.StateDir))
	}
	if _, err := os.Stat(option.Config.BpfDir); os.IsNotExist(err) {
		logging.Fatal(logger, "BPF template directory: NOT OK. Please run 'make install-bpf'", slog.Any(logfields.Error, err))
	}

	if err := probes.CreateHeaderFiles(filepath.Join(option.Config.BpfDir, "include/bpf"), probes.ExecuteHeaderProbes(logger)); err != nil {
		logging.Fatal(logger, "failed to create header files with feature macros", slog.Any(logfields.Error, err))
	}

	if err := pidfile.Write(defaults.PidFilePath); err != nil {
		logging.Fatal(logger, "Failed to create Pidfile", slog.Any(logfields.Error, err), slog.String(logfields.Path, defaults.PidFilePath))
	}
	...
	bpf.CheckOrMountFS(logging.DefaultLogger, option.Config.BPFRoot)
	...
	if err != nil {
		logging.Fatal(logger, fmt.Sprintf("Failed to parse %s", option.MonitorAggregationName), slog.Any(logfields.Error, err))
	}
	...
	if err := identity.AddUserDefinedNumericIdentitySet(option.Config.FixedIdentityMapping); err != nil {
		logging.Fatal(logger, "Invalid fixed identities provided", slog.Any(logfields.Error, err))
	}
	...
	if err := labelsfilter.ParseLabelPrefixCfg(option.Config.Labels, option.Config.NodeLabels, option.Config.LabelPrefixFile); err != nil {
		logging.Fatal(logger, "Unable to parse Label prefix configuration", slog.Any(logfields.Error, err))
	}
	...
	default:
		logging.Fatal(logger, "Invalid datapath mode", slog.String(logfields.DatapathMode, option.Config.DatapathMode))
	}
	...
	if option.Config.EnableIPSec && option.Config.TunnelingEnabled() {
		if err := ipsec.ProbeXfrmStateOutputMask(); err != nil {
			logging.Fatal(logger, "IPSec with tunneling requires support for xfrm state output masks (Linux 4.19 or later).", slog.Any(logfields.Error, err))
		}
	}
	...
	if option.Config.EnableIPSec &&
		...
		if err != nil {
			logging.Fatal(logger, "Ipsec default interface lookup failed, consider \"encrypt-interface\" to manually configure interface.", slog.Any(logfields.Error, err))
		}
	...
	if option.Config.EnableSRv6 {
		if !option.Config.EnableIPv6 {
			logging.Fatal(logger, fmt.Sprintf("SRv6 requires IPv6."))
		}
	}
	...
	if option.Config.IPAM == ipamOption.IPAMMultiPool {
		...
		if option.Config.EnableIPSec {
			logging.Fatal(logger, fmt.Sprintf("Cannot specify IPAM mode %s with %s.", option.Config.IPAM, option.EnableIPSecName))
		}
	}
...
func (d *Daemon) initKVStore(resolver *dial.ServiceResolver) {
	...
	if d.clientset.IsEnabled() {
		log := logging.DefaultLogger.With(slog.String(logfields.LogSubsys, "etcd"))
	...
	params.Lifecycle.Append(cell.Hook{
		OnStart: func(cell.HookContext) (err error) {
			...
			if !option.Config.DryMode {
				...
				if err != nil {
					d.logger.Error("Unable to store Cilium's configuration", slog.Any(logfields.Error, err))
				}

				err = option.StoreViperInFile(option.Config.StateDir)
				if err != nil {
					d.logger.Error("Unable to store Viper's configuration", slog.Any(logfields.Error, err))
				}
			}
			...
			if option.Config.DryMode {
				...
				go func() {
					...
					if err := startDaemon(daemon, restoredEndpoints, cleaner, params); err != nil {
						d.logger.Error("Daemon start failed", slog.Any(logfields.Error, err))
						daemonResolver.Reject(err)
					} else {
						daemonResolver.Resolve(daemon)
					}
...
func startDaemon(d *Daemon, restoredEndpoints *endpointRestoreState, cleaner *daemonCleanup, params daemonParams) error {
	...
	if err := d.ipcache.WaitForRevision(d.ctx, 1); err != nil {
		d.logger.Error("Failed to wait for initial IPCache revision", slog.Any(logfields.Error, err))
	}
	...
	if params.WGAgent != nil {
		go func() {
			...
			if err := params.WGAgent.RestoreFinished(d.clustermesh); err != nil {
				d.logger.Error("Failed to set up WireGuard peers", slog.Any(logfields.Error, err))
			}
		}()
	}
	...
	if err != nil {
		d.logger.Warn("Failed to send agent start monitor message", slog.Any(logfields.Error, err))
	}
...
func initClockSourceOption(logger logging.FieldLogger) {
	...
	if option.Config.EnableBPFClockProbe {
		if probes.HaveProgramHelper(logger, ebpf.XDP, asm.FnJiffies64) == nil {
