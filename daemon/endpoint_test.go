...
func (ds *DaemonSuite) TestEndpointAddReservedLabel(c *C) {
	...
	code, err := ds.d.createEndpoint(context.Background(), epTemplate, strconv.FormatInt(epTemplate.ID, 10), lbls)
...
func (ds *DaemonSuite) TestEndpointAddNoLabels(c *C) {
	...
	epTemplate := getEPTemplate(c)
	code, err := ds.d.createEndpoint(context.Background(), epTemplate, strconv.FormatInt(epTemplate.ID, 10), nil)
	c.Assert(err, IsNil)
