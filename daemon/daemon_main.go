...
func runDaemon() {
	...
	if option.Config.IsPolicyEnforcementInterfaceSet() {
		waitForHostDeviceWhenReady(option.Config.PolicyEnforcementInterface)
	}
	...
	if option.Config.IsPolicyEnforcementInterfaceSet() {
		err := node.SetInternalIPv4From(option.Config.PolicyEnforcementInterface)
		if err != nil {
			log.WithError(err).WithField("device", option.Config.PolicyEnforcementInterface).Fatal("Unable to set internal IPv4")
		}
