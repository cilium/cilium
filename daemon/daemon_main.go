	...
	case option.DatapathModeVeth:
		...
		if option.Config.IsFlannelMasterDeviceSet() {
			...
			if option.Config.FlannelManageExistingContainers && !option.Config.WorkloadsEnabled() {
				log.Warnf("Running Cilium in flannel mode requires container workloads. Changing %s to %q", option.ContainerRuntime, "auto")
...
func runDaemon() {
	...
	if option.Config.IsFlannelMasterDeviceSet() {
		...
		log.Warnf("Running Cilium in flannel mode doesn't support health checking. Changing %s mode to %q", option.EnableHealthChecking, false)
