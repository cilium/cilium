...
func (d *Daemon) prepareAllocationCIDR(family datapath.NodeAddressingFamily) (routerIP net.IP, err error) {
	...
	if routerIP != nil && !allocRange.Contains(routerIP) {
		...
		// The restored router IP is not part of the allocation
		// range. This indicates that the allocation range has
		// changed.
		...
		if err != nil {
			err = fmt.Errorf("unable to lookup host device %s: %s", defaults.HostDevice, err)
			return
		}
		if err = netlink.LinkDel(link); err != nil {
			err = fmt.Errorf("unable to delete host device %s to change allocation CIDR: %s",
				defaults.HostDevice, err)
			return
		}
