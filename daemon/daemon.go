...
func (d *Daemon) AnnotateEndpoint(e *endpoint.Endpoint, annotationKey, annotationValue string) {
	...
	go func(e *endpoint.Endpoint) {
		...
		for {
			...
			if len(split) < 2 {
				scopedLog.Error("k8s: unable to update pod with annotation: namespace and pod name should be delimited by :")
...
func removeCiliumRules(table string) {
	...
	if err != nil {
		cmd := fmt.Sprintf("%s %s", prog, strings.Join(args, " "))
		log.WithError(err).WithField("cmd", cmd).Warning("Command execution failed")
		return
	}
	...
	for scanner.Scan() {
		rule := scanner.Text()
		log.WithField("obj", logfields.Repr(rule)).Debug("Considering removing iptables rule")
		...
		if strings.Contains(strings.ToLower(rule), "cilium") &&
			...
			log.WithField("obj", logfields.Repr(deleteRule)).Debug("Removing iptables rule")
			err = runProg("iptables", deleteRule, true)
			if err != nil {
				log.WithError(err).WithField("obj", rule).Warningf("Unable to delete Cilium iptables rule")
			}
		}
	}
}
...
func (d *Daemon) compileBase() error {
	...
	scopedLog := log.WithField(logfields.BPFFilter, d.conf.DevicePreFilter)
	if d.conf.DevicePreFilter != "undefined" {
		if err := policy.ProbePreFilter(d.conf.DevicePreFilter, d.conf.ModePreFilter); err != nil {
			scopedLog.WithError(err).Warning("Turning off prefilter")
			d.conf.DevicePreFilter = "undefined"
		}
	}
	if d.conf.DevicePreFilter != "undefined" {
		if d.preFilter, ret = policy.NewPreFilter(); ret != nil {
			scopedLog.WithError(ret).Warning("Unable to init prefilter")
			return ret
		}

		if err := d.writePreFilterHeader("./"); err != nil {
			scopedLog.WithError(err).Warning("Unable to write prefilter header")
			return err
		}
	...
	if d.conf.Device != "undefined" {
		...
		if err != nil {
			// REVIEW can this field be named something more generic?
			log.WithError(err).WithField("receiveDevice", d.conf.Device).Warning("Link does not exist")
			return err
		}
...
func (d *Daemon) removeStaleMap(path string) {
	if err := os.RemoveAll(path); err != nil {
		log.WithError(err).WithField(logfields.Path, path).Warning("Error while deleting stale map file")
	} else {
...
func (h *patchConfig) Handle(params PatchConfigParams) middleware.Responder {
	log.WithField("req", logfields.Repr(params)).Debug("PATCH /config request")
...
func (h *getConfig) Handle(params GetConfigParams) middleware.Responder {
	log.WithField("req", logfields.Repr(params)).Debug("GET /config request")
