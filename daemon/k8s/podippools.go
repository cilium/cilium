...
const (
	PodIPPoolTableName = "podippools"
)

var (
	PodIPPoolNameIndex = statedb.Index[LocalPodIPPool, string]{
		Name: "name",
	...
	PodIPPoolByName    = PodIPPoolNameIndex.Query
	PodIPPoolTableCell = cell.Provide(NewPodIPPoolTableAndReflector)
)

// NewPodIPPoolTableAndReflector returns the read-only Table[LocalPodIPPool] and registers
...
// to initialize.
func NewPodIPPoolTableAndReflector(jg job.Group, db *statedb.DB, cs client.Clientset) (statedb.Table[LocalPodIPPool], error) {
	pools, err := NewPodIPPoolTable(db)
	if err != nil {
		return nil, err
	}
	...
	cfg := podIPPoolReflectorConfig(cs, pools)
	err = k8s.RegisterReflector(jg, db, cfg)
	return pools, err
}

func NewPodIPPoolTable(db *statedb.DB) (statedb.RWTable[LocalPodIPPool], error) {
...
func podIPPoolReflectorConfig(cs client.Clientset, podIPPools statedb.RWTable[LocalPodIPPool]) k8s.ReflectorConfig[LocalPodIPPool] {
