...
func retrieveNodeInformation(ctx context.Context, log logging.FieldLogger, localNodeResource LocalNodeResource, localCiliumNodeResource LocalCiliumNodeResource) *nodeTypes.Node {
	...
	if option.Config.IPAM == ipamOption.IPAMClusterPool {
		for event := range localCiliumNodeResource.Events(ctx) {
			if event.Kind == resource.Upsert {
				...
				n = &no
				log.Info("Retrieved node information from cilium node", slog.Any(logfields.NodeName, n.Name))
				if err := waitForCIDR(); err != nil {
					log.Warn("Waiting for k8s node information", slog.Any(logfields.Error, err))
				} else {
					event.Done(nil)
					break
				}
		...
		for event := range localNodeResource.Events(ctx) {
			if event.Kind == resource.Upsert {
				...
				log.Info("Retrieved node information from kubernetes node", slog.Any(logfields.NodeName, n.Name))
				if err := waitForCIDR(); err != nil {
					log.Warn("Waiting for k8s node information", slog.Any(logfields.Error, err))
				} else {
					event.Done(nil)
...
// received.
func WaitForNodeInformation(ctx context.Context, log logging.FieldLogger, localNode LocalNodeResource, localCiliumNode LocalCiliumNodeResource) error {
	...
	if n := retrieveNodeInformation(ctx, log, localNode, localCiliumNode); n != nil {
		...
		log.Info(
			...
			slog.Any(logfields.Labels, n.Labels),
			slog.Any(logfields.IPAddr+".ipv4", nodeIP4),
			slog.Any(logfields.IPAddr+".ipv6", nodeIP6),
			slog.Any(logfields.V4Prefix, n.IPv4AllocCIDR),
			slog.Any(logfields.V6Prefix, n.IPv6AllocCIDR),
			slog.Any(logfields.K8sNodeIP, k8sNodeIP),
		)
