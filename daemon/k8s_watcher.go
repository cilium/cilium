...
func cnpNodeStatusController(ciliumV2Store cache.Store, cnp *cilium_v2.CiliumNetworkPolicy, rev uint64, logger *logging.Entry, policyImportErr error) error {
...
func (d *Daemon) updateK8sPodV1(oldK8sPod, newK8sPod *v1.Pod) {
	...
	newIdtyLabels, _ := filter.FilterLabels(newLabels)
	oldLabels := labels.Map2Labels(oldPodLabels, labels.LabelSourceK8s)
	oldIdtyLabels, _ := filter.FilterLabels(oldLabels)
