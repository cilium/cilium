				...
				cache.ResourceEventHandlerFuncs{
					...
					DeleteFunc: func(obj interface{}) {
						...
						if ok {
							...
							// exists.
							if ciliumNode, ok := deletedObj.Obj.(*cilium_v2.CiliumNode); ok {
								n := node.ParseCiliumNode(ciliumNode)
								serNodes.Enqueue(func() error {
						...
						} else if ciliumNode, ok := obj.(*cilium_v2.CiliumNode); ok {
							n := node.ParseCiliumNode(ciliumNode)
							serNodes.Enqueue(func() error {
								d.nodeDiscovery.Manager.NodeDeleted(n)
								return nil
				...
				cache.ResourceEventHandlerFuncs{
					...
					DeleteFunc: func(obj interface{}) {
						...
						if ok {
							...
							// known state and the object no longer
							// exists.
							if ciliumEndpoint, ok := deletedObj.Obj.(*types.CiliumEndpoint); ok {
								endpoint := ciliumEndpoint.DeepCopy()
								serCiliumEndpoints.Enqueue(func() error {
						...
						} else if ciliumEndpoint, ok := obj.(*types.CiliumEndpoint); ok {
							endpoint := ciliumEndpoint.DeepCopy()
							serCiliumEndpoints.Enqueue(func() error {
								endpointDeleted(endpoint)
								return nil
