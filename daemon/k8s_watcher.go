...
func (d *Daemon) EnableK8sWatcher(reSyncPeriod time.Duration) error {
	...
	serPods := serializer.NewFunctionQueue(20)
...
func (d *Daemon) updateK8sPodV1(oldK8sPod, newK8sPod *v1.Pod) {
	...
	if reflect.DeepEqual(oldPodLabels, newPodLabels) {
		return
	}
	eps := endpointmanager.GetEndpoints()

	podNSName := k8sUtils.GetObjNamespaceName(&newK8sPod.ObjectMeta)
	var podEP *endpoint.Endpoint
	for _, ep := range eps {
		ep.RLock()
		epNSName := ep.GetK8sNamespaceAndPodNameLocked()
		ep.RUnlock()
		if podNSName == epNSName {
			podEP = ep
			break
		}
	}
