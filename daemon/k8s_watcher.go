...
func (d *Daemon) k8sServiceHandler() {
	for {
		...
		switch event.Action {
		case k8s.UpdateService, k8s.UpdateEndpoints, k8s.UpdateIngress:
			//log.WithFields(logrus.Fields{
			...
			//}).Debug("Received service/endpoint update")
		...
		case k8s.DeleteService, k8s.DeleteEndpoints, k8s.DeleteIngress:
			log.WithFields(logrus.Fields{
				logfields.K8sSvcName:   event.ID.Name,
				logfields.K8sNamespace: event.ID.Namespace,
...
func parsingV1beta1(ing *v1beta1.Ingress, host net.IP) (*k8s.Service, error) {
	...
	ingressSvcInfo := k8s.NewService(host, false, nil, nil)
	portName := loadbalancer.FEPortName(ing.Spec.Backend.ServiceName + "/" + ing.Spec.Backend.ServicePort.String())
	ingressSvcInfo.Ports[portName] = loadbalancer.NewFEPort(loadbalancer.TCP, uint16(ingressPort))
