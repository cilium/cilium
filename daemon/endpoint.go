...
// EndpointMonitorDeleterNotifier is a wrapper around the monitor notification
// that a specified Endpoint has been deleted.
type EndpointMonitorDeleterNotifier struct {
	d *Daemon
}

// NotifyMonitorDeleted notifies the monitor that an endpoint has been deleted.
func (emd *EndpointMonitorDeleterNotifier) NotifyMonitorDeleted(ep *endpoint.Endpoint) {
	repr, err := monitorAPI.EndpointDeleteRepr(ep)
	// Ignore endpoint deletion if EndpointDeleteRepr != nil
	if err == nil {
		emd.d.SendNotification(monitorAPI.AgentNotifyEndpointDeleted, repr)
	}
}

// EpManager is a wrapper around the global endpointmanager, which doesn't
// have a type, so we can pass it around as an interface
type EpManager struct{}

func (epm *EpManager) Remove(ep *endpoint.Endpoint) <-chan struct{} {
	return endpointmanager.Remove(ep)
}
...
func (d *Daemon) deleteEndpointQuiet(ep *endpoint.Endpoint, conf endpoint.DeleteConfig) []error {
	...
	emd := &EndpointMonitorDeleterNotifier{d: d}
	epm := &EpManager{}
	return ep.Delete(emd, d.ipam, epm, conf)
}
