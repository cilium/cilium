...
func (d *Daemon) PolicyAdd(rules policyAPI.Rules, opts *AddOptions) (newRev uint64, err error) {
	...
	d.recordPolicyDelayMetric(newRev, policyAddStartTime, source)
	...
	log.WithField(logfields.PolicyRevision, newRev).Info("Policy imported via API, recalculating...")
	...
	if err != nil {
		log.WithField(logfields.PolicyRevision, newRev).Warn("Failed to represent policy update as monitor notification")
	} else {
