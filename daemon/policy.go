...
func (d *Daemon) policyAdd(rules policyAPI.Rules, opts *AddOptions, resChan chan interface{}) {
	...
	epsIdentityConsumers := make([]policy.Endpoint, len(allEndpoints))
		...
		if opts.Replace {
			for _, r := range rules {
				...
				if len(oldRules) > 0 {
					...
					deletedRules.UpdateRulesEndpointsCaches(epsIdentityConsumers, endpointsToRegen, &policySelectionWG)
				}
			}
		}
	...
	addedRules.UpdateRulesEndpointsCaches(epsIdentityConsumers, endpointsToRegen, &policySelectionWG)
...
type PolicyReactionEvent struct {
	d                    *Daemon
	wg                   *sync.WaitGroup
	epsIdentityConsumers []policy.Endpoint
