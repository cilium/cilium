...
func (d *Daemon) PolicyAdd(rules policyAPI.Rules, opts *AddOptions) (uint64, error) {
	...
	if len(removedPrefixes) > 0 {
		...
		if err := ipcache.ReleaseCIDRs(removedPrefixes); err != nil {
			...
			if errReleaseCIDR := ipcache.ReleaseCIDRs(prefixes); errReleaseCIDR != nil {
				log.WithError(errReleaseCIDR).WithField("prefixes", prefixes).Warn(
					"Failed to release CIDRs allocated for new rules during policy add with replace error cleanup")
			}
			return d.policy.GetRevision(), err
		}
	}
