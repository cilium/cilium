...
func (d *Daemon) restoreOldEndpoints(dir string, clean bool) ([]*endpoint.Endpoint, error) {
	restoredEndpoints := []*endpoint.Endpoint{}

	if !option.Config.RestoreState {
		log.Info("Endpoint restore is disabled, skipping restore step")
		return restoredEndpoints, nil
	}
	...
	for _, ep := range possibleEPs {
		...
		restoredEndpoints = append(restoredEndpoints, ep)
	}

	log.WithFields(logrus.Fields{
		"count.restored": len(restoredEndpoints),
...
func (d *Daemon) regenerateRestoredEndpoints(endpoints []*endpoint.Endpoint) {
	log.Infof("Regenerating %d restored endpoints", len(endpoints))
