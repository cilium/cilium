...
func (d *Daemon) SyncState(dir string, clean bool) error {
	...
	for _, ep := range possibleEPs {
		...
		if err := d.syncLabels(ep); err != nil {
			log.WithError(err).WithField(logfields.EndpointID, ep.ID).Warning("Unable to restore endpoint")
			continue
		}
	...
	for k := range endpointmanager.Endpoints {
		...
		go func(ep *endpoint.Endpoint, wg chan<- bool) {
			...
			if buildSuccess := <-ep.Regenerate(d); !buildSuccess {
				scopedLog.Warning("Failed while regenerating endpoint")
				wg <- false
				return
			}
			ep.Mutex.RLock()
			log.WithError(err).WithField(logfields.IPAddr, []string{ep.IPv4.String(), ep.IPv6.String()}).Info("Restored endpoint with IPs")
			ep.Mutex.RUnlock()
			wg <- true
		}(ep, wg)
	}
...
func readEPsFromDirNames(basePath string, eptsDirNames []string) []*endpoint.Endpoint {
	...
	for _, epID := range eptsDirNames {
		...
		readDir := func() string {
			...
			if err != nil {
				scopedLog.Warning("Error while reading directory. Ignoring it...")
				return ""
			}
		...
		if err != nil {
			log.WithError(err).Warning("Unable to read the C header file")
			continue
		}
