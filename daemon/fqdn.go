...
func NewGetTofqdnsHandler(d *Daemon) GetTofqdnsHandler {
	return &getTofqdns{daemon: d}
}

func (h *getTofqdns) Handle(params GetTofqdnsParams) middleware.Responder {
	...
	switch {
	case err != nil:
		return api.Error(GetTofqdnsBadRequestCode, err)
	case len(lookups) == 0:
		return NewGetTofqdnsIDNotFound()
	}

	return NewGetTofqdnsIDOK().WithPayload(lookups)
}
...
func NewGetTofqdnsIDHandler(d *Daemon) GetTofqdnsIDHandler {
	return &getTofqdnsID{daemon: d}
}

func (h *getTofqdnsID) Handle(params GetTofqdnsIDParams) middleware.Responder {
	var endpoints []*endpoint.Endpoint
	if params.ID != "" {
		ep, err := endpointmanager.Lookup(params.ID)
		switch {
		case err != nil:
			return api.Error(GetTofqdnsIDBadRequestCode, err)
		case ep == nil:
			return api.Error(GetTofqdnsIDNotFoundCode, fmt.Errorf("Cannot find endpoint %s", params.ID))
		default:
