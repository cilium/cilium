...
func (d *Daemon) bootstrapFQDN(restoredEndpoints *endpointRestoreState, preCachePath string) (err error) {
	cfg := fqdn.Config{
		Cache:          fqdn.NewDNSCacheWithLimit(option.Config.ToFQDNsMinTTL, option.Config.ToFQDNsMaxIPsPerHost),
	...
	if preCachePath != "" {
		...
		if err != nil {
			...
			d.dnsRuleGen.LookupCache().UpdateFromCache(precache, nil)
		}
	}
	...
	for _, restoredEP := range restoredEndpoints.restored {
		...
		if restoredEP.DNSHistory != nil {
			d.dnsRuleGen.LookupCache().UpdateFromCache(restoredEP.DNSHistory, []string{})
		}
	}
...
func (h *deleteFqdnCache) Handle(params DeleteFqdnCacheParams) middleware.Responder {
	...
	namesToRegen, err := deleteDNSLookups(h.daemon.dnsRuleGen.LookupCache(), endpoints, time.Now(), matchPatternStr)
	if err != nil {
...
func deleteDNSLookups(cache *fqdn.DNSCache, endpoints []*endpoint.Endpoint, expireLookupsBefore time.Time, matchPatternStr string) (namesToRegen []string, err error) {
	...
	namesToRegen = append(namesToRegen, cache.ForceExpire(expireLookupsBefore, nameMatcher)...)
	for _, ep := range endpoints {
		namesToRegen = append(namesToRegen, ep.DNSHistory.ForceExpire(expireLookupsBefore, nameMatcher)...)
		cache.UpdateFromCache(ep.DNSHistory, nil)
	}
