...
func (d *Daemon) svcAdd(feL3n4Addr types.L3n4AddrID, bes []types.LBBackEnd, addRevNAT bool) (bool, error) {
	log.WithFields(log.Fields{
		...
		logfields.LBBackend: logfields.Repr(bes),
...
func (h *putServiceID) Handle(params PutServiceIDParams) middleware.Responder {
	log.WithField("params", logfields.Repr(params)).Debug("PUT /service/{id} request")
...
func (h *deleteServiceID) Handle(params DeleteServiceIDParams) middleware.Responder {
	log.WithField("req", logfields.Repr(params)).Debug("DELETE /service/{id} request")
	...
	if err != nil {
		log.WithError(err).Warning("error, DeleteL3n4AddrIDByUUID failed")
	}

	if err := h.d.svcDelete(svc); err != nil {
		log.WithError(err).WithField("obj", logfields.Repr(svc)).Warning("DELETE /service/{id}: error deleting service")
...
func (h *getServiceID) Handle(params GetServiceIDParams) middleware.Responder {
	log.WithField("req", logfields.Repr(params)).Debug("GET /service/{id} request")
...
func (h *getService) Handle(params GetServiceParams) middleware.Responder {
	log.WithField("req", logfields.Repr(params)).Debug("GET /service request")
...
func (d *Daemon) SyncLBMap() error {
	...
	addSVC2BPFMap := func(oldID types.ServiceID, svc types.LBSVC) error {
		...
		if ok {
			...
			if err := lbmap.DeleteRevNat(revNATK); err != nil {
				scopedLog.WithError(err).Warning("Unable to remove old rev NAT entry")
			}
			...
			log.WithFields(log.Fields{
				...
				// REVIEW revNAT is of type L3n4Addr which we can print field by field
				"revNAT": revNAT,
	...
	parseSVCEntries := func(key bpf.MapKey, value bpf.MapValue) {
		...
		if err != nil {
			scopedLog.WithError(err).Errorf("SyncLBMap.parseSVCEntries")
			return
		}
	...
	if !d.conf.IPv4Disabled {
		...
		if err != nil {
			log.WithError(err).Warning("error dumping Service4Map")
		}
		err = lbmap.RevNat4Map.Dump(lbmap.RevNat4DumpParser, parseRevNATEntries)
		if err != nil {
			log.WithError(err).Warning("error dumping RevNat4Map")
		}
	}
	...
	if err != nil {
		log.WithError(err).Warning("error dumping Service6Map")
	}
	lbmap.RevNat6Map.Dump(lbmap.RevNat6DumpParser, parseRevNATEntries)
	if err != nil {
		log.WithError(err).Warning("error dumping RevNat6Map")
	}
	...
	for _, svc := range newSVCList {
		...
		if err != nil {
			log.WithError(err).WithFields(log.Fields{
				...
				logfields.L4Addr:    svc.FE.L3n4Addr.String(),
		...
		if svc.FE.ID != kvL3n4AddrID.ID {
			...
			}).Infof("Frontend service ID read from BPF map was out of sync with KVStore, got new ID")
			oldID := svc.FE.ID
	...
	for _, svc := range failedSyncSVC {
		log.WithField(logfields.L4Addr, svc.FE).Debug("Unable to restore, so removing service")
		if err := d.svcDeleteBPF(&svc); err != nil {
			log.WithError(err).WithField(logfields.L4Addr, svc.FE).Warning("Unable to clean service from BPF map")
		}
	}
	...
	for id, revNAT := range failedSyncRevNAT {
		...
		if err := lbmap.DeleteRevNat(revNATK); err != nil {
			log.WithError(err).WithFields(log.Fields{
				logfields.ServiceID: id,
				"revNAT":            revNAT,
			}).Warning("Unable to clean rev NAT from BPF map")
		}
	}
