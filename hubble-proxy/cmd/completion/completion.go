...
func New(ctx context.Context, cctx *cmdContext.Context) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runCompletion(ctx, cctx, cmd, args)
		},
		ValidArgs: []string{"bash", "zsh"},
	}
}

func runCompletion(ctx context.Context, cctx *cmdContext.Context, cmd *cobra.Command, args []string) error {
	if len(args) > 1 {
		return fmt.Errorf("too many arguments; expected only the shell type: %s", args)
	}

	if len(args) == 0 || args[0] == "bash" {
		if _, err := cctx.Stdout.Write([]byte(copyRightHeader)); err != nil {
			return err
		}
		return cmd.Root().GenBashCompletion(cctx.Stdout)
	}
	if args[0] == "zsh" {
		if _, err := cctx.Stdout.Write([]byte(copyRightHeader)); err != nil {
			return err
		}
		return cmd.Root().GenZshCompletion(cctx.Stdout)
	}
