...
func (igc *GC) gc(ctx context.Context) error {
	...
	if ctx.Err() == nil {
		igc.successfulRuns++
		igc.metrics.IdentityGCRuns.WithLabelValues(LabelValueOutcomeSuccess, LabelIdentityTypeCRD).Set(float64(igc.successfulRuns))
	} else {
		igc.failedRuns++
		igc.metrics.IdentityGCRuns.WithLabelValues(LabelValueOutcomeFail, LabelIdentityTypeCRD).Set(float64(igc.failedRuns))
	}
