...
func (igc *GC) runKVStoreModeGC(ctx context.Context) error {
	...
	for {
		...
		if err != nil {
			...
			igc.failedRuns[LabelIdentityTypeKVStore]++
			igc.metrics.IdentityGCRuns.WithLabelValues(LabelValueOutcomeFail, LabelIdentityTypeKVStore).Set(float64(igc.failedRuns[LabelIdentityTypeKVStore]))
		} else {
			...
			igc.successfulRuns[LabelIdentityTypeKVStore]++
			igc.metrics.IdentityGCRuns.WithLabelValues(LabelValueOutcomeSuccess, LabelIdentityTypeKVStore).Set(float64(igc.successfulRuns[LabelIdentityTypeKVStore]))
