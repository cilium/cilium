			...
			p.JobGroup.Add(
				job.OneShot("network-driver-ipam-pool-handler", func(ctx context.Context, _ cell.Health) error {
					...
					for ev := range p.CiliumResourceIPPools.Events(ctx) {
						...
						ev.Done(err)
						if err != nil {
							p.Logger.ErrorContext(ctx, fmt.Sprintf("failed to %s pool %q", action, ev.Key), logfields.Error, err)
						}
					}
					return nil
				}),
...
func upsertPool(allocator *multipool.PoolAllocator, pool *ciliumv2alpha1.CiliumResourceIPPool) error {
	var ipv4 *ciliumv2.IPv4PoolSpec
	if pool.Spec.IPv4 != nil {
		ipv4 = &ciliumv2.IPv4PoolSpec{
			CIDRs:    pool.Spec.IPv4.CIDRs,
			MaskSize: pool.Spec.IPv4.MaskSize,
		}
	}

	var ipv6 *ciliumv2.IPv6PoolSpec
	if pool.Spec.IPv6 != nil {
		ipv6 = &ciliumv2.IPv6PoolSpec{
			CIDRs:    pool.Spec.IPv6.CIDRs,
...
func autoCreatePools(
	...
	for poolName, poolSpecString := range poolMap {
		...
		if parsed.IPv4 != nil {
			pool.Spec.IPv4 = &ciliumv2alpha1.IPv4PoolSpec{
				CIDRs:    parsed.IPv4.CIDRs,
				MaskSize: parsed.IPv4.MaskSize,
			}
		}
		if parsed.IPv6 != nil {
			pool.Spec.IPv6 = &ciliumv2alpha1.IPv6PoolSpec{
				CIDRs:    parsed.IPv6.CIDRs,
				MaskSize: parsed.IPv6.MaskSize,
			}
		}
