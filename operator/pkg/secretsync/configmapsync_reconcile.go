...
func (r *configMapSyncer) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	scopedLog := r.logger.With(
		logfields.Resource, req.NamespacedName,
	)
	scopedLog.DebugContext(ctx, "Reconciling ConfigMap")

	original := &corev1.ConfigMap{}
	if err := r.client.Get(ctx, req.NamespacedName, original); err != nil {
		if k8serrors.IsNotFound(err) {
			scopedLog.DebugContext(ctx, "Unable to get ConfigMap - either deleted or not yet available", logfields.Error, err)
			...
			scopedLog.DebugContext(ctx, "Successfully reconciled ConfigMap", logfields.Action, action(synced))
	...
	for _, reg := range r.registrations {
		if reg.RefObjectCheckFunc(ctx, r.client, r.logger, original) {
			desiredSync := configMapToSyncSecret(reg.SecretsNamespace, original)

			scopedLog.DebugContext(ctx, "Syncing ConfigMap", logfields.K8sNamespace, reg.SecretsNamespace)
	...
	if synced {
		...
		scopedLog.DebugContext(ctx, "Successfully reconciled ConfigMap with resync",
	...
	scopedLog.DebugContext(ctx, "Successfully reconciled ConfigMap",
		logfields.Action, action(changed))
	return ctrl.Result{}, nil
}

func (r *configMapSyncer) getJitteredResyncInterval() time.Duration {
...
func configMapToSyncSecret(secretsNamespace string, original *corev1.ConfigMap) *corev1.Secret {
	s := &corev1.Secret{}
...
func (r *configMapSyncer) ensureSyncedSecret(ctx context.Context, desired *corev1.Secret) error {
