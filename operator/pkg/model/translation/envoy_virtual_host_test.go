...
func Test_envoyHTTPSRoutes_disablesExtAuthzFilters(t *testing.T) {
	...
	result := envoyHTTPSRoutes(routes, []string{"example.com"}, false, authFilters, []model.ExtensionRefFilter{extProcFilter}, false)
...
func Test_envoyHTTPRoutes_directResponseDisablesExternalFilters(t *testing.T) {
	...
	result := envoyHTTPRoutes([]model.HTTPRoute{route}, []string{"example.com"}, false, 80, []*model.HTTPExternalAuthFilter{auth}, []model.ExtensionRefFilter{extProcFilter}, false)
	require.Len(t, result, 1)
...
func Test_envoyHTTPRoutes_extProcAndExternalAuth(t *testing.T) {
	t.Run("route with active ExternalAuth and ext_proc filter", func(t *testing.T) {
		...
		res := envoyHTTPRoutes(httpRoutes, []string{"*"}, false, 80, []*model.HTTPExternalAuthFilter{auth}, allExtProcFilters, false)
		require.Len(t, res, 1)
