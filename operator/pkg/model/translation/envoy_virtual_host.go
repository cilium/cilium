...
// getTypedPerFilterConfigForSyntheticRoute disables external filters while retaining
// non-external per-route configuration such as CORS.
func getTypedPerFilterConfigForSyntheticRoute(route model.HTTPRoute, allAuthFilters []*model.HTTPExternalAuthFilter, allExtProcFilters []model.ExtensionRefFilter) map[string]*anypb.Any {
	route.ExternalAuth = nil
	route.ExtensionRefFilters = nil
	return getTypedPerFilterConfig(nil, allAuthFilters, allExtProcFilters, route)
}
	...
	for _, r := range httpRoutes {
		...
		route := envoy_config_route_v3.Route{
			Match: getRouteMatch(hostnames,
				...
				hRoutes[0].Method),
			Action:               rRedirect,
			TypedPerFilterConfig: getTypedPerFilterConfigForSyntheticRoute(r, allAuthFilters, allExtProcFilters),
		}
...
func envoyHTTPRoutes(httpRoutes []model.HTTPRoute, hostnames []string, hostNameSuffixMatch bool, listenerPort uint32, allAuthFilters []*model.HTTPExternalAuthFilter, allExtProcFilters []model.ExtensionRefFilter) []*envoy_config_route_v3.Route {
	...
	for _, r := range httpRoutes {
		...
		if hRoutes[0].DirectResponse != nil {
			noBackendRoute := envoyHTTPRouteDirectResponse(hRoutes[0], hostnames, hostNameSuffixMatch, allAuthFilters, allExtProcFilters)
...
func envoyHTTPRouteDirectResponse(route model.HTTPRoute, hostnames []string, hostNameSuffixMatch bool, allAuthFilters []*model.HTTPExternalAuthFilter, allExtProcFilters []model.ExtensionRefFilter) *envoy_config_route_v3.Route {
	...
	return &envoy_config_route_v3.Route{
		...
		TypedPerFilterConfig: getTypedPerFilterConfigForSyntheticRoute(route, allAuthFilters, allExtProcFilters),
	}
}
