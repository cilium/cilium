		...
		r := &envoy_config_route_v3.Route_Route{
			Route: &envoy_config_route_v3.RouteAction{
				ClusterSpecifier: &envoy_config_route_v3.RouteAction_Cluster{
					Cluster: routeClusterName(backends[0]),
				},
			},
		}
...
// routeClusterName returns the cluster a backend's traffic should target. For an inferencepool
// backend, it is the ORIGINAL_DST cluster that routes to the EPP selected endpoint. OTherwise,
// it's the backend's normal EDS
func routeClusterName(be model.Backend) string {
	if be.EndpointPicker != nil {
		return getInferenceDestinationClusterName(be.Namespace, be.Name, be.Port.GetPort())
	}
	return getClusterName(be.Namespace, be.Name, be.Port.GetPort())
}
