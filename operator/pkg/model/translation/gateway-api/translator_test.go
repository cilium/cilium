	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			trans := &gatewayAPITranslator{
				cecTranslator: translation.NewCECTranslator("cilium-secrets", false, true, 60, false, nil, false, false, 0, nil),
			}
...
func Test_translator_Translate_HostNetwork(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			trans := &gatewayAPITranslator{
				cecTranslator:      translation.NewCECTranslator("cilium-secrets", false, true, 60, true, tt.nodeLabelSelector, tt.ipv4Enabled, tt.ipv6Enabled, 0, nil),
...
func Test_translator_Translate_WithXffNumTrustedHops(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			trans := &gatewayAPITranslator{
				cecTranslator:      translation.NewCECTranslator("cilium-secrets", false, true, 60, false, nil, false, false, 2, nil),
				hostNetworkEnabled: true,
			}
			cec, svc, ep, err := trans.Translate(tt.args.m)
			require.Equal(t, tt.wantErr, err != nil, "Error mismatch")
