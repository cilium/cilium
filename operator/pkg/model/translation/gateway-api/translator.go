...
func (t *gatewayAPITranslator) Translate(m *model.Model) (*ciliumv2.CiliumEnvoyConfig, *ciliumv2alpha1.CiliumGatewayL4Config, *corev1.Service, *discoveryv1.EndpointSlice, error) {
	listeners := m.GetListeners()
	if len(listeners) == 0 || len(listeners[0].GetSources()) == 0 {
		return nil, nil, nil, nil, fmt.Errorf("model source can't be empty, %d listeners", len(listeners))
	}
	...
	if source == nil || source.Name == "" {
		return nil, nil, nil, nil, fmt.Errorf("model source name can't be empty")
	}
	...
	if m.IsL4ListenerConfigured() {
		var err error
		gl4c, err = t.gl4cTranslator.Translate(source.Namespace, generatedName, m)
		if err != nil {
			return nil, nil, nil, nil, err
		}
	}
