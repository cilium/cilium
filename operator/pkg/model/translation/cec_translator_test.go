...
func Test_getUniqueExtProcFilters_conflictResolution(t *testing.T) {
	...
	makeFilter := func(name, routeNS, routeName string, ts time.Time) model.ExtensionRefFilter {
		return model.ExtensionRefFilter{
			Name:    name,
			TypeURL: "type.googleapis.com/test.Filter",
			SourceRouteRule: &model.HTTPRouteRule{
		...
		m := &model.Model{HTTP: []model.HTTPListener{{
			Routes: []model.HTTPRoute{
				{ExtensionRefFilters: []model.ExtensionRefFilter{
					makeFilter("alpha", "default", "old-route", old),
					makeFilter("beta", "default", "old-route", old),
				}},
				{ExtensionRefFilters: []model.ExtensionRefFilter{
					makeFilter("beta", "default", "new-route", new_),
					makeFilter("alpha", "default", "new-route", new_),
				}},
			},
		}}}
	...
	t.Run("alphabetic namespace/name tiebreak on equal creationTimestamp", func(t *testing.T) {
		...
		m := &model.Model{HTTP: []model.HTTPListener{{
			Routes: []model.HTTPRoute{
				{ExtensionRefFilters: []model.ExtensionRefFilter{
					makeFilter("beta", "aaa", "route", ts),
					makeFilter("alpha", "aaa", "route", ts),
				}},
				{ExtensionRefFilters: []model.ExtensionRefFilter{
					makeFilter("alpha", "zzz", "route", ts),
					makeFilter("beta", "zzz", "route", ts),
				}},
			},
		}}}
		result := i.getUniqueExtProcFilters(m)
	...
	t.Run("duplicate references from different routes collapse to one", func(t *testing.T) {
		...
		m := &model.Model{HTTP: []model.HTTPListener{{
			Routes: []model.HTTPRoute{
				{ExtensionRefFilters: []model.ExtensionRefFilter{
					makeFilter("shared", "default", "route-a", old),
				}},
				{ExtensionRefFilters: []model.ExtensionRefFilter{
					makeFilter("shared", "default", "route-b", new_),
				}},
			},
		}}}
		result := i.getUniqueExtProcFilters(m)
