...
func GatewayAPI(log *slog.Logger, input Input) *model.Model {
	...
	for _, l := range listeners {
		...
		case gatewayv1.HTTPProtocolType, gatewayv1.HTTPSProtocolType, gatewayv1.TLSProtocolType:
			...
			httpRoutes = append(httpRoutes, toHTTPRoutes(log, l.Listener, listenerHostnamesByProtocol, filteredHTTPRoutes, input.Services, input.ServiceImports, input.ReferenceGrants, input.BackendTLSPolicyMap, input.EnableExtensionRefFilters, input.CiliumEnvoyExtProcFilters)...)
			httpRoutes = append(httpRoutes, toGRPCRoutes(log, l.Listener, listenerHostnamesByProtocol, filteredGRPCRoutes, input.Services, input.ServiceImports, input.ReferenceGrants, input.EnableExtensionRefFilters, input.CiliumEnvoyExtProcFilters)...)
...
func sourceGRPCRouteRule(grpcr gatewayv1.GRPCRoute, ruleIndex, matchIndex int) *model.HTTPRouteRule {
	return &model.HTTPRouteRule{
		Source:     sourceGRPCRoute(grpcr),
		RuleIndex:  ruleIndex,
		MatchIndex: matchIndex,
	}
}

func sourceGRPCRoute(grpcr gatewayv1.GRPCRoute) model.FullyQualifiedResource {
...
func resolveExtensionRef(
	...
	if ref.Group != v2alpha1.CustomResourceDefinitionGroup || ref.Kind != v2alpha1.CEEPFKindDefinition {
		log.Debug(
	...
	return crdToExtensionRefFilter(log, found, grants, services)
}
...
func crdToExtensionRefFilter(log *slog.Logger, crd *v2alpha1.CiliumEnvoyExtProcFilter, grants []gatewayv1.ReferenceGrant, services []corev1.Service) (*model.ExtensionRefFilter, bool) {
	...
	service := getServiceSpec(crd.Spec.BackendRef.Name, ns, services)
	if service == nil || !serviceHasPort(service, crd.Spec.BackendRef.Port) {
		log.Debug(
			"CiliumEnvoyExtProcFilter backend Service or port was not found",
			logfields.ResourceName, crd.Name,
			logfields.K8sNamespace, crd.Namespace,
			logfields.BackendName, crd.Spec.BackendRef.Name,
			logfields.ServiceNamespace, ns,
			logfields.Port, crd.Spec.BackendRef.Port,
		)
		return nil, false
	}
	...
	if crd.Spec.MessageTimeout != nil {
		messageTimeout := crd.Spec.MessageTimeout.Duration
		if messageTimeout < 0 || messageTimeout > time.Hour {
			log.Debug(
				"CiliumEnvoyExtProcFilter messageTimeout is outside Envoy's supported range",
				logfields.ResourceName, crd.Name,
				logfields.K8sNamespace, crd.Namespace,
				logfields.Duration, messageTimeout,
			)
			return nil, false
		}
		extProc.MessageTimeout = durationpb.New(messageTimeout)
	}
...
var extProcStatReplacer = strings.NewReplacer("-", "_2d", ".", "_2e")

// sanitizeExtProcStatPart escapes characters that are illegal in Envoy stat
// names. The escape sequences are injective because underscores are not valid
// in Kubernetes DNS-subdomain resource names.
...
func toHeaderSendMode(mode v2alpha1.ExtProcHeaderMode) ext_procv3.ProcessingMode_HeaderSendMode {
	switch mode {
	case v2alpha1.ExtProcHeaderModeSend:
		return ext_procv3.ProcessingMode_SEND
	case v2alpha1.ExtProcHeaderModeSkip:
...
func toBodySendMode(mode v2alpha1.ExtProcBodyMode) ext_procv3.ProcessingMode_BodySendMode {
	switch mode {
	case v2alpha1.ExtProcBodyModeNone:
		return ext_procv3.ProcessingMode_NONE
	case v2alpha1.ExtProcBodyModeStreamed:
		return ext_procv3.ProcessingMode_STREAMED
	case v2alpha1.ExtProcBodyModeBuffered:
		return ext_procv3.ProcessingMode_BUFFERED
	case v2alpha1.ExtProcBodyModeBufferedPartial:
