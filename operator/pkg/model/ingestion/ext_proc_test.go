...
func Test_resolveExtensionRef(t *testing.T) {
	extProcServices := []corev1.Service{
		testService("default", "ext-proc-service", 9001),
		testService("other-namespace", "ext-proc-service", 9002),
	}
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			filter, ok := resolveExtensionRef(logger, tc.enableExtensionRefFilters, tc.namespace, tc.ref, extProcFilters, tc.grants, extProcServices)
...
func Test_crdToExtensionRefFilter(t *testing.T) {
	tests := map[string]struct {
		...
		"missing backend Service": {
			crd: &v2alpha1.CiliumEnvoyExtProcFilter{
				ObjectMeta: metav1.ObjectMeta{Name: "missing-service-filter", Namespace: "default"},
				Spec:       v2alpha1.CiliumEnvoyExtProcFilterSpec{BackendRef: v2alpha1.ExtProcBackendRef{Name: "missing-service", Port: 50051}},
			},
			expectOK: false,
		},
		"missing Service port": {
			crd: &v2alpha1.CiliumEnvoyExtProcFilter{
				ObjectMeta: metav1.ObjectMeta{Name: "missing-port-filter", Namespace: "default"},
				Spec:       v2alpha1.CiliumEnvoyExtProcFilterSpec{BackendRef: v2alpha1.ExtProcBackendRef{Name: "ext-proc-svc", Port: 50052}},
			},
			expectOK: false,
		},
		"cross-namespace backendRef with ReferenceGrant": {
	...
	extProcServices := []corev1.Service{
		testService("default", "my-grpc-service", 50051),
		testService("default", "ext-proc-svc", 50051),
		testService("other-namespace", "ext-proc-svc", 50051),
	}

	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			logger := hivetest.Logger(t, hivetest.LogLevel(slog.LevelDebug))
			filter, ok := crdToExtensionRefFilter(logger, tc.crd, tc.grants, extProcServices)
...
func Test_extractRoutes_multipleExtensionRefFilters(t *testing.T) {
	...
	t.Run("two extension ref filters preserved in declaration order", func(t *testing.T) {
		...
		services := []corev1.Service{
			testService("default", "backend", 8080),
			testService("default", "svc-a", 9001),
			testService("default", "svc-b", 9002),
		}
