...
var Cell = cell.Module(
	...
	cell.Config(ingressConfig{
		...
		IngressHostnetworkSharedPort:        0,
...
type ingressConfig struct {
	...
	IngressHostnetworkSharedPort        uint32
...
func (r ingressConfig) Flags(flags *pflag.FlagSet) {
	...
	flags.Uint32("ingress-hostnetwork-shared-port", r.IngressHostnetworkSharedPort, "Port on the host network that gets used for the shared listener (TLS Passthrough, HTTP & HTTPS)")
...
func registerReconciler(params ingressParams) error {
	...
	reconciler := newIngressReconciler(
		...
		params.IngressConfig.IngressHostnetworkEnabled,
		params.IngressConfig.IngressHostnetworkSharedPort,
	)
