...
func (ic *Controller) deleteService(svc *corev1.Service) error {
	...
	_, exists, err := ic.serviceManager.getByKey(resourceName)
	if err != nil {
	...
	err = ic.clientset.CoreV1().Services(svc.GetNamespace()).Delete(context.Background(), resourceName, metav1.DeleteOptions{})
	if err != nil && !k8serrors.IsNotFound(err) {
