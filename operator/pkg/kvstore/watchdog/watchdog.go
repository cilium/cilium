...
func runWatchdog(p params) {
	if !p.Client.IsEnabled() {
		return
	}

	p.Logger.Info("Starting kvstore watchdog", logfields.Interval, defaults.LockLeaseTTL)
	...
	if err != nil {
		logging.Fatal(p.Logger, "Unable to initialize kvstore backend for identity garbage collection", logfields.Error, err)
	}
	...
	p.JobGroup.Add(
		job.Timer("kvstore-watchdog", func(ctx context.Context) error {
			...
			if err != nil && !errors.Is(ctx.Err(), context.Canceled) {
				p.Logger.Warn("Unable to run security identity garbage collector", logfields.Error, err)
				return err
			}
