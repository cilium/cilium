...
func (c *Controller) processNamespaceEvents(ctx context.Context) error {
	for event := range c.namespace.Events(ctx) {
		...
		case resource.Upsert:
			c.logger.DebugContext(ctx, "Got Upsert Namespace event ", logfields.K8sNamespace, event.Key.String())

			c.onNamespaceUpsert(event.Object)
		case resource.Delete:
			c.logger.DebugContext(ctx, "Got Delete Namespace event ", logfields.K8sNamespace, event.Key.String())
