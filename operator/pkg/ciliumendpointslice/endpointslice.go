...
func (c *Controller) Start(ctx cell.HookContext) error {
	...
	// CES or Pod event is retrieved and checked for whether it is from a priority namespace
	...
	c.reconciler = newReconciler(c.context, c.clientset.CiliumV2alpha1(), c.manager, c.logger, c.pods, c.ciliumEndpointSlice, c.ciliumNodes, c.namespace, c.ciliumIdentity, c.metrics, c.wireguardEnabled, c.ipsecEnabled)
