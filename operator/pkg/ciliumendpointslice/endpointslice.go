...
func (c *Controller) runCiliumEndpointsUpdater(ctx context.Context) error {
	for event := range c.ciliumEndpoint.Events(ctx) {
		...
		case resource.Upsert:
			c.logger.DebugContext(ctx, "Got Upsert Endpoint event", logfields.CEPName, event.Key.String())
			c.onEndpointUpdate(event.Object)
		case resource.Delete:
			c.logger.DebugContext(ctx, "Got Delete Endpoint event", logfields.CEPName, event.Key.String())
...
func (c *Controller) runCiliumEndpointSliceUpdater(ctx context.Context) error {
	for event := range c.ciliumEndpointSlice.Events(ctx) {
		switch event.Kind {
		case resource.Upsert:
			c.logger.DebugContext(ctx, "Got Upsert Endpoint Slice event", logfields.CESName, event.Key.String())
			c.onSliceUpdate(event.Object)
		case resource.Delete:
			c.logger.DebugContext(ctx, "Got Delete Endpoint Slice event", logfields.CESName, event.Key.String())
