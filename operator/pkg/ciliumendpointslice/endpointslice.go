...
func (c *Controller) onNodeUpdate(node *cilium_api_v2.CiliumNode) {
	touchedCESs := c.manager.UpdateNodeMapping(node)
	c.enqueueCESReconciliation(touchedCESs)
}

func (c *Controller) onNodeDelete(node *cilium_api_v2.CiliumNode) {
	touchedCESs := c.manager.RemoveNodeMapping(node)
	c.enqueueCESReconciliation(touchedCESs)
}

func (c *Controller) onIdentityUpdate(cid *cilium_api_v2.CiliumIdentity) {
	touchedCESs := c.manager.UpdateIdentityMapping(cid)
	c.enqueueCESReconciliation(touchedCESs)
}

func (c *Controller) onIdentityDelete(cid *cilium_api_v2.CiliumIdentity) {
