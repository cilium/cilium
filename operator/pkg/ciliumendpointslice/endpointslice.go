...
func (c *Controller) onPodUpdate(pod *slim_corev1.Pod) {
	...
	if err != nil {
		c.logger.Info("could not get endpointnetworking for pod",
	...
	if err != nil {
		c.logger.Info("could not get node name for pod",
			logfields.K8sPodName, pod.Name,
			logfields.Error, err)
		return
	}

	pCid, err := c.reconciler.getPodIdentity(pod)
	if err != nil {
		...
		if errLabels != nil {
			c.logger.Info("could not get labels for pod",
				logfields.K8sPodName, pod.Name,
				logfields.Error, err)
			return
		}
	...
	touchedCESs := c.manager.UpdatePodMapping(pod, node, pCid)
...
func (c *Controller) runCiliumNodesUpdater(ctx context.Context) error {
	...
	for event := range c.ciliumNodes.Events(ctx) {
		switch event.Kind {
		case resource.Upsert:
			c.logger.Debug("Got Upsert CiliumNode event", logfields.NodeName, event.Key.String())
			c.onNodeUpdate(event.Object)
		case resource.Delete:
			c.logger.Debug("Got Delete CiliumNode event", logfields.NodeName, event.Key.String())
...
func (c *Controller) runCiliumIdentitiesUpdater(ctx context.Context) error {
	for event := range c.ciliumIdentity.Events(ctx) {
		switch event.Kind {
		case resource.Upsert:
			c.logger.Debug("Got Upsert CiliumIdentity event", logfields.CIDName, event.Key.String())
			c.onIdentityUpdate(event.Object)
		case resource.Delete:
			c.logger.Debug("Got Delete CiliumNode event", logfields.CIDName, event.Key.String())
...
func (c *Controller) syncCESsInLocalCache(ctx context.Context) error {
	...
	for _, pod := range podStore.List() {
		...
		cidName, gidLabels := cidToGidLabels(pCid)
		c.manager.initializeMappingPodToNode(pod, pod.Namespace, node, cepToCes[pod.Name], cidName, gidLabels)
	}
