...
func (r *reconciler) reconcileCESUpdate(cesName CESName, cesObj *cilium_v2a1.CiliumEndpointSlice) (err error) {
	...
	data := r.cesManager.getCESNamespace(cesName)
	if updatedCES.Namespace != data {
		updatedCES.Namespace = data
		cesEqual = false
	}
...
func (r *reconciler) getPodIdentityID(pod *slim_corev1.Pod) (int64, error) {
	cid, err := r.getPodIdentity(pod)
	if err != nil {
		return 0, err
	}
	identityId, err := strconv.ParseInt(cid.Name, 10, 64)
	if err != nil {
...
func (r *reconciler) getPodIdentity(pod *slim_corev1.Pod) (*cilium_v2.CiliumIdentity, error) {
...
func (r *reconciler) getEndpointEncryptionKey(pod *slim_corev1.Pod) (int, error) {
	...
	case option.Config.EnableIPSec:
		...
		r.logger.Debug("Getting encryption key for pod")
...
func (r *reconciler) getNodeNameForPod(pod *slim_corev1.Pod) (string, error) {
	ciliumNode, err := r.ciliumNodeStore.ByIndex(op_k8s.CiliumNodeIPIndex, pod.GetHostIP())
	if err != nil || ciliumNode == nil || len(ciliumNode) == 0 {
		return "", fmt.Errorf("failed to get CiliumNode from store: %w", err)
	}
	return ciliumNode[0].Name, nil
}
