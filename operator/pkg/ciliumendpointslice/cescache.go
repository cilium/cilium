...
func (c *CESCache) upsertCEP(cepName CEPName, cesName CESName, nodeName NodeName, gidLabels Labels, cid CID) {
	c.updateCEPInCache(cepName, nodeName, gidLabels, cesName)
	c.globalIdLabelsToCIDSet[gidLabels].setSelectedID(cid)
...
func (c *CESCache) updateCEPInCache(cepName CEPName, nodeName NodeName, gidLabels Labels, cesName CESName) {
	...
	if _, ok := c.nodeData[nodeName]; !ok {
		c.nodeData[nodeName] = NewNodeData(0)
	}
...
func (c *CESCache) deleteCID(cid CID, gidLabels Labels) []CESKey {
