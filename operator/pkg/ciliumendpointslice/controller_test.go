...
func TestRegisterController(t *testing.T) {
	...
	cesCreated, err := createPodandVerifyCESCreated(fakeClient, *pod, *ciliumEndpointSlice, *namespace, *ciliumIdentity, *ciliumNode)
	if err != nil {
...
func createPodandVerifyCESCreated(fakeClient k8sClient.Clientset, pod resource.Resource[*slim_corev1.Pod], ciliumEndpointSlice resource.Resource[*cilium_v2a1.CiliumEndpointSlice], ns resource.Resource[*slim_corev1.Namespace], cid resource.Resource[*cilium_v2.CiliumIdentity], cnode resource.Resource[*cilium_v2.CiliumNode]) (bool, error) {
	// Create a node, with pod1 (in ns with labels testLbsA)
	node := tu.CreateStoreNode("node1")
	fakeClient.CiliumV2().CiliumNodes().Create(context.Background(), node, meta_v1.CreateOptions{})
	ciliumNodeStore, _ := cnode.Store(context.Background())
	...
	ns1 := idtu.CreateNSObj("ns", testLbsA)
	fakeClient.Slim().CoreV1().Namespaces().Create(context.Background(), ns1, meta_v1.CreateOptions{})
	nsStore, _ := ns.Store(context.Background())
	if err := testutils.WaitUntil(func() bool {
	...
	fakeClient.Slim().CoreV1().Pods("ns").Create(context.Background(), pod1, meta_v1.CreateOptions{})
	...
	id1 := idtu.CreateCIDObj("1", testLbsA)
	fakeClient.CiliumV2().CiliumIdentities().Create(context.Background(), id1, meta_v1.CreateOptions{})
	idStore, _ := cid.Store(context.Background())
	if err := testutils.WaitUntil(func() bool {
