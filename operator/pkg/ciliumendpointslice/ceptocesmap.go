...
type CESToCEPMapping struct {
	...
	// cesNameToCESData is used to map CiliumEndpointSlice name to all CiliumEndpoints names it contains
	// and the namespace associated with it
	cesNameToCESData map[CESName]*CESData
}
...
func newCESToCEPMapping() *CESToCEPMapping {
	return &CESToCEPMapping{
		...
		cesNameToCESData: make(map[CESName]*CESData),
	}
}
...
func (c *CESToCEPMapping) insertCEP(cepName CEPName, cesName CESName) {
	...
	c.cesNameToCESData[cesName].ceps.Insert(cepName)
}
...
func (c *CESToCEPMapping) deleteCEP(cepName CEPName) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	c.cesNameToCESData[c.cepNameToCESName[cepName]].ceps.Delete(cepName)
...
func (c *CESToCEPMapping) countCEPsInCES(ces CESName) int {
	...
	if cesData, ok := c.cesNameToCESData[ces]; ok {
...
func (c *CESToCEPMapping) getCEPsInCES(ces CESName) []CEPName {
	c.mutex.RLock()
	defer c.mutex.RUnlock()
	ceps := make([]CEPName, 0, c.cesNameToCESData[ces].ceps.Len())
	for cep := range c.cesNameToCESData[ces].ceps {
...
func (c *CESToCEPMapping) insertCES(cesName CESName, ns string) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	c.cesNameToCESData[cesName] = &CESData{
...
func (c *CESToCEPMapping) deleteCES(cesName CESName) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	delete(c.cesNameToCESData, cesName)
}

func (c *CESToCEPMapping) hasCESName(cesName CESName) bool {
	c.mutex.RLock()
	defer c.mutex.RUnlock()
	_, ok := c.cesNameToCESData[cesName]
	return ok
}
...
func (c *CESToCEPMapping) getAllCESs() []CESName {
	c.mutex.RLock()
	defer c.mutex.RUnlock()
	cess := make([]CESName, 0, len(c.cesNameToCESData))
	for ces := range c.cesNameToCESData {
...
func (c *CESToCEPMapping) getCESNamespace(name CESName) string {
	c.mutex.RLock()
	defer c.mutex.RUnlock()
	if cesData, ok := c.cesNameToCESData[name]; ok {
		return cesData.ns
	}
	return ""
}
