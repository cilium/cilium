...
// UpdatePodMapping is used to insert coreCEP in local cache, this may result in creating a new
// CES object or updating an existing CES object.
func (c *cesManager) UpdatePodMapping(pod *slim_corev1.Pod, nodeName string, cid *cilium_v2.CiliumIdentity) []CESKey {
	cepName, cesName := c.upsertPodIntoCES(pod)
...
func (c *cesManager) UpdateNodeMapping(node *cilium_v2.CiliumNode) []CESKey {
	...
	if c.mapping.hasNode(name) {
		// If node encryption key is changed, update affected CES.
		oldKey := c.mapping.getNodeEncryptionKey(name)
		if oldKey != newKey {
...
func (c *cesManager) RemoveNodeMapping(node *cilium_v2.CiliumNode) []CESKey {
	if c.mapping.hasNode(NodeName(node.Name)) {
...
func (c *cesManager) UpdateNamespaceMapping(ns *slim_corev1.Namespace) []CESKey {
...
// This function is called when we are syncing the cesStore with the manager cache.
// Therefore we only sync pods that are already mapped to a CES.
func (c *cesManager) initializeMappingPodToNode(pod *slim_corev1.Pod, ns string, nodeName string, ces CESName, cid CID, gidLabels string) {
	if _, ok := c.mapping.cesNameToCEPNameSet[ces]; !ok {
		return
	}
