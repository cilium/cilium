...
func TestFrontendTLSConfigMapSyncUsesOnlyFirstRef(t *testing.T) {
	...
	handler.EnqueueFrontendTLSConfigMapsGateway().Create(t.Context(), event.CreateEvent{Object: gw}, queue)
...
func TestEnqueueFrontendTLSConfigMapsReferenceGrant(t *testing.T) {
	logger := hivetest.Logger(t)
	refNamespace := gatewayv1.Namespace("ca-namespace")
	gw := gatewayWithFrontendTLSConfigMapRefs("gateway", []gatewayv1.ObjectReference{{
		Group:     "",
		Kind:      "ConfigMap",
		Name:      "frontend-ca",
	...
	grant := &gatewayv1.ReferenceGrant{
		ObjectMeta: metav1.ObjectMeta{Namespace: string(refNamespace)},
	}
	handler := NewSecretSyncHandler(c, logger, defaultControllerName)
	queue := workqueue.NewTypedRateLimitingQueue(workqueue.DefaultTypedControllerRateLimiter[reconcile.Request]())
	t.Cleanup(queue.ShutDown)

	handler.EnqueueFrontendTLSConfigMapsReferenceGrant().Generic(
		t.Context(),
		event.GenericEvent{Object: grant},
		queue,
	)
	...
	item, shutdown := queue.Get()
	require.False(t, shutdown)
	require.Equal(t, reconcile.Request{NamespacedName: clientObjectKey(string(refNamespace), "frontend-ca")}, item)
	queue.Done(item)

	handler.EnqueueFrontendTLSConfigMapsReferenceGrant().Delete(
		t.Context(),
		event.DeleteEvent{Object: grant},
		queue,
	)
