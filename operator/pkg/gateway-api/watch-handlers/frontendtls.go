...
func frontendTLSConfigMapMatches(gw *gatewayv1.Gateway, cfgMap *corev1.ConfigMap) bool {
	...
	if ref, ok := helpers.FirstFrontendTLSCACertificateRef(frontend.Default.Validation); ok &&
		configMapMatchesReference(ref, gw.Namespace, cfgMap) {
		return true
	}
...
func configMapMatchesReference(ref gatewayv1.ObjectReference, defaultNamespace string, cfgMap *corev1.ConfigMap) bool {
