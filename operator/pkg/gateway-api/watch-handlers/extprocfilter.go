...
func EnqueueRequestForExtProcFilter(c client.Client, logger *slog.Logger, controllerName string) handler.EventHandler {
	return handler.EnqueueRequestsFromMapFunc(func(ctx context.Context, o client.Object) []reconcile.Request {
		...
		for i := range hrList.Items {
			for _, req := range getGatewayReconcileRequestsForRoute(ctx, c, &hrList.Items[i], hrList.Items[i].Spec.CommonRouteSpec, scopedLog, controllerName) {
				reconcileRequests[req] = struct{}{}
			}
		}
		for i := range grpcRouteList.Items {
			for _, req := range getGatewayReconcileRequestsForRoute(ctx, c, &grpcRouteList.Items[i], grpcRouteList.Items[i].Spec.CommonRouteSpec, scopedLog, controllerName) {
				reconcileRequests[req] = struct{}{}
			}
		}
