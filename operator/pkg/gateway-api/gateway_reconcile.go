		...
		if err != nil {
			...
			for _, original := range collection.Valid {
				...
				input := &policychecks.BackendTLSPolicyInput{
					Ctx:              ctx,
					Logger:           scopedLog.With("BackendTLSPolicy", client.ObjectKeyFromObject(original)),
					Client:           r.Client,
...
func (r *gatewayReconciler) updateBackendTLSPolicyStatus(ctx context.Context, scopedLog *slog.Logger, original *gatewayv1.BackendTLSPolicy, new *gatewayv1.BackendTLSPolicy) error {
	...
	diff := cmp.Diff(original.Status, new.Status)
	if len(diff) > 0 {
		scopedLog.Debug("BackendTLSPolicy status update", backendTLSPolicy, types.NamespacedName{Name: original.Name, Namespace: original.Namespace}, "status", diff)
	}
