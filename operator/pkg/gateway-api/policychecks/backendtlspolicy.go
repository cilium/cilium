...
func (b *BackendTLSPolicyInput) ValidateSpec(ancestorRef gatewayv1.ParentReference) (bool, error) {
	if len(b.BackendTLSPolicy.Spec.Validation.CACertificateRefs) > 0 {
		...
		err := b.Client.Get(b.Ctx, caCertRefKey, caCert)
		if err != nil {
			...
			b.Logger.ErrorContext(b.GetContext(), fmt.Sprintf("Failed trying to get CA Certificate Ref Configmap: %s", caCertRefKey), logfields.Error, err)
