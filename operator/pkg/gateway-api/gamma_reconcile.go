...
func (r *gammaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !gammaService {
		// The Service is no longer the parent of any GAMMA route, so whatever the
		// reconciler generated for it while it was one has to go. Without this the
		// CiliumEnvoyConfig keeps intercepting traffic to the Service ClusterIP.
		if err := r.cleanupOwnedResources(ctx, svc); err != nil {
			return controllerruntime.Fail(fmt.Errorf("failed to clean up resources generated for the GAMMA Service: %w", err))
		}
		return controllerruntime.Success()
	}
...
// ensureOwnedEnvoyConfigDeleted deletes the CiliumEnvoyConfig that was generated
// for svc while it was still a GAMMA Service.
//
// The CiliumEnvoyConfig is named after the Service, but its controller owner
// reference points at the HTTPRoute or GRPCRoute that attached to the Service
// rather than at the Service itself. metav1.IsControlledBy(cec, svc) is therefore
// always false here, unlike in the Gateway reconciler, so ownership is established
// from the owner's kind instead.
func (r *gammaReconciler) ensureOwnedEnvoyConfigDeleted(ctx context.Context, svc *corev1.Service) error {
	cec := &ciliumv2.CiliumEnvoyConfig{}
	key := types.NamespacedName{
		Namespace: svc.Namespace,
		Name:      shortener.ShortenDNSLabelK8sName(svc.Name),
	}
	...
	if !isGammaOwnedEnvoyConfig(cec) {
		return nil
	}
...
// isGammaOwnedEnvoyConfig reports whether cec was generated by this reconciler,
// that is, whether it is controlled by a Gateway API route.
func isGammaOwnedEnvoyConfig(cec *ciliumv2.CiliumEnvoyConfig) bool {
	...
	if err != nil || gv.Group != gatewayv1.GroupVersion.Group {
		return false
	}

	return owner.Kind == "HTTPRoute" || owner.Kind == "GRPCRoute"
}

// cleanupOwnedResources removes everything the reconciler generated for svc while
// it was a GAMMA Service.
func (r *gammaReconciler) cleanupOwnedResources(ctx context.Context, svc *corev1.Service) error {
	if err := r.ensureOwnedEnvoyConfigDeleted(ctx, svc); err != nil {
		return err
	}
	if err := r.ensureOwnedEndpointSlicesDeleted(ctx, svc); err != nil {
		return err
	}
	return nil
}

// ensureOwnedEndpointSlicesDeleted deletes the dummy EndpointSlice generated
// alongside the CiliumEnvoyConfig while svc was a GAMMA Service.
//
// Unlike the CiliumEnvoyConfig, this object's controller owner reference carries
// the Service's own UID, so ownership is checked with metav1.IsControlledBy here.
func (r *gammaReconciler) ensureOwnedEndpointSlicesDeleted(ctx context.Context, svc *corev1.Service) error {
	epsList := &discoveryv1.EndpointSliceList{}
	matchingLabels := client.MatchingLabels{
		gatewayApiTranslation.EndpointSliceServiceNameLabel: shortener.ShortenDNSLabelK8sName(
			gatewayApiTranslation.CiliumGatewayPrefix + svc.Name,
		),
	}

	if err := r.client.List(ctx, epsList, client.InNamespace(svc.Namespace), matchingLabels); err != nil {
		return client.IgnoreNotFound(err)
	}

	for _, eps := range epsList.Items {
		if !metav1.IsControlledBy(&eps, svc) {
			continue
		}
