...
func (r *gammaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if len(httpRouteList.Items) == 0 {
		// scopedLog.Debug("Service is not a GAMMA Service")
		return controllerruntime.Success()
	}
	...
	// TODO: Run the HTTPRoute route checks here and update the status accordingly.
	...
	if err := r.updateStatus(ctx, originalSvc, svc); err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to update Gamma Service status: %w", err)
	}
