...
func Test_gatewayReconciler_Reconcile(t *testing.T) {
	...
	t.Run("valid http gateway", func(t *testing.T) {
		...
		require.Len(t, gw.Status.Conditions, 3)
		require.Equal(t, "Accepted", gw.Status.Conditions[0].Type)
		...
		require.Equal(t, "Gateway successfully configured", gw.Status.Conditions[1].Message)
		require.Equal(t, "Programmed", gw.Status.Conditions[2].Type)
		require.Equal(t, "True", string(gw.Status.Conditions[2].Status))
		require.Equal(t, "Gateway successfully reconciled", gw.Status.Conditions[2].Message)
