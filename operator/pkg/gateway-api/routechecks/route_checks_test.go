...
func TestCheckExtensionRefs_HTTPRouteNilGrants(t *testing.T) {
	parentRef := gatewayv1.ParentReference{Name: "gateway"}
	input := httpRouteInputWithExtensionRef(
		"default",
		parentRef,
	...
	require.Equal(t, string(gatewayv1.RouteReasonRefNotPermitted), input.HTTPRoute.Status.Parents[0].Conditions[0].Reason)
}

func TestCheckExtensionRefs_ListenerSetParent(t *testing.T) {
	// ListenerSet parents reach CheckExtensionRefs via backendCheckFuncs, the
	// same path as Gateway parents. Verify the condition is recorded against
	// the ListenerSet ParentReference, not silently dropped.
	...
	gotResult, err := CheckExtensionRefs(input, lsParentRef)
	require.NoError(t, err)
	require.False(t, gotResult)
	require.Len(t, input.HTTPRoute.Status.Parents, 1)
