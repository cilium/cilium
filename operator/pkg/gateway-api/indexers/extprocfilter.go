...
// no Namespace field), so the referenced filter always lives in the HTTPRoute's
// own namespace.
...
// IndexGRPCRouteByExtProcFilter is a client.IndexerFunc that takes a single GRPCRoute
// and returns the full names (`namespace/name`) of all CiliumEnvoyExtProcFilters
// referenced via ExtensionRef filters on that GRPCRoute.
//
// ExtensionRef filters are same-namespace only (gatewayv1.LocalObjectReference has
// no Namespace field), so the referenced filter always lives in the GRPCRoute's
// own namespace.
