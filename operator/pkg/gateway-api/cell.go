...
func discoverCRDsWithRetry(ctx context.Context, client k8sClient.Clientset, logger *slog.Logger, health cell.Health) (*gatewayAPIPreconditions, error) {
	...
	for {
		...
		// If the retry context has expired, the API server was unreachable for the
		// entire retry period. Return an error to crash the operator so that kubelet
		// restarts it and Gateway API initialization is retried on the fresh start.
		// This check must come before isTransientError to avoid a race where checkCRDs
		// returns a context deadline error that isTransientError doesn't recognize,
		// which would silently disable Gateway API instead of triggering a restart.
		if ctx.Err() != nil {
			logger.Error(
