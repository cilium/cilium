...
func TestConformance(t *testing.T) {
	...
	if os.Getenv(usableNetworkAddressesEnv) == "" {
		...
		usableNetworkAddressesStr := os.Getenv(usableNetworkAddressesEnv)
		for _, value := range strings.Split(usableNetworkAddressesStr, ",") {
	...
	if os.Getenv(unusableNetworkAddressesEnv) == "" {
		t.Logf("Set %s to run this test", features.SupportGatewayStaticAddresses)
		skipTests = append(skipTests, string(features.SupportGatewayStaticAddresses))
	} else {
		var addressType = v1.IPAddressType
		unusableNetworkAddressesStr := os.Getenv(unusableNetworkAddressesEnv)
		for _, value := range strings.Split(unusableNetworkAddressesStr, ",") {
