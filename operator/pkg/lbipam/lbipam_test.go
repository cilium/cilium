...
func TestLBIPAMStartupRestartShutdown(t *testing.T) {
	...
	_, err = fakeK8s.Services("default").Create(context.TODO(), &slim_core_v1.Service{
		ObjectMeta: slim_meta_v1.ObjectMeta{
			Name: "service-a",
		},
		Spec: slim_core_v1.ServiceSpec{
	...
	_, err = fakePools.Create(context.TODO(), &cilium_api_v2alpha1.CiliumLoadBalancerIPPool{
	...
	svc1, err := fakeK8s.Services("default").Get(context.TODO(), "service-a", meta_v1.GetOptions{})
	require.NoError(t, err)
	...
	err = fakePools.Delete(context.TODO(), "pool-a", meta_v1.DeleteOptions{})
	require.NoError(t, err)
