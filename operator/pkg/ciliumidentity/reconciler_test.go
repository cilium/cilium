...
import (
	...
	testutils "github.com/cilium/cilium/operator/pkg/ciliumidentity/testutils"
...
func TestReconcileCID(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			initialStoreState: testutils.NewCID(cidName, testLbsA),
			usedInPods:        false,
		...
		},
		{
			...
			usedInPods:     true,
			expectedCreate: testutils.NewCID(cidName, testLbsA), // CID is created
		},
		{
		...
		},
		{
			...
			cidKey:            cidResourceKey(cidName),
			initialStoreState: testutils.NewCID(cidName, testLbsB),
			desiredState:      testLbsA,
			usedInPods:        true,
			expectedUpdate:    testutils.NewCID(cidName, testLbsA),
		},
		{
...
func TestReconcilePod(t *testing.T) {
	ns1 := testutils.NewNamespace("ns1", nil)

	pod1 := testutils.NewPod("pod1", ns1.Name, testLbsA, "node1")
	pod2 := testutils.NewPod("pod2", ns1.Name, testLbsA, "node1")
	...
	testCases := []struct {
		...
		},
		{
			...
			newPod:       pod1,
			existingPods: []*slim_corev1.Pod{pod1},
			existingCIDs: []*capi_v2.CiliumIdentity{testutils.NewCIDWithNamespace(cidName, pod1, ns1)},
			expected: expectedState{
				queueSize:           0,
				numDesiredCIDLabels: 1,
				expectedPods:        1,
		...
		},
		{
			...
			newPod:          testutils.NewPod("pod1", ns1.Name, testLbsB, "node1"),
			existingPods:    []*slim_corev1.Pod{testutils.NewPod("pod1", ns1.Name, testLbsB, "node1")},
			existingCIDs:    []*capi_v2.CiliumIdentity{testutils.NewCIDWithNamespace(cidName, pod1, ns1)},
			initialPodToCID: map[resource.Key]string{podResourceKey(pod1.Name, ns1.Name): cidName},
			expected: expectedState{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, pod := range tc.existingPods {
				_ = reconciler.nsStore.CacheStore().Add(testutils.NewNamespace(pod.GetNamespace(), nil))
...
func TestReconcileNS(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := reconciler.nsStore.CacheStore().Add(testutils.NewNamespace(tc.nsName, nil)); err != nil {
				t.Errorf("Unexpected error behavior: got error %v", err)
			}
