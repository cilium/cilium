...
import (
	...
	"github.com/cilium/cilium/operator/pkg/ciliumidentity/testutils"
...
func TestReconcileCID(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			initialStoreState: testutils.CreateCIDObj(cidName, testLbsA),
			usedInPods:        false,
		...
		},
		{
			...
			usedInPods:     true,
			expectedCreate: testutils.CreateCIDObj(cidName, testLbsA), // CID is created
		},
		{
		...
		},
		{
			...
			cidKey:            cidResourceKey(cidName),
			initialStoreState: testutils.CreateCIDObj(cidName, testLbsB),
			desiredState:      testLbsA,
			usedInPods:        true,
			expectedUpdate:    testutils.CreateCIDObj(cidName, testLbsA),
		},
		{
...
func TestReconcilePod(t *testing.T) {
	ns1 := testutils.CreateNSObj("ns1", nil)

	pod1 := testutils.CreatePodObj("pod1", ns1.Name, testLbsA, nil)
	pod2 := testutils.CreatePodObj("pod2", ns1.Name, testLbsA, nil)
	...
	testCases := []struct {
		...
		},
		{
			...
			newPod:       pod1,
			existingPods: []*slim_corev1.Pod{pod1},
			existingCIDs: []*capi_v2.CiliumIdentity{testutils.CreateCIDObjNs(cidName, pod1, ns1)},
			expected: expectedState{
				queueSize:           0,
				numDesiredCIDLabels: 1,
				expectedPods:        1,
		...
		},
		{
			...
			newPod:          testutils.CreatePodObj("pod1", ns1.Name, testLbsB, nil),
			existingPods:    []*slim_corev1.Pod{testutils.CreatePodObj("pod1", ns1.Name, testLbsB, nil)},
			existingCIDs:    []*capi_v2.CiliumIdentity{testutils.CreateCIDObjNs(cidName, pod1, ns1)},
			initialPodToCID: map[resource.Key]string{podResourceKey(pod1.Name, ns1.Name): cidName},
			expected: expectedState{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, pod := range tc.existingPods {
				_ = reconciler.nsStore.CacheStore().Add(testutils.CreateNSObj(pod.GetNamespace(), nil))
...
func TestReconcileNS(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := reconciler.nsStore.CacheStore().Add(testutils.CreateNSObj(tc.nsName, nil)); err != nil {
				t.Errorf("Unexpected error behavior: got error %v", err)
			}
