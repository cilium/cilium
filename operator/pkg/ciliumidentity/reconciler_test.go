...
func TestReconcileCID(t *testing.T) {
	...
	clientset.CiliumFakeClientset.PrependReactor("delete", "*", func(action k8sTesting.Action) (handled bool, ret runtime.Object, err error) {
		return true, nil, fmt.Errorf("forced error on any deletion")
	})
	...
	assert.NoError(t, reconciler.upsertDesiredState(cidName, key.GetCIDKeyFromK8sLabels(testLblsA)))
	...
	assert.Equal(t, true, isMarked, "CID is marked for deletion when it isn't in desired state and isn't used")

	reconciler.cidDeletionTracker.markedForDeletion[cidName] = time.Now().Add(-cidDeleteDelay)

	reconciler.reconcileCID(cidResourceKey(cidName))
	_, isMarked = reconciler.cidDeletionTracker.MarkedTime(cidName)
	assert.Equal(t, true, isMarked, "CID is still marked for deletion because deletion failed")
	...
	reconciler.reconcileCID(cidResourceKey(cidName))
	assert.Equal(t, cidName, deleteCIDName, "CID deletion completed")
	_, isMarked = reconciler.cidDeletionTracker.MarkedTime(cidName)
	assert.Equal(t, false, isMarked, "CID is unmarked for deletion after deletion is completed")
	deleteCIDName = ""
...
func TestReconcilePod(t *testing.T) {
	...
	assert.Equal(t, 0, len(reconciler.desiredCIDState.idToLabels), assertTxt)
	...
	assert.NoError(t, err, assertTxt)
	assert.Equal(t, 1, len(queueOps.fakeWorkQueue), assertTxt)
	assert.Equal(t, 1, len(reconciler.desiredCIDState.idToLabels), assertTxt)
	...
	assert.Equal(t, 2, len(reconciler.desiredCIDState.idToLabels), assertTxt)
	...
	pod6 := testCreatePodObj("pod6", "ns2", testLblsA, nil)
	reconciler.podStore.CacheStore().Add(pod6)

	cidName6 := "6000"
	lbls, _ := reconciler.getRelevantLabelsForPod(pod6)
	cid6 := testCreateCIDObj(cidName6, lbls)
	reconciler.cidStore.CacheStore().Add(cid6)

	err = reconciler.reconcilePod(podResourceKey("pod6", "ns2"))
	assert.NoError(t, err)
	assert.Equal(t, 3, len(reconciler.desiredCIDState.idToLabels), assertTxt)
	testVerifyCIDUsageInPods(t, reconciler.cidUsageInPods, 3, 4, "4 Pods and 3 CID expected")

	assertTxt = "Pod is quickly deleted and recreated"
	reconciler.podStore.CacheStore().Delete(pod6)
	err = reconciler.reconcilePod(podResourceKey("pod6", "ns2"))
	assert.NoError(t, err)
	_, marked := reconciler.cidDeletionTracker.MarkedTime(cidName6)
	assert.Equal(t, true, marked, assertTxt)
	// Here CID desired state is separately verified from usage in pods, because
	// the desired state is only removed after CID is successfully deleted.
	assert.Equal(t, 3, len(reconciler.desiredCIDState.idToLabels), assertTxt)
	testVerifyCIDUsageInPods(t, reconciler.cidUsageInPods, 2, 3, assertTxt)

	reconciler.podStore.CacheStore().Add(pod6)
	err = reconciler.reconcilePod(podResourceKey("pod6", "ns2"))
	assert.NoError(t, err)
	_, marked = reconciler.cidDeletionTracker.MarkedTime(cidName6)
	assert.Equal(t, false, marked, assertTxt+" (recreation part)")
	assert.Equal(t, 3, len(reconciler.desiredCIDState.idToLabels), assertTxt)
	testVerifyCIDUsageInPods(t, reconciler.cidUsageInPods, 3, 4, "4 Pods and 3 CID expected")
}
