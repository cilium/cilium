...
func checkAndMarkNode(c kubernetes.Interface, nodeGetter slimNodeGetter, nodeName string, options markNodeOptions) bool {
	...
	if options.SetCiliumIsUpCondition && !HasCiliumIsUpCondition(node) {
		...
		log.WithFields(logrus.Fields{logfields.NodeName: node.GetName()}).Debug("Node with CiliumIsUp condition")
	}
	return true
}
...
// HandleNodeConditions remove node
func HandleNodeConditions(stopCh <-chan struct{}) {
	mno = markNodeOptions{
