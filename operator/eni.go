...
func (k *k8sAPI) Update(node, origNode *v2.CiliumNode) (*v2.CiliumNode, error) {
	...
	case k8sCapabilities.UpdateStatus:
		// Always update the status first to ensure that the ENI
		// information is synced for all addresses that are marked as
		// available.
		...
		if !reflect.DeepEqual(origNode.Spec, node.Spec) {
			newNode, err := ciliumK8sClient.CiliumV2().CiliumNodes("default").Update(node)
			if err != nil {
				return newNode, err
			}
		}
	default:
		if !reflect.DeepEqual(origNode, node) {
			return ciliumK8sClient.CiliumV2().CiliumNodes("default").Update(node)
		}
	}
...
func startENIAllocator() error {
	...
	log.Infof("Running on instance %s in region %s", instance.InstanceID, instance.Region)
	...
	instances := eni.NewInstancesManager(ec2Client)
