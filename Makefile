...
GOFILES_EVAL := $(subst github.com/cilium/cilium,.,$(shell $(GO_LIST) -find -e ./...))
GOFILES ?= $(GOFILES_EVAL)

# Exclude some packages from tests:
# - ./api/v1/... - autogenerated, doesn't come with tests, ignore.
# - ./test - contains Ginkgo test boilerplate in _test.go files, not testable.
# - ./test/bpf_tests - needs a test flag to be specified, currently doesn't skip automatically.
#
# Make converts newlines to spaces when storing strings in variables.
# Only a '\s'->'\n' conversion before the grep is necessary.
#
# TODO(tb): This is a hack. All packages should be 'go test'able out of the box.
TESTPKGS_EVAL := $(shell echo $(GOFILES) | \
		sed 's/ /\n/g' | \
		grep -Ev './api/v1|./test$$|./test/bpf_tests')

TESTPKGS ?= $(TESTPKGS_EVAL)

GOLANG_SRCFILES := $(shell for pkg in $(GOFILES); do find $$pkg -name *.go -print; done | sort | uniq)
