...
dev-docker-image: GIT_VERSION
	$(CONTAINER_ENGINE_FULL) build \
		...
		--build-arg V=${V} \
		--build-arg LIBNETWORK_PLUGIN=${LIBNETWORK_PLUGIN} \
		-t "cilium/cilium-dev:$(DOCKER_IMAGE_TAG)-${ARCH}" .
	$(QUIET)echo "Push like this when ready:"
	$(QUIET)echo "${CONTAINER_ENGINE} push cilium/cilium-dev:$(DOCKER_IMAGE_TAG)-${ARCH}"

docker-cilium-dev-manifest:
	./push_manifest.sh cilium-dev $(DOCKER_IMAGE_TAG)

docker-operator-image: GIT_VERSION
	$(CONTAINER_ENGINE_FULL) build --build-arg LOCKDEBUG=${LOCKDEBUG} -f cilium-operator.Dockerfile -t "cilium/operator:$(DOCKER_IMAGE_TAG)-${ARCH}" .
	$(QUIET)echo "Push like this when ready:"
	$(QUIET)echo "docker push cilium/operator:$(DOCKER_IMAGE_TAG)-${ARCH}"

docker-operator-manifest:
	./push_manifest.sh operator $(DOCKER_IMAGE_TAG)

docker-plugin-image: GIT_VERSION
	$(CONTAINER_ENGINE_FULL) build --build-arg LOCKDEBUG=${LOCKDEUBG} -f cilium-docker-plugin.Dockerfile -t "cilium/docker-plugin:$(DOCKER_IMAGE_TAG)-${ARCH}" .
	$(QUIET)echo "Push like this when ready:"
	$(QUIET)echo "docker push cilium/docker-plugin:$(DOCKER_IMAGE_TAG)-${ARCH}"

docker-plugin-manifest:
	./push_manifest.sh docker-plugin $(DOCKER_IMAGE_TAG)

docker-image-init:
	$(QUIET)cd contrib/packaging/docker && ${CONTAINER_ENGINE} build -t "cilium/cilium-init:$(UTC_DATE)-${ARCH}" -f Dockerfile.init .

docker-cilium-init-manifest:
	./push_manifest.sh cilium-init $(UTC_DATE)

docker-image-runtime:
	cd contrib/packaging/docker && ${CONTAINER_ENGINE} build -t "cilium/cilium-runtime:$(UTC_DATE)-${ARCH}" -f Dockerfile.runtime .

docker-cilium-runtime-manifest:
	./push_manifest.sh cilium-runtime $(UTC_DATE)

docker-image-builder:
	${CONTAINER_ENGINE_FULL} build -t "cilium/cilium-builder:$(UTC_DATE)-${ARCH}" -f Dockerfile.builder .

docker-cilium-builder-manifest:
	./push_manifest.sh cilium-builder $(UTC_DATE)
