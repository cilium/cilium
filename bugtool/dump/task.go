...
const (
	resultsFileName      = "bugtool-task-status-results.yaml"
	resultsHeaderComment = `
# This file was generated by Cilium bugtool.
# It contains the results of all tasks that were run, including their
# status and any error messages.
# As well as the time taken to run the task and peak memory/cpu usage.
...
func (c Context) GetResults() error {
	for p, v := range c.collect.results {
		file := path.Join(p, resultsFileName)
		...
		if _, err := fd.Write([]byte(resultsHeaderComment)); err != nil {
			return fmt.Errorf("failed to write results header comment %q: %w", file, err)
		}
