...
# Master's IPv4 address. Workers' IPv4 address will have their IP incremented by 1.
# The netmask used will be /24
export 'MASTER_IPV4'=${MASTER_IPV4:-"192.168.33.11"}
# NFS address is only set if NFS option is active. This will create a new network
# interface for each VM with starting on this IP. This IP will be available to reach from
# the host.
export 'MASTER_IPV4_NFS'=${NFS+"192.168.34.11"}
# Enable IPv4 mode. It's enabled by default since it's required for several runtime tests
export 'IPV4'=${IPV4:-1}
# Exposed IPv6 node CIDR, only set if IPV4 is disabled. Each node will be setup with a
# IPv6 network available from the host with $IPV6_PUBLIC_CIDR + 6to4($MASTER_IPV4).
# For IPv4 "192.168.33.11" we will have for example:
#   master  : FD00::B/16
#   worker 1: FD00::C/16
# The netmask used will be /16
export 'IPV6_PUBLIC_CIDR'=${IPV4+"FD00::"}
# Internal IPv6 node CIDR, always set up by default. Each node will be setup with a
# IPv6 network available from the host with IPV6_INTERNAL_CIDR + 6to4($MASTER_IPV4).
# For IPv4 "192.168.33.11" we will have for example:
#   master  : FD01::B/16
#   worker 1: FD01::C/16
# The netmask used will be /16
...
# $CILIUM_IPV6_NODE_CIDR + 6to4($MASTER_IPV4). For IPv4 "192.168.33.11" we will have for
# example: master  : FD02::C0A8:210B:0:0/96
...
# get_cilium_node_addr sets the cilium node address in ${1} for the IPv4 address in ${2}.
...
# configuration for the vm IP ${1}. Sets the master's hostname with IPv6 address in ${2}.
...
# Uses the IPv4 suffix in ${1} for the IPv4 route and cilium IPv6 in ${2} via ${3} for
# the IPv6 route. Sets the worker's hostname based on the index defined in ${4} with the
# IPv6 defined in ${5}.
...
# write_nodes_routes writes in file ${3} the routes for all nodes in the clusters except
# for node with index ${1}. All routes will be based on IPv4 defined in ${2}.
...
function write_cilium_cfg() {
    ...
    if [[ "${IPV4}" -eq "1" ]]; then
        cilium_options+=" --ipv4 --ipv4-range 10.${index}.0.1"
    fi

    if [ -n "${K8S}" ]; then
        cilium_options+=" -k http://${MASTER_IPV4}:8080"
        cilium_options+=" --etcd-config-path /var/lib/cilium/etcd-config.yml"
    else
        if [[ "${IPV4}" -eq "1" ]]; then
            cilium_options+=" -c ${MASTER_IPV4}:8500"
        else
            cilium_options+=" -c [${ipv6_addr}]:8500"
        fi
    fi
...
if [ -n "\$(grep DISTRIB_RELEASE=14.04 /etc/lsb-release)" ]; then
    ...
    echo 'exec cilium -D daemon run ${ubuntu_1404_interface} ${cilium_options}' >> /etc/init/cilium-net-daemon.conf
    service cilium-net-daemon restart
else
    # Adding debug flag to cilium daemon
...
function create_master(){
    ...
    write_cilium_cfg 1 "${master_cilium_ipv6}" "${output_file}"
}
...
function create_workers(){
    ...
    if [ -n "${NWORKERS}" ]; then
        for i in `seq 2 $(( NWORKERS + 1 ))`; do
            ...
            get_cilium_node_addr worker_cilium_ipv6 "${MASTER_IPV4}"

            write_cilium_cfg "${i}" "${worker_cilium_ipv6}" "${output_file}"
        done
    fi
}
