...
// check-workflow-check-name-collisions flags GitHub Actions workflows whose
// check name collides with another workflow that can run for the same pull
// request. Each workflow publishes a commit status named after the workflow, so
// two workflows sharing a name make that name ambiguous as a required status
// check. A passing status from one workflow then hides a missing status from
// the other.
//
// A collision only matters when both workflows can run against the same base
// branch. Per-version workflow families such as foo.yaml and foo-v1.18.yaml
// share a name on purpose but have disjoint branch triggers, so they never run
// together and count as benign.
//
// It scans .github/workflows, or the directory given as the first argument. Use
// -v to also list the benign collisions.
package main
...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, "Scan GitHub Actions workflows for colliding check names.")
		fmt.Fprintln(os.Stderr, "dir defaults to .github/workflows.")
		flag.PrintDefaults()
	}
	flag.Parse()
...
// branchScope reduces a workflow's on: triggers to the base branches it can run for.
