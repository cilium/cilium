...
# If any entry which writes any messagedoesn't contaion the 'subsys' field,
...
if git grep -n "log[ ]*= logging\.DefaultLogger*" ':(exclude)test/' ':(exclude)pkg/debugdetection/' | grep -v LogSubsys; then
