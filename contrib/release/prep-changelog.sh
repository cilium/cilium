...
usage() {
    ...
    logecho "OLD-BRANCH     Branch of the previous release version if"
    logecho "               NEW-VERSION is a new minor version"
    logecho
...
main() {
    ...
    if [ "${old_branch}" = "" ] ; then
      ...
      echo $RELNOTES --last-stable $old_branch --base $old_version --head $(git rev-parse HEAD)
